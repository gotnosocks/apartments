...
def family(term):
    ...
    if term.startswith('season:'):
        return 'season'
    if term=='size_missing' or term.endswith('.unknown') or term.endswith('=__unknown__'):
        return 'missingness'
...
def residual(model,row):
    predicted=model.predict(row,row['period']);fitted=predicted['predicted_rent']
    asking=float(row['asking_rent']);groups=defaultdict(list)
    ...
    return {'audit_id':row['audit_id'],'unit_id':row['unit_id'],'building_id':row['building_id'],
        'source_listing_id':row['source_listing_id'],'period':row['period'],
        ...
        'analysis_price_basis':row.get('analysis_price_basis'),'current_capture':current,
        'asking_rent':asking,'fitted_rent':fitted,'asking_minus_fitted':asking-fitted,
        'asking_vs_fitted_percent':100*math.expm1(log_error),'log_residual':log_error,
        'absolute_log_residual':abs(log_error),'log_contributions_by_family':grouped,
        'group_adjustment_percent':100*math.expm1(grouped.get('building',0)+grouped.get('unit',0)),
        'residual_type':'in_sample_fitted_diagnostic',
        'current_capture_in_fit':str(row.get('capture_id')) in model.artifact['training'].get('current_capture_ids',[]),
...
def review_queue(residuals,*,top_units=20):
    ...
    for label,reverse in [('positive_residual',True),('negative_residual',False)]:
        units=set()
        for row in sorted(residuals,key=lambda r:(r['log_residual'],r['audit_id']),reverse=reverse):
            if (label=='positive_residual' and row['log_residual']<=0) or (label=='negative_residual' and row['log_residual']>=0):
                continue
            if row['unit_id'] in units:
                continue
            units.add(row['unit_id']);chosen[row['audit_id']]=row;reasons[row['audit_id']].append(label)
            if len(units)>=top_units:
                break
    for row in residuals:
        if row['current_capture']:
            chosen[row['audit_id']]=row;reasons[row['audit_id']].append('current_capture')
    return [{**row,'selection_reasons':reasons[key],'review_status':'unreviewed',
            ...
            for key,row in sorted(chosen.items(),key=lambda item:(-item[1]['absolute_log_residual'],item[0]))]
...
def summarize(rows):
    ...
    return {'rows':len(rows),'units':len({r['unit_id'] for r in rows}),
        ...
        'median_absolute_log_residual':statistics.median(r['absolute_log_residual'] for r in rows),
        'median_asking_vs_fitted_percent':statistics.median(r['asking_vs_fitted_percent'] for r in rows),
        'median_absolute_dollar_residual':statistics.median(abs(r['asking_minus_fitted']) for r in rows)}


def markdown(queue,summary):
    lines=['# Fitted-residual review','',
    ...
    for title,predicate in [('Current captures',lambda r:r['current_capture']),
                            ('Large positive residuals',lambda r:'positive_residual' in r['selection_reasons']),
                            ('Large negative residuals',lambda r:'negative_residual' in r['selection_reasons'])]:
        lines.extend(['## '+title,'','| Advertisement | Month | Ask | Fitted | Ask − fitted | Difference |',
                      '| --- | --- | ---: | ---: | ---: | ---: |'])
        for row in sorted(filter(predicate,queue),key=lambda r:-r['absolute_log_residual']):
            lines.append(f"| [{row['source_listing_id']}]({row['advertisement_url']}) | {row['period'][:7]} | ${row['asking_rent']:,.0f} | ${row['fitted_rent']:,.0f} | ${row['asking_minus_fitted']:+,.0f} | {row['asking_vs_fitted_percent']:+.1f}% |")
        lines.append('')
...
def run(model_bundle,dataset,output,*,top_units=20):
    ...
    if (manifest.get('dataset_version')!='historical-plus-current-capture-analysis-v1'
            or manifest!=model.artifact['training']['source_manifest']):
        raise ValueError('Residuals require the exact verified dataset used for this analysis fit')
    source=[json.loads(s) for s in files['observations.jsonl'].decode().splitlines() if s.strip()]
    results=[residual(model,row) for row in source]
    queue=review_queue(results,top_units=top_units);by_id={row['audit_id']:row for row in source}
    for row in queue:
        original=by_id[row['audit_id']]
        row['source_record']=original
        row['log_components']=model.predict(original,original['period'])['log_components']
    summary={'version':VERSION,'all':summarize(results),
        'current':summarize([r for r in results if r['current_capture']]),'review_rows':len(queue),
        'selection_counts':dict(Counter(reason for r in queue for reason in r['selection_reasons'])),
        'top_units_per_tail':top_units,'review_state':'Unreviewed queue; no diagnoses or corrections inferred from residual magnitude.',
        'limitations':model.artifact['limitations']}
    encoder=model.encoder
    support={'core_layout':model.artifact['training'].get('layout_support',{}),
             'amenity_numeric':encoder['amenity_numeric'],'amenity_category_counts':encoder['amenity_category_counts'],
    ...
    return publish_bundle(output,{
        ...
        {'version':VERSION,'model_manifest':model.manifest,'dataset_manifest':manifest,'top_units_per_tail':top_units,
...
if __name__=='__main__':
    ...
    parser.add_argument('--output',type=Path,required=True);parser.add_argument('--top-units',type=int,default=20)
    args=parser.parse_args()
    print(canonical(run(args.model,args.dataset,args.output,top_units=args.top_units)))
