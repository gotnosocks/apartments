...
def plan():
    ...
    def add(path):
        ...
        if (
            path.name.endswith(".sqlite3-shm")
        ...
        if path.is_symlink():
            ...
            target = (
                (path.parent / original).resolve()
                if not original.startswith("/")
                else Path(original).resolve()
            )
            try:
                ...
                target_relative = target.relative_to(
                    Path("/__modal/volumes") / volume.object_id
                )
            rec.update(
                type="symlink",
                target=os.path.relpath(BASE / target_relative, path.parent),
    ...
    batch = []
    size = 0
    (EXPORT / "jobs").mkdir(exist_ok=True)
    (EXPORT / "packs").mkdir(exist_ok=True)
    ...
    for record in records:
        ...
        if batch and (
            size + record["size"] > 2 * 1024**3 or len(batch) >= 5000 or previous != top
        ):
            flush()
        batch.append(record)
        size += record["size"]
    ...
    out = {
        ...
        "source": "chelsea-archive",
        "total_files": len(records),
        "total_bytes": sum(r["size"] for r in records),
        "groups": groups,
        "by_root": totals,
    }
    write_json(EXPORT / "plan.json", out)
    volume.commit()
...
class HashReader:
    def __init__(self, stream, label=None):
        ...
        self.label = label
        self.bytes = 0
        self.started = self.last_log = time.time()
...
def pack(ident):
    ...
    with partial.open("wb") as output, manifest_partial.open("w") as mf:
        proc = subprocess.Popen(
            ["zstd", "-3", "-T2", "--quiet", "-c"], stdin=subprocess.PIPE, stdout=output
        )
        try:
            with tarfile.open(
                fileobj=proc.stdin, mode="w|", bufsize=1024 * 1024
            ) as archive:
                for record in records:
                    source = BASE / record["path"]
                    before = source.lstat()
                    if before.st_mtime_ns != record["mtime_ns"] or (
                        record.get("type") != "symlink"
                        and before.st_size != record["size"]
                    ):
                        raise RuntimeError(
                            "Source changed since freeze: " + record["path"]
                        )
                    info = tarfile.TarInfo(record["path"])
                    info.mode = record["mode"]
                    info.mtime = before.st_mtime
                    if record.get("type") == "symlink":
                        if os.readlink(source) != record["original_target"]:
                            raise RuntimeError("Source symlink changed")
                        ...
                        with source.open("rb") as raw:
                            reader = HashReader(raw, record["path"])
                        ...
                        if (
                            ...
                            raise RuntimeError(
                                "Source mutated during export: " + record["path"]
                            )
                        final = {
                            "path": record["path"],
                            "size": record["size"],
...
def run():
    planned = plan.remote()
    errors = []
    for result in pack.map(
        [g["id"] for g in planned["groups"]],
        order_outputs=False,
        return_exceptions=True,
    ):
        if isinstance(result, Exception):
            errors.append(str(result))
            print(json.dumps({"pack_error": str(result)}), flush=True)
        else:
            print(json.dumps(result), flush=True)
    if errors:
        raise RuntimeError(
            "Export incomplete; successful bundles retained: " + str(errors)
        )
    volume.reload()
    ready = [
        json.loads((EXPORT / "packs" / f"{g['id']:05d}.ready.json").read_text())
        for g in planned["groups"]
    ]
    out = {
        "completed_at": time.time(),
        "files": sum(r["files"] for r in ready),
        "bytes": sum(r["bytes"] for r in ready),
        "uncompressed_bytes": sum(r["uncompressed_bytes"] for r in ready),
    }
    write_json(EXPORT / "complete.json", out)
    volume.commit()
    return out
...
def status():
    volume.reload()
    planned = (
        json.loads((EXPORT / "plan.json").read_text())
        if (EXPORT / "plan.json").exists()
        else None
    )
    ready = (
        [json.loads(p.read_text()) for p in (EXPORT / "packs").glob("*.ready.json")]
        if (EXPORT / "packs").exists()
        else []
    )
    return {
        "plan": planned,
        "completed_groups": len(ready),
        "compressed_bytes": sum(r["bytes"] for r in ready),
        "completed_uncompressed_bytes": sum(r["uncompressed_bytes"] for r in ready),
        "complete": (EXPORT / "complete.json").exists(),
    }
