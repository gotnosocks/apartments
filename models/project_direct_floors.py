...
def run(dataset, policy, output, reviewed_at):
    ...
    source_hash, policy_bundle_hash = digest(dataset/'complete.json'), digest(policy/'complete.json')
    if policy_bundle_hash != 'b447545431c5ab427c2995d30fbe0bdd50b6c5d491b41fb5acd8d870c29057d2':
    ...
    known = lambda rows: sum(r.get('listed_floor') is not None or r.get('advertised_floor') is not None for r in rows)
    summary = {'source_rows': len(before), 'rows': len(after), 'floor_additions': len(changes),
        'known_floor_before': known(before), 'known_floor_after': known(after),
        'other_fields_unchanged': True, 'measurement': 'advertised floor, not physical height'}
    products = {name: value.decode() for name, value in files.items() if name not in {'observations.jsonl', 'summary.json'}}
    if {contract.SIDECAR, 'direct-floor-parent-summary.json', 'direct-floor-policy.json'} & products.keys():
    ...
    metadata = {'version': contract.VERSION, 'source_manifest': parent,
        'source_manifest_sha256': source_hash, 'policy': decision, 'policy_sha256': policy_hash,
        'policy_bundle_manifest_sha256': policy_bundle_hash,
        'source_rows': len(before), 'reviewed_at': reviewed_at, 'summary': summary}
    prospective = {**metadata, 'files': {name: hashlib.sha256(value.encode()).hexdigest()
    ...
    if digest(dataset/'complete.json') != source_hash or digest(policy/'complete.json') != policy_bundle_hash:
