...
def analyze(experiment, output):
    ...
    for split in protocol["splits"]:
        ...
        manifest, files = _verified_bundle(
            root / name / "comparison", retain={"predictions.jsonl"}
        )
        if manifest.get("protocol_sha256") != expected:
            raise ValueError("Prediction protocol mismatch")
        table = pd.DataFrame(
            json.loads(line)
            for line in files["predictions.jsonl"].decode().split("\n")
            if line.strip()
        )
        if (
            len(table) != split["test_rows"]
            or table.duplicated(["unit_id", "period"]).any()
        ):
    ...
    def pool(names, expected_rows, expected_table=None):
        ...
        if expected_table is not None:
            identities = lambda frame: set(
                zip(frame.unit_id, frame.period, frame.audit_id)
            )
            if identities(combined) != identities(expected_table):
                raise ValueError("Crossed folds differ from annual holdout identities")
        metrics = {
            variant: baseline.metrics(combined, np.log(combined[variant].to_numpy()))
        ...
        for reference in ("baseline", "missingness"):
            ...
            comparisons["full_minus_" + reference] = model.paired_building_comparison(
                paired, draws=2000
            )
        return {
            "rows": len(combined),
    ...
    pooled = (
        pool(names, protocol["development_rows"])
    ...
    years = sorted(
        {
            name.split("-")[1]
            for name in tables
            if name.startswith("year-") and "-building-" in name
        }
    )
    for year in years:
        ...
        if all(name in tables for name in names):
            ...
            expected_rows = (
                ...
                else sum(len(tables[name]) for name in names)
            )
            crossed[year] = pool(names, expected_rows, annual)
    report = {
        "version": "amenity-ablation-analysis-v1",
        "protocol": protocol,
        "pooled_building_holdouts": pooled,
        "pooled_crossed_holdouts": crossed,
        "splits": summary["results"],
        "limitations": [
            "Retrospective reconstructed asking rents and later-collected attributes.",
