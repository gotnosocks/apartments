...
def install(benchmark, recovery):
    ...
    with (benchmark/'.recovery-install.lock').open('a') as lock:
        ...
        _, files = _verified_bundle(recovery/'validation', retain={'recovery.json', 'raw-inventory.json'})
        ...
        if (record['benchmark'] != str(benchmark.resolve()) or record['settings'] != settings
                or record['new_draws_generated'] != 0 or not record['original_raw_files_unchanged']
                or record['conversion']['chains'] != settings['chains']
                or record['conversion']['draws'] != settings['draws_per_chain']
                or record['sampling_progress'] != json.loads((benchmark/'progress.json').read_text())
                or record['posterior_sha256'] != digest(recovery/'posterior.nc')):
            raise ValueError('Recovery source, dimensions, progress or posterior differs')
        inventory = {p.name: digest(p) for p in sorted((benchmark/'unconstrained-trace').glob('*.npy'))}
        ...
        if any(p.exists() for p in (destination, benchmark/'posterior.partial', benchmark/'sampled.json')):
            raise ValueError('Existing posterior/completion products must not be overwritten')
        ...
        timings = {k: v for k, v in record['sampling_progress'].items() if k.endswith('_seconds') or k == 'retained_batches'}
        timings['recovery_transform_and_write_seconds'] = record['recovery_transform_and_write_seconds']
        timings['recovery_install_seconds'] = time.perf_counter()-start
        completed = {'settings': settings, 'timings': timings,
            'sample_statistics': {k: record['conversion'][k] for k in ('divergences', 'mean_n_steps', 'max_n_steps', 'max_tree_depth')},
            'posterior_sha256': record['posterior_sha256'],
            'status': 'sampled_diagnostics_and_ess_comparison_pending',
            'pymc_call_completed': False,
            'postprocessing_recovery': {'directory': str(recovery.resolve()),
