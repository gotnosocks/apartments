...
def verify_revision(reference, candidate):
    ...
    if sha(projection.parent_rows(bundles[1][0], after, changes)) != sha((bundles[0][0], before)):
        raise ValueError('Direct floor revision does not restore exact reference source')
...
def residual_slices(before, after, movements):
    ...
    masks = {'all': np.ones(len(after), dtype=bool),
        ...
        'previously_known_floor': np.isfinite(old), 'remaining_unknown_floor': ~np.isfinite(new)}
    ...
    return {name: {**shared.laundry.summarize_slice(
        [indexed[r['audit_id']] for r, keep in zip(after, mask, strict=True) if keep]),
...
def run(output, **kwargs):
    modules = (projection, checks, expanded, shared, shared.report, shared.source, shared.common,
        shared.laundry, shared.floors, smooth, smooth.increment, smooth.contrasts, smooth.elevator,
        spline, checks.experiment, checks.execution, smooth.publisher)
