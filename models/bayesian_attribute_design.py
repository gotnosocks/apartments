...
TEXT_PATTERNS = {
    ...
    "private_outdoor_unit": (
        r"\b(?:private|your own|own private|exclusive)\s+(?:outdoor space|terrace|balcony|"
...
def attribute_units(data, captures):
    ...
    for name, pattern in LABEL_PATTERNS.items():
        ...
        result[name] = sorted(
            {
                u
                for u, url in zip(data.unit_id, data.canonical_unit_url)
                if regex.match(unit_label(url))
            }
        )
    for name, pattern in TEXT_PATTERNS.items():
        ...
        result[name] = sorted(
            {
                u
                for u, audit in zip(data.unit_id, data.audit_id)
                if any(
...
class FeatureDesign(spline.FeatureDesign):
    def __init__(
        ...
        *,
        ...
        if set(attribute_units) != set(ATTRIBUTES):
            raise ValueError(
                "Attribute units must cover exactly the declared attributes"
            )
    ...
    def attribute_matrix(self, data):
        return np.column_stack(
            [
                data.unit_id.isin(set(self.attribute_units[name])).to_numpy(dtype=float)
    ...
    def _validate_saved_structure(self):
        ...
        if own == VERSION:
            ...
            if (
                ...
                raise ValueError(
                    "Saved attribute columns, policy or evidence binding differ"
                )
