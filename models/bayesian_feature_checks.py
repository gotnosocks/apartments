"""Legacy helper API; execution delegates to the corrected v2 checker.

Original v1 results used an unsafe design reload and must not be interpreted.
The frozen original implementation remains with those historical artifacts.
"""
...
VERSION = 'bayesian-feature-posterior-checks-v1'
VARIABLE_DIMS = {
...
def load_draws(path, protocol, design, per_chain=50):
    with xr.open_dataset(path, group='posterior', engine='h5netcdf') as posterior:
        ...
        selection = []
        for chain, draw in chosen:
            # Select before load: never materialize the complete posterior.
            part = posterior[list(VARIABLE_DIMS)].isel(chain=chain, draw=draw).load()
            ...
            for name, dimensions in VARIABLE_DIMS.items():
                samples[name].append(np.asarray(part[name].transpose(*dimensions).values).copy())
...
def run(experiment, dataset, output, *, per_chain=50, seed=20260919):
    """Route legacy calls through the ordered loader; v1 artifacts are invalid."""
    from .bayesian_feature_checks_v2 import run as corrected_run
    return corrected_run(experiment, dataset, output, per_chain=per_chain, seed=seed)
