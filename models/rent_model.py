...
def prepare_data(db_path: Path, frequency="monthly", start=None):
    ...
    with duckdb.connect(str(db_path), read_only=True) as db:
        ...
        names = dict(
            db.execute("""SELECT building_slug,
    ...
    furnished_ids = set(
        events.loc[events.is_furnished.fillna(False), "source_listing_id"]
    )
    evidence = (
        events.event_json.fillna("") + " " + events.event_type.fillna("")
    ).str.lower()
    furnished_ids.update(
        events.loc[
            evidence.str.contains(r"blueground|\bfurnished\b", regex=True),
            "source_listing_id",
        ]
    )
    for source_id, raw in captures:
        item = json.loads(raw)
        if any(
            str(feature).upper() == "FURNISHED"
            for feature in item.get("home_features", [])
        ):
            furnished_ids.add(source_id)
    coverage = {
        "archived_rental_units": int(events.source_listing_id.nunique()),
        "archived_rental_buildings": int(events.building_slug.nunique()),
        "raw_events": len(events),
        "furnished_units_excluded": len(furnished_ids),
    }
    valid = (
        events.asking_rent.gt(0)
        & events.event_date.notna()
        & events.unit_is_specific.fillna(True)
    )
    if start is not None:
        valid &= events.event_date.ge(pd.Timestamp(start))
    coverage["invalid_or_outside_window_events"] = int((~valid).sum())
    coverage["furnished_events_excluded"] = int(
        (valid & events.source_listing_id.isin(furnished_ids)).sum()
    )
    ...
    events.loc[events.floor_inference.eq("heuristic-first-digit"), "physical_floor"] = (
        np.nan
    )
    ...
    data["square_feet_imputed"] = data.square_feet.fillna(group_median).fillna(
        overall if pd.notna(overall) else 700
    )
    ...
    for col, idx in [
        ("building_slug", "building_idx"),
        ("unit_key", "unit_idx"),
        ("floor_level", "floor_idx"),
    ]:
        values = sorted(data[col].unique())
        data[idx] = data[col].map(dict(zip(values, range(len(values))))).astype(int)
    data["period_idx"] = data.period.map(
        dict(zip(periods, range(len(periods))))
    ).astype(int)
    coverage.update(
        modeled_units=int(data.unit_key.nunique()),
        modeled_buildings=int(data.building_slug.nunique()),
        unit_period_observations=len(data),
        unknown_floor_observations=int(data.physical_floor.isna().sum()),
        missing_size_observations=int(data.sqft_missing.sum()),
        earliest_date=str(events.event_date.min().date()),
        latest_date=str(events.event_date.max().date()),
    )
    data.attrs.update(
        coverage=coverage,
        names=names,
        excluded_furnished_units=sorted(furnished_ids),
        size_log_mean=float(log_size.mean()),
        size_log_scale=size_scale,
    )
    return data, periods
...
def fit_model(
    ...
    *,
    ...
    coords = {
        "building": buildings,
        "unit": units,
        "floor": [str(x) for x in floors],
        "period": periods.strftime("%Y-%m-%d").tolist(),
        "step": np.arange(max(0, len(periods) - 1)),
        "feature": FEATURES,
    }
    with pm.Model(coords=coords):
        alpha = pm.Normal("alpha", np.log(5000), 1)
        sigma_building = pm.HalfNormal("sigma_building", 0.5)
        building_z = pm.Normal("building_z", 0, 1, dims="building")
        building = pm.Deterministic(
            "building_offset",
            (building_z - pt.mean(building_z)) * sigma_building,
            dims="building",
        )
        sigma_unit = pm.HalfNormal("sigma_unit", 0.4)
        unit_z = pm.Normal("unit_z", 0, 1, dims="unit")
        unit = pm.Deterministic("unit_effect", unit_z * sigma_unit, dims="unit")
        sigma_floor = pm.HalfNormal("sigma_floor", 0.15)
        floor_z = pm.Normal("floor_z", 0, 1, dims="floor")
        floor = pm.Deterministic(
            "floor_effect", (floor_z - pt.mean(floor_z)) * sigma_floor, dims="floor"
        )
        sigma_rw = pm.HalfNormal("sigma_rw", 0.08 if frequency == "monthly" else 0.04)
        step_z = pm.Normal("step_z", 0, 1, dims="step")
        trend = pm.Deterministic(
            "trend",
            pt.concatenate([pt.zeros(1), pt.cumsum(step_z * sigma_rw)]),
            dims="period",
        )
        beta = pm.Normal(
            "beta",
            mu=[0.15, 0.1, 0.3, 0, 0, 0],
            sigma=[0.3, 0.3, 0.3, 0.3, 0.3, 0.3],
            dims="feature",
        )
        mu = (
            alpha
            + trend[train.period_idx.to_numpy()]
            + building[train.building_idx.to_numpy()]
            + unit[train.unit_idx.to_numpy()]
            + floor[train.floor_idx.to_numpy()]
        ...
        pm.StudentT(
            ...
            mu=mu,
            sigma=sigma,
            observed=train.log_rent.to_numpy(dtype=float),
        )
        return pm.sample(
            draws=draws,
            tune=tune,
            ...
            backend=backend,
            compile_kwargs=(
                {"gradient_backend": gradient_backend} if gradient_backend else {}
            ),
            target_accept=0.95,
            random_seed=150130,
            progressbar=False,
...
def predict(inference, data):
    ...
    return (
        samples(p, "alpha")[:, None]
        + samples(p, "trend")[:, data.period_idx.to_numpy()]
...
def save_outputs(inference, data, periods, frequency, output, validation=None):
    ...
    counts = data.groupby("period").agg(
        observations=("unit_key", "size"), units=("unit_key", "nunique")
    )
    chart = pd.DataFrame(
        {
            "period": periods,
            "index_median": np.median(index, axis=0),
            "index_lower": np.quantile(index, 0.025, axis=0),
            "index_upper": np.quantile(index, 0.975, axis=0),
        }
    )
        ...
        rows.append(
            {
                ...
                "building_name": data.attrs["names"].get(building, building),
                **interval(100 * np.expm1(offsets[:, i])),
                "units": group.unit_key.nunique(),
    ...
    pd.DataFrame(
        [
            {"term": name, **interval(100 * np.expm1(beta[:, i]))}
    ...
    print(
        json.dumps({**metadata["coverage"], **diagnostic_summary(inference)}, indent=2),
        flush=True,
    )
...
def main():
    ...
    parser.add_argument(
        "--start", help="Optional ISO start date; default uses all history"
    )
    ...
    parser.add_argument(
        "--validate-from", help="Also refit withholding prices from this date onward"
    )
    parser.add_argument(
        "--validation-draws",
        type=int,
        help="Override posterior draws for the withheld-price fit",
    )
    parser.add_argument(
        "--validation-tune",
        type=int,
        help="Override tuning draws for the withheld-price fit",
    )
    ...
    if args.validate_from:
        ...
        if not train_mask.any() or train_mask.all():
            raise ValueError(
                "Validation split needs both training and withheld periods"
            )
        ...
        validation = {
            "from": args.validate_from,
            "observations": len(held),
            "previously_seen_unit_observations": int(seen.sum()),
            "draws_per_chain": validation_draws,
            "tune_per_chain": validation_tune,
            "chains": args.chains,
            "median_absolute_percent_error": float(
                np.median(np.abs(predictions / held.asking_rent.to_numpy() - 1)) * 100
            ),
            "seen_unit_model_median_absolute_percent_error": float(
                np.median(
                    np.abs(predictions[seen] / held.asking_rent.to_numpy()[seen] - 1)
                )
                * 100
            )
            if seen.any()
            else None,
            "seen_unit_last_rent_median_absolute_percent_error": float(
                np.median(
                    np.abs(
                        baseline.to_numpy()[seen] / held.asking_rent.to_numpy()[seen]
                        - 1
                    )
                )
                * 100
