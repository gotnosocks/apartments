...
v2 = previous.v2
load_data, graph_kwargs, validate_posterior = previous.load_data, previous.graph_kwargs, previous.validate_posterior
residual_scale_summary, floor_contrasts = previous.residual_scale_summary, previous.floor_contrasts
...
def attributes(args, data):
    ...
    return attribute.attribute_units(data, captures), digest(Path(args.evidence)/'complete.json')
...
def make_protocol(args, data, source, code, configuration, units, evidence_sha):
    ...
    result.update(version=VERSION, feature_design_version=attribute.VERSION,
        attribute_policy=attribute.policy(), evidence_manifest_sha256=evidence_sha,
...
def run(args):
    ...
    with (root/'.run.lock').open('a') as lock:
        ...
        protocol = make_protocol(args, data, source, code, configuration, units, evidence_sha)
        ...
        design = attribute.FeatureDesign(data, args.spec, floor_prior_scale=args.floor_prior_scale,
                                         attribute_units=units, evidence_manifest_sha256=evidence_sha)
        design.save(target)
