...
def main():
    ...
    with (root / 'run.lock').open('a') as lock:
        ...
        started = time.time()
        state = {'started_at': started, 'root': str(root), 'workers': args.workers, 'completed_shards': 0}
        ...
        try:
            ...
            with concurrent.futures.ProcessPoolExecutor(max_workers=args.workers, mp_context=multiprocessing.get_context('spawn')) as pool:
                futures = [pool.submit(process_shard, args.snapshot, root, i, args.bodies) for i in range(plan['shards'])]
            ...
            progress('finalizing')
            report = finish(root)
            progress('complete', counts=report['tables']['counts'],
