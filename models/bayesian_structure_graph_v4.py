"""Structure graph v4: v3 plus a per-building bedroom slope (research screening).

v3 is frozen by the running combined fit's hash. ``building_bedroom_slope=True``
adds s_b * (min(bedrooms, 4) - 1), s_b = tau * z_b, z_b ~ N(0, 1),
tau ~ HalfNormal(``bedroom_slope_prior``): how much steeper or flatter the
studio-to-4BR price step is in each building than the Chelsea-wide bedroom
premiums. Suggested by the from-scratch model session's ablations. With
default arguments v4 builds exactly the v3 graph.
"""
...
VERSION = "bayesian-structure-graph-v4"
