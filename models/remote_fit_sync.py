...
def code_files(repo):
    ...
    repo = Path(repo)
    listed = subprocess.run(['git', 'ls-files', '-z', '-co', '--exclude-standard', '--', *CODE_PATHS],
                            cwd=repo, check=True, capture_output=True).stdout.decode().split('\0')
    result = {}
...
def git_state(repo):
    ...
    return {'head': run('rev-parse', 'HEAD'),
            'dirty_paths': [line[3:] for line in run('status', '--porcelain', '--', *CODE_PATHS).splitlines()]}


def build_request(*, repo, dataset, runner, runner_args, run_id, inputs=(), returned=RETURNED,
                  keep_auxiliary=False, resources=None, extra_code=()):
    ...
    if not re.fullmatch(r'models\.[A-Za-z0-9_]+', runner):
        raise ValueError('Runner must be a module under models, e.g. models.bayesian_floor_spline_experiment')
    if any(a in ('--dataset', '--output') or a.startswith(('--dataset=', '--output=')) for a in runner_args):
    ...
    extra = {relative_posix(name): Path(path) for name, path in dict(extra_code).items()}
    ...
    for root in dict.fromkeys([dataset, *(Path(p).resolve() for p in inputs)]):
        if not root.is_absolute() or root == Path('/') or Path(CODE_ROOT) in (root, *root.parents):
    ...
    for name in returned:
        relative_posix(name)
    request = {'version': REQUEST_VERSION, 'run_id': safe_id(run_id),
               'created_at': datetime.now(timezone.utc).isoformat(),
               'runner': runner, 'runner_args': list(runner_args), 'dataset': dataset.as_posix(),
               'code_root': CODE_ROOT, 'git': git_state(repo), 'extra_code': sorted(extra),
               'returned': list(returned), 'keep_auxiliary': bool(keep_auxiliary),
               'resources': resources or {}, 'files': files}
    ...
    for item in request['files']:
        ...
        if target.exists():
            if digest(target) != item['sha256']:
                raise ValueError(f'Worker path already holds different bytes: {destination}')
            continue
...
def execute(request, *, blob_root, run_root, scratch, filesystem_root='/', python=None, commit=None):
    ...
    if output_root.exists():
        raise ValueError('Remote run output already exists; outputs are never overwritten')
    ...
    command = [python or sys.executable, '-m', request['runner'],
               '--dataset', str(local(request['dataset'])), '--output', str(fit_output), *request['runner_args']]
    ...
    env = {**os.environ, 'PYTHONPATH': os.pathsep.join([str(code/'src'), str(code)]),
    ...
    result = {'version': RESULT_VERSION, 'run_id': request['run_id'], 'returncode': returncode,
              'succeeded': returncode == 0 and bool(inventory),
              'command': command[1:], 'materialized_bytes': written, 'timings': timings,
              'runner_peak_rss_mib': peak_rss_mib,
              'returned_bytes': sum(item['size'] for item in inventory), 'files': inventory,
              'scratch_bytes': sum(p.stat().st_size for p in fit_output.rglob('*') if p.is_file())
                               if fit_output.exists() else 0,
    ...
    for marker in sorted(item['path'] for item in downloaded if PurePosixPath(item['path']).name == 'complete.json'):
        ...
        for name, expected in manifest.get('files', {}).items():
            ...
            if path not in hashes:
                raise ValueError(f'Bundle {folder} lists a file that was not returned: {name}')
...
def modal_worker(run_id, volume_name):
    ...
    with tempfile.TemporaryDirectory(prefix='fit-') as scratch:
        return execute(request, blob_root=root/'blobs', run_root=run_root, scratch=Path(scratch),
                       python=python, commit=volume.commit)
