...
def load_experiment(root):
    ...
    results = json.loads(sf["results.json"])
    cm, cf = _verified_bundle(
        root / "standard/comparison", retain={"fitted-values.jsonl"}
    )
    if cm["protocol_sha256"] != sm["protocol_sha256"]:
...
def run(structured, corroborated, interior, output):
    ...
    if (
        sp["dataset_manifest"]["interior_manifest"]
        != cp["dataset_manifest"]["interior_manifest"]
    ):
        raise ValueError("Source policies do not share interior inputs")
    if (
        cp["dataset_manifest"].get("structured_projection_manifest")
        != sp["dataset_manifest"]
    ):
        raise ValueError(
            "Corroboration policy must derive from this structured dataset"
        )
    sby = {r["audit_id"]: r for r in srows}
    cby = {r["audit_id"]: r for r in crows}
    if set(sby) != set(cby) or len(sby) != len(srows) or len(cby) != len(crows):
    ...
    for key in sby:
        if any(
            ...
            for f in (
                ...
                "building",
                "period",
                "shared_outdoor_category",
                "advertised_ceiling_feet",
                "advertised_levels",
            )
        ):
            raise ValueError(
                "Source-policy comparison changed targets or nonprivate inputs"
            )
    reference, _ = _verified_bundle(Path(interior) / "summary")
    ipm, ipf = _verified_bundle(Path(interior) / "protocol", retain={"protocol.json"})
    if (
        ipm["protocol_sha256"] != reference["protocol_sha256"]
        or json.loads(ipf["protocol.json"])["dataset_manifest"]
        != sp["dataset_manifest"]["interior_manifest"]
    ):
        raise ValueError("Reference interior source dataset differs")
    parity = {}
    for name in ("standard", "weaker_groups", "stronger_groups"):
        im, inf = _verified_bundle(
            Path(interior) / name / "values", retain={"log-fitted.json"}
        )
        if im["protocol_sha256"] != reference["protocol_sha256"]:
            raise ValueError("Prior interior protocol mismatch")
        old = np.exp(np.array(json.loads(inf["log-fitted.json"])))
        for label, root in [("structured", structured), ("corroborated", corroborated)]:
            _, f = _verified_bundle(
                Path(root) / name / "interior", retain={"log-fitted.json"}
            )
            new = np.exp(np.array(json.loads(f["log-fitted.json"])))
            if len(new) != len(old):
                raise ValueError("Reference row count differs")
            error = float(max(abs(new - old)))
            if error > 0.02:
                raise ValueError("Reference model differs beyond numerical tolerance")
            ...
            comparison.append(
                {
                    ...
                    "triplex_contrast_by_variant": {
                        v: next(
                            c["rent_percent_change"]
                            for c in d["interior_contrasts"]
                            if c["field"] == "advertised_levels"
                        )
                        for v, d in item["results"].items()
                    },
                    "outdoor_contrasts": values["outdoor_contrasts"],
    ...
    for key, r in sby.items():
        ...
        diagnostics.append(
            ...
            | {
                "structured_private_category": r["private_outdoor_category"],
                "corroborated_private_category": other["private_outdoor_category"],
                "interior_fitted_rent": r["interior_fitted_rent"],
                "structured_fitted_rent": r["types_fitted_rent"],
                "corroborated_fitted_rent": other["types_fitted_rent"],
                "corroborated_minus_structured_dollars": other["types_fitted_rent"]
                - r["types_fitted_rent"],
                "structured_residual_dollars": r["asking_rent"]
                - r["types_fitted_rent"],
                "corroborated_residual_dollars": r["asking_rent"]
                - other["types_fitted_rent"],
            }
        )
    current = [
        r
        for r in diagnostics
        if r["analysis_price_basis"] == "current_capture_gross_ask"
    ]
    selected = []
    units = set()
    for r in sorted(
        diagnostics,
        key=lambda r: (-abs(r["corroborated_minus_structured_dollars"]), r["audit_id"]),
    ):
    ...
    cross_policy = {
        ...
        "median_absolute_fitted_change_dollars": float(np.median(changes)),
        "maximum_absolute_fitted_change_dollars": float(max(changes)),
    }
    lines = [
        "# Outdoor source-policy sensitivity",
        "",
        f"Both policies retain the same {len(diagnostics):,} observations and price targets. Eighteen fits compare interior-only, outdoor reporting, and outdoor-type contrasts under three group penalties.",
        "",
    ...
    for r in comparison:
        ...
        numbers = " | ".join(
            f"{r['metrics'][v]['log_rmse']:.6f}"
            for v in ("interior", "reporting", "types")
        )
        effects = [
            r["triplex_contrast_by_variant"]["types"],
            outdoor[0]["rent_percent_change"],
            outdoor[1]["rent_percent_change"],
        ]
        effect_text = " | ".join(
            "unsupported" if v is None else f"{v:+.3f}%" for v in effects
        )
        lines.append(f"| {r['policy']} | {r['settings']} | {numbers} | {effect_text} |")
    lines += [
        "",
        f"At standard penalties, switching source policy changes fitted rent by a median absolute ${cross_policy['median_absolute_fitted_change_dollars']:.2f}; the maximum is ${cross_policy['maximum_absolute_fitted_change_dollars']:,.2f}.",
    ...
    for r in current:
        lines.append(
            f"| {r['source_listing_id']} | ${r['asking_rent']:,.0f} | ${r['interior_fitted_rent']:,.0f} | ${r['structured_fitted_rent']:,.0f} | ${r['corroborated_fitted_rent']:,.0f} | {r['corroborated_residual_dollars']:+,.0f} |"
        )
    lines += [
        "",
        "These are in-sample residual diagnostics. Review the largest policy-driven changes against source evidence before promoting an encoding. The main reviewed analysis model remains unchanged.",
    ]
    return publish_bundle(
        ...
        {
            "version": "outdoor-policy-comparison-v1",
            "structured_summary": sm,
            "corroborated_summary": cm,
            "interior_summary": reference,
...
if __name__ == "__main__":
    ...
    for key in ("structured", "corroborated", "interior", "output"):
