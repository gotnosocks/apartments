...
def build(dataset):
    args = spline.argument_parser().parse_args(
        ["--dataset", str(dataset), "--output", "/dev/null"]
    )
    data, _ = spline.load_data(dataset)
    design = floor.FeatureDesign(
        data, args.spec, floor_prior_scale=args.floor_prior_scale
    )
    return spline.graph.build_model(
        data, design, args.prior_multiplier, **spline.graph_kwargs(args)
    )
...
def points(model, count=3):
    ...
    result = []
    for scale in (0, 0.025, 0.1)[:count]:
        point = {
            k: np.asarray(initial[k]) + rng.normal(0, scale, np.shape(initial[k]))
            for k in names
        }
...
def numba_probe(model, x, processes, seconds):
    ...
    for count in processes:
        queue = context.Queue()
        workers = [
            context.Process(target=numba_worker, args=(function, x[0], seconds, queue))
        ...
        scaling.append(
            {
                "processes": count,
                "seconds_per_gradient_per_chain": per_chain,
...
def jax_probe(dataset, batches, seconds, output):
    ...
    logp = get_jaxified_logp(
        model
    )  # PyMC's flag is inverted: the default returns +logp
    ...
    np.savez(
        output,
        logp=np.asarray([v[0] for v in values], np.float64),
...
def errors(reference_logp, reference_grad, path):
    other = np.load(path)
    grad_scale = np.maximum(np.abs(reference_grad), 1.0)
    return {
        "max_abs_logp_error": float(np.max(np.abs(other["logp"] - reference_logp))),
        "max_relative_logp_error": float(
            np.max(np.abs(other["logp"] - reference_logp) / np.abs(reference_logp))
        ),
        "max_scaled_gradient_error": float(
            np.max(np.abs(other["grad"] - reference_grad) / grad_scale)
        ),
    }
...
def run(args):
    ...
    result = {
        "version": "spline-gradient-hardware-probe-v1",
        "hardware": hardware(),
    ...
    if args.processes:
        ...
        reference = (
            np.asarray([v[0] for v in values]),
            np.asarray([v[1] for v in values]),
        )
    ...
    for precision in args.jax_precisions:
        ...
        if completed.returncode:
            ...
            continue
        result[f"jax{precision}"] = json.loads(
    ...
    if (output / "jax64.npz").exists() and (output / "jax32.npz").exists():
        ...
        result["jax32_vs_jax64"] = errors(
            wide["logp"], wide["grad"], output / "jax32.npz"
        )
...
def argument_parser():
    ...
    parser.add_argument(
        "--jax-precisions", type=int, nargs="*", default=[], choices=(32, 64)
    )
...
if __name__ == "__main__":
    ...
    if arguments.jax_child:
        summary = jax_probe(
            arguments.dataset, arguments.batches, arguments.seconds, arguments.jax_child
        )
