...
def vector(design, frame, level, access=None):
    ...
    if REFERENCE_FLOOR not in levels or level not in levels:
        raise ValueError(
            "Floor curve requires supported endpoints; extrapolation refused"
        )
    if access is not None and type(access) is not bool:
        raise ValueError("Known elevator state required")
    if level == REFERENCE_FLOOR:
        return np.zeros(len(design.features))
    ...
    if hasattr(design, "base"):
        raise ValueError("Interaction curve requires explicit elevator state")
    result = np.zeros(len(design.features))
    for threshold in design.floor_thresholds:
        result[design.features.index(floor._name(threshold))] = float(
            level > threshold
        ) - float(REFERENCE_FLOOR > threshold)
    return result


def curve(fit, access=None):
    design = fit["design"]
    beta = elevator_comparison.beta_draws(fit)
    if REFERENCE_FLOOR not in design.floor_levels:
        raise ValueError("Reference floor 2 absent")
    definitions = [
        {
            "id": f"floor:{REFERENCE_FLOOR:g}->{level:g}:access={access}",
            "floor": level,
            "reference_floor": REFERENCE_FLOOR,
            "elevator": access,
            "design_vector": vector(design, fit["data"], level, access).tolist(),
        }
        for level in design.floor_levels
        if level != REFERENCE_FLOOR
    ]
    calculated, _ = categories.calculate(beta, definitions)
    calculated.append(
        {
            "id": f"floor:2->2:access={access}",
            "floor": REFERENCE_FLOOR,
            "reference_floor": REFERENCE_FLOOR,
            "elevator": access,
            "design_vector": [0.0] * len(design.features),
            "log_effect": {"median": 0.0, "lower_95": 0.0, "upper_95": 0.0},
            "percent_effect": {"median": 0.0, "lower_95": 0.0, "upper_95": 0.0},
            "status": "deterministic_reference",
            "diagnostics": {
                "deterministic": True,
                "reason": "Identical endpoint contrast; no sampling ESS or R-hat claimed.",
            },
        }
    )
    for row in calculated:
        row["normal_prior_log_sd"] = float(
            np.linalg.norm(np.asarray(row["design_vector"]) * design.prior_scales)
            * fit["protocol"]["prior_multiplier"]
        )
    ...
    access_values = (
        np.array(
            [
                v[1]
                for v in elevator_comparison.contract.source_values(
                    fit["data"].to_dict("records")
                )
            ],
            dtype=object,
        )
    ...
    for row in calculated:
        ...
        if access is not None:
            mask &= access_values == int(access)
            baseline_mask &= access_values == int(access)
        row["endpoint_rows_at_access"] = int(mask.sum())
        row["reference_rows_at_access"] = int(baseline_mask.sum())
        row["both_endpoints_observed_at_access"] = bool(
            mask.any() and baseline_mask.any()
        )
        supported = row["both_endpoints_observed_at_access"]
        row["source_support_status"] = (
            "both_endpoints_observed"
            if supported
            else "withheld_missing_joint_floor_access_endpoint"
        )
        row["source_support_reason"] = (
            None
            if supported
            else "No fitted observations at the reference or target floor with this known elevator state."
        )
        row["display_percent_effect"] = row["percent_effect"] if supported else None
    return {
        "elevator": access,
        "points": sorted(calculated, key=lambda r: r["floor"]),
        "withheld_points": sum(
            r["status"] == "withheld_derived_diagnostics" for r in calculated
        ),
        "source_withheld_points": sum(
            not r["both_endpoints_observed_at_access"] for r in calculated
        ),
        "reference_floor": REFERENCE_FLOOR,
    }
...
def source_counts(frame):
    ...
    proxy = np.array(
        [
            r.get("floor_label_provenance", {}).get("status") == "label_proxy"
            for r in records
        ]
    )
    ...
    for level in np.unique(values[known]):
        ...
        result.append(
            {
                "floor": float(level),
                "explicit_rows": int((mask & ~proxy).sum()),
                "label_proxy_rows": int((mask & proxy).sum()),
                "units": int(frame.loc[mask, "unit_id"].nunique()),
...
def load_fit(root, dataset, rows):
    ...
    protocol = json.loads(
        shared.common.bound_bytes(
            root / "protocol", "protocol.json", provenance["protocol_manifest"]
        )
    )
    ...
    return {
        ...
        "data": frame,
        "design": shared.load_design(root / "fit", frame, protocol),
    }
...
def build(
    ...
    before, after = label_comparison.verify_revision(
        reference_dataset, candidate_dataset
    )
    third_manifest, third_files = _verified_bundle(
        interaction_dataset, retain={"observations.jsonl"}
    )
    second_manifest, _ = _verified_bundle(candidate_dataset, retain=set())
    if (
        third_manifest != second_manifest
        or shared.report.jsonl(third_files["observations.jsonl"]) != after
    ):
        raise ValueError("Interaction fit must use the exact expanded-floor source")
    fits = [
        load_fit(root, data, rows)
        for root, data, rows in [
            (reference, reference_dataset, before),
            (candidate, candidate_dataset, after),
            (interaction_fit, interaction_dataset, after),
        ]
    ]
    a, b, c = fits
    label_comparison.check_protocols(a["protocol"], b["protocol"])
    label_comparison.check_designs(a, b)
    elevator_comparison.check_protocols(b["protocol"], c["protocol"])
    elevator_comparison.check_designs(b, c)
    result = {
        "version": VERSION,
        "reference_floor": REFERENCE_FLOOR,
        "main_selection_changed": False,
        "curves": {
            "explicit_source_baseline": curve(a),
            "expanded_label_floor": curve(b),
            "expanded_with_elevator": curve(c, True),
            "expanded_without_elevator": curve(c, False),
        },
        "source_counts": {
            "baseline": source_counts(a["data"]),
            "expanded": source_counts(b["data"]),
        },
        "fits": [
            {
                "root": str(f["root"]),
    ...
    for f in fits:
        if (
            digest(f["root"] / "fit/posterior.nc")
            != f["provenance"]["fit_manifest"]["files"]["posterior.nc"]
        ):
...
def render(result):
    ...
    with plt.rc_context(
        ...
        counts = fig.add_subplot(grid[1, :])

        def draw(ax, key, label, color, levels=None, connect=True):
            points = [
                r
                for r in result["curves"][key]["points"]
                if levels is None or r["floor"] in levels
            ]
            x = [r["floor"] for r in points]
            ys = [
                [
                    r["display_percent_effect"][k]
                    if r["display_percent_effect"] is not None
                    else np.nan
                    for r in points
                ]
            ...
            if connect:
                ...
                ax.fill_between(x, ys[1], ys[2], alpha=0.15, color=color)
            else:
                low = np.asarray(ys[0]) - np.asarray(ys[1])
                high = np.asarray(ys[2]) - np.asarray(ys[0])
                ax.errorbar(
                    x,
                    ys[0],
                    yerr=[low, high],
        ...
        draw(
            left,
            "explicit_source_baseline",
            "Explicit-source floor baseline",
            "#6c757d",
            connect=False,
        )
        draw(
            left, "expanded_label_floor", "Expanded with unit-label proxies", "#1368aa"
        )
            ...
            *[
                {
                    r["floor"]
                    for r in result["curves"][key]["points"]
                    if r["both_endpoints_observed_at_access"]
                }
                for key in ("expanded_without_elevator", "expanded_with_elevator")
            ]
        )
        draw(
            right,
            "expanded_without_elevator",
            "Known no elevator",
            "#bd5c22",
            common_levels,
        )
        draw(
            right, "expanded_with_elevator", "Known elevator", "#2a8060", common_levels
        )
        for ax, title in [
            (left, "Adding advertised unit-label floors"),
            (right, "Floor + elevator: shared floor support"),
        ]:
            ax.axhline(0, color="#555555", linewidth=0.7)
            ax.set(
                title=title,
                xlabel="Supported listed-floor label",
                ylabel="Floor component vs floor 2 (%)",
            )
            ax.legend(fontsize=9)
            ax.grid(alpha=0.15)
        levels = result["source_counts"]["expanded"]["levels"]
        x = [r["floor"] for r in levels]
        explicit = [r["explicit_rows"] for r in levels]
        proxy = [r["label_proxy_rows"] for r in levels]
        ...
        unsupported = sum(
            v["source_withheld_points"] for v in result["curves"].values()
        )
        fig.suptitle(
            "Conditional floor contributions • joint posterior pointwise 95% intervals\n"
            f"Not physical height • Withheld: {withheld} sampling diagnostics; {unsupported} missing floor/access support",
            fontsize=13,
        )
        payload = {}
...
def run(output, **kwargs):
    ...
    if any(digest(p) != hashes[p.name] for p in paths):
        raise ValueError("Plot implementation changed during analysis")
    files.update(
        {
    ...
    publisher._publish(
        ...
        {
            "version": VERSION,
            "fits": [f["bindings"] for f in result["fits"]],
