...
def load(fit):
    ...
    units = (
        effects[effects.kind.eq("unit")]
        .set_index("id")
        .log_effect.map(lambda v: v["median"])
    )
    residuals["deviation"] = residuals.residual_log + residuals.unit_id.map(
        units
    ).fillna(0.0)
    coefficients = pd.DataFrame(
        json.loads((fit / "fit/coefficients.json").read_text())
    ).set_index("feature")
...
def main():
    ...
    if (
        base_protocol["source_observations_sha256"]
        != cand_protocol["source_observations_sha256"]
    ):
    ...
    frame = base[
        ...
        cand[["audit_id", "fitted_rent", "deviation", "residual_log"]],
        on="audit_id",
    ...
    frame["group"] = pd.Categorical.from_codes(
        graph.bedroom_groups(frame.bedrooms), graph.GROUP_LABELS
    )
    frame["year"] = pd.to_datetime(frame.period).dt.year
    cells = frame.groupby(["year", "group"], observed=True).agg(
        rows=("audit_id", "size"),
        base=("deviation_base", "mean"),
        candidate=("deviation_cand", "mean"),
    )
    table = (cells[["base", "candidate"]] * 100).round(2).unstack("group")
    weighted_abs = {
        k: float((cells[k].abs() * cells.rows).sum() / cells.rows.sum() * 100)
        for k in ("base", "candidate")
    }
    current = frame[frame.analysis_price_basis.eq("current_capture_gross_ask")]
    current_change = (
        current.assign(
            change=100 * (current.fitted_rent_cand / current.fitted_rent_base - 1)
        )
        .groupby("group", observed=True)
        .change.describe()[["count", "mean", "min", "max"]]
    )
    joined = base_coef[["median"]].join(
        cand_coef[["median"]], lsuffix="_base", rsuffix="_cand"
    )
    ...
    params = {
        label: scalars(fit, names)
        for label, fit in (("base", args.baseline), ("candidate", args.candidate))
    }
    result = {
        ...
        "median_abs_residual_log": {
            "base": float(frame.residual_log_base.abs().median()),
            "candidate": float(frame.residual_log_cand.abs().median()),
        },
        "parameters": params,
        "largest_coefficient_shifts": json.loads(
            joined.reindex(joined["shift"].abs().sort_values(ascending=False).index)
            .head(12)
            .round(4)
            .to_json(orient="index")
        ),
        "current_fitted_rent_change_percent": json.loads(
            current_change.round(2).to_json(orient="index")
        ),
    }
