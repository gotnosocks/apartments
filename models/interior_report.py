...
def run(experiment, dataset, parent_residuals, output):
    ...
    inputs={r['audit_id']:r for r in (json.loads(s) for s in df['observations.jsonl'].decode().split('\n') if s)}
    rm,rf=_verified_bundle(parent_residuals,retain={'residuals.jsonl'})
    if rm['dataset_manifest']!=protocol['dataset_manifest']['parent_dataset_manifest']:
        raise ValueError('Parent residuals do not bind the matched cohort')
    parent={r['audit_id']:r for r in (json.loads(s) for s in rf['residuals.jsonl'].decode().split('\n') if s)}
    ...
    parity=max(abs(r['baseline_fitted_rent']-parent[r['audit_id']]['fitted_rent']) for r in rows)
    ...
    enriched=[]
    for r in rows:
        enriched.append({**r,'values_minus_reporting_dollars':r['values_fitted_rent']-r['reporting_fitted_rent'],
            'baseline_residual_dollars':r['asking_rent']-r['baseline_fitted_rent'],
            'reporting_residual_dollars':r['asking_rent']-r['reporting_fitted_rent'],
            'values_residual_dollars':r['asking_rent']-r['values_fitted_rent']})
    changed=[];seen=set()
    for r in sorted(enriched,key=lambda r:(-abs(r['values_minus_reporting_dollars']),r['audit_id'])):
        if r['unit_id'] in seen:continue
        seen.add(r['unit_id']);changed.append(r)
        if len(changed)==20:break
    current=[r for r in enriched if r['analysis_price_basis']=='current_capture_gross_ask']
    ...
    frame=pd.DataFrame(source_rows);frame['period']=pd.to_datetime(frame.period)
    enc=fitted['encoder'];matrix=enc.matrix(frame);a,b=enc.offsets['interior']
    terms=matrix[:,a:b].toarray()*fitted['beta'][a:b]
    for r,values in zip(selected.values(),terms):
        r['interior_log_contributions']=dict(zip(enc.interior_features,values.tolist()))
        r['interpretation']='Diagnostic in-sample residual; reported claims and group effects enter the fit. Direct terms are not the whole refit change.'
    stability={field:[] for field in ('advertised_ceiling_feet','advertised_levels')}
    for summary in summaries:
        for contrast in summary['results']['values']['contrasts']:
            stability[contrast['field']].append({'settings':summary['settings'],'rent_percent_change':contrast['rent_percent_change']})
    support=protocol['dataset_manifest']['support']
    lines=['# Advertised interior features: matched descriptive experiment','',
        ...
        '|---|---:|---:|---:|---:|---:|']
    for summary in summaries:
        results=summary['results'];contrasts=results['values']['contrasts']
        lines.append(f"| {summary['settings']} | "+' | '.join(f"{results[v]['metrics']['log_rmse']:.6f}" for v in ('baseline','reporting','values'))+
    ...
    lines+=['',f"Adding known values beyond reporting changes fitted rent by a median absolute ${comparison['median_absolute_fitted_change_dollars']:.2f}; the largest change is ${comparison['maximum_absolute_fitted_change_dollars']:,.2f}.",
    ...
    for field,v in support.items():
        lines.append(f"| {field} | {v['known_rows']} | {v['known_units']} | {v['known_buildings']} | {v['buildings_with_known_value_variation']} |")
    lines+=['', 'Ceiling height means an unambiguous advertised height somewhere in the home; it can be room-specific. It is not an independently measured or uniform height.',
        ...
        '|---|---:|---:|---:|---:|---:|']
    for r in current:
        lines.append(f"| {r['source_listing_id']} | ${r['asking_rent']:,.0f} | ${r['baseline_fitted_rent']:,.0f} | ${r['reporting_fitted_rent']:,.0f} | ${r['values_fitted_rent']:,.0f} | ${r['values_residual_dollars']:+,.0f} |")
    lines+=['',f"The recomputed baseline matches the prior scientific fit within ${parity:.6f} across all {len(rows):,} rows. All saved fits enforce convergence and exact coefficient-reload parity.",
    ...
    return publish_bundle(output,{'report.md':'\n'.join(lines)+'\n','contrast-stability.json':canonical(stability)+'\n',
        ...
        'baseline-parity.json':canonical({'rows':len(rows),'maximum_absolute_dollars':parity})+'\n',
        'report.py':Path(__file__).read_text()},
        {'version':'advertised-interior-experiment-report-v1','summary_manifest':sm,'parent_residual_manifest':rm,
         'protocol_sha256':pm['protocol_sha256'],'implementation_sha256':digest(__file__)})


if __name__=='__main__':
    parser=argparse.ArgumentParser(description=__doc__)
    for key in ('experiment','dataset','parent-residuals','output'):parser.add_argument('--'+key,type=Path,required=True)
    args=parser.parse_args();run(args.experiment,args.dataset,args.parent_residuals,args.output)
