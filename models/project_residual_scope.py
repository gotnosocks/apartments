...
def run(dataset, decisions, output):
    ...
    parent_hash, decisions_hash = (
        digest(dataset / "complete.json"),
        digest(decisions / "complete.json"),
    )
    ...
    if (
        parent["version"] != contract.PARENT
        or dm["version"] != contract.DECISION_VERSION
    ...
    current = lambda rows: [
        r for r in rows if r["analysis_price_basis"] == "current_capture_gross_ask"
    ]
    ...
    summary = {
        ...
        "units": len({r["unit_id"] for r in kept}),
        "buildings": len({r["building"] for r in kept}),
        "current_rows": len(current(kept)),
    ...
    metadata = {
        ...
        "decisions_manifest_sha256": decisions_hash,
        "source_rows": len(before),
        "reviewed_at": dm["reviewed_at"],
        "decision_ids": sorted(d["decision_id"] for d in edits),
        "summary": summary,
    ...
    if {
        "parent-summary.json",
        contract.SIDECAR,
        "residual-scope-decisions.jsonl",
    } & products.keys():
    ...
    products.update(
        {
            ...
            "parent-summary.json": files["summary.json"].decode(),
    ...
    prospective = {
        **metadata,
        "files": {
            k: hashlib.sha256(v.encode()).hexdigest() for k, v in products.items()
        },
    }
    ...
    source_lineage(
        prospective,
        kept,
        residual_scope_changes=excluded,
    ...
    if (
        digest(dataset / "complete.json") != parent_hash
        or digest(decisions / "complete.json") != decisions_hash
    ):
