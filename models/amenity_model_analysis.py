...
def category_contrast(fitted, field, before, after, *, baseline_rent=4000.0):
    ...
    positions = [
        start + enc.amenity_features.index(f"{field}={level}")
        for level in (before, after)
    ]
    delta = float(fitted["beta"][positions[1]] - fitted["beta"][positions[0]])
    return {
        "log_rent_difference": delta,
        "percent_difference": float(100 * np.expm1(delta)),
...
def analyze(dataset, experiment, output, *, year=2024):
    ...
    for field, before, after in CONTRASTS:
        ...
        for level in (before, after):
            ...
            support[level] = {
                "unit_months": len(matching),
                "units": len({r["unit_id"] for r in matching}),
                "buildings": len(buildings[level]),
            }
        effects = {}
        ...
        contrasts.append(
            {
                ...
                "after": after,
                "support": support,
                "buildings_observed_in_both_categories": len(
                    buildings[before] & buildings[after]
                ),
                "effects": effects,
            }
        )
    report = {
        "training_end_exclusive": f"{year}-01-01",
        "contrasts": contrasts,
        "limitations": [
    ...
    return publish_bundle(
        output,
        {"contrasts.json": canonical(report) + "\n"},
        {
            "version": "chelsea-supported-contrasts-v1",
            "experiment_protocol_sha256": digest(experiment / "protocol.json"),
            "fit_manifest_sha256": hashes,
            "implementation_sha256": digest(__file__),
            "runtime_versions": {
                p: importlib.metadata.version(p)
                for p in ("numpy", "pandas", "scipy", "duckdb")
            },
        },
    )
...
if __name__ == "__main__":
    ...
    print(
        json.dumps(
            analyze(args.dataset, args.experiment, args.output, year=args.year),
            indent=2,
        )
    )
