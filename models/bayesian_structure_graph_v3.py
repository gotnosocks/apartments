"""Structure graph v3: v2 plus sampler-geometry options (iteration-speed E1).

v2 is frozen by a running protocol fit's hash, so geometry experiments live
here. With default arguments v3 builds exactly the v2 graph.

* ``unit_centering="building"``: unit effects sum to zero within each
  building (sigma_unit * (z_u - mean of z in its building)); the building
  effect carries each building's level alone.
* ``intercept="building_mean"``: building effects are drawn around the
  intercept, b_j ~ N(alpha, sigma_building), instead of zero-sum effects added
  to alpha (``building_level`` is the Deterministic b_j - alpha, comparable to
  ``building_effect`` in the default graph).
"""
...
VERSION = "bayesian-structure-graph-v3"
