...
def split(data, fraction, seed):
    ...
    keep[data[counts.ge(2)].groupby("unit_id").sample(1, random_state=seed).index] = (
        True
    )
...
def subset(train, test, fraction, by, seed):
    ...
    chosen = set(
        np.random.default_rng(seed).choice(
            ids, size=int(round(fraction * len(ids))), replace=False
        )
    )
    return (
        train[train[column].isin(chosen)].reset_index(drop=True),
...
def map_posterior(model, seed, maxeval, fixed=None):
    ...
    if fixed:
        model = pm.do(
            model,
            {
                model[name]: np.asarray(value, dtype=float)
    ...
    for rv in original.free_RVs + original.deterministics:
        ...
        variables[name] = xr.DataArray(
            value[None, None], dims=("chain", "draw", *dims), coords=coords
        )
...
def pseudo_groups(data, seed):
    ...
    first = (
        data.drop_duplicates("unit_id")
        .set_index("unit_id")
        .bedrooms.reindex(units)
        .to_numpy()
    )
    ...
    frame = pd.DataFrame(
        {
            "lpd": lpd,
            "error": error,
            "group": graph.bedroom_groups(test.bedrooms),
            "year": test.period.dt.year.to_numpy(),
        }
    )
    frame["era"] = pd.cut(
        frame.year,
        [e[0] - 1 for e in ERAS] + [ERAS[-1][1]],
        labels=[f"{a}-{b}" for a, b in ERAS],
    )
    cells = (
        frame.groupby(["group", "era"], observed=True)
        .agg(rows=("lpd", "size"), mean_error=("error", "mean"), lpd=("lpd", "sum"))
        .reset_index()
    )
    cells["group"] = cells.group.map(dict(enumerate(graph.GROUP_LABELS)))
    return {
        "rows": len(frame),
        "elpd": float(frame.lpd.sum()),
        "elpd_se": float(frame.lpd.std() * math.sqrt(len(frame))),
        "rmse_log": float(np.sqrt((frame.error**2).mean())),
        "mae_log": float(frame.error.abs().mean()),
...
def main():
    ...
    parser.add_argument("--walk-prior-scale", type=float, default=0.05)
    parser.add_argument("--linear-prior-scale", type=float, default=0.02)
    parser.add_argument(
    ...
    if args.subset_fraction < 1:
        ...
        train, test = subset(
            train, test, args.subset_fraction, args.subset_by, args.subset_seed
        )
        design = floor.FeatureDesign(train, "full_half_balance", floor_prior_scale=0.10)
        values = floor.listed_floor_values(test)
        supported = ~np.isfinite(values) | (
        ...
        in_horizon = test.period.between(
    ...
    if group_column == "bedrooms":
        model = graph.build_model(
            ...
            design,
            mode=args.mode,
            walk_prior_scale=args.walk_prior_scale,
    ...
    if args.method == "map":
        ...
        if args.fix_scales_from:
            ...
            fixed = {
                n: saved[n]["mean"]
                for n in ("sigma", "sigma_unit", "sigma_building", "trend_scale")
            }
    ...
    result = {
        "mode": args.mode,
        "method": args.method,
        "negative_control_seed": args.negative_control_seed,
    ...
    if args.mode != "none":
        ...
        result["curves"] = {
            g: {
                design.time.periods[i].strftime("%Y"): {
                    "median": float(curve.sel(bedroom_group=g).isel(period=i).median()),
                    "lower_95": float(
                        curve.sel(bedroom_group=g).isel(period=i).quantile(0.025)
                    ),
                    "upper_95": float(
                        curve.sel(bedroom_group=g).isel(period=i).quantile(0.975)
                    ),
                }
                for i in jan
            }
    ...
    print(
        json.dumps(
            ...
            | {
                ...
                "rmse_log": result["heldout"]["rmse_log"],
            }
        )
    )
