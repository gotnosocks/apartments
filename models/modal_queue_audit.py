...
def grouped_summary(groups):
    ...
    return {
        ...
        "urls": sum(map(len, duplicates)),
        "extra_urls": sum(len(rows) - 1 for rows in duplicates),
        "pending_urls": sum(
            state == "pending" for rows in duplicates for url, state in rows
        ),
...
def unit_url_audit(db, generation):
    rows = db.execute(
        """SELECT f.url,f.state FROM frontier f JOIN scope_urls s USING(generation,url)
                         WHERE f.generation=? AND f.kind='listing'""",
        (generation,),
    ).fetchall()
    ...
    latest = db.execute(
        """SELECT url,body_hash,max(id) FROM snapshots WHERE generation=?
                           GROUP BY url""",
        (generation,),
    ).fetchall()
    ...
    return {
        ...
        "total": len(rows),
        "states": dict(states),
        "path_states": dict(paths),
        "query_states": dict(queries),
...
def audit_or_migrate(
    workspace_id="chelsea-resume", apply=False, expected_retirements=-1
):
    ...
    if not locks.put(
        "writer",
        {
            ...
            "action": "queue-audit-migration",
            "started_at_epoch": time.time(),
        },
        skip_if_exists=True,
    ):
        raise RuntimeError(
            "Archive writer active; stop the batch and await its commit first"
        )
    committed = False
    store = None
    try:
        ...
        if apply and preview["superseded"] != expected_retirements:
            raise ValueError(
                "Migration differs from reviewed dry run; refusing to apply"
            )
        report = {
            "at": time.time(),
        ...
        report["scope_before"] = dict(
            store.db.execute(
                """SELECT f.state,count(*) FROM frontier f
            JOIN scope_urls s USING(generation,url) WHERE f.generation=? GROUP BY f.state""",
                (generation,),
            )
        )
        if apply:
            report["applied"] = store.deduplicate_building_views(generation)
        report["scope_after"] = dict(
            store.db.execute(
                """SELECT f.state,count(*) FROM frontier f
            JOIN scope_urls s USING(generation,url) WHERE f.generation=? GROUP BY f.state""",
                (generation,),
        ...
        (
            destination
            / ("queue-alias-migration.json" if apply else "queue-alias-preview.json")
...
def main(apply: bool = False, expected_retirements: int = -1):
    result = audit_or_migrate.remote(
        apply=apply, expected_retirements=expected_retirements
    )
    destination = Path(
        "/tmp/chelsea-queue-alias-migration.json"
        if apply
        else "/tmp/chelsea-queue-alias-preview.json"
    )
    destination.write_text(json.dumps(result, indent=2))
    print("Saved " + str(destination))
    print(
        json.dumps(
            {key: value for key, value in result.items() if key != "units"}, indent=2
        )
    )
