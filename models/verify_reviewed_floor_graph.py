...
import numpy as np
from models import bayesian_feature_model as reference, bayesian_feature_graph_v3 as compressed, bayesian_floor_increment_design as feature
...
design = feature.FeatureDesign(data, 'full_half_balance', floor_increment_prior_scale=.15)
...
result={'version':'bayesian-floor-graph-parity-v1','rows':len(data),
 ...
 'source_observations_sha256':protocol['source_observations_sha256'],'floor_increment_prior_scale':design.floor_increment_prior_scale,'floor_levels':design.floor_levels,'implementation_sha256':{p.name:digest(p) for p in paths}}
