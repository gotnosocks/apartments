...
import numpy as np
from models import (
    bayesian_feature_model as reference,
    bayesian_feature_graph_v3 as compressed,
    bayesian_floor_increment_design as feature,
)
...
paths = [*runner.implementation_paths(), Path(feature.__file__)]
hashes = {str(p): digest(p) for p in paths}
design = feature.FeatureDesign(
    data, "full_half_balance", floor_increment_prior_scale=0.15
)
...
for name, module in [("reference", reference), ("compressed", compressed)]:
    ...
    for target in ("logp", "dlogp"):
        print(
            canonical({"phase": "compiling", "graph": name, "target": target}),
            flush=True,
        )
        started = time.perf_counter()
        functions[name][target] = getattr(models[name], "compile_" + target)(
            mode="NUMBA"
        )
...
assert [str(rv.owner.op) for rv in old.free_RVs] == [
    str(rv.owner.op) for rv in new.free_RVs
]
...
points = [
    ...
    *[
        {
            key: np.asarray(value) + rng.normal(0, scale, np.shape(value))
...
point_hashes = [
    hashlib.sha256(
        canonical({k: np.asarray(v).tolist() for k, v in p.items()}).encode()
    ).hexdigest()
    for p in points
]
...
for name in functions:
    for target, fn in functions[name].items():
        print(
            canonical({"phase": "first_invocation", "graph": name, "target": target}),
            flush=True,
        )
        started = time.perf_counter()
        value = fn(points[0])
        timings[name][target + "_first_invocation_seconds"] = (
...
for i, point in enumerate(points):
    ...
    results.append(
        {
            "point": i,
            "point_sha256": point_hashes[i],
            "reference_log_density": float(a["logp"]),
            "compressed_log_density": float(b["logp"]),
            "absolute_log_density_difference": float(abs(a["logp"] - b["logp"])),
            "gradient_parameters": int(a["dlogp"].size),
            "maximum_absolute_gradient_difference": float(absolute.max()),
            "maximum_scaled_gradient_difference": float(
                np.max(absolute / np.maximum(1.0, np.abs(a["dlogp"])))
            ),
            "all_finite": True,
        }
    )
...
result = {
    ...
    "units": int(data.unit_id.nunique()),
    "specification": design.spec,
    "graph_configuration": new.graph_configuration,
    "feature_columns": len(design.features),
    "points": results,
    "timings": timings,
    "compression": new.compression_summary,
    "criterion": {
        "logp_rtol": 1e-11,
        "logp_atol": 1e-6,
        "gradient_rtol": 1e-10,
...
for r in results:
    lines.append(
        f"| {r['point']} | {r['absolute_log_density_difference']:.10g} | {r['maximum_absolute_gradient_difference']:.10g} |"
    )
lines += [
    "",
    "| Graph | Logp warm median (ms) | Gradient warm median (ms) |",
    "| --- | ---: | ---: |",
]
for name, t in timings.items():
    lines.append(
        f"| {name} | {1000 * t['logp_warm_median_seconds']:.4f} | {1000 * t['dlogp_warm_median_seconds']:.4f} |"
    )
lines += [
    "",
    result["timing_notes"],
    "",
    "Full timings, criterion, parameter-point hashes, source/design/code hashes and dependency versions are in `parity.json`. These checks establish numerical equivalence at tested points, not sampler convergence or coefficient validity.",
    "",
]
output = args.output
publish_bundle(
