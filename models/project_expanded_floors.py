...
from apartments import corrections, expanded_floor_projection as contract, floor_label_projection as original
...
REASON = ('The floor-3 assertion describes reference photography of a different apartment, '
...
REVIEW_PATH = Path(__file__).resolve().parents[1]/'docs/analysis/chelsea-expanded-label-rule-conflicts-2026-09-19.md'
...
def mask_evidence(row, captures, reviewed_at):
    if sha(row) != MASKS[row['source_listing_id']] or row.get('advertised_floor') != 3 or row.get('listed_floor') is not None:
    ...
    for capture in captures:
        ...
        if (not isinstance(text, str) or text.count(PHRASE) != 1
        ...
        result.append({**deepcopy(capture), 'spans': [
...
def run(dataset, evidence, ledger, reviewed_at, output):
    ...
    retained = {'observations.jsonl', original.SIDECAR, 'elevator-corrections.jsonl',
                'quarantined.jsonl', 'current-source-evidence.jsonl'}
    parent, files = _verified_bundle(dataset, retain=retained)
    rows, old_changes = records(files['observations.jsonl']), records(files[original.SIDECAR])
    original.parent_rows(parent, rows, old_changes)
    mapping = load_evidence(dataset, evidence)
    selected = {r['source_listing_id']: r for r in rows if r['source_listing_id'] in MASKS}
    if selected.keys() != MASKS.keys():
        raise ValueError('Reviewed correction membership differs')
    prepared = {ad: mask_evidence(row, mapping[row['audit_id']], reviewed_at)
    ...
    if (any(e['target'].get('version_id') not in expected for e in existing)
            or len({e['target']['version_id'] for e in existing}) != len(existing)):
        raise ValueError('Correction ledger contains unrelated or repeated active versions')
    for ad, row in sorted(selected.items()):
        edit = {'target': {'source': 'streeteasy', 'source_listing_id': ad, 'version_id': sha(row)},
            'validity': {'all_time': True}, 'patch': [
                {'op': 'test', 'path': '/advertised_floor', 'value': 3},
                {'op': 'replace', 'path': '/advertised_floor', 'value': None}]}
        previous = next((e for e in existing if e['target'].get('version_id') == sha(row)), None)
        if previous:
            if any(previous[k] != edit[k] for k in edit):
                raise ValueError('Existing floor correction differs from reviewed patch')
        else:
            if (output/'complete.json').exists():
                raise ValueError('Published output cannot add new correction records')
            corrections.append(ledger, author='codex', reason=REASON, edit=edit,
                evidence=[str(REVIEW_PATH), 'review_document_sha256:'+review_hash,
                          'source_manifest_sha256:'+SOURCE, PHRASE])
    prior = json.loads((output/'complete.json').read_text()) if (output/'complete.json').exists() else None
    ...
    masks = []
    for ad, row in sorted(selected.items()):
        updated, applied = overlay.apply(row, {'source': 'streeteasy',
            'source_listing_id': ad, 'version_id': sha(row)})
        expected_row = deepcopy(row); expected_row['advertised_floor'] = None
        if updated != expected_row or len(applied) != 1:
            raise ValueError('Correction overlay changed an unreviewed field')
        masks.append({'audit_id': row['audit_id'], 'source_row_sha256': sha(row),
            'reason': REASON, 'reviewed_at': reviewed_at, 'before_advertised_floor': 3,
            'correction_record': active[sha(row)], 'captures': prepared[ad]})
    policy = {'excluded_buildings': parent['excluded_buildings'],
        'building_floor_evidence': parent['building_floor_evidence'],
        'parent_interpreted_at': parent['interpreted_at'],
        'excluded_units': [conflict[0]['unit_id']], 'floor_masks': masks}
    revised, changes = [], []
    ...
    def floor(row):
        return row.get('listed_floor') if row.get('listed_floor') is not None else row.get('advertised_floor')
    allowed = {'listed_floor', 'advertised_floor', 'expanded_floor_provenance'}
    if any({k: v for k, v in a.items() if k not in allowed} !=
           {k: v for k, v in b.items() if k not in allowed}
           for a, b in zip(rows, revised, strict=True)):
        raise ValueError('Expanded projection changed prices, identity or unrelated fields')
    new = [b for a, b in zip(rows, revised, strict=True) if floor(a) is None and floor(b) is not None]
    summary = {'rows': len(rows), 'units': len({r['unit_id'] for r in rows}),
        'buildings': len({r['building'] for r in rows}), 'known_before': sum(floor(r) is not None for r in rows),
        'known_after': sum(floor(r) is not None for r in revised), 'newly_known_rows': len(new),
        'known_after_percent': 100*sum(floor(r) is not None for r in revised)/len(revised),
        'known_units_after': len({r['unit_id'] for r in revised if floor(r) is not None}),
        'current_rows': sum(r['analysis_price_basis']=='current_capture_gross_ask' for r in rows),
        'current_known_after': sum(r['analysis_price_basis']=='current_capture_gross_ask' and floor(r) is not None for r in revised),
        'corrected_advertisements': sorted(MASKS),
        'changed_floor_values': sum(floor(a)!=floor(b) for a,b in zip(rows,revised,strict=True)),
        'newly_known_by_rule': dict(Counter(r['expanded_floor_provenance']['rule'] for r in new)),
        'statuses': dict(Counter(r['expanded_floor_provenance']['status'] for r in revised)),
    ...
    outputs = {name: content.decode() for name, content in files.items() if name != 'observations.jsonl'}
    outputs.update({'observations.jsonl': ''.join(canonical(r)+'\n' for r in revised),
        contract.SIDECAR: ''.join(canonical(c)+'\n' for c in changes),
        'expanded-floor-policy.json': canonical(policy)+'\n',
        'expanded-floor-corrections.jsonl': ''.join(canonical(c)+'\n' for c in overlay.records),
    ...
    metadata = {'version': contract.VERSION, 'source_manifest': parent,
        'source_manifest_sha256': SOURCE, 'source_rows': len(rows), 'interpreted_at': as_of,
        'policy': policy, 'policy_sha256': sha(policy), 'overlay': overlay.manifest, 'evidence_manifest_sha256': digest(evidence/'complete.json'),
        'review_document_sha256': review_hash, 'summary': summary}
    # Validate before publishing; readers verify the same forward/inverse contract.
    preview = {**metadata, 'files': {name: hashlib.sha256(text.encode()).hexdigest() for name,text in outputs.items()}}
