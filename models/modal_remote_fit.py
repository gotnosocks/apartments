...
def image(repo=REPO, jax=False):
    ...
    if jax:
        ...
        result = result.run_commands(
            f"/.uv/uv pip install --python /.uv/.venv/bin/python {quoted}"
        )
...
GPU_USD_PER_SECOND = {
    ...
    "H200": 0.001261,
    "B200": 0.001736,
}  # modal.com/pricing, 2026-09-22


def estimate_usd(result, resources):
    seconds = result["timings"]["worker_seconds"]
    gpu = GPU_USD_PER_SECOND.get((resources.get("gpu") or "").upper(), 0.0)
    return seconds * (
        resources["cpu"] * USD_PER_CORE_SECOND
        + resources["memory_mib"] / 1024 * USD_PER_GIB_SECOND
        + gpu
    )
...
def submit(args):
    ...
    resources = {
        ...
        "ephemeral_disk_mib": args.ephemeral_disk,
        "gpu": args.gpu,
        "jax_stack": JAX_STACK if jax else None,
    }
    extra_code = {
        Path(path).resolve().relative_to(REPO).as_posix(): path
        for path in args.add_code
    }
    ...
    with store.batch_upload(force=True) as batch:
        ...
        batch.put_file(
            io.BytesIO(json.dumps(request, indent=2).encode()),
            f"/runs/{run_id}/request.json",
        )
    total = {item["sha256"]: item["size"] for item in request["files"]}
    transfer = {
        "input_files": len(request["files"]),
        "input_blobs": len(total),
        "input_bytes": sum(total.values()),
        "uploaded_blobs": len(missing),
        "uploaded_bytes": sum(missing.values()),
        "hash_seconds": hashing,
    ...
    worker = app.function(
        image=image(args.code_root, jax),
        cpu=args.cpu,
        memory=args.memory,
        timeout=args.timeout,
        **options,
        volumes={sync.VOLUME_MOUNT: store},
        retries=0,
        max_containers=1,
                ...
                json.dumps(
                    {
                        ...
                        "fetch": f"python -m models.modal_remote_fit fetch --run-id {run_id} --output {args.output}",
                    },
                    indent=2,
                )
            )
    ...
    fetch_run(
        ...
        include_auxiliary=args.keep_auxiliary,
        full=args.full,
    )


def fetch_run(
    store, run_id, destination, remote_seconds=None, include_auxiliary=False, full=False
):
    ...
    if not result["succeeded"]:
        ...
        (partial / "remote-runner.log").write_bytes(
            b"".join(store.read_file(f"/runs/{run_id}/runner.log"))
        )
        raise SystemExit(
            f"Remote runner failed (exit {result['returncode']}); log in {partial / 'remote-runner.log'}"
        )
    include = [*request["returned"], *(sync.AUXILIARY if include_auxiliary else ())]
    started = time.monotonic()
    downloaded = sync.download(
        result,
        lambda path: store.read_file(f"/runs/{run_id}/output/{path}"),
        destination,
        include=include,
    ...
    record = (
        json.loads(submitted.read_text()) if submitted.exists() else {"run_id": run_id}
    )
    record.update(
        ...
        json.dumps(
            {
                ...
                for k in (
                    ...
                    "download",
                    "remote_call_seconds",
                    "estimated_worker_usd",
                )
            }
            | {"timings": result["timings"], "output": str(destination)},
            indent=2,
        )
    )
    ...
    if args.unreferenced_blobs:
        ...
        for entry in store.listdir("/runs"):
            try:
                referenced |= {
                    item["sha256"]
                    for item in read_json(store, f"{entry.path}/request.json")["files"]
                }
            except (FileNotFoundError, NotFoundError):
                raise SystemExit(
                    f"{entry.path} has no request; refusing to prune blobs"
                )
...
def argument_parser():
    ...
    run = commands.add_parser(
        "run", help="Upload changed inputs, fit remotely, download results"
    )
    run.add_argument("--dataset", type=Path, required=True)
    run.add_argument(
        "--output", type=Path, required=True, help="New local experiment directory"
    )
    ...
    run.add_argument(
        "--timeout", type=int, default=6 * 3600, help="Seconds (Modal maximum 86400)"
    )
    run.add_argument(
        "--ephemeral-disk",
        type=int,
    ...
    run.add_argument(
        "--gpu", help="Modal GPU type, e.g. A100-80GB or H100; implies --jax"
    )
    run.add_argument(
        "--jax", action="store_true", help="Add the pinned JAX/NumPyro/BlackJAX layer"
    )
    run.add_argument(
        "--add-code",
        action="append",
    ...
    run.add_argument(
        "--detach", action="store_true", help="Submit and return; fetch later"
    )
    run.add_argument(
        "--full",
        action="store_true",
    ...
    run.add_argument(
        ...
        help="Arguments after -- go to the runner",
    )
    fetch = commands.add_parser(
        "fetch", help="Download a finished run (resumes partial downloads)"
    )
    fetch.add_argument("--run-id", required=True)
    fetch.add_argument("--output", type=Path, required=True)
    fetch.add_argument("--include-auxiliary", action="store_true")
    fetch.add_argument(
        "--full", action="store_true", help="Also download posterior.nc/prior.nc"
    )
    finish = commands.add_parser(
        "complete", help="Download the posterior files a summary fetch left remote"
    )
