...
LABELS = {
    "annual_frozen": "Annual frozen",
    "monthly": "Monthly refit",
...
def render(experiment, output):
    ...
    report = json.loads(sf["report.json"])
    ...
    if [r["month"] for r in months] != [
        s["month"][:7] for s in protocol["splits"]
    ...
    for month in months:
        manifest, files = _verified_bundle(
            root / month["month"] / "forecast", retain={"report.json"}
        )
        if (
            manifest.get("protocol_sha256") != ph
            or json.loads(files["report.json"]) != month
        ):
            raise ValueError("Monthly report provenance mismatch")
        chain.append(hashlib.sha256(canonical(manifest).encode()).hexdigest())
    if hashlib.sha256(canonical(chain).encode()).hexdigest() != sm.get(
    ...
    figure = make_subplots(
        ...
        subplot_titles=(
            ...
            "Nominal 95% band: observed coverage",
    ...
    for policy, label in LABELS.items():
        ...
        series = [
            ([v["median_absolute_percent_error"] for v in values], "Median error (%)"),
            (
                [v["intervals"]["95"].get("coverage_percent") for v in values],
                "Coverage (%)",
            ),
            (
                [
                    v["intervals"]["95"].get("median_width_percent_of_prediction")
                    for v in values
                ],
                "Width (%)",
            ),
    ...
    rows = []
    for name, group in [
        ("All months", report["pooled"]),
        *report["years"].items(),
        *report["strata"].items(),
    ]:
        for policy, label in LABELS.items():
            m = group["policies"][policy]
            interval = m["intervals"]["95"]
            fmt = lambda value: "Unavailable" if value is None else f"{value:.2f}%"
            display_name = {
                "seen_unit": "Seen units",
                "new_unit_seen_building": "New units, seen buildings",
                "new_building": "New buildings",
            }.get(name, name)
            rows.append(
                [
                    display_name,
                    label,
                    str(group["rows"]),
                    fmt(m["median_absolute_percent_error"]),
                    fmt(m["median_signed_percent_error"]),
                    fmt(interval.get("coverage_percent")),
                    str(interval["available_rows"]),
                    str(interval.get("pooled_fallback_rows", 0)),
                    fmt(interval.get("median_width_percent_of_prediction")),
                ]
            )
    table = (
        "<table><thead><tr>"
        + "".join("<th>" + html.escape(x) + "</th>" for x in header)
        + "</tr></thead><tbody>"
    )
    table += (
        "".join(
            "<tr>" + "".join("<td>" + html.escape(x) + "</td>" for x in row) + "</tr>"
            for row in rows
        )
        + "</tbody></table>"
    )
...
and month-specific calibration rules are in the accompanying JSON report.</p></html>"""
    return publish_bundle(
        ...
        {
            "report.html": body,
            "report.json": sf["report.json"].decode(),
            "figure.json": figure.to_json(),
            "driver.py": Path(__file__).read_text(),
        },
        {
            "version": "monthly-validation-report-v1",
            "protocol_sha256": ph,
