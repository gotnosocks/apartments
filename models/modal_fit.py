...
PACKAGES = [
    'pymc==6.2.0', 'pytensor==3.2.3', 'nutpie==0.16.11',
    'arviz==1.2.0', 'numpy==2.4.6', 'pandas==3.0.5', 'pyarrow==24.0.0',
    'h5netcdf==1.8.1', 'h5py==3.16.0', 'duckdb==1.5.5', 'scipy==1.18.0',
]
base_image = modal.Image.debian_slim(python_version='3.12').pip_install(*PACKAGES)

def add_source(image):
    return (image.add_local_file(MODEL_FILE, '/root/rent_model.py')
            .add_local_file(Path(__file__), '/root/modal_fit.py'))

cpu_image = add_source(base_image.env({'PYTENSOR_FLAGS': 'floatX=float64,cxx=',
                                      'OMP_NUM_THREADS': '1', 'OPENBLAS_NUM_THREADS': '1'}))
gpu_image = add_source(base_image.pip_install('jax[cuda13]==0.11.1').env({
    'JAX_ENABLE_X64': 'true', 'JAX_PLATFORMS': 'cuda',
    'XLA_PYTHON_CLIENT_PREALLOCATE': 'false', 'PYTENSOR_FLAGS': 'floatX=float64,cxx=',
    'OMP_NUM_THREADS': '1', 'OPENBLAS_NUM_THREADS': '1'}))
...
def run_fit(snapshot, run_id, settings, backend):
    ...
    print(json.dumps({'devices': devices, 'backend': backend, 'snapshot': snapshot}), flush=True)
    ...
    periods = pd.date_range(data.period.min(), data.period.max(),
    ...
    with tempfile.TemporaryDirectory() as folder:
        ...
        result['execution'] = {
            ...
            'input_sha256': input_digest, 'model_sha256': digest(MODEL_FILE),
            'source_metadata_sha256': digest(source/'metadata.json'),
            'input_snapshot_metadata': metadata.get('snapshot', {}),
            'sampler': 'nutpie', 'backend': backend, 'gradient_backend': 'pytensor',
            'float_precision': 64, 'devices': devices, 'settings': settings,
            'sampling_compile_seconds': sampling_seconds, 'worker_seconds': elapsed,
            'min_ess_bulk_per_sampling_second': result['min_ess_bulk']/sampling_seconds,
            'estimated_worker_compute_usd': elapsed*(4*.0000131+8*.00000222+(.000164 if backend=='jax' else 0)),
        ...
        volume.commit()
    return {'volume': 'chelsea-archive', 'path': str(destination.relative_to('/archive')),
            'files': sorted(path.name for path in destination.iterdir()),
            'execution': result['execution'], 'max_rhat': result['max_rhat'],
            'min_ess_bulk': result['min_ess_bulk'], 'divergences': result['divergences']}


@app.function(image=cpu_image, cpu=(4,4), memory=(8192,8192), timeout=1800,
...
def run_gpu(snapshot, run_id, settings):
    ...
    worker = gpu_app.function(image=gpu_image, gpu='T4', cpu=(4,4),
        memory=(8192,8192), timeout=1800, max_containers=1, retries=0,
        volumes={'/archive': volume}, include_source=False)(fit_gpu)
    with gpu_app.run():
        return worker.remote(snapshot, run_id, settings)


@app.local_entrypoint()
def main(snapshot: str = 'chelsea-20260908', run_id: str = '', gpu: bool = False,
         output: str = '', draws: int = 1000, tune: int = 1000, chains: int = 4):
    ...
    if destination and destination.exists():
        raise ValueError('Output must be a new directory; existing fits are never overwritten')
    settings = dict(draws=draws, tune=tune, chains=chains)
    result = (run_gpu(snapshot, run_id, settings) if gpu
