...
def export(dataset, experiment, output, *, prediction_month):
    ...
    at = robust_pricing.month(prediction_month)
    ...
    if (
        protocol.get("version") != "chelsea-sequential-monthly-v1"
        or pm.get("protocol_sha256") != protocol_hash
    ):
    ...
    for module in (amenities, baseline, ablation, contrasts, pricing):
        if (
            digest(module.__file__)
            != protocol["implementation_sha256"][Path(module.__file__).name]
        ):
    ...
    split = next(
        (s for s in protocol["splits"] if s["month"] == at.strftime("%Y-%m-%d")), None
    )
    if split is None:
        raise ValueError("Month absent from completed experiment")
    fm, ff = _verified_bundle(
        root / name / "fit", retain={"models.json", "diagnostics.json"}
    )
    if fm.get("protocol_sha256") != protocol_hash or any(
        fm.get(k) != v for k, v in split.items()
    ):
    ...
    naive = pd.Timestamp(at.date())
    train = data[data.period < naive]
    test = data[data.period.eq(naive)]
    if (
        contrasts.membership(train) != split["train_sha256"]
    ...
    artifact = {
        ...
        "coefficients": saved["beta"],
        "center": saved["center"],
        "training": {
            "start_month": str(train.period.min().date()),
            "end_month": str(train.period.max().date()),
            "rows": len(train),
            "units": int(train.unit_id.nunique()),
            "buildings": int(train.building.nunique()),
            "knowledge_cutoff": source.get("as_of"),
            "latest_evidence_known_at": str(train.known_at.max())
            if "known_at" in train
            else None,
            "unit_buildings": dict(
                train[["unit_id", "building"]]
                .drop_duplicates()
                .itertuples(index=False, name=None)
            ),
            "source_listing_ids": sorted(set(train.source_listing_id.astype(str))),
            "source_manifest": source,
            "membership_sha256": split["train_sha256"],
        },
        "validation": {
            "experiment_protocol_sha256": protocol_hash,
            "fit_manifest": fm,
            "prediction_month": name,
            "mode": "retrospective development; later-collected attributes",
            "max_serving_horizon_months": 1,
        },
        "limitations": [
            "Market associations and renter willingness to pay are separate.",
    ...
    fitted = {
        "encoder": encoder,
        "beta": np.asarray(saved["beta"]),
        "center": saved["center"],
    }
    expected = np.exp(baseline.predict(fitted, test))
    actual = np.array(
        [
            portable.predict(row, name + "-01")["predicted_rent"]
    ...
    if not np.allclose(actual, expected, rtol=1e-11, atol=1e-8):
        raise ValueError(
            "Portable encoder predictions differ from scientific implementation"
        )
    parity = {
        "rows": len(test),
        "maximum_absolute_dollar_difference": float(np.max(abs(actual - expected))),
        "maximum_relative_difference": relative,
        "test_membership_sha256": split["test_sha256"],
    }
