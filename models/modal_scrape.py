    ...
    with log.open("wb") as output:
        with subprocess.Popen(
            ...
            reader = threading.Thread(
                target=forward, args=(process.stdout, output), daemon=True
            )
            reader.start()
            try:
...
def clone_snapshot(base, workspace, snapshot):
    ...
    if workspace.exists():
        raise ValueError(
            "Incomplete workspace exists; inspect it or choose a new workspace ID"
        )
    ...
    marker.write_text(
        json.dumps({"snapshot": snapshot, "created_at_epoch": time.time()})
    )
...
def resume(
    ...
    if not locks.put(
        "writer",
        {"owner": owner, "workspace": workspace_id, "started_at_epoch": time.time()},
        skip_if_exists=True,
    ):
    ...
    try:
        ...
        volume.commit()
        print(
            f"Starting crawl batch: budget={max_requests}, concurrency={concurrency}, api_rps={api_rps}",
            flush=True,
        )
        with sqlite3.connect(workspace / "archive.sqlite3") as before_db:
            done_before = before_db.execute(
                "SELECT count(*) FROM frontier f JOIN scope_urls s USING(generation,url) WHERE f.generation=(SELECT max(id) FROM generations) AND f.state='done'"
            ).fetchone()[0]
        before_db.close()
        ...
        with sqlite3.connect(workspace / "archive.sqlite3") as db:
            ...
            scoped = dict(
                db.execute(
                    "SELECT f.state,count(*) FROM frontier f JOIN scope_urls s USING(generation,url) WHERE f.generation=? GROUP BY f.state",
                    (generation,),
                )
            )
            state = db.execute(
                "SELECT status,cooldown FROM generations WHERE id=?", (generation,)
            ).fetchone()
        db.close()
        result = {
...
def continuation(result):
    ...
    if result["exit_code"] != 0 and not (
        result["exit_code"] == 124 and result.get("made_progress")
    ):
...
def publish_workspace(
    workspace, destination, snapshots, new_snapshot, seed_database=False
):
    ...
    if seed_database and (
        not seed.exists()
        or not (original / "prepared/metadata.json").exists()
        or Path(str(seed) + ".wal").exists()
    ):
        raise ValueError(
            "Seed database must have a completed preparation and no active WAL"
        )
    ...
    provenance = {
        ...
        "parent_snapshot": origin["snapshot"],
        "created_at_epoch": time.time(),
        "source_run": run,
        "seeded_derived_database": seed_database,
        "body_root": "/bodies",
    }
...
def publish_snapshot(workspace_id: str, new_snapshot: str, seed_database: bool = False):
    ...
    try:
        ...
        result = publish_workspace(
            Path("/archive/crawls") / workspace_id,
            root / new_snapshot,
            root,
            new_snapshot,
            seed_database,
...
def main(
    ...
    safe_id(workspace)
    if action == "finish":
        result = finish_backfill.remote(
            snapshot, workspace, total_budget, batch_size, concurrency, api_rps
        )
...
def submit_backfill(
    snapshot="chelsea-20260908", workspace="chelsea-resume", total_budget=1000
):
    ...
    if latest and runs.get(latest, {}).get("status") == "running":
        raise RuntimeError(
            "Controller reports running; verify its state before replacement"
        )
    if runs.get("stop-after-batch:" + workspace, False):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "action", choices=["setup-secret", "show-lock", "clear-lock", "submit"]
    )
    ...
    if args.action == "setup-secret":
        ...
        if not args.owner:
            parser.error(
                "--owner is required; first stop the worker and inspect show-lock"
            )
