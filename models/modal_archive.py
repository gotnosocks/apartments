...
image = (modal.Image.debian_slim(python_version='3.12')
         .pip_install('pymc==6.2.0', 'pytensor==3.2.3', 'nutpie==0.16.11',
                      'arviz==1.2.0', 'numpy==2.4.6', 'pandas==3.0.5',
                      'pyarrow==24.0.0', 'h5netcdf==1.8.1', 'h5py==3.16.0',
                      'duckdb==1.5.5', 'scipy==1.18.0', 'beautifulsoup4==4.14.3',
                      'parsel==1.10.0')
         .env({'PYTHONPATH':'/root/src:/root/models', 'APARTMENTS_DB_MEMORY_LIMIT':'2GB',
               'OMP_NUM_THREADS':'2', 'OPENBLAS_NUM_THREADS':'2', 'PYTENSOR_FLAGS':'cxx='})
...
def safe_id(value):
    ...
    if not re.fullmatch(r'[a-zA-Z0-9_-]+', value):
        raise ValueError('Snapshot IDs may contain letters, digits, dash and underscore only')
    return value
...
def upload_files(snapshot, archive, database):
    ...
    with (archive/'crawler.lock').open('a') as lock:
        ...
        if not backup.exists() and 'sqlite' not in ledger:
            source = sqlite3.connect((archive/'archive.sqlite3').resolve().as_uri()+'?mode=ro', uri=True)
            partial = backup.with_suffix('.partial')
            partial.unlink(missing_ok=True)
            target = sqlite3.connect(partial)
            source.execute('PRAGMA cache_size=-8192')
            source.backup(target, pages=256, sleep=.05)
        ...
        if database and database.exists():
            if Path(str(database)+'.wal').exists():
                raise RuntimeError('Database writer/WAL is active; finish its import before uploading')
        ...
        complete.write_text(json.dumps({'snapshot':snapshot, 'created_at_epoch':time.time(),
                                       'files':ledger, 'body_files':len(shared)}))
        with volume.batch_upload(force=True) as batch:
...
def prepare(snapshot: str):
    ...
    counts = import_archive(base, base/'apartments.duckdb', body_root=Path('/archive/bodies'))
    if counts['failed']:
        raise RuntimeError(f"Archive import failed for {counts['failed']} captures; rerun after fixing errors")
    volume.commit()
    ...
    building_ids = set(); totals = collections.defaultdict(lambda: [0,0])
    ...
    with (result/'buildings.jsonl').open('w') as output:
        for url, raw in rows:
            data = json.loads(raw); slug = url.rsplit('/',1)[-1]
            ...
            if not primary or primary.get('id') in building_ids:
                continue
            refs = {}; stream = flight_text(data.get('scripts',[]))
            ...
            if isinstance(area,str) and area.startswith('$'): area=refs.get(area[1:])
            if not isinstance(area,dict) or not ({area.get('id'),area.get('slug')} & {'chelsea','west-chelsea'}):
                continue
            building_ids.add(primary['id'])
            entry = {k:primary.get(k) for k in ['id','slug','name','type','residentialUnitCount']}
            entry.update(url=url,area=area)
            output.write(json.dumps(entry)+'\n')
            totals[str(entry['type'])][0] += 1
            totals[str(entry['type'])][1] += entry['residentialUnitCount'] or 0
    queue = dict(source.execute('SELECT f.state,count(*) FROM frontier f JOIN scope_urls s USING(generation,url) WHERE f.generation=(SELECT max(id) FROM generations) GROUP BY f.state'))
    source.close()
    ...
    if data.duplicated(['unit_key','period']).any() or not np.isfinite(data[FEATURES+['log_rent']].to_numpy(dtype=float)).all():
        raise ValueError('Invalid model input')
    path = result/'training_data.parquet'; data.to_parquet(path,index=False)
    metadata = {'frequency':'monthly', 'coverage':data.attrs['coverage'],
                'excluded_furnished_units':data.attrs['excluded_furnished_units'],
                'size_log_scale':data.attrs['size_log_scale'], 'buildings':data.attrs['names'],
                'snapshot':{'id':snapshot,'training_sha256':digest(path),'scoped_queue':queue,
                            'discovered_queue_drained':not queue.get('pending',0) and not queue.get('inflight',0),
                            'coverage_is_exhaustive':False,
                            'building_totals':dict(totals),'import':counts,'worker_seconds':time.monotonic()-started,
