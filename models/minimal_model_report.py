...
def layout(fig, height=380):
    fig.update_layout(
        ...
        height=height,
        font={"family": "Arial, sans-serif", "size": 13, "color": BLUE},
        margin={"l": 55, "r": 25, "t": 35, "b": 48},
        paper_bgcolor="white",
        plot_bgcolor="white",
...
def report(root, output):
    ...
    c = results["coverage"]
    temporal = results["temporal_validation"]["scores"]
    cold = results["unseen_unit_validation"]["scores"]
    main = temporal["model"]
    index = pd.read_parquet(root / "rent_index.parquet")
    validation = pd.read_parquet(root / "temporal_validation.parquet")
    data = pd.read_parquet(root / "model_data.parquet")
    diagnostics = pd.read_parquet(root / "fit_diagnostics.parquet")
    encoder = json.loads((root / "encoder.json").read_text())
    with np.load(root / "model.npz", allow_pickle=False) as model:
        a, b = encoder["offsets"]["season"]
        season = model["beta"][a:b]
    season_pct = 100 * np.expm1(season - season.mean())
    indexed = index.set_index("period")
    yoy = 100 * (
        indexed.loc["2026-08-01", "smooth_index"]
        / indexed.loc["2025-08-01", "smooth_index"]
        - 1
    )
    pre = index[index.period.eq("2020-02-01")].smooth_index.iloc[0]
    trough = (
        index[index.period.between("2020-03-01", "2021-12-01")]
        .sort_values("smooth_index")
        .iloc[0]
    )
    decline = 100 * (trough.smooth_index / pre - 1)
    season_peak = int(np.argmax(season_pct))
    season_low = int(np.argmin(season_pct))
    month_names = [
        "Jan",
        "Feb",
        "Mar",
    ...
    fig.add_trace(
        go.Scatter(
            x=index.period,
            y=index.smooth_index,
            name="Adjusted trend, excluding seasonality",
    ...
    fig.add_trace(
        go.Scatter(
            x=index.period,
            y=index.index_with_seasonality,
            name="Adjusted trend + seasonality",
    ...
    layout(fig, 470)
    fig.update_yaxes(title_text="Index · Jan 2022 = 100", row=1, col=1)
    fig.update_yaxes(title_text="Count", row=2, col=1)
    trend_chart = fig.to_html(
        full_html=False,
    ...
    for name, color, mask in [
        ("Previously seen unit", TEAL, validation.seen_unit),
        ("New unit", ORANGE, ~validation.seen_unit),
    ]:
        part = validation[mask]
        fig.add_trace(
            go.Scatter(
                x=part.asking_rent,
                y=part.predicted_rent,
                mode="markers",
                name=name,
                marker={"size": 5, "color": color, "opacity": 0.40},
                hovertemplate="Asked $%{x:,.0f}<br>Predicted $%{y:,.0f}<extra>"
                + name
                + "</extra>",
            )
        )
    ...
    fig.update_xaxes(type="log", title="Actual initial asking rent ($/month)")
    fig.update_yaxes(type="log", title="Held-out prediction ($/month)")
    ...
    fig = go.Figure(
        go.Bar(
            x=month_names,
            y=season_pct,
            marker_color=[TEAL if x >= 0 else MUTED for x in season_pct],
        )
    )
    layout(fig, 300)
    fig.update_layout(showlegend=False)
    fig.update_yaxes(title="Difference from annual center (%)")
    season_chart = fig.to_html(
        full_html=False,
        include_plotlyjs=False,
        config={"displaylogo": False, "responsive": True},
    )
    metrics_rows = []
    for label, value in [
        ("Model · all 2026 test listings", main),
        ("Bedroom-only recent-rent baseline", temporal["bedroom_baseline"]),
        (
            "Model without unit effects",
            results["temporal_validation"]["without_unit_effect"],
        ),
        ...
        metrics_rows.append(
            [
                ...
                f"{value['median_absolute_percent_error']:.1f}%",
                f"{value['within_20_percent']:.1f}%",
            ]
        )
    error_by = []
    validation["absolute_percent_error"] = 100 * np.abs(
        validation.predicted_rent / validation.asking_rent - 1
    )
    for beds, group in validation.groupby("bedrooms"):
        error_by.append(
            [
                "Studio"
                if beds == 0
                else f"{int(beds)} bedroom{'s' if beds != 1 else ''}",
                f"{len(group):,}",
                f"{group.absolute_percent_error.median():.1f}%",
            ]
        )
    labels = {
        "no_dated_own_active_event": "No dated initial active event belonging to this listing",
        ...
        "furnished": "Explicit furnished evidence",
        "short_term": "Explicit lease shorter than six months",
    ...
    exclusion_rows = [
        [labels.get(r["reason"], r["reason"]), f"{r['listing_ids']:,}"]
        for r in c["exclusions"]
        if r["listing_ids"]
    ]
    years = data.groupby(data.period.dt.year).agg(
        observations=("unit_id", "size"), units=("unit_id", "nunique")
    )
    years.to_parquet(output / "coverage_by_year.parquet")
    summary = {
        "year_over_year_august_2026_percent": float(yoy),
        "covid_era_trough_month": str(trough.period.date()),
        "trough_vs_february_2020_percent": float(decline),
        "seasonal_peak": month_names[season_peak],
        "seasonal_low": month_names[season_low],
        "temporal_validation_by_bedrooms": error_by,
    ...
    bedroom_description = (
        "Bedrooms use cumulative indicators: >0, >1, >2, >3, and >4. A two-bedroom listing activates the first two terms; each coefficient is an additional-bedroom contribution on the log-rent scale. Regularization shrinks those increments separately. Studios activate none of them."
        if incremental
        else "Bedrooms use exact-category indicators for 1–5 bedrooms, with studios as the reference."
    )
...
<footer>Dataset: <code>{html.escape(Path(c["dataset"]).name)}</code><br>Model artifacts: <code>{html.escape(str(root))}</code><br>Saved inputs, selection audit, coefficients, weights, held-out predictions and source hashes accompany this report.</footer>"""
    ...
    (output / "report.html").write_text(
        '<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Chelsea · Minimal asking-rent model</title><style>'
        + styles
...
"""
    ...
    print(
        json.dumps(
            {"report": str(output / "report.html"), "summary": summary}, indent=2
        )
    )
