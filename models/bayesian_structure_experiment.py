"""Bedroom-time spline model plus a per-building random-walk drift.

Same source loading, feature design, floor spline, bedroom-group time curve,
disk sampling and report stages as `bayesian_bedroom_time_experiment`. The
graph (`bayesian_structure_graph`, walk mode) adds each building's own
piecewise-linear random walk on knots every ``building_knot_years``, centered
over that building's training months. Fitted rents and residuals include it.
Building-walk draws are read one building at a time from a building-major
memory-mapped cache, never as one samples x buildings x knots array.
...
VERSION = location_terms.STRUCTURE_EXPERIMENT
...
VARIABLES = ('bedroom_walk_scale', 'bedroom_walk_z', 'bedroom_time', 'building_time_scale', 'building_time_z')
...
def validate_args(args):
    ...
    for name in ('bedroom_walk_prior_scale', 'building_scale_prior', 'building_knot_years'):
...
def graph_kwargs(args):
    return {'building_time': 'walk', 'building_knot_years': args.building_knot_years,
            'building_scale': None, 'building_scale_prior': args.building_scale_prior,
...
def implementation_paths(base=None):
    return [*previous.previous.implementation_paths(), Path(previous.graph.__file__), Path(previous.__file__),
            Path(graph.__file__), Path(location_terms.__file__), Path(__file__)]
...
def make_protocol(args, data, source, code, configuration):
    ...
    result.update(version=VERSION, bedroom_time='walk', bedroom_walk_prior_scale=args.bedroom_walk_prior_scale,
        bedroom_linear_prior_scale=None, building_time='walk', building_knot_years=args.building_knot_years,
...
def validate_posterior(inference, args, design, configuration):
    ...
    basis, _ = location_terms.building_knots(design.time.periods, args.building_knot_years)
    expected = len(design.time.buildings)*basis.shape[1]
    if posterior['building_time_z'].dims != ('chain', 'draw', 'building_knot') or posterior.sizes['building_knot'] != expected:
...
def building_cache(posterior, directory, posterior_sha256):
    ...
    identity = {'version': BUILDING_TIME_VERSION, 'posterior_sha256': posterior_sha256, 'chains': chains,
                'draws': draws, 'building_knots': width, 'dtype': str(variable.dtype),
    ...
    mapped = np.lib.format.open_memmap(path, mode='w+', dtype=variable.dtype, shape=(chains*draws, width),
                                       fortran_order=True)
    ...
    marker.write_text(canonical({'identity': identity, 'cache_sha256': digest(path)})+'\n')
    return np.load(path, mmap_mode='r', allow_pickle=False)
...
def fitted_summary(inference, design, data, protocol, building_samples):
    ...
    samples = {name: base.sample_values(p, name) for name in
               ('alpha', 'beta', 'trend_coefficients', 'annual_drift', 'season_coefficients',
                'building_effect', 'sigma_unit', 'unit_z', 'building_time_scale')}
    ...
    centers = location_terms.building_centers(a['building'], a['period'], len(d.buildings), basis)
    width = basis.shape[1]
    rows = [None]*len(data)
    ...
    for building in np.unique(a['building']):
        ...
        for start in range(0, len(members), 128):
            ...
            walk = location_terms.building_walk(z, samples['building_time_scale'], years,
                                                basis[a['period'][sl]], centers[building])
            ...
            for j, i in enumerate(sl):
                ...
                rows[i] = {'audit_id': row.audit_id, 'source_listing_id': str(row.source_listing_id),
...
def building_time_summary(inference, design, data, protocol, building_samples):
    ...
    diagnostic, _ = v2.base.diagnostics({'posterior': xr.Dataset({'building_time_scale': scale}),
    ...
    centers = location_terms.building_centers(a['building'], a['period'], len(design.time.buildings), basis)
    latest = basis[-1]
    width = basis.shape[1]
    ...
    for building, name in enumerate(design.time.buildings):
        z = np.asarray(building_samples[:, building*width:(building+1)*width])
        value = location_terms.building_walk(z, s, years, latest[None], centers[building])[:, 0]
        shifts.append({'building': name, 'latest_log_offset': v2.reports.interval(value)})
    return {'version': BUILDING_TIME_VERSION, 'knot_years': years,
            'knots': [design.time.periods[int(k)].strftime('%Y-%m') for k in knots],
            'scale': v2.reports.interval(s), 'diagnostics': diagnostic,
            'latest_period': design.time.periods[-1].strftime('%Y-%m'), 'latest_building_offsets': shifts,
...
def write_reports(target, inference, design, data, protocol_hash, protocol, building_samples):
    ...
    residuals, samples = fitted_summary(inference, design, data, protocol, building_samples)
    ...
    acceptable = acceptable and curves['diagnostics']['acceptable'] and walk['diagnostics']['acceptable']
...
def completed_fit(target, protocol_hash, configuration):
    manifest, files = previous.previous._verified_bundle(target, retain={'summary.json', 'graph-configuration.json',
...
def run(args):
    ...
    with (root/'.run.lock').open('a') as lock:
        ...
        configuration = graph.configuration(data, **{k: v for k, v in graph_kwargs(args).items()})
        ...
        if checkpoint.exists():
            ...
            (target/'compression.json').write_text(canonical({'version': graph.VERSION, 'observations': len(data),
                'features': len(design.features), 'unique_feature_rows': unique_rows,
        ...
        try:
            ...
            building_samples = building_cache(v2.base.posterior_dataset(inference), root/'building-cache', posterior_hash)
            with report_cache.bounded_unit_samples(v2.base, inference, root/'report-cache', posterior_hash) as cached:
                result = write_reports(target, inference, design, data, ph, protocol, building_samples)
...
def argument_parser():
    ...
    parser.add_argument('--building-knot-years', type=float, default=.5)
    parser.add_argument('--building-scale-prior', type=float, default=.1)
    return parser
