...
from apartments import outdoor_scope_v2 as outdoor_scope
...
def run(review, output):
    ...
    for case in cases:
        ...
        for label in case['annotations']:
            ...
            found = [(i,c) for i,c in enumerate(claims)
            ...
            if found: used.add(found[0][0])
            predicted = prediction['scope'] if prediction else 'missing_mention'
            confusion[label['scope'],predicted] += 1
            results.append({'case_id':case['case_id'], 'audit_id':case['audit_id'],
                            'source_listing_id':case['source_listing_id'], 'capture_id':case['capture_id'],
                            'label':label, 'prediction':prediction, 'scope_matches':predicted==label['scope']})
        extras.extend({'case_id':case['case_id'], 'audit_id':case['audit_id'], 'claim':claim}
                      for i,claim in enumerate(claims) if i not in used)
    positive = {'private_explicit','unit_access'}
    promoted = [r for r in results if r['prediction'] and r['prediction']['scope'] in positive]
    summary = {'cases':len(cases), 'labeled_mentions':len(results),
               ...
               'matched_unit_positive_claims_with_positive_label':sum(r['label']['scope'] in positive for r in promoted),
               'unlabeled_unit_positive_claims':sum(r['claim']['scope'] in positive for r in extras),
               'confusion':[{'label':a,'prediction':b,'count':n} for (a,b),n in sorted(confusion.items())],
    ...
    return publish_bundle(output, {'summary.json':canonical(summary)+'\n',
        ...
        'outdoor_scope.py':Path(outdoor_scope.__file__).read_text(), 'evaluation.py':Path(__file__).read_text()},
        {'version':'outdoor-scope-evaluation-v2', 'extractor_version':outdoor_scope.VERSION,
         'review_manifest':rm, 'summary':summary,
         'implementation_sha256':{'outdoor_scope.py':digest(outdoor_scope.__file__),'evaluation.py':digest(__file__)}})
