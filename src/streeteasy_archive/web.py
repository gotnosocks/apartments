...
def create_app(data_dir="data", *, allowed_hosts=None):
    ...
    def db():
        if "db" not in g:
            if not (root / "archive.sqlite3").exists():
                abort(
                    404,
                    description="Archive not initialized. Run backfill or import-har first.",
                )
            g.db = sqlite3.connect(
                f"file:{quote(str(root / 'archive.sqlite3'))}?mode=ro",
                uri=True,
                timeout=5,
            )
    ...
    def local_headers(response):
        response.headers["Content-Security-Policy"] = (
            "default-src 'self'; script-src 'self'; style-src 'self'; connect-src 'self'; img-src 'none'; frame-ancestors 'none'; base-uri 'none'"
        )
    ...
    def generation():
        ...
        if (
            result is not None
            and not db()
            .execute("SELECT 1 FROM generations WHERE id=?", (result,))
            .fetchone()
        ):
    ...
    def summary():
        ...
        generations = [
            ...
            for row in db().execute(
                "SELECT * FROM generations ORDER BY id DESC LIMIT 200"
            )
        ]
        state = (
            dict(
                db().execute("SELECT * FROM generations WHERE id=?", (gen,)).fetchone()
            )
            if gen
            else None
        )
        queue = {
            row[0]: row[1]
            for row in db().execute(
                "SELECT state,count(*) FROM frontier WHERE generation=? GROUP BY state",
                (gen,),
            )
        }
        captured = {
            row[0]: row[1]
            for row in db().execute(
                """SELECT f.kind,count(*) FROM frontier f
            ...
            AND (o.status BETWEEN 200 AND 299 OR o.status=304)) GROUP BY f.kind""",
                (gen, gen),
            )
        }
        counts = (
            db().execute("SELECT count(*),COALESCE(sum(size),0) FROM bodies").fetchone()
        )
        errors = [
            dict(row)
            for row in db().execute(
                """SELECT e.id,e.url,e.status,e.error,e.fetched
        ...
        profile_row = (
            db()
            .execute(
                "SELECT value FROM metadata WHERE key=?", (f"crawl_profile:{gen}",)
            )
            .fetchone()
        )
        ...
        scoped = {
            r[0]: r[1]
            for r in db().execute(
                "SELECT f.state,count(*) FROM frontier f JOIN scope_urls s ON s.generation=f.generation AND s.url=f.url WHERE f.generation=? GROUP BY f.state",
                (gen,),
            )
        }
        return jsonify(
            building_coverage=building_coverage(db(), gen),
            profile=profile,
            scope_queue=scoped,
            generation=state,
    ...
    def pages():
        ...
        if state == "captured":
            where.append(
                "o.body_hash IS NOT NULL AND o.error IS NULL AND (o.status BETWEEN 200 AND 299 OR o.status=304)"
            )
        elif state == "errors":
        ...
        total = (
            db().execute("SELECT count(*) " + join + condition, params).fetchone()[0]
        )
        rows = (
            db()
            .execute(
                """SELECT f.url,f.kind,f.state,f.attempts,f.lastmod,o.id observation_id,
            o.status,o.fetched,o.body_hash,o.error """
                + join
                + condition
                + " ORDER BY f.url LIMIT 40 OFFSET ?",
                [*params, (page - 1) * 40],
            )
            .fetchall()
        )
        return jsonify(
            items=[dict(row) for row in rows], page=page, total=total, page_size=40
        )
    ...
    def page_detail():
        ...
        row = (
            db()
            .execute("SELECT * FROM frontier WHERE generation=? AND url=?", (gen, url))
            .fetchone()
        )
        if row is None:
        ...
        history = [
            ...
            for item in db().execute(
                """SELECT id,generation,fetched,status,content_type,
            headers,body_hash,not_modified,error FROM observations WHERE url=? AND generation<=?
            ORDER BY fetched DESC,id DESC LIMIT 100""",
                (url, gen),
            )
        ]
        for item in history:
        ...
        return jsonify(
            ...
            .execute(
                "SELECT count(*) FROM observations WHERE url=? AND generation<=?",
                (url, gen),
            )
            .fetchone()[0],
        )
    ...
    def extraction(obs_id):
        ...
        snap = (
            ...
            .execute(
                ...
                (row["generation"], row["url"], row["body_hash"]),
            )
            .fetchone()
        )
        if snap:
        ...
        with gzip.open(body_file(row), "rb") as source:
            if request.args.get("download") == "1":
                ...
                response.headers["Content-Disposition"] = (
                    f'attachment; filename="observation-{obs_id}.bin"'
                )
                return response
            preview = source.read(200001)
        return jsonify(
            text=preview[:200000].decode("utf-8", errors="replace"),
            truncated=len(preview) > 200000,
