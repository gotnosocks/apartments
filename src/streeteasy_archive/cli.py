...
def resolve_delay(explicit, transport, neighborhood, profile):
    if explicit is not None:
        ...
        delay = (
            profile.get("delay", 0)
            if profile.get("pacing_version") == 2
            and profile.get("transport") == transport
            else 0
        )
    else:
        delay = (
            profile.get("delay", 60 if neighborhood else 10)
            if profile.get("transport") == transport
            else (60 if neighborhood else 10)
        )
    minimum = 0 if transport == "oxylabs" else 10
    if not math.isfinite(delay) or delay < minimum:
        raise ValueError(
            f"--delay must be finite and at least {minimum} seconds for {transport}"
        )
    return delay
...
def main(argv=None):
    ...
    parser.add_argument(
        "--data",
        default=str(Path(__file__).resolve().parents[2] / "data/archive"),
        help="archive directory (default: apartments/data/archive)",
    )
    ...
    for name in ("backfill", "update", "resume"):
        ...
        command.add_argument(
            "--concurrency", type=int, help="Oxylabs concurrent jobs (1-20)"
        )
        command.add_argument(
            "--api-rps",
            type=float,
            help="Oxylabs submissions per second (0-50, exclusive of zero)",
        )
        command.add_argument(
            "--oxylabs-render",
            action="store_true",
            help="request rendered HTML instead of server HTML for this run",
        )
        command.add_argument(
            "--neighborhood",
            choices=("chelsea",),
            help="persistent Chelsea + West Chelsea scope, excluding Hudson Yards",
        )
        command.add_argument(
            "--delay",
            type=float,
            help="request delay seconds; defaults to 0 (submission pacing uses --api-rps)",
        )
        command.add_argument(
            "--wait-for-cooldown",
            action="store_true",
            help="wait for an existing cooldown once; a new challenge still exits",
        )
        command.add_argument(
            "--building",
            help="restrict this run to a building URL and child detail URLs; repeat on resume",
        )
        ...
        command.add_argument(
            ...
            type=int,
            default=0,
            help="request budget; zero is unlimited",
        )
        command.add_argument(
            "--revisit-interval",
            type=float,
            default=0,
            help="update interval in seconds for known buildings/listings",
        )
    ...
    serve_parser.add_argument(
        "--listen",
        default=os.environ.get("ARCHIVE_LISTEN"),
    ...
    if (
        getattr(args, "max_requests", 0) < 0
        or getattr(args, "revisit_interval", 0) < 0
        or not math.isfinite(getattr(args, "revisit_interval", 0))
    ):
    ...
    try:
        ...
        previous_profile = store.db.execute(
        ...
        row = store.db.execute(
            "SELECT value FROM metadata WHERE key=?", (profile_key,)
        ).fetchone()
        profile = (
            json.loads(row[0])
            if row
            else json.loads(previous_profile[0])
            if previous_profile
            else {}
        )
        args.neighborhood = args.neighborhood or (
            None if explicit_building else profile.get("neighborhood")
        )
        args.building = args.building or (
            None if explicit_neighborhood else profile.get("building")
        )
        ...
        args.concurrency = (
            ...
            else profile.get("concurrency", 5)
        )
        args.api_rps = (
            args.api_rps if args.api_rps is not None else profile.get("api_rps", 2)
        )
        if (
            not 1 <= args.concurrency <= 20
            or not math.isfinite(args.api_rps)
            or not 0 < args.api_rps <= 50
        ):
            raise ValueError(
                "concurrency must be 1-20 and api-rps must be finite and greater than 0, up to 50"
            )
        args.delay = resolve_delay(
            args.delay, args.transport, args.neighborhood, profile
        )
        ...
        if args.building:
            ...
            configure_building(
                store, generation, args.building, args.include_unavailable
            )
        ...
        while (
            args.wait_for_cooldown
            and state["cooldown"]
            and state["cooldown"] > time.time()
        ):
            print(
                f"Waiting for cooldown until {state['cooldown']}; no site requests",
                flush=True,
            )
        ...
        if state["cooldown"] and state["cooldown"] > time.time():
            ...
            print(
                "crawl is cooling down; resume after the reported UTC epoch timestamp",
                file=sys.stderr,
            )
            return 3
...
def import_har(store, path):
    ...
    for entry in doc.get("log", {}).get("entries", []):
        ...
        if store.db.execute(
            "SELECT 1 FROM har_entries WHERE fingerprint=?", (fingerprint,)
        ).fetchone():
            skipped += 1
            continue
        headers = {
        ...
        if previous is None or fetched is None or fetched >= previous["fetched"]:
            store.enqueue(
                generation, [{"url": url, "kind": kind_for(url) or "homepage"}]
            )
        store.record(
            generation,
            url,
            ...
            data,
            error=error,
            discovered=approved_links(data["links"], url) if error is None else [],
            fetched=fetched,
            har_fingerprint=fingerprint,
        )
        imported += 1
    store.finish(generation)
    print(
        json.dumps({"generation": generation, "imported": imported, "skipped": skipped})
    )
    return 0
...
def export(store, generation, path, offline):
    generations = (
        [generation]
        if generation
        else [r[0] for r in store.db.execute("SELECT id FROM generations ORDER BY id")]
    )
    output = Path(path).resolve()
    if (
        output.parent == store.root
        and (output.name.startswith("archive.sqlite3") or output.name == "crawler.lock")
    ) or store.root / "bodies" in output.parents:
            ...
            for row in store.observations(gen):
                ...
                if row["body_hash"]:
                    snap = store.db.execute(
                        "SELECT extracted FROM snapshots WHERE generation=? AND url=? AND body_hash=?",
                        (gen, row["url"], row["body_hash"]),
                    ).fetchone()
                ...
                item["body_path"] = (
                    str(store.body_path(item["body_sha256"]).relative_to(store.root))
                    if item["body_sha256"]
                    else None
                )
...
def run_crawler(args, generation, lock=None):
    ...
    settings = {
        "LOG_LEVEL": "INFO",
        "DOWNLOAD_HANDLERS": {
            "https": "streeteasy_archive.oxylabs.OxylabsDownloadHandler"
        },
    ...
    if errors or crawler.stats.get_value("spider_exceptions/count", 0):
        print(
            "\n".join(errors)
            or "crawler callback failed; resume after fixing the error",
            file=sys.stderr,
        )
        return 2
