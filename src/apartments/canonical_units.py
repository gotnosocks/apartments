...
def memberships(captures):
    ...
    pages = {
        lid: common_unit_page(c["canonical_unit_url"] for c in rows)
    ...
    ambiguous_pages = {
        c["canonical_unit_url"]
        for lid, rows in listings.items()
        if pages[lid] is None
        for c in rows
        if c["canonical_unit_url"]
    }
    result = []
    for lid, rows in sorted(listings.items()):
        page = pages[lid]
        reason = (
            "Missing or conflicting canonical unit URLs across captures"
            if not page
            else "Canonical unit URL is also used by a listing with conflicting captures"
        ...
        result.append(
            {
                "listing_id": lid,
                "unit_id": canonical_unit_id(page) if not reason else None,
                "canonical_unit_url": page,
                "capture_count": len(rows),
                "status": "unresolved" if reason else "associated",
                "reason": reason,
...
def build_canonical_units(root):
    ...
    try:
        db.read_parquet(
            [str(p) for p in sorted((root / "listing_observations").glob("*.parquet"))]
        ).create_view("listings")
        rows = db.execute(
            "SELECT snapshot_id,listing_id,canonical_unit_url FROM listings WHERE listing_type='rental' ORDER BY snapshot_id"
        ).fetchall()
    finally:
        db.close()
    captures = [
        dict(zip(("snapshot_id", "listing_id", "canonical_unit_url"), row))
        for row in rows
    ]
        ...
        if member["unit_id"]:
            unit = units.setdefault(
                member["unit_id"],
                {
                    ...
                    "canonical_unit_url": member["canonical_unit_url"],
                    "listing_count": 0,
                    "capture_count": 0,
                    "rule": ASSOCIATION_RULE,
                },
        ...
        observations.append(
            {
                **capture,
                ...
                if member
                else "Missing rental listing ID",
            }
        )
    ...
    tables = {
        "rental_units": sorted(
            units.values(), key=lambda row: row["canonical_unit_url"]
        ),
        "rental_unit_memberships": members,
        "rental_unit_observations": observations,
    }
    digests = {}
    for name, output in tables.items():
        ...
        pq.write_table(
            pa.Table.from_pylist(output, schema=SCHEMAS[name]),
            temporary,
            compression="zstd",
        )
    ...
    summary = {
        ...
        "listing_ids_in_multi_listing_units": sum(
            u["listing_count"] for u in units.values() if u["listing_count"] > 1
        ),
