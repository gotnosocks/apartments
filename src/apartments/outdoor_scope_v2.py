...
VERSION = 'outdoor-access-scope-v2'
...
_ADJ = (r'(?:large|small|huge|spacious|expansive|oversized|landscaped|planted|sunny|'
        ...
        r'roof|rooftop|roof\s+top|rear|back|front|lush|lovely|spectacular|perfect|serene|quiet|courtyard|'
...
_VIEW = re.compile(rf'\b(?:views?\s+(?:of|over|onto)|overlooks?|overlooking|look(?:s|ing)?\s+out\s+(?:to|onto)|faces|(?<!-)facing)\s+(?:(?:the\s+)?building[’\x27]s\s+)?{_MOD}$', re.I)
_AFTER_VIEW = re.compile(r'^\s*(?:[ -]facing\b|views?\b)|^\s+(?:garden|courtyard)\s+views?\b', re.I)
...
_BUILDING = re.compile(r'\b(?:the|this|our)\s+(?:building|community|complex)\b|'
                       r'\b(?:building|community|residents?)\s+(?:amenities|features|offers?|includes?|enjoy|have|has|access)\b|'
                       r'\bamenities(?:\s+and\s+features)?\s+(?:include|offer|feature)\b|'
                       r'\b(?:building|condominium|co-op)\s+that\s+offers\s+residents\b', re.I)
_UNIT = re.compile(r'\b(?:this|your|the)\s+(?:(?:beautiful|bright|spacious|stunning|lovely|unique|exceptional|luxurious|sun-filled)\s+){0,3}(?:apartment|unit|home|residence|townhouse|penthouse)\b', re.I)
_UNIT_VERB = re.compile(r'\b(?:has|have|offers?|features?|includes?|boasts?|with|enjoys?)\b', re.I)
_ROOM = r'(?:(?:your|the|a|an|its|primary|master|main|second|guest|spacious|large|beautiful)\s+){0,3}(?:living(?:/dining)?\s+(?:room|area)|dining\s+(?:room|area)|bedroom|kitchen|home\s+office|great\s+room|apartment|unit|home|residence|suite)'
_OFF_ROOM = re.compile(rf'^\s+(?:(?:just|directly)\s+)?(?:off\s+of|off|adjoining|attached\s+to|accessible\s+from)\s+{_ROOM}\b', re.I)
_FROM_ROOM = re.compile(rf'\b{_ROOM}.{{0,180}}\b(?:access\s+to|(?:opens?|opening)\s+(?:out\s+)?(?:to|onto)|(?:leads?|leading)\s+(?:out\s+)?(?:to|onto)|doorway\s+to)\s+{_MOD}$', re.I)
_DOORS = re.compile(rf'\b(?:french\s+|sliding\s+)?doors?\s+(?:opens?|leads?)\s+(?:out\s+)?(?:to|onto)\s+{_MOD}$', re.I)
_ROOM_WITH = re.compile(rf'\b{_ROOM}\s+(?:with|featuring|has|offers|includes)\s+{_MOD}$', re.I)
...
def scope_text(text: str | None, source_path='/description') -> list[dict]:
    ...
    for noun in _NOUN.finditer(text):
        ...
        view_relation = _VIEW.search(prefix)
        view = bool(view_relation or _AFTER_VIEW.search(after))
        ...
        proper_name = bool((noun.lastgroup == 'YARD' and re.search(r'\b(?:Hudson|Navy)\s+$', prefix, re.I)) or
                           (noun.lastgroup == 'TERRACE' and re.search(r'\bLondon\s+$', prefix, re.I)))
        nonamenity = bool(proper_name or (noun.lastgroup == 'GARDEN' and re.match(r'^[ -]+(?:level|floor|apartment)\b', after, re.I)))
        access = bool(_OFF_ROOM.search(after) or _FROM_ROOM.search(prefix) or _DOORS.search(prefix) or _ROOM_WITH.search(prefix))
        ...
        if nonamenity: flags.append('non_amenity_name_or_level')
        ...
        elif nonamenity or multiple or semi or not_private or (shared_heading and private and not shared_adj and not after_shared and not building):
        ...
        building_view = bool(re.search(r'\b(?:lobby|lobbies|common areas?)\b', prefix, re.I))
        view_subject_prefix = prefix[:view_relation.start()] if view_relation else prefix
        if scope == 'view':
            # The garden's owner is not necessarily the viewer: apartment
            # windows may overlook the building's front garden.
            subject = ('building' if building_view else 'apartment' if _UNIT.search(view_subject_prefix) or
                       re.search(r'\b(?:bedroom|living room|kitchen)\b', view_subject_prefix, re.I) else
                       'building' if _BUILDING.search(view_subject_prefix) else 'unspecified')
        else:
            subject = ('not_an_amenity' if nonamenity else 'building' if shared or building_view else
                       'multiple_units' if multiple else 'apartment' if units or access else 'unspecified')
