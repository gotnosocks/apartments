...
def retain_capture(db, capture_id, item, manifest=None, *, collected_at=None, time_basis=None):
    ...
    source_id = item.get('source_listing_id') or urlparse(item['canonical_url']).path.strip('/').removeprefix('building/')
    listing = item.get('archive_listing') or {}
    episode = item.get('street_easy_rental_id') or listing.get('id') or item.get('canonical_url')
    ...
    payload = {'capture_id': capture_id, 'item': item, 'manifest': manifest,
    ...
    if db.execute('SELECT 1 FROM attribute_versions WHERE version_id=?', [version_id]).fetchone():
        evidence_count = db.execute('SELECT count(*) FROM event_capture_evidence WHERE version_id=?', [version_id]).fetchone()[0]
        if evidence_count == len(item.get('price_history', [])):
            return version_id
    numeric = item.get('attributes') or {}
    db.execute('''INSERT INTO attribute_versions(
        ...
        [version_id,capture_id,item.get('source','streeteasy'),source_id,str(episode) if episode is not None else None,
         item.get('building_slug'),item.get('unit'),collected_at,basis,PARSER_VERSION,
         listing.get('createdAt'),listing.get('updatedAt'),listing.get('onMarketAt'),
         numeric.get('bedrooms'),numeric.get('bathrooms'),numeric.get('square_feet'),
         canonical(attrs),canonical(item),canonical(manifest)])
    ...
    if evidence:
        db.executemany('INSERT INTO event_capture_evidence VALUES (?,?,?) ON CONFLICT DO NOTHING', evidence)
    return version_id
...
def sync_observations(source, target):
    ...
    first = source.execute('SELECT url,fetched FROM observations ORDER BY id LIMIT 1').fetchone()
    if first is None:
        return 0
    ...
    known = {r[0] for r in target.execute('SELECT observation_id FROM archive_observations WHERE archive_key=?', [archive_key]).fetchall()}
    ...
    batch = [[archive_key,*list(r[:3]),datetime.fromtimestamp(r[3],UTC),*list(r[4:])]
             for r in rows if r[0] not in known]
    if batch:
        target.executemany('''INSERT INTO archive_observations(archive_key,observation_id,generation,url,
...
def backfill_capture_history(db, source=None):
    ...
    if source is not None:
        original_times = dict(source.execute('''SELECT s.id, min(o.fetched) FROM snapshots s
            JOIN observations o ON o.generation=s.generation AND o.url=s.url AND o.body_hash=s.body_hash
            WHERE o.error IS NULL AND (o.status BETWEEN 200 AND 299 OR o.status=304)
            GROUP BY s.id''').fetchall())
    rows = db.execute('SELECT capture_id,structured_json,manifest_json FROM captures').fetchall()
    ...
    for start in range(0, len(rows), 20):
        ...
        try:
            for cid, raw, provenance in rows[start:start+20]:
                ...
                collected_at = datetime.fromtimestamp(epoch,UTC).isoformat() if epoch is not None else None
                retain_capture(db,cid,item,manifest,collected_at=collected_at,
                               time_basis='archive_response_fetched' if epoch is not None else None)
            db.execute('COMMIT')
        except Exception:
            db.execute('ROLLBACK')
            raise
