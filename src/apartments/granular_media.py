...
def page_type(item):
    ...
    return 'media_gallery' if is_gallery_url(item['url']) else item.get('kind') or kind_for(item['url'])
...
def parse_media_gallery(body, url):
    ...
    matching = [c for c in candidates if requested_id is None or str(c.get('id')) == requested_id]
    ...
    error = None if gallery is not None else ('Conflicting media-gallery objects' if galleries else 'Media-gallery object missing or unresolved')
    if gallery is not None and any(gallery.get(k) is not None and not isinstance(gallery[k], (dict, list)) for k in ('media', 'signatureMediaGallery')):
        error = 'Gallery media payload missing or unresolved'
    return {
        **canonical_fields(body, url), 'url':url, 'page_type':'media_gallery',
        'listing_id':str(gallery['id']) if gallery and gallery.get('id') is not None else requested_id,
        'listing_type':requested_type,
        'building_id':str(gallery['buildingId']) if gallery and gallery.get('buildingId') is not None else None,
        'property_details_json':_json(gallery.get('propertyDetails')) if gallery else None,
        'media_json':_json(gallery.get('media')) if gallery else None,
        'signature_media_gallery_json':_json(gallery.get('signatureMediaGallery')) if gallery else None,
        'raw_gallery_json':_json(gallery), 'parse_status':('partial' if error else 'ok') if gallery is not None else 'failed',
        'error':error,
    }
