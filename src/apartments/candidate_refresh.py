...
async def refresh(snapshot, output, *, max_age_days=7, max_targets=50, max_new_requests=None, fetch=None, ledger=None):
    if max_new_requests is not None and (not isinstance(max_new_requests,int) or max_new_requests<1):
    ...
    with (root/'.refresh.lock').open('a') as lock:
        ...
        try:
            ...
            for index,target in enumerate(plan['targets']):
                ...
                binding={'version':VERSION,'plan_sha256':ph,'target_sha256':_hash(target),'index':index}
                ...
                if existing is None and (started/'complete.json').exists():
                    ...
                    if observation['body_hash'] and hashlib.sha256(body).hexdigest()!=observation['body_hash']:
                        raise ValueError('Archived body integrity failure')
                    result=interpret(target,observation,body,ph,interpreted_at=datetime.now(UTC).isoformat(),overlay=overlay)
            ...
            if any(digest(p)!=plan['implementation_sha256'][p.name] for p in _code_paths()):
