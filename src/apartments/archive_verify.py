...
def _load_manifest(path: Path) -> dict[str, dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as stream:
        for line_number, line in enumerate(stream, 1):
            ...
            if rel in records:
                raise ManifestError(
                    f"manifest line {line_number}: duplicate path {rel!r}"
                )
            ...
            if kind == "file":
                ...
                if not isinstance(size, int) or isinstance(size, bool) or size < 0:
                    raise ManifestError(
                        f"manifest line {line_number}: file size must be a non-negative integer"
                    )
                if (
                    ...
                    raise ManifestError(
                        f"manifest line {line_number}: file sha256 must be 64 hex characters"
                    )
                ...
                if not isinstance(target, str) or not target:
                    raise ManifestError(
                        f"manifest line {line_number}: symlink target must be a non-empty string"
                    )
                records[rel] = {"type": "symlink", "target": target}
            else:
                raise ManifestError(
                    f"manifest line {line_number}: unsupported record type {kind!r}"
                )
    return records
...
def _parse_prefix_maps(values: list[str]) -> list[tuple[str, str]]:
    ...
    for value in values:
        ...
        if new.startswith("/"):
            ...
            _relative_path(
                new.rstrip("/") or ".", label="prefix map destination"
            ) if new not in ("", ".") else None
...
def _mapped_target(target: str, maps: list[tuple[str, str]]) -> str:
    if target.startswith("/"):
        ...
        raise ManifestError(
            f"absolute symlink target needs an explicit prefix map: {target!r}"
        )
    return target
    ...
    for current, dirs, files in os.walk(root, followlinks=False):
        ...
        for name in files:
            ...
            found[rel] = (
                "symlink"
                if item.is_symlink()
                else ("file" if item.is_file() else "other")
            )
    return found
    ...
    *,
    ...
    for database in databases or []:
        ...
        try:
            ...
            result["sqlite_checks"].append(
                {
                    "path": str(rel),
                    "ok": False,
                    "error": "database path is missing or outside destination root",
                }
            )
            continue
    ...
    result["ok"] = (
        ...
        and all(x["ok"] for x in result["sqlite_checks"])
    )
    return result
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--root", required=True, type=Path, help="transferred archive directory"
    )
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help="verification JSON output, outside --root",
    )
    parser.add_argument(
        "--check-unexpected",
        action="store_true",
        help="report all unlisted files and symlinks",
    )
    parser.add_argument(
        "--symlink-prefix-map",
        action="append",
        default=[],
        metavar="OLD=NEW",
        help="map an absolute legacy target prefix to a destination-root-relative prefix",
    )
    parser.add_argument(
        "--database",
        action="append",
        default=[],
        help="relative SQLite path to check with read-only PRAGMA quick_check",
    )
    parser.add_argument(
        "--sqlite-cache-kib",
        type=int,
        default=8192,
        help="SQLite page cache limit for each requested quick_check",
    )
    ...
    try:
        ...
        if not 1 <= args.sqlite_cache_kib <= MAX_SQLITE_CACHE_KIB:
            raise ManifestError(
                f"SQLite cache limit must be between 1 and {MAX_SQLITE_CACHE_KIB} KiB"
            )
        result = verify(
            root,
            args.manifest,
            check_unexpected=args.check_unexpected,
            prefix_maps=args.symlink_prefix_map,
