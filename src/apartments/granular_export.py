...
SCHEMAS = {
    name: pa.schema(
        [
            (
                x.split(":")[0],
                {"i": pa.int64(), "f": pa.float64(), "s": pa.string()}[x.split(":")[1]],
            )
...
def implementation_hash():
    ...
    h.update(
        (Path(__file__).parents[1] / "streeteasy_archive" / "flight.py").read_bytes()
    )
    return h.hexdigest()
...
class Tables:
    ...
    def flush(self, name):
        path = self.root / name / f"{self.part}.parquet"
        path.parent.mkdir(parents=True, exist_ok=True)
        if name not in self.writers:
            self.writers[name] = pq.ParquetWriter(
                path, SCHEMAS[name], compression="zstd"
            )
        if self.buffers[name]:
            self.writers[name].write_table(
                pa.Table.from_pylist(self.buffers[name], schema=SCHEMAS[name])
            )
    ...
    def close(self):
        for n in self.names:
            self.flush(n)
            self.writers[n].close()


def connect(snapshot):
    c = sqlite3.connect(
        Path(snapshot).resolve().as_uri() + "?mode=ro&immutable=1", uri=True
    )
...
def prepare(snapshot, root, ledger, chunk_size=2000):
    ...
    config["ledger_sha256"] = hashlib.sha256(ledger_bytes).hexdigest()
    ...
    if plan_path.exists():
        ...
        for key in (
            ...
            if previous.get(key) != config[key]:
                raise ValueError(f"Run identity changed: {key}; choose a new output ID")
        return previous
    (root / "corrections.jsonl").write_bytes(ledger_bytes)
    c = connect(snapshot)
    tables = Tables(
        root, "metadata", ["snapshots", "fetch_observations", "frontier", "url_aliases"]
    )
    jobs = []
    batch = []
    for r in c.execute(
        "SELECT p.id snapshot_id,p.generation,p.url,p.body_hash,p.observed observed_at,p.extraction_version,f.kind FROM snapshots p JOIN scope_urls s USING(generation,url) LEFT JOIN frontier f USING(generation,url) ORDER BY p.id"
    ):
        row = dict(r)
        row["page_type"] = page_type(row)
        tables.add("snapshots", row)
        batch.append(row)
    ...
    for r in c.execute(
        "SELECT o.id observation_id,o.generation,o.url,o.fetched fetched_at,o.status,o.content_type,o.headers,o.body_hash,o.not_modified,o.error FROM observations o JOIN scope_urls s USING(generation,url) ORDER BY o.id"
    ):
        tables.add("fetch_observations", dict(r))
    for r in c.execute(
        "SELECT f.generation,f.url,f.kind,f.state,f.attempts FROM frontier f JOIN scope_urls s USING(generation,url)"
    ):
        tables.add("frontier", dict(r))
    for r in c.execute(
        "SELECT a.* FROM url_aliases a JOIN scope_urls s USING(generation,url)"
    ):
        tables.add("url_aliases", dict(r))
    c.close()
    tables.close()
    for i, job in enumerate(jobs):
        write_json(root / "jobs" / f"{i:05d}.json", job)
    config.update(
        created_at=time.time(), shards=len(jobs), metadata_counts=tables.counts
    )
...
def read_bodies(job, body_root):
    ...
    def read(item):
        if page_type(item) not in {"listing", "media_gallery"}:
            return item, None
        try:
            h = item["body_hash"]
            return item, gzip.decompress(
                (Path(body_root) / h[:2] / f"{h}.gz").read_bytes()
            )
...
def process_shard(snapshot, root, part, body_root):
    ...
    root = Path(root)
    marker = root / "checkpoints" / f"{part:05d}.json"
    if marker.exists():
        previous = json.loads(marker.read_text())
        if previous.get("implementation_sha256") != implementation_hash():
            raise ValueError("Parser changed; choose a new run ID")
        if shard_files_valid(root, previous):
            return previous
        marker.unlink()
    job = json.loads((root / "jobs" / f"{part:05d}.json").read_text())
    c = connect(snapshot)
    tables = Tables(
        root,
        f"part-{part:05d}",
    ...
    for n, (item, body) in enumerate(read_bodies(job, body_root)):
        ...
        if kind == "media_gallery":
            try:
                if isinstance(body, Exception):
                    raise body
                if body is None:
                    raise ValueError("Missing gallery body")
            ...
            try:
                if isinstance(body, Exception):
                    raise body
                if body is None:
                    raise ValueError("Missing listing body")
                row, events = parse_listing(body, url)
            except Exception as e:
                row = {
                    **(canonical_fields(body, url) if isinstance(body, bytes) else {}),
                    "parse_status": "error",
                    "error": f"{type(e).__name__}: {e}",
                    "listing_type": "sale"
                    if re.search(r"/(?:sale|sales)/", url)
                    else "rental",
                }
                events = []
            row.update(
            ...
            if row.get("listing_type") == "rental" and not row.get(
                "canonical_unit_url"
            ):
                tables.add(
                    "listing_exclusions",
                    {
                        **row,
                        "reason": "rental_missing_canonical_unit_page",
                        "canonical_unit_error": row.get("canonical_unit_error")
                        or "Canonical unit page could not be read",
                    },
                )
            else:
                tables.add("listing_observations", row)
                for ev in events:
                    tables.add("event_mentions", dict(ev, snapshot_id=sid))
                source = (
                    json.loads(row["raw_listing_json"])
                    if row.get("raw_listing_json")
                    else {}
                )
                for path, changes in (
                    (
                        "pricing.priceChanges",
                        (source.get("pricing") or {}).get("priceChanges"),
                    ),
                    ("statusChanges", source.get("statusChanges")),
                ):
                    ...
                    for j, change in enumerate(changes):
                        ...
                        tables.add(
                            "source_changes",
                            {
                                ...
                                "source_path": path,
                                "change_index": j,
                                "source_timestamp": obj.get("changedAt"),
                                "price": price,
                                "source_json": dumps(change),
                            },
                        )
            if row.get("parse_status") != "ok":
                errors.append(
                    {"snapshot_id": sid, "url": url, "error": row.get("error")}
                )
        elif kind in ("building", "inventory"):
            try:
                raw = c.execute(
                    "SELECT extracted FROM snapshots WHERE id=?", (sid,)
                ).fetchone()[0]
                data = json.loads(raw)
                if kind == "inventory":
                    ...
                    for j in range(max(len(rows), len(records))):
                        ...
                        tables.add(
                            "inventory_rows",
                            {
                                "snapshot_id": sid,
                                "row_index": j,
                                "listing_url": rec.get("url"),
                                "row_kind": rec.get("kind"),
                                "row_html": rows[j] if j < len(rows) else None,
                                "record_json": dumps(rec),
                            },
                        )
                    tables.add(
                        "inventory_observations",
                        {
                            "snapshot_id": sid,
                            "count": inv.get("count"),
                            "row_count": max(len(rows), len(records)),
                            "expected_counts_json": dumps(inv.get("expected_counts")),
                            "raw_inventory_json": dumps(inv),
                            "parse_status": "ok" if inv else "missing",
                            "error": None if inv else "No saved inventory",
                        },
                    )
                else:
                    ...
                    tables.add(
                        "building_observations",
                        {
                            "snapshot_id": sid,
                            "building_slug": slug,
                            "building_id": str(primary["id"])
                            if primary and primary.get("id") is not None
                            else None,
                            "residential_units": primary.get("residentialUnitCount")
                            if primary
                            else None,
                            "latitude": primary.get("latitude") if primary else None,
                            "longitude": primary.get("longitude") if primary else None,
                            "raw_building_json": dumps(primary),
                            "parse_status": "ok" if primary else "missing",
                            "error": None if primary else "No primary building object",
                        },
                    )
            except Exception as e:
        ...
        if n and n % 250 == 0:
            print(
                f"Shard {part}: {n}/{len(job)} ({time.time() - started:.0f}s)",
                flush=True,
            )
    c.close()
    tables.close()
