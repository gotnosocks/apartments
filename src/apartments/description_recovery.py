...
def _implementation():
    return {name: digest(path) for name, path in {
        'description_recovery': __file__, 'granular_parse': granular_parse.__file__,
        'flight': flight.__file__, 'extract': extract.__file__,
        'unit_canonical': unit_canonical.__file__, 'research_pipeline': research_pipeline.__file__,
    }.items()}
...
def recover_capture(capture, body_root, *, max_body_bytes=32 * 1024 * 1024):
    ...
    row = {
        'snapshot_id': capture['snapshot_id'], 'listing_id': str(capture['listing_id']),
        'source_url': capture['url'], 'canonical_unit_url': capture.get('canonical_unit_url'),
        ...
        'recovery_version': VERSION, 'parser_version': granular_parse.VERSION,
    ...
    try:
        body, actual_hash = _read_body(Path(body_root) / body_hash[:2] / (body_hash + '.gz'), max_body_bytes)
    ...
    if parsed.get('parse_status') != 'ok':
        return reject('reparse_not_ok', parse_status=parsed.get('parse_status'), detail=parsed.get('error'))
    if str(parsed.get('listing_id')) != str(capture['listing_id']) or str(original.get('id')) != str(capture['listing_id']):
        return reject('listing_identity_mismatch')
    identity_fields = ('canonical_unit_url', 'unit_label', 'building_slug', 'listing_type')
    changes = [name for name in identity_fields if parsed.get(name) != capture.get(name)]
    if changes:
    ...
    if _json(old_other) != _json(new_other):
        changed = sorted(k for k in old_other.keys() | new_other.keys()
                         if _json(old_other.get(k)) != _json(new_other.get(k)) or (k in old_other) != (k in new_other))
        return reject('non_description_payload_changed', changed_top_level_fields=changed,
                      reparsed_raw_listing_sha256=hashlib.sha256(parsed['raw_listing_json'].encode()).hexdigest())
    return {**row, 'status': 'accepted', 'resolved_description': text,
...
def build_description_recovery(dataset, body_root, output, *, workers=4, batch_size=64,
                               max_body_bytes=32 * 1024 * 1024, progress=None):
    ...
    """
    import duckdb
    if not 1 <= workers <= 8 or not 1 <= batch_size <= 256 or not 1024 <= max_body_bytes <= 32 * 1024 * 1024:
        raise ValueError('Invalid bounded worker, batch or body-size configuration')
    root, bodies, out = Path(dataset).resolve(), Path(body_root).resolve(), Path(output).resolve()
    ...
    plan = {'recovery_version': VERSION, 'source_dataset': str(root), 'body_root': str(bodies),
            'source_manifest_sha256': _hash(source_files), 'source_files': source_files,
            'implementation_sha256': implementation, 'batch_size': batch_size,
            'max_body_bytes': max_body_bytes,
            'runtime_versions': {name: importlib.metadata.version(name) for name in ('duckdb', 'parsel', 'lxml')}}
    ...
    with (out / '.build.lock').open('a') as lock:
        ...
        if (out / 'recovery-plan.json').exists():
            if json.loads((out / 'recovery-plan.json').read_text()) != plan:
                raise ValueError('Recovery inputs or implementation changed; choose a new output directory')
        ...
        try:
            for name in ('listing_observations', 'snapshots'):
                db.read_parquet([str(root / p) for p in source_files if p.startswith(name + '/')]).create_view(name)
            duplicate = db.execute('SELECT snapshot_id FROM snapshots GROUP BY snapshot_id HAVING count(*) > 1 LIMIT 1').fetchone()
            if duplicate:
            ...
            previous_id = None
            with ProcessPoolExecutor(max_workers=workers, mp_context=multiprocessing.get_context('spawn')) if workers > 1 else _InlineExecutor() as pool:
                while batch := db.fetchmany(batch_size):
                    captures = [dict(zip(keys, values)) for values in batch]
                    for capture in captures:
                        if previous_id is not None and capture['snapshot_id'] <= previous_id:
                            raise ValueError('Duplicate or unordered listing snapshot candidates')
                        previous_id = capture['snapshot_id']
                    path, checkpoint = parts / f'{part:06d}.jsonl', checkpoints / f'{part:06d}.json'
                    candidate_hash = _hash(captures)
                    if checkpoint.exists():
                        saved = json.loads(checkpoint.read_text())
                        if saved['candidate_sha256'] != candidate_hash or digest(path) != saved['artifact_sha256']:
                        ...
                        if len(result) != len(captures) or [r['snapshot_id'] for r in result] != [r['snapshot_id'] for r in captures]:
                            raise ValueError('Recovery checkpoint row identity failure')
                    else:
                        result = list(pool.map(_worker, [(c, str(bodies), max_body_bytes) for c in captures]))
                        _atomic(path, ''.join(_json(r) + '\n' for r in result))
                        _atomic(checkpoint, _json({'candidate_sha256': candidate_hash,
                                                  'artifact_sha256': digest(path), 'rows': len(result)}) + '\n')
                    counts.update(r['status'] for r in result)
                    reasons.update(r['reason'] for r in result if r['status'] == 'quarantined')
                    total += len(result)
                    part += 1
                    if progress:
                        progress({'processed': total, 'accepted': counts['accepted'], 'quarantined': counts['quarantined'], 'batches': part})
        finally:
            db.close()
        ...
        coverage = {'candidates': total, 'accepted': counts['accepted'], 'quarantined': counts['quarantined'],
                    'quarantine_reasons': dict(sorted(reasons.items())), 'batches': part}
        ...
        for status, name in (('accepted', 'accepted.jsonl'), ('quarantined', 'quarantined.jsonl')):
            with tempfile.NamedTemporaryFile('w', dir=out, encoding='utf-8', prefix='.partial-', delete=False) as stream:
                for index in range(part):
                    with (parts / f'{index:06d}.jsonl').open(encoding='utf-8', newline='\n') as source:
                        for line in source:
        ...
        files = {name: digest(out / name) for name in ('accepted.jsonl', 'quarantined.jsonl', 'source-files.json', 'coverage.json')}
        manifest = {'recovery_version': VERSION, 'parser_version': granular_parse.VERSION,
                    'flight_decoder_version': flight.VERSION, 'source_dataset': str(root),
                    'source_manifest_sha256': _hash(source_files), 'implementation_sha256': implementation,
                    'runtime_versions': plan['runtime_versions'], 'coverage': coverage, 'files': files,
...
class _InlineExecutor:
    ...
    def map(self, function, tasks): return map(function, tasks)
...
if __name__ == '__main__':
    ...
    manifest = build_description_recovery(args.dataset, args.body_root, args.output,
