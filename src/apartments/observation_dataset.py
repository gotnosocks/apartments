...
def iter_observations(db, overlay: Overlay, *, collected_as_of=None,
                      interpreted_as_of=None, effective_at=None, version_id=None, known_as_of=None):
    ...
    if known_as_of is not None:
        if collected_as_of is not None or interpreted_as_of is not None:
            raise CorrectionError('known_as_of cannot be combined with separate collection/interpretation cutoffs')
        if overlay.enabled and overlay.as_of > instant(known_as_of):
            raise CorrectionError('Overlay contains later knowledge; set corrections_as_of to known_as_of or earlier')
        collected_as_of = interpreted_as_of = known_as_of
    filters, args = [], []
    for column, value in [('collected_at', collected_as_of), ('recorded_at', interpreted_as_of)]:
    ...
    cursor = db.execute('''SELECT version_id,capture_id,source,source_listing_id,
        ...
        structured_json,provenance_json FROM attribute_versions''' + where + ' ORDER BY version_id', args)
    ...
    while batch := cursor.fetchmany(32):
        for row in batch:
            ...
            context = {k: values[k] for k in ('version_id', 'capture_id', 'source', 'source_listing_id', 'episode_id', 'building_slug', 'unit')}
            ...
            clocks = {k: v.isoformat() if hasattr(v, 'isoformat') else v for k, v in values.items()}
            yield {'observation': clocks, 'provenance': provenance, 'raw': raw,
                   'corrected': corrected, 'corrections': edits,
                   'effective_at': instant(effective_at).isoformat() if effective_at else None,
    ...
    try:
        ...
        with partial.open('x', encoding='utf-8') as stream:
            for row in iter_observations(db, overlay, **selection):
                input_hash.update((canonical({k: row[k] for k in ('observation', 'raw', 'provenance')})+'\n').encode())
                line = canonical(row)+'\n'
                stream.write(line)
        ...
        (output/'corrections.jsonl').write_text(''.join(canonical(r)+'\n' for r in overlay.records), encoding='utf-8')
        manifest = {'dataset_version': VERSION, 'observations': count, 'corrected_observations': corrected,
                    'applied_correction_ids': sorted(applied), 'overlay': overlay.manifest,
                    'selection': {k: v.isoformat() if isinstance(v, datetime) else v for k, v in selection.items()},
                    'source_observations_sha256': input_hash.hexdigest(), 'observations_sha256': output_hash.hexdigest(),
                    'limitations': ['Observation export, not a fitted model or a verified physical-unit/episode training table.',
        ...
        (output/'metadata.json').write_text(json.dumps(manifest, indent=2), encoding='utf-8')
        return manifest
    finally:
        db.close()
