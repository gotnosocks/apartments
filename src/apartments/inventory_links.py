...
SCHEMA = pa.schema([
    ("snapshot_id", pa.int64()), ("row_index", pa.int64()),
    ("listing_url", pa.string()), ("row_kind", pa.string()),
    ("record_url", pa.string()), ("link_basis", pa.string()),
    ("parsed_at", pa.float64()), ("candidate_urls_json", pa.string()),
])
...
def _candidates(row):
    ...
    for value in values:
        ...
        if url and kind_for(url) in {"listing", None} and (kind_for(url) == "listing" or "/closing/" in url):
            if url not in valid:
                valid.append(url)
    return valid
...
def _is_placeholder(row):
    ...
    if not html.strip():
        return False
    try:
        text = _selector(html.encode()).xpath("normalize-space(.)").get().lower().rstrip(".")
    except Exception:
        return False
    return text in {"no info for unavailable units", "no units available", "no listings available", "no records", "no units", "no listings"}
...
def build_inventory_links(root: Path) -> dict:
    ...
    files = sorted(p for p in source_dir.rglob("*.parquet") if p.is_file()) if source_dir.exists() else []
    ...
    counts = {"listing": 0, "closing": 0, "placeholder": 0, "unclassified": 0, "ambiguous": 0}
    stats = {"rows": 0, "recovered_legacy_links": 0, "mismatched_nonnull_record_url": 0, "unresolved": 0, "ambiguous": 0}
    basis_counts = {}
    ...
    try:
        for path in files:
            ...
            for batch in parquet.iter_batches(batch_size=256):
                ...
                for row in batch.to_pylist():
                    ...
                    input_hash.update(json.dumps([row.get("snapshot_id"), row.get("row_index"), row.get("row_html"), record_url], ensure_ascii=False, separators=(",", ":"), default=str).encode("utf-8"))
                    input_hash.update(b"\n")
                    ...
                    elif record_canonical and (kind_for(record_canonical) == "listing" or "/closing/" in record_canonical):
                        chosen = record_canonical
                    ...
                    if kind == "unclassified":
                        stats["unresolved"] += 1
                    if row.get("listing_url") is None and chosen and basis in {"html", "record_fallback"}:
                    ...
                    output_rows.append({
                        "snapshot_id": row.get("snapshot_id"), "row_index": row.get("row_index"),
                        "listing_url": chosen, "row_kind": kind, "record_url": record_url,
                        "link_basis": basis, "parsed_at": parsed_at,
                        "candidate_urls_json": json.dumps(candidates, separators=(",", ":")),
                    })
                if output_rows:
        ...
        writer.close()
        raise
    return {"parsed_at": parsed_at, "rows": stats["rows"], "rows_by_kind": counts, "basis_counts": basis_counts,
            "recovered_legacy_links": stats["recovered_legacy_links"],
            "mismatched_nonnull_record_url": stats["mismatched_nonnull_record_url"],
            "unresolved": stats["unresolved"], "ambiguous": stats["ambiguous"],
