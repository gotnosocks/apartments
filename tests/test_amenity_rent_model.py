    ...
    for year in range(2016,2020):
        for month in (1,4,7,10):
            for unit in range(20):
                rows.append({'unit_id':f'u{unit}','building':f'b{unit%5}',
                    'period':pd.Timestamp(year,month,1),'bedrooms':float(unit%3),
                    'bathrooms':1.,'square_feet':500.+unit*10,
                    'asking_rent':float(np.exp(8+.2*(unit%3)+.08*(unit%2)+.02*(year-2016))),
                    'physical_floor':unit%6+1,'listed_floor':unit%6+1,
                    'elevator':unit%2==1,'laundry_type':'in_unit' if unit%2 else 'none',
                    'doorman_type':None,'hvac_type':'central_ac',
...
def test_robust_fit_converges_and_saved_design_roundtrips(tmp_path):
    data = fixture()
    fitted = base.fit(data,model.SETTINGS,unit_effect=False,iterations=12,encoder_class=model.AmenityEncoder)
    assert fitted['robust_objective_relative_change'] < 1e-5
    original = base.predict(fitted,data)
    assert np.sqrt(np.mean((original-data.log_rent)**2)) < .03
    content = {'amenities': {'encoder':fitted['encoder'].metadata(),
                            'beta':fitted['beta'].tolist(),'center':fitted['center']}}
    publish_bundle(tmp_path,{'models.json':json.dumps(content,sort_keys=True)}, {'version':model.VERSION})
...
def test_paired_comparison_resamples_same_buildings():
    data = pd.DataFrame({'building':['a','a','b'],'asking_rent':[2000.,3000.,4000.],
                         'baseline_rent':[2200.,3300.,4400.],'amenity_rent':[2000.,3000.,4000.]})
...
def test_dataframe_missing_values_do_not_become_category_levels_or_hide_aliases():
    row = model.feature_record({'physical_floor':np.nan,'floors_above_ground':5,
                                'pet_policy':pd.NA,'period':'2024-01-01'})
...
def test_extracted_view_vocabulary_is_accepted_by_model():
    ...
    attrs = extract_attribute_evidence({'propertyDetails':{'features':{'views':['CITY','PARK']}}})['attributes']
    raw = model.pricing._raw_features({**attrs,'observed_at':'2024-01-01'},'2020-01-01')
...
def test_category_contrast_matches_prediction_difference_and_rejects_unknown():
    ...
    contrast = category_contrast(fitted,'laundry_type','none','in_unit',baseline_rent=before)
