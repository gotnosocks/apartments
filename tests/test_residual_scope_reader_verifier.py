...
spec = importlib.util.spec_from_file_location('scope_readers', Path(__file__).parents[1]/'docs/analysis/scripts/verify_residual_scope_readers.py')
...
def test_identical_design_is_explicit(spline_train):
    ...
    assert result['floor_knots'] == [1., 5., 10., 20., 35., 52.]
...
def test_design_drift_rejected(spline_train, fault):
    ...
    b = deepcopy(a)
    if fault == 'prior': b.prior_scales[0] *= 2
    elif fault == 'knots': b.floor_knots[-1] += 1
    elif fault == 'basis': b.floor_basis[0][0] += .1
    elif fault == 'category': next(iter(b.categories.values()))['basis'][0][0] += .1
    elif fault == 'rank':
...
def test_empirical_changes_reported_not_hidden(spline_train):
    ...
    assert result['numeric_normalization_changes'][0]['raw_unit_prior_sd_relative_change'] == -.5
