...
import pytest
from apartments.attribute_evidence import extract_attribute_evidence as extract, DIRECT_OFFER_FLOOR_RULE


def test_complete_reviewed_descriptions_recover_independent_floor_claims():
    cases = json.loads((Path(__file__).parent/'fixtures/direct_floor_offers.json').read_text())
    for case in cases:
        text = case['description']
        assert hashlib.sha256(text.encode()).hexdigest() == case['description_sha256']
        result = extract({'description': text})
        assert result['attributes']['advertised_floor'] == {'3967693': 2, '776029': 1}[case['source_listing_id']]
...
@pytest.mark.parametrize('text', [
    ...
    'Not this first floor apartment.', 'This first floor apartment might be available.',
    ...
    'The first floor gym is shared.', 'Unit two, floor unknown.',
])
...
def test_structured_floor_disagreement_preserves_both_claims():
    result = extract({'propertyDetails': {'floor': 3},
    ...
    assert {e['value'] for e in result['evidence'] if e['attribute'] == 'advertised_floor'} == {1, 3}
