...
def test_quarantine_is_explicit_and_never_rewrites_source_price():
    rows=[{'audit_id':'a','source_listing_id':'10','asking_rent':30000},
          {'audit_id':'b','source_listing_id':'20','asking_rent':3000}]
    edit={'audit_id':'a','source_listing_id':'10','action':'quarantine','reasons':['commercial'],
          'evidence':['source hash'],'author':'reviewer'}
...
def test_real_revision_fit_exact_cohort_comparison_and_replay(tmp_path,monkeypatch):
    ...
    for period in pd.date_range('2017-01-01','2019-12-01',freq='MS'):
        for unit in range(6):
            ...
            rows.append(dict(period=period,unit_id=f'u{unit}',building=f'b{unit//2}',building_id=f'b{unit//2}',
                audit_id=f'{period}-{unit}',source_listing_id=f'{period}-{unit}',bedrooms=1.,bathrooms=1.,square_feet=600.,
    ...
    capture=dict(unit_id='u0',building_id='b0',source='streeteasy',source_listing_id='current',capture_id='c',
        collected_at='2020-01-02T00:00:00Z',known_at='2020-01-02T01:00:00Z',listing_status='ACTIVE',
        rent=3900,bedrooms=1,bathrooms=1,price_basis='gross_advertised_rent')
    ...
    parent=tmp_path/'parent';analysis.run(tmp_path,source,parent,as_of='2020-01-02T02:00:00Z')
    dm,_=_verified_bundle(parent/'dataset')
    edits=[{'audit_id':r['audit_id'],'source_listing_id':r['source_listing_id'],
          'action':'quarantine','reasons':['fixture'],'author':'reviewer','evidence':['fixture']} for r in rows[:6]]
    decisions=tmp_path/'decisions'
    publish_bundle(decisions,{'decisions.jsonl':''.join(canonical(edit)+'\n' for edit in edits)},
    ...
    assert revision.run(parent/'model',parent/'dataset',decisions,output)==result
