...
def test_rapid_initial_source_changes_require_date_value_size_and_time_match():
    ...
    assert len(found)==1 and found[0]['elapsed_seconds']==23 and found[0]['price_ratio']==10
...
def test_order_duplicates_and_finite_values():
    payload=prices();events=payload['pricing']['priceChanges']
...
def test_language_preserves_context_for_review_not_automatic_classification():
    ...
    assert found[0]['context']==text  # This is a false-positive candidate to reject in review.
    assert not any('exclude' in f or 'replacement_price' in f for f in found)
    assert screen({'description':'Apartment with a home office, near shops.'},'2020-01-01',3000)==[]
    found=screen({'description':'Advertise rent it’s NET EFFECTIVE'},'2020-01-01',3000)
