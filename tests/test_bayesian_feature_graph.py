    ...
    rows = pd.DataFrame(
        {
            ...
            "unit_id": ["u" + str(i) for i in range(n)],
...
def test_compression_is_exact_even_for_adjacent_floating_point_values():
    ...
    matrix = np.array([[1.0, 2.0], [near, 2.0], [1.0, 2.0], [-3.0, 4.0], [near, 2.0]])
    ...
    with pytest.raises(ValueError, match="Finite two-dimensional"):
        compressed.compress(np.array([1.0, 2.0]))
    with pytest.raises(ValueError, match="Finite two-dimensional"):
        compressed.compress(np.array([[1.0, np.nan]]))
