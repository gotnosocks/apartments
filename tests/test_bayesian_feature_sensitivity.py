...
def rewrite(root, protocol_change=None, fit_change=None):
    ...
    publish_binary(root/'protocol',files,{'version':m.verified.EXPERIMENT_VERSION,'protocol_sha256':ph})
    ...
    if fit_change:
        fit_change(files)
    publish_binary(root/'fit',files,{'version':m.verified.EXPERIMENT_VERSION,'protocol_sha256':ph})


@pytest.fixture
def pair(experiment,tmp_path):
    first,dataset,*_ = experiment
    rewrite(first,{'seed':71,'adaptation':'diag','target_accept':.95,'versions':{'synthetic':'1'}})
    second = tmp_path/'second'
    shutil.copytree(first,second)
    rewrite(second,{'prior_multiplier':.5,'seed':72,'draws':1200,'tune':1500,'chains':6})
...
def test_saved_summary_comparison_and_immutable_replay(pair,tmp_path):
    ...
    def change(files):
        ...
        for index,row in enumerate(residuals):
            fit = row['asking_rent']*(.95+.02*index)
    ...
    assert full['changes']['percent_effect']['median_change'] == pytest.approx(.02)
    assert full['support_before'] == {'rows':1,'units':1,'buildings':1}
    coef = next(r for r in comparison['coefficients']['encoded_value_coefficients'] if r['feature']=='bedrooms_gt_1')
    assert coef['changes']['log_coefficient']['median_change'] == pytest.approx(.01)
    ...
    assert residuals['signed_log_residual']['spearman_rho'] == pytest.approx(1/math.sqrt(5))
    ...
    assert current['source_record']['analysis_price_basis'] == 'current_capture_gross_ask'
    assert set(full['changes']['percent_effect']) == {'reference','candidate','median_change','lower_endpoint_change','upper_endpoint_change','intervals_overlap_descriptively'}
...
@pytest.mark.parametrize('change',[
    {'adaptation':'low_rank'}, {'target_accept':.99}, {'versions':{'synthetic':'2'}},
    {'purpose':'changed estimand'}, {'likelihood':'changed target'},
])
...
@pytest.mark.parametrize('name',['feature-design.json','time-design.json','time-design.npz'])
def test_design_must_match_exactly_despite_rehashed_products(pair,name):
    first,second,dataset = pair
    def change(files):
        files[name] += b' '  # Valid JSON whitespace or changed binary, with a valid manifest.
    rewrite(second,fit_change=change)
...
@pytest.mark.parametrize('change',[{'draws':0},{'tune':-1},{'chains':True},{'seed':-1}])
...
def test_both_diagnostic_gates_apply(pair,family):
    ...
    def change(files):
        ...
        files['summary.json'] = canonical(summary)+'\n'
        name = 'diagnostics.json' if family == 'diagnostics' else 'derived-diagnostics.json'
...
def test_same_valid_archived_implementation_required(pair):
    first,second,dataset = pair
    for root,content in [(first,b'original implementation'),(second,b'changed implementation')]:
        ...
        rewrite(root,{'implementation_sha256':{'implementation.py':manifest['files']['implementation.py']}})
    with pytest.raises(ValueError,match='Fixed protocol identity'):
        m.build_comparison([first,second],dataset)
...
def test_nonunit_reference_rule_and_three_fits(pair,tmp_path):
    ...
    assert [r['candidate_prior_multiplier'] for r in report['comparisons']] == [.5,2]
    report,_ = m.build_comparison([third,second],dataset)
    assert report['reference_prior_multiplier'] == .5
...
def test_support_semantics_and_duplicate_contrasts_cannot_be_paired():
    interval = {'median':1,'lower_95':0,'upper_95':2,'probability_positive':.9}
