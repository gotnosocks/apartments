...
def source(tmp_path, count=2, overlay=None):
    ...
    for index in range(count):
        ...
        rows.append(dict(unit_id=canonical_unit_id(url),building_id='demo',canonical_unit_url=url,
            source='streeteasy',source_listing_id=str(100+index),capture_id=str(index),
            collected_at='2026-09-17T10:00:00Z',known_at='2026-09-17T11:00:00Z',
            listing_status='ACTIVE',rent=3500,price_basis='gross_advertised_rent',bedrooms=0,bathrooms=1))
    root=tmp_path/'source'
    publish_bundle(root,{'candidates.jsonl':''.join(canonical(r)+'\n' for r in rows)},
        {'snapshot_version':'canonical-candidate-captures-v1','as_of':'2026-09-18T16:00:00Z','overlay':overlay})
    return root,rows


def html(row, *, status='ACTIVE', rent=3700, canonical_url=None):
    listing={'id':int(row['source_listing_id']),'status':status,'pricing':{'price':rent},
             'description':'Studio apartment with a washer/dryer in unit.',
             'propertyDetails':{'address':{'displayUnit':row['canonical_unit_url'].split('/')[-1]},
                                'bedroomCount':0,'fullBathroomCount':1}}
    # Current price/status are usable even when the historical event panel is absent.
    return ('<html><head><link rel="canonical" href="'+(canonical_url or row['canonical_unit_url'])+
            '"></head><body><script type="application/json">'+json.dumps({'listing':listing})+
...
def test_bounded_refresh_resume_replay_and_changed_status(tmp_path):
    src,rows=source(tmp_path);output=tmp_path/'refresh';calls=[]
    async def fetch(request):
        calls.append(request.url);row=next(r for r in rows if request.url.endswith(r['source_listing_id']))
        request.meta['archive_provider']={'submission_attempts':1,'password':'fixture-secret',
            'results':[{'content':'must-not-duplicate-body','status_code':200,'job_id':'fixture-job'}]}
        return HtmlResponse(request.url,request=request,body=html(row,status='RENTED' if len(calls)==2 else 'ACTIVE'))
    ...
    assert all(r['rent']==3700 and r['capture_id'].startswith('refresh:') for r in fresh)
    assert all(r['refresh_provenance']['history_warning']=='propertyHistory missing or unresolved' for r in fresh)
    ...
    try:
        metadata=[r[0] for r in archive.db.execute('SELECT capture_metadata FROM observations')]
        assert len(metadata)==2
        assert all('fixture-secret' not in m and 'must-not-duplicate-body' not in m for m in metadata)
...
def test_failed_refresh_never_falls_back(tmp_path,failure):
    ...
    async def fetch(request):
        return HtmlResponse(request.url,request=request,status=404 if failure=='http' else 200,
            body=html(rows[0],canonical_url='https://streeteasy.com/building/demo/wrong'))
    output=tmp_path/'refresh';result=run(src,output,fetch=fetch)
...
def test_interrupted_unknown_submission_does_not_repeat(tmp_path):
    src,_=source(tmp_path,count=1);output=tmp_path/'refresh';plan,ph=refresh.prepare(src,output)
    publish_bundle(output/'requests'/'0000',{'intent.json':'{}\n'},
        {'version':refresh.VERSION,'plan_sha256':ph,'target_sha256':refresh._hash(plan['targets'][0]),'index':0})
...
def test_blocked_response_pause_survives_restart_and_checkpoint_gap(tmp_path):
    src,_=source(tmp_path);output=tmp_path/'refresh';calls=[]
    async def fetch(request):
        calls.append(request.url)
        return HtmlResponse(request.url,request=request,status=429,body=b'Rate limited')
...
def test_frozen_correction_ledger_applies_to_new_capture(tmp_path,monkeypatch):
    ...
    monkeypatch.setattr(corrections,'now',lambda:corrections.instant('2026-09-18T12:00:00Z'))
    corrections.append(ledger,author='reviewer',reason='confirmed layout',edit={
        'target':{'source':'streeteasy','source_listing_id':'100'},'validity':{'from':'2026-09-18'},
    ...
    output=tmp_path/'refresh';run(src,output,fetch=fetch,ledger=ledger)
    row=json.loads((output/'snapshot'/'candidates.jsonl').read_text())
    assert row['bedrooms']==1 and row['corrections'][0]['reason']=='confirmed layout'
