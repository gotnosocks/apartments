...
def test_literal_amounts_preserve_basis_and_exact_offsets(text, net, gross):
    ...
    assert {(r["basis_label"], r["amount"]) for r in result["amounts"]} == {
        ("net", net),
        ("gross", gross),
    }
...
@pytest.mark.parametrize(
    "text",
    [
        "Net effective price listed.",
        "NET EFFECTIVE RENT ADVERTISED.",
        "The listed rent is net effective.",
        "Rent is advertised as net effective.",
        "Advertised rent is net.",
    ],
)
...
def test_shown_net_signal_retains_negation_and_ignores_unrelated_shown_text():
    ...
    for text in [
        "Net income shown.",
        "Internet shown in photos.",
        "Price shown is base rent.",
    ]:
...
@pytest.mark.parametrize(
    ...
    [
        "Not net rent $3,000.",
        "This is not a net effective rent of $3,000.",
...
def test_negated_amount_labels_are_not_positive_target_matches(text):
    result = measure(text, 3000)
    assert (
        result["target_matches"] == [] and result["amounts"][0]["preceded_by_negation"]
    )
...
def test_missing_or_ambiguous_text_does_not_imply_gross():
    ...
    assert (
        measure("Net effective rent listed, gross is 7,300.", 7300)["target_matches"]
        == []
    )
    assert measure("Net $3,000, gross $3,000.", 3000)["target_matches"] == [
        "gross",
        "net",
    ]
...
def test_a_following_net_label_does_not_relabel_the_preceding_gross_amount(separator):
    text = (
        "Gross rent: $1895"
        + separator
        + "Net rent (2 weeks free for an immediate lease start): $1815"
    )
    result = measure(text, 1895)
    assert result["target_matches"] == ["gross"]
    assert [(c["basis_label"], c["amount"]) for c in result["amounts"]] == [
        ("gross", 1895),
        ("net", 1815),
    ]
...
@pytest.mark.parametrize(
    "text",
    ["$4095 Gross Rent - $3755 Net Effective Rent", "Gross rent $4095 Net rent $3755"],
)
...
def test_gross_price_per_month_is_not_relabelled_by_following_net_offer():
    ...
    assert result["target_matches"] == ["gross"]
    assert (
        result["amounts"][0]["amount"] == 4500
        and result["amounts"][0]["basis_label"] == "gross"
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            ["Net rent $3,000.", "Net effective rent $3,000."],
            "all_captures_match_explicit_net_only",
        ),
        (["Net rent $3,000.", None], "some_captures_match_explicit_net"),
        (
            ["Net rent $3,000.", "Gross rent $3,000."],
            "target_matches_both_net_and_gross_quotes",
        ),
        (
            ["Net effective rent advertised. Gross rent $3,000."],
            "target_matches_explicit_gross",
        ),
        (["Legal rent $3,000."], "target_matches_explicit_legal"),
        (
            ["Net effective rent advertised."],
            "advertised_net_statement_without_target_amount_match",
        ),
        (
            ["Income approval uses net rent $3,000."],
            "net_mention_without_target_amount_match",
        ),
    ],
)
def test_all_attached_captures_and_administrative_context_affect_review_priority(
    texts, expected
):
...
def test_explicit_dollarless_net_quotes_keep_literal_amounts_without_arithmetic(
    text, value
):
...
@pytest.mark.parametrize(
    "text",
    [
        "Internet 3000 included.",
        "Net 3000 square feet.",
        "Lease effective 2020.",
        "Net monthly cost with 1 month free.",
        "Net effective rent 2,99.",
        "Net effective rent 3,000,000.",
...
def test_bare_quote_retains_negation_and_administrative_context_and_avoids_duplicate_currency():
    ...
    assert measure("Gross rent $3000 Net effective rent 2800.", 3000)[
        "target_matches"
    ] == ["gross"]
