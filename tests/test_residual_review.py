...
class Model:
    ...
    def predict(self, row, month):
        components = {
            "center": math.log(3000),
            "building:b": 0.1,
            "unit:u": -0.05,
            "bedrooms_gt_0": 0.2,
            "amenity:laundry_type=in_unit": 0.03,
            "amenity:window_exposures.north.unknown": -0.01,
        }
        return {
            "predicted_rent": math.exp(math.fsum(components.values())),
...
def test_residual_sign_group_contributions_and_deliberate_training_inclusion():
    ...
    assert math.exp(
        math.fsum(result["log_contributions_by_family"].values())
    ) == pytest.approx(result["fitted_rent"])
    assert result["log_contributions_by_family"]["missingness"] == -0.01
    assert result["current_capture_in_fit"] is True
    assert result["residual_type"] == "in_sample_fitted_diagnostic"
    assert result["asking_vs_fitted_percent"] == pytest.approx(
        100 * (5000 / result["fitted_rent"] - 1)
    )


def test_review_tails_are_distinct_units_and_retain_all_current_rows():
    rows = []
    for i, (unit, error, current) in enumerate(
        [
            ("u", 2, False),
            ("u", 1.9, False),
            ("v", 1, False),
        ...
        rows.append(
            dict(
                ...
                unit_id=unit,
                log_residual=error,
                absolute_log_residual=abs(error),
    ...
    assert next(r for r in queue if r["audit_id"] == "5")["selection_reasons"] == [
        "current_capture"
    ]
