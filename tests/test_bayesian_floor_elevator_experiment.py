...
def configure(setup):
    ...
    args.interaction_prior_scale = .15
...
@pytest.mark.parametrize('fault', [None, 'source', 'mode', 'scale', 'thresholds', 'code', 'missing_code', 'version', 'failed'])
def test_graph_proof_is_bound_to_full_interaction_contract(setup, fault):
    ...
    keys = ('source_manifest_sha256', 'source_observations_sha256', 'specification',
            'floor_increment_prior_scale', 'floor_levels', 'graph_configuration', 'rows',
            'interaction_mode', 'interaction_prior_scale', 'interaction_thresholds')
    proof = {k: deepcopy(protocol[k]) for k in keys}
    proof.update(version=m.PARITY_VERSION, passed=True, implementation_sha256=deepcopy(code))
    if fault == 'source': proof['source_observations_sha256'] = 'wrong'
    elif fault == 'mode': proof['interaction_mode'] = 'separate'
    elif fault == 'scale': proof['interaction_prior_scale'] = .3
    elif fault == 'thresholds': proof['interaction_thresholds'].append(5)
    elif fault == 'code': proof['implementation_sha256']['bayesian_floor_elevator_design.py'] = 'wrong'
    elif fault == 'missing_code': del proof['implementation_sha256']['bayesian_feature_graph.py']
    elif fault == 'version': proof['version'] = 'old-proof'
    elif fault == 'failed': proof['passed'] = False
    ...
    if fault:
        with pytest.raises(ValueError, match='Interaction graph proof'): m.make_protocol(args, frame, source, code, config)
    else:
        assert m.make_protocol(args, frame, source, code, config)['graph_verification']['rows'] == len(frame)
...
def test_joint_floor_access_contrasts_preserve_covariance_and_gate_intervals(fault):
    frame = data(); design = m.feature.FeatureDesign(frame, mode='separate')
    rng = np.random.default_rng(148)
    beta = rng.normal(0, .02, size=(4, 1200, len(design.features)))
    i, j = [design.features.index(k) for k in ('listed_floor_gt_2', 'floor_elevator_gt_2')]
    beta[:, :, i] = rng.normal(0, .3, size=(4, 1200))
    beta[:, :, j] = -2*beta[:, :, i]+rng.normal(.1, .002, size=(4, 1200))
    if fault == 'unmixed': beta[0, :, i] += 10
    coords = design.features[::-1] if fault == 'coordinates' else design.features
    p = xr.Dataset({'beta': (('chain', 'draw', 'feature'), beta)}, coords={'feature': coords})
    if fault == 'coordinates':
        with pytest.raises(ValueError, match='coordinates'): m.joint_contrasts({'posterior': p}, design, frame)
        return
    ...
    if fault == 'unmixed':
        ...
        expected = (beta[:, :, i]+.5*beta[:, :, j]).ravel()
        np.testing.assert_allclose([case['log_effect'][k] for k in ('lower_95', 'median', 'upper_95')],
                                  np.quantile(expected, [.025, .5, .975]))
        assert case['log_contrast_prior_sd'] == pytest.approx(2*np.hypot(.15, .075))


def test_absent_endpoint_is_explicit_even_when_global_floor_is_supported():
    frame = data(); design = m.feature.FeatureDesign(frame)
    reduced = frame.loc[~(frame.advertised_floor.eq(8) & frame.elevator.eq(False))]
    case = next(r for r in m.construct_contrasts(design, reduced) if r['id'] == 'floor:5->8:elevator=0')
...
def test_no_parity_proof_refuses_sampling_before_creating_output(tmp_path):
    args = m.argument_parser().parse_args(['--dataset', str(tmp_path/'missing'), '--output', str(tmp_path/'output')])
    with pytest.raises(ValueError, match='parity'): m.run(args)
...
def configure_runner(setup, monkeypatch, acceptable=True):
    ...
    args.interaction_mode = 'pooled'; args.interaction_prior_scale = .15
    ...
    monkeypatch.setattr(m, 'joint_contrasts', lambda *a, **k: ({'version': m.CONTRAST_VERSION,
        'all_contrasts_acceptable': acceptable}, pd.DataFrame({'ess_bulk': [1000]})))
    original = m.previous.v2.write_reports
    def report(*a):
        value = original(*a); value['status'] = 'exploratory_converged'; return value
    monkeypatch.setattr(m.previous.v2, 'write_reports', report)
    frame, source = m.previous.load_data(args.dataset)
    code = {p.name: m.digest(p) for p in m.implementation_paths()}
    config = m.previous.graph.graph_configuration(frame)
    protocol = m.make_protocol(args, frame, source, code, config)
    fields = ('source_manifest_sha256', 'source_observations_sha256', 'specification',
              'floor_increment_prior_scale', 'floor_levels', 'graph_configuration', 'rows',
              'interaction_mode', 'interaction_prior_scale', 'interaction_thresholds')
    proof = {k: protocol[k] for k in fields}
    proof.update(version=m.PARITY_VERSION, passed=True, implementation_sha256=code)
    args.graph_validation = args.output.parent/'proof'
    publish_bundle(args.graph_validation, {'parity.json': json.dumps(proof)}, {'version': m.PARITY_VERSION})
    return args, events


@pytest.mark.parametrize('acceptable', [False, True])
def test_disk_lifecycle_gates_joint_contrasts_and_reuses_completed_fit(setup, monkeypatch, acceptable):
    args, events = configure_runner(setup, monkeypatch, acceptable)
    result = m.run(args)
    assert result['floor_elevator_contrasts_acceptable'] is acceptable
    assert result['status'] == ('exploratory_converged' if acceptable else 'diagnostic_only_do_not_interpret_intervals')
    manifest = json.loads((args.output/'fit/complete.json').read_text())
    assert manifest['version'] == m.VERSION and m.REQUIRED_FIT <= manifest['files'].keys()
...
def test_reporting_resume_keeps_existing_joint_draws(setup, monkeypatch):
    ...
    original = m.previous.v2.write_reports
    monkeypatch.setattr(m.previous.v2, 'write_reports', lambda *a: (_ for _ in ()).throw(RuntimeError('report interrupted')))
