...
@pytest.mark.parametrize('fault', [None, 'rows', 'draws', 'prior', 'math', 'extra', 'missing', 'source', 'storage'])
def test_protocol_changes_are_confined_to_verified_source_readers(fault):
    a, b = protocols()
    if fault == 'rows': b['rows'] = 15
    elif fault == 'draws': b['draws'] = 50
    elif fault == 'prior': b['prior_multiplier'] = 2
    elif fault == 'math': b['implementation_sha256']['math.py'] = 'changed'
    elif fault == 'extra': b['implementation_sha256']['extra.py'] = 'added'
    elif fault == 'missing': del b['implementation_sha256']['elevator_corrections.py']
    elif fault == 'source': b['source_version'] = m.correction.PARENT
...
def test_published_revision_has_exact_inverse_and_no_membership_or_price_change(tmp_path, fault):
    row, *_ = fixture()
    ...
    if fault == 'unrelated': rows[0]['asking_rent'] = 9999
    if fault == 'parent': revised_manifest['source_manifest_sha256'] = 'wrong'
    publish_bundle(child, {'observations.jsonl': ''.join(canonical(r)+'\n' for r in rows),
        m.correction.SIDECAR: ''.join(canonical(r)+'\n' for r in changes)},
        {k: v for k, v in revised_manifest.items() if k != 'files'})
    if fault:
        with pytest.raises(ValueError): m.verify_revision(parent, child)
    else:
...
def designs():
    ...
    return [{'design': FeatureDesign(frame), 'data': frame} for frame in (before, after)]


@pytest.mark.parametrize('fault', [None, 'price_feature', 'centering', 'prior', 'category'])
def test_only_elevator_design_columns_change(fault):
    a, b = designs()
    if fault == 'price_feature': b['data'].loc[0, 'square_feet'] *= 2
    elif fault == 'centering': b['design'].means[0] += .1
    elif fault == 'prior': b['design'].prior_scales[0] *= 2
    elif fault == 'category': b['design'].categories['laundry_type']['frequencies'][0] += .01
    if fault:
...
def test_raw_elevator_and_missingness_scenarios_have_correct_units_and_prior_variance():
    ...
    for fit, vectors in zip((a, b), output):
        d = fit['design']; center, scale = [d.numeric['elevator'][k] for k in ('center', 'scale')]
        i, j = [d.features.index(k) for k in ('elevator', 'elevator.unknown')]
        np.testing.assert_allclose(np.array([v['design_vector'] for v in vectors])[:, [i, j]],
                                   [[1/scale, 0], [center/scale, 1], [(center-1)/scale, 1]])
        assert vectors[0]['raw_contrast_prior_sd'] == pytest.approx(.15/scale)
        assert vectors[1]['raw_contrast_prior_sd'] == pytest.approx(np.hypot(.15*center/scale, .2))
        assert vectors[0]['support_before']['rows'] == int(fit['data'].elevator.eq(False).sum())
        assert vectors[1]['support_after']['rows'] == int(fit['data'].elevator.isna().sum())
    assert output[0][0]['raw_contrast_prior_sd'] != output[1][0]['raw_contrast_prior_sd']
...
def test_all_joint_draws_preserve_elevator_missingness_covariance(tmp_path, fault):
    fit, _ = designs(); design = fit['design']
    rng = np.random.default_rng(211)
    beta = rng.normal(0, .01, size=(4, 1000, len(design.features)))
    i, j = [design.features.index(k) for k in ('elevator', 'elevator.unknown')]
    beta[:, :, i] = rng.normal(0, .2, size=(4, 1000))
    c = design.numeric['elevator']['center']/design.numeric['elevator']['scale']
    beta[:, :, j] = -.99*c*beta[:, :, i]+rng.normal(.1, .001, size=(4, 1000))
    ...
    fit.update(root=tmp_path, protocol={'chains': 4, 'draws': 50 if fault == 'draw_count' else 1000, 'prior_multiplier': 2})
    if fault:
        with pytest.raises(ValueError): m.elevator_contrasts(fit)
    else:
        result = m.elevator_contrasts(fit)
        expected = (c*beta[:, :, i]+beta[:, :, j]).ravel()
        np.testing.assert_allclose([result[1]['log_effect'][k] for k in ('lower_95', 'median', 'upper_95')],
                                  np.quantile(expected, [.025, .5, .975]))
        assert result[0]['raw_contrast_prior_sd'] == pytest.approx(.3/design.numeric['elevator']['scale'])
