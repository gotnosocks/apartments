...
def test_density_ratio_exactly_changes_only_five_normal_priors():
    ...
    beta = rng.normal(0, 0.04, (4, 500, 5))
    for alt in (0.05, 0.20):
        expected = (norm.logpdf(beta, scale=alt) - norm.logpdf(beta, scale=0.1)).sum(
            axis=2
        )
        np.testing.assert_allclose(
            m.prior_log_ratios(beta, 0.1, alt), expected, rtol=1e-13, atol=1e-13
        )
    np.testing.assert_array_equal(
        m.prior_log_ratios(beta, 0.1, 0.1), np.zeros((4, 500))
    )
...
def test_bad_scales_rejected(scale):
    with pytest.raises(ValueError):
        m.prior_log_ratios(np.zeros((4, 1000, 5)), 0.1, scale)


def test_weight_normalization_and_diagnostics_preserve_chain_order():
    rng = np.random.default_rng(4012)
    beta = rng.normal(0, 0.025, (4, 3000, 5))
    ratios = m.prior_log_ratios(beta, 0.1, 0.05)
    weights, diagnostic = m.importance_weights(ratios, 0.8)
    assert diagnostic["acceptable"]
    assert diagnostic["pareto_k"] < 0.5
    assert weights.shape == (4, 3000) and weights.sum() == pytest.approx(1.0)
...
def test_weight_concentration_and_low_reff_fail_closed():
    ...
    ratios = m.prior_log_ratios(rng.normal(0, 0.02, (4, 3000, 5)), 0.1, 0.05)
    _, diag = m.importance_weights(ratios, 0.001)
    assert not diag["acceptable"]
...
def test_weighted_quantiles_are_empirical_not_resampled():
    value = m.weighted_interval([-100.0, 1.0, 2.0, 4.0], [0.0, 0.025, 0.925, 0.05])
    assert value == {
        "lower_95": 1.0,
        "median": 2.0,
        "upper_95": 4.0,
...
def test_reweighting_matches_analytic_gaussian_changed_prior():
    ...
    for alt in (0.05, 0.2):
        weights, diagnostic = m.importance_weights(
            m.prior_log_ratios(beta, baseline, alt), 1.0
        )
        assert diagnostic["acceptable"]
        newvar = 1 / (1 / alt**2 + 1 / likelihood_sd**2)
        newmean = newvar * likelihood_mean / likelihood_sd**2
        interval = m.weighted_interval(beta[:, :, 0], weights)
        expected = norm.ppf([0.025, 0.5, 0.975], loc=newmean, scale=np.sqrt(newvar))
        np.testing.assert_allclose(
            [interval[k] for k in ("lower_95", "median", "upper_95")],
            expected,
            atol=0.001,
        )
...
def simple_case():
    features = [
        ...
        *[f"listed_floor_spline_{i}" for i in range(5)],
    ]

    def direction(low, high):
        return np.r_[0.0, 0.0, np.array([1.0, 0.4, -0.3, 0.2, 0.1]) * (high - low) / 50]

    design = SimpleNamespace(
        version=m.contract.DESIGN,
        floor_prior_scale=0.1,
        floor_levels=[1.0, 2.0, 10.0, 52.0],
        features=features,
        prior_scales=np.array([0.2, 0.2, *([0.1] * 5)]),
        floor_support={"test": True},
        contrast_vector=direction,
    )
    beta = np.random.default_rng(4216).normal(0, 0.025, (4, 3000, len(features)))
    return design, beta
...
def test_only_five_floor_priors_change_and_intervals_match_base_rules(simple_case):
    ...
    assert (
        result["posterior_refitted"] is False
        and result["main_selection_changed"] is False
    )
...
def test_failed_weights_withhold_all_alternative_nonzero_intervals(
    simple_case, monkeypatch
):
    design, beta = simple_case
...
def test_failed_baseline_coefficients_refused(simple_case):
    design, beta = simple_case
    beta[0, :, 2:] += 1.0
    with pytest.raises(ValueError, match="coefficient diagnostics fail"):
        m.calculate(design, beta)
...
def test_psis_adapter_smooths_large_weights_not_their_inverse():
    ...
    ratios = rng.normal(0, 0.1, (4, 3000))
    ratios[0] += 2.0
    weights, diagnostic = m.importance_weights(ratios, 1.0)
    assert diagnostic["raw"]["chain_mass"][0] > 0.70
    assert diagnostic["psis"]["chain_mass"][0] > 0.70
    assert weights[0].sum() > 0.70
...
def test_degenerate_psis_tail_withholds_instead_of_inventing_k():
    _, diagnostic = m.importance_weights(np.zeros((4, 3000)), 1.0)
