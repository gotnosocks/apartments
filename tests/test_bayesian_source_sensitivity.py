...
def test_rehashed_revision_changes_still_rejected(revision, mutation):
    ...
    if mutation == "unit":
        rows[0]["unit_id"] = "other"
    if mutation == "count":
        rows[0]["reported_full_bathrooms"] = 2
    if mutation == "flag":
...
def protocols():
    a = {
        ...
        "implementation_sha256": {"original.py": "abc"},
        "source_manifest_sha256": "old",
        "source_observations_sha256": "old-observations",
        "source_version": m.overlay.SOURCE_VERSION,
    ...
    b = {
        **a,
        ...
        "source_observations_sha256": "new-observations",
        "source_version": m.overlay.VERSION,
        "source_directory": "revision",
        "residual_scale": "shared",
        "building_prior_scale": 0.35,
        "unit_prior_scale": 0.25,
        "graph_configuration": {"residual_scale": "shared"},
        "implementation_sha256": {
            **a["implementation_sha256"],
            **{n: "extra" for n in m.noise.V3_CODE},
        },
    }
    return a, b
...
def test_shared_source_comparison_allows_recorded_computational_parameterization(
    parameterization,
):
    a, b = protocols()
...
def test_physical_contrast_identity_preserved_with_separate_support_and_omissions():
    before = [
        {
            ...
            "after_full_half": [2, 0],
            "support_before": {"rows": 5},
            "support_after": {"rows": 6},
            "log_effect": interval(0.2),
        }
    ]
    after = deepcopy(before)
    after[0]["support_after"]["rows"] = 4
    after[0]["log_effect"] = interval(0.19)
    before.append(
        {
            "bedrooms": 2,
            "before_full_half": [4, 0],
            "after_full_half": [5, 0],
            "support_before": {"rows": 2},
            "support_after": {"rows": 1},
            "log_effect": interval(0.3),
        }
    )
    ...
    assert row["changes"]["log_effect"]["median_change"] == pytest.approx(-0.01)
...
def test_matched_residuals_exclude_quarantines_from_both_metrics_and_pair_by_id():
    ...
    def fitted(row, fit):
        return {
            **row,
            "fitted_rent": fit,
            "residual_log": math.log(3000 / fit),
            "residual_dollars": 3000 - fit,
        }
...
def design_archive(tmp_path, rows, version=m.noise.V2):
    ...
    fm = publish_binary(
        target,
        {name: (target / name).read_bytes() for name in m.comparison.DESIGNS},
        {"version": version, "protocol_sha256": ph},
    )
...
def test_independently_rehashed_design_changes_rejected(actual_design, mutation):
    ...
    name = (
        "time-design.json"
        if mutation in {"time_center", "size_median", "group_order"}
        else "feature-design.json"
    )
    path = root / "fit" / name
    value = json.loads(path.read_text())
    if mutation == "center":
        value["means"][0] += 0.1
    if mutation == "numeric_scale":
        value["numeric"]["listed_floor"]["scale"] *= 2
    if mutation == "prior_scale":
        value["prior_scales"][0] *= 2
    if mutation == "active":
        value["active"][0] = not value["active"][0]
    if mutation == "category_basis":
        value["categories"]["laundry_type"]["basis"][0][0] += 0.1
    if mutation == "category_order":
        value["categories"]["laundry_type"]["levels"].reverse()
    if mutation == "time_center":
        value["time_center"][0] += 0.1
    if mutation == "size_median":
        value["size_default"] += 10
    if mutation == "group_order":
        value["buildings"].reverse()
...
@pytest.mark.parametrize(
    "mutation", ["time_values", "season_order", "dtype", "shape", "missing", "extra"]
)
def test_rehashed_npz_array_values_types_shapes_and_membership_rejected(
    actual_design, mutation
):
    root, source, protocol, provenance = actual_design
    path = root / "fit" / "time-design.npz"
    with np.load(path, allow_pickle=False) as saved:
        arrays = {name: saved[name].copy() for name in saved.files}
    if mutation == "time_values":
        arrays["time_matrix"][0, 0] += 0.1
    if mutation == "season_order":
        arrays["season_matrix"] = arrays["season_matrix"][:, ::-1]
    if mutation == "dtype":
        arrays["time_matrix"] = arrays["time_matrix"].astype("float32")
    if mutation == "shape":
        arrays["time_matrix"] = arrays["time_matrix"].reshape(-1)
    if mutation == "missing":
        del arrays["season_matrix"]
    if mutation == "extra":
        arrays["undeclared"] = np.zeros(3)
    np.savez_compressed(path, **arrays)
    provenance["fit_manifest"]["files"][path.name] = digest(path)
...
def test_json_format_and_npz_container_bytes_are_not_design_semantics(actual_design):
    ...
    for name in ("feature-design.json", "time-design.json"):
        ...
        value = json.loads(path.read_text())
        path.write_text(
            json.dumps(dict(reversed(list(value.items()))), indent=5) + "\n"
        )
        provenance["fit_manifest"]["files"][name] = digest(path)
    path = root / "fit" / "time-design.npz"
    with np.load(path, allow_pickle=False) as saved:
        arrays = {name: saved[name].copy() for name in reversed(saved.files)}
...
@pytest.mark.parametrize(
    "mutation",
    ["missing_code", "changed_code", "changed_environment", "changed_source"],
)
def test_reconstruction_fails_before_use_of_mismatched_dependencies(
    actual_design, mutation, monkeypatch
):
    root, source, protocol, provenance = actual_design
    if mutation == "missing_code":
        del protocol["implementation_sha256"]["bayesian_feature_model.py"]
    if mutation == "changed_code":
        protocol["implementation_sha256"]["bayesian_feature_model.py"] = "0" * 64
    if mutation == "changed_environment":
        protocol["versions"]["numpy"] = "different"
    if mutation == "changed_source":
        protocol["source_observations_sha256"] = "0" * 64
    ph = m.overlay.sha(protocol)
    p = root / "protocol" / "protocol.json"
    p.write_text(canonical(protocol) + "\n")
    provenance["protocol_manifest"]["files"][p.name] = digest(p)
    provenance["protocol_manifest"]["protocol_sha256"] = ph
    provenance["fit_manifest"]["protocol_sha256"] = ph
    feature, _ = m.reconstruction_dependencies()
    monkeypatch.setattr(
        feature,
        "FeatureDesign",
        lambda *a: pytest.fail("Must reject before reconstruction"),
    )
...
def complete_synthetic_fit(root, source, rows, version):
    ...
    if version == m.noise.V3:
        ...
        code.update(
            {
                Path(x.__file__).name: Path(x.__file__).read_bytes()
                for x in (graph, runner)
            }
        )
    protocol = {
        "version": version,
        ...
        "rows": len(data),
        "units": data.unit_id.nunique(),
        "buildings": data.building.nunique(),
        "current_rows": 1,
        "specification": "full_half_balance",
        "prior_multiplier": 1.0,
        "chains": 4,
        ...
        "likelihood": "Synthetic accepted summary, no sampling",
    ...
    target = root / "fit"
    target.mkdir(parents=True)
    design = feature.FeatureDesign(data)
    design.save(target)
    beta = np.full((10, len(design.features)), 0.01)
    ...
    residuals = [
        ...
        | {
            "fitted_rent": r["asking_rent"] * 0.95,
            "latent_rent_lower_95": r["asking_rent"] * 0.9,
            "latent_rent_upper_95": r["asking_rent"],
            "residual_dollars": r["asking_rent"] * 0.05,
            "residual_log": math.log(1 / 0.95),
        }
        for r in rows
    ]
    groups = [
    ...
    summary = {
        ...
        "median_absolute_log_residual": math.log(1 / 0.95),
    }
    ...
    files.update(
        {
            ...
            "coefficients.json": canonical(
                [{"feature": name, **interval(0.01)} for name in design.features]
            )
            + "\n",
            "residuals.jsonl": "".join(canonical(r) + "\n" for r in residuals),
            "group-effects.jsonl": "".join(canonical(r) + "\n" for r in groups),
            "posterior.nc": b"SYNTHETIC; no posterior samples",
        }
    )
    if version == m.noise.V3:
        scales = {
            ...
            "global_sigma": {**interval(0.2), "probability_positive": 1.0},
            "global_sigma_role": "Shared observation scale",
            "by_bedroom": [],
        ...
        files.update(
            {
                "graph-configuration.json": canonical(config) + "\n",
...
def comparison_pair(tmp_path, train):
    ...
    edits = []
    for index, action in [
        (1, "mask_bathroom_composition"),
        (2, "quarantine_nonresidential"),
    ]:
        row = rows[index]
        text = "The apartment has 2 FULL BATHS."
        capture = {k: row[k] for k in m.overlay.IDENTITY} | {
            "capture_id": index,
            "body_sha256": "a" * 64,
        ...
        edits.append(decision(row, capture, action))
    publish_bundle(
        audit,
        {"captures.jsonl": "".join(canonical(c) + "\n" for c in captures)},
        {"version": "synthetic-audit"},
    )
    publish_bundle(
        decisions,
        {"decisions.jsonl": "".join(canonical(d) + "\n" for d in edits)},
        {
            "version": m.overlay.DECISION_VERSION,
            "source_manifest_sha256": digest(source / "complete.json"),
            "source_observations_sha256": digest(source / "observations.jsonl"),
...
def test_full_comparison_reconstructs_both_source_designs_and_replays(
    comparison_pair, tmp_path
):
    ...
    first = m.run(*comparison_pair, output)
    assert m.run(*comparison_pair, output) == first


def test_full_comparison_rejects_independent_design_change_after_report_gate(
    comparison_pair,
):
...
def test_source_comparison_accepts_verified_storage_change_but_not_mean_change():
    ...
    a, b = protocols()
    b.update(
        execution_version=disk.VERSION,
        storage_policy=disk.POLICY,
        storage_versions={
            name: "test" for name in ("zarr", "obstore", "xarray", "h5py")
        },
    )
...
def test_source_comparison_does_not_ignore_unknown_storage_policy():
    ...
    b.update(
        execution_version=disk.VERSION,
        storage_policy={**disk.POLICY, "all_retained_draws": False},
        storage_versions={
            name: "test" for name in ("zarr", "obstore", "xarray", "h5py")
        },
    )
    b["implementation_sha256"].update({name: "disk" for name in disk.CODE})
    with pytest.raises(ValueError, match="disk execution"):
        m.check_protocols(a, b)
