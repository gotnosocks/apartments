...
def source(tmp_path, monkeypatch):
    monkeypatch.setattr(
        corrections, "now", lambda: datetime(2026, 9, 19, 4, tzinfo=timezone.utc)
    )
    rows, reviews = [], []
    for key, decision in [
        ("a", "withhold_floor_due_to_scope"),
        ("b", "retain_explicit_floor_claim"),
    ]:
        row = {
            "audit_id": key,
            "unit_id": "unit",
            ...
            "capture_ids": [1 if key == "a" else 2],
            "advertised_floor": 3.0,
            "listed_floor": None,
            "physical_floor": None,
        ...
        text = (
            "Video of the third floor."
            if key == "a"
            else "Apartment is on the third floor."
        )
        capture = {
            k: row[k] for k in ("audit_id", "unit_id", "source_listing_id", "known_at")
        }
        capture.update(
            ...
            review_spans=[{"start": 0, "end": len(text), "literal": text}],
        )
        review = {
            k: row[k] for k in ("audit_id", "unit_id", "building", "source_listing_id")
        }
        review.update(
            decision=decision,
            ...
            explicit_floor=3.0,
            interpreted_at="2026-09-19T03:00:00+00:00",
            proposed_analytical_floor=None if key == "a" else 3.0,
            captures=[capture],
        )
        rows.append(row)
...
def test_mask_changes_only_the_named_row_and_preserves_evidence_and_other_unit_history(
    source,
):
    ...
    assert {
        k: v
        for k, v in revised[0].items()
        if k not in ("advertised_floor", "attribute_review_history")
    } == {k: v for k, v in original[0].items() if k != "advertised_floor"}
    assert changes[0]["before_advertised_floor"] == 3.0
    assert changes[0]["recorded_at"] == WHEN
    assert len(changes[0]["captures"]) == 1
    before, after = (
        listed_floor_values(pd.DataFrame(original)),
        listed_floor_values(pd.DataFrame(revised)),
    )
    assert before.tolist() == [3.0, 3.0]
    assert pd.isna(after[0]) and after[1] == 3.0
...
def test_unreviewed_or_stale_evidence_cannot_change_floors(source, fault):
    ...
    if fault == "typed_capture":
        reviews[0]["captures"][0]["capture_id"] = "1"
    if fault == "changed_row":
        rows[0]["asking_rent"] += 100
    if fault == "changed_source":
        originals[0]["asking_rent"] += 100
    if fault == "missing_review":
        reviews.pop(0)
    if fault == "retained_mask":
        reviews[0]["decision"] = "retain_explicit_floor_claim"
    if fault == "wrong_span":
        reviews[0]["captures"][0]["review_spans"][0]["literal"] = "Wrong"
    if fault == "future_evidence":
        reviews[0]["captures"][0]["known_at"] = "2026-09-20T00:00:00+00:00"
    if fault == "alternative_floor":
        rows[0]["listed_floor"] = originals[0]["listed_floor"] = 4.0
...
def test_source_revision_does_not_reuse_old_ledger_even_when_review_row_matches(source):
    ...
    with pytest.raises(ValueError, match="exact reviewed source version"):
        apply(source)


def test_publication_replay_and_cutoff_use_real_ledger(source, tmp_path):
    rows, originals, reviews, ledger = source
    parent, original, review = [
        tmp_path / name for name in ("parent", "original", "review")
    ]
    publish_bundle(
        parent,
    ...
    publish_bundle(
        ...
        {"decisions.jsonl": "".join(canonical(r) + "\n" for r in reviews)},
        {
            ...
            "source_manifest_sha256": digest(original / "complete.json"),
            "summary": {"observations": 2},
        },
    )
    kwargs = dict(
        dataset=parent,
        reviewed_source=original,
        review=review,
        ledger=ledger,
        as_of=WHEN,
