...
def prepared(tmp_path):
    ...
    publish_bundle(
        benchmark / "protocol", {"settings.json": canonical(settings) + "\n"}, {}
    )
    ...
    record = {
        ...
        "settings": settings,
        "new_draws_generated": 0,
        "original_raw_files_unchanged": True,
        "conversion": {
            "chains": 4,
            "draws": 6000,
            "divergences": 0,
        ...
        "posterior_sha256": digest(recovery / "posterior.nc"),
        "recovery_transform_and_write_seconds": 10,
    }
    publish_bundle(
        recovery / "validation",
        {
            "recovery.json": canonical(record) + "\n",
            "raw-inventory.json": canonical(
                {"field-000.npy": digest(scratch / "field-000.npy")}
            )
            + "\n",
        },
        {},
...
def test_install_preserves_sampling_times_without_inventing_successful_pymc_call(
    tmp_path,
):
    ...
    assert (benchmark / "posterior.nc").read_bytes() == (
        recovery / "posterior.nc"
    ).read_bytes()
...
def test_install_rejects_changed_recovery_or_original_evidence(tmp_path, fault):
    benchmark, recovery = prepared(tmp_path)
    path = {
        "raw": benchmark / "unconstrained-trace/field-000.npy",
        "posterior": recovery / "posterior.nc",
        "progress": benchmark / "progress.json",
    }[fault]
    path.write_text("{}\n")
