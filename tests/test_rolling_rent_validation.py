...
def sample():
    ...
    for period in pd.date_range('2017-01-01','2019-12-01',freq='MS'):
        for unit in range(6):
            rent = 3000.+100*unit+5*(period.year-2017)
            rows.append(dict(unit_id=f'u{unit}',building=f'b{unit//2}',
                audit_id=f'{period.date()}-{unit}',period=period,asking_rent=rent,
                log_rent=np.log(rent),bedrooms=1.,bathrooms=1.,square_feet=600.,
...
def history():
    ...
    for month in range(1,7):
        for unit in range(40):
            rows.append(dict(unit_id=str(unit),building=str(unit//2),audit_id=f'{month}-{unit}',
                             period=f'2019-{month:02d}-01',stratum='seen_unit',
                             asking_rent=3000.*np.exp(.01*month),
                             monthly=3000.,annual_frozen=2900.,monthly_recent=3100.))
    return pd.DataFrame(rows)


def test_current_and_future_targets_cannot_change_forecasts_or_intervals():
    data = sample();origin = pd.Timestamp('2019-04-01')
    train=data[data.period < origin];test=data[data.period.eq(origin)]
    hist=history()
    log_pred=np.full(len(test),np.log(3000.))
    table,rules=rolling.forecast_table(train,test,log_pred,log_pred,hist,origin)
    changed=hist.copy();changed.loc[changed.period >= '2019-04-01','asking_rent']=1e8
    changed_test=test.copy();changed_test['asking_rent']=1e9
    replay,replayed_rules=rolling.forecast_table(train,changed_test,log_pred,log_pred,changed,origin)
    assert rules==replayed_rules
    pd.testing.assert_frame_equal(table.drop(columns='asking_rent'),replay.drop(columns='asking_rent'))
    assert rules['recent_adjustment']['log_offset']==pytest.approx(.02)
    assert rules['interval_rules']['monthly']['seen_unit']['end']=='2019-03-01'
    assert rules['interval_rules']['monthly']['new_building']['scope']=='pooled_fallback'
...
def test_calendar_windows_strata_and_tail_scoring():
    hist=history()
    assert rolling.earlier_history(hist,pd.Timestamp('2020-04-01'),12).period.min()=='2019-04-01'
    data=sample();train=data[data.period < '2019-01-01'];test=data[data.period.eq('2019-01-01')].copy()
    test.loc[test.index[0],'unit_id']='new-unit'
    test.loc[test.index[1],['unit_id','building']]=['new-unit-2','new-building']
    assert rolling.familiarity(train,test).tolist()[:3]==['new_unit_seen_building','new_building','seen_unit']
    # Scoring must not drop uncalibrated rows from point-prediction denominators.
    table,_=rolling.forecast_table(train,test,np.log(test.asking_rent.to_numpy()),
                                   np.log(test.asking_rent.to_numpy()),pd.DataFrame(),pd.Timestamp('2019-01-01'))
    ...
    for policy in rolling.POLICIES:
        for level in (80,95):
            table[f'{policy}_{level}_lower']=table.asking_rent*.9
...
def test_monthly_checkpoint_resume_and_swapped_forecast_rejection(tmp_path,monkeypatch):
    data=sample()
    monkeypatch.setattr(amenities,'load_analytical',lambda _: (data,{'fixture':True},{}))
    root=tmp_path/'run';kwargs=dict(start_year=2019,end_year=2019,min_rows=1)
    ...
    assert report['pooled']['policies']['monthly']['intervals']['80']['unavailable_rows']==18
    january=pd.read_json(root/'2019-01'/'forecast'/'predictions.jsonl',lines=True)
    ...
    source=root/'2019-01'/'forecast';target=root/'2019-02'/'forecast'
...
def test_protocol_change_and_incomplete_months_rejected(tmp_path,monkeypatch):
    data=sample()
    monkeypatch.setattr(amenities,'load_analytical',lambda _: (data,{'fixture':True},{}))
    rolling.run(tmp_path,tmp_path/'run',start_year=2019,end_year=2019,min_rows=1,prepare_only=True)
    with pytest.raises(ValueError,match='identity changed'):
        rolling.run(tmp_path,tmp_path/'run',start_year=2019,end_year=2019,min_rows=2,prepare_only=True)
    data=data[~data.period.eq('2019-06-01')]
    with pytest.raises(ValueError,match='Insufficient rows'):
        rolling.run(tmp_path,tmp_path/'missing',start_year=2019,end_year=2019,min_rows=1,prepare_only=True)
