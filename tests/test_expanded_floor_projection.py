...
def extend(parent, rows, original_changes):
    policy = {
        "excluded_buildings": deepcopy(parent["excluded_buildings"]),
...
def bundle(parent, rows, original_changes, policy):
    projected = [
        f.project_row(row, change, policy, AS_OF)
        for row, change in zip(rows, original_changes, strict=True)
    ]
    changes = [
        f.make_change(i, row, change)
        for i, (row, change) in enumerate(zip(rows, original_changes, strict=True))
    ]
    manifest = {
        "version": f.VERSION,
        "source_manifest": parent,
        "source_manifest_sha256": records_hash([parent]),
        "policy": policy,
        "policy_sha256": sha(policy),
        "interpreted_at": AS_OF,
        "source_rows": len(rows),
        "files": {
            f.SIDECAR: records_hash(changes),
            "observations.jsonl": records_hash(projected),
        },
    }
...
def example(label="601", advertised=None, count=12, excluded=False):
    row = {
        ...
        "advertised_floor": advertised,
    }
    captures = [
        {
            **{
                k: row[k]
                for k in ("audit_id", "unit_id", "source_listing_id", "known_at")
            },
            "capture_id": cid,
            ...
            "body_sha256": "b" * 64,
            "literal": label,
            "candidate_floor": old.candidate(label),
    ...
    building = (
        ...
        if count is None
        else [
            {
                "floor_count": count,
                "source_collected_at": row["known_at"],
                "source_path": "/floorCount",
                "source_url": "https://streeteasy.com/building/b",
                "body_sha256": "c" * 64,
                "raw_building_sha256": "d" * 64,
            }
        ]
    )
    as_of = "2026-09-19T12:00:00Z"
    policy = {
        "excluded_buildings": ["b"] if excluded else [],
        "building_floor_evidence": {"b": building},
        "parent_interpreted_at": as_of,
        "excluded_units": [],
        "floor_masks": [],
    }
    result = old.project_row(
        row, captures, policy["excluded_buildings"], building, as_of
    )
    change = {
        "source_index": 0,
        "source_row_sha256": sha(row),
        "listed_floor_was_present": False,
        "before_listed_floor": None,
        "captures": captures,
    }
    ancestor = {
        "version": old.PARENT,
    ...
    parent = {
        "version": old.VERSION,
        "source_manifest": ancestor,
        "source_manifest_sha256": records_hash([ancestor]),
        "source_rows": 1,
        "interpreted_at": as_of,
        "excluded_buildings": policy["excluded_buildings"],
        "building_floor_evidence": policy["building_floor_evidence"],
        "files": {
            old.SIDECAR: records_hash([change]),
            "observations.jsonl": records_hash([result]),
        },
    }
...
def mask_for(row, change):
    ...
    record = {
        ...
        "action": "edit",
        "id": "correction-1",
        "recorded_at": "2026-09-19T22:00:00Z",
        "author": "reviewer",
        "reason": "reference photo",
        "previous_hash": "0" * 64,
        "target": {
            "source": "streeteasy",
    ...
    return {
        ...
        "source_row_sha256": sha(row),
        "reason": "Reviewed reference photo",
        "reviewed_at": "2026-09-19T21:00:00Z",
...
@pytest.mark.parametrize(
    "label,rule,want",
    [
        ("601", "numeric_hundreds", 6),
        ("1404", "numeric_hundreds", 14),
        ("#906", "numeric_hundreds", 9),
        ("S15K", "north_south_wing_prefix", 15),
        ("N3D", "north_south_wing_prefix", 3),
        ("4FE", "front_rear_suffix", 4),
        ("3RW", "front_rear_suffix", 3),
        ("11THFLOOR", "explicit_ordinal_label", 11),
        ("2NDFL", "explicit_ordinal_label", 2),
        (" #3d ", old.RULE, 3),
    ],
)
...
@pytest.mark.parametrize(
    "count,status",
    [
        (None, "missing_building_floor_count"),
        (5, "above_captured_building_floor_count"),
        (6, "label_proxy"),
    ],
)
...
def test_mask_requires_exact_review_and_ledger_witness(mutation):
    ...
    if mutation == "source_hash":
        mask["source_row_sha256"] = "a" * 64
    elif mutation == "missing_capture":
        mask["captures"].pop()
    elif mutation == "typed_capture":
        mask["captures"][0]["capture_id"] = "1"
    elif mutation == "body_hash":
        mask["captures"][0]["body_sha256"] = "d" * 64
    elif mutation == "description_hash":
        mask["captures"][0]["description"] += "!"
    elif mutation == "span":
        mask["captures"][0]["spans"][0]["start"] += 1
    elif mutation == "partial_sentence":
        span = mask["captures"][0]["spans"][0]
        span["start"] += 10
        span["literal"] = span["literal"][10:]
    elif mutation == "clock":
        mask["reviewed_at"] = "2026-09-17T00:00:00Z"
    elif mutation == "correction_scope":
        mask["correction_record"]["target"]["source_listing_id"] = "other"
    elif mutation == "correction_patch":
        mask["correction_record"]["patch"][1]["path"] = "/bedrooms"
    elif mutation == "correction_clock":
        mask["correction_record"]["recorded_at"] = "2026-09-20T00:00:00Z"
    elif mutation == "correction_hash":
        mask["correction_record"]["hash"] = "a" * 64
    elif mutation == "mask_value":
        mask["before_advertised_floor"] = 4
    elif mutation == "duplicate_mask":
        policy["floor_masks"].append(deepcopy(mask))
...
def test_original_capture_and_projection_are_revalidated(mutation):
    ...
    if mutation == "row_value":
        row["bedrooms"] = 10
    elif mutation == "capture_identity":
        change["captures"][0]["source_listing_id"] = 12
    elif mutation == "capture_type":
        change["captures"][0]["capture_id"] = True
    elif mutation == "capture_literal":
        change["captures"][0]["literal"] = "602"
    elif mutation == "capture_membership":
        change["captures"].pop()
    elif mutation == "candidate_type":
        change["captures"][0]["candidate_floor"] = False
    elif mutation == "old_provenance":
        row["floor_label_provenance"]["status"] = "label_proxy"
    elif mutation == "original_hash":
        change["source_row_sha256"] = "d" * 64
...
def test_inverse_rejects_tampering_even_with_updated_artifact_hashes(mutation):
    ...
    m, rows, changes = bundle(parent, [row], [change], policy)
    if mutation == "new_row":
        rows[0]["listed_floor"] = 7
    elif mutation == "sidecar":
        changes[0]["source_row_sha256"] = "a" * 64
    elif mutation == "policy":
        m["policy_sha256"] = "a" * 64
    elif mutation == "parent_context":
        m["policy"]["excluded_buildings"] = ["other"]
        m["policy_sha256"] = sha(m["policy"])
    elif mutation == "parent_hash":
        m["source_manifest_sha256"] = "a" * 64
    elif mutation == "order":
        changes[0]["source_index"] = True
    elif mutation == "before_presence":
        changes[0]["before"]["listed_floor"]["present"] = 0
