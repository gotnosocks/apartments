    ...
    return pd.DataFrame(
        {
            ...
            "unit_id": ["u" + str(i % 60) for i in range(n)],
            "building": ["b" + str(i % 6) for i in range(n)],
...
def points(model):
    ...
    return [
        ...
        *[
            {
                k: np.asarray(v) + rng.normal(0, scale, np.shape(v))
                for k, v in initial.items()
            }
            for scale in (0.035, 0.17)
        ],
    ]
...
def test_shared_defaults_match_existing_density_gradient_and_individual_priors(spec):
    data = training()
    design = feature.FeatureDesign(data, spec)
    ...
    functions = [
        (
            m.compile_logp(mode="FAST_COMPILE"),
            m.compile_dlogp(mode="FAST_COMPILE"),
            m.compile_logp(vars=m.free_RVs, sum=False, mode="FAST_COMPILE"),
        )
        for m in [old, new]
    ]
...
def test_bedroom_scales_likelihood_and_gradients_match_explicit_formula():
    ...
    model = graph.build_model(
        data, design, residual_scale="bedroom", residual_parameterization="noncentered"
    )
    config = model.graph_configuration
    assert config == graph.graph_configuration(
        data, residual_scale="bedroom", residual_parameterization="noncentered"
    )
    assert config["residual_bedroom_levels"] == list(range(6))
    assert config["residual_bedroom_counts"] == [
        int(data.bedrooms.eq(i).sum()) for i in range(6)
    ]
    assert model.named_vars_to_dims["sigma_by_bedroom"] == ("residual_bedroom",)
    value_outputs = model.replace_rvs_by_values(
        [
            model["sigma"],
            model["residual_bedroom_scale"],
            model["residual_bedroom_z"],
            model["sigma_by_bedroom"],
            model["log_rent"].owner.inputs[-2],
        ]
    )
    evaluated = model.compile_fn(
        value_outputs,
    ...
    density = model.compile_logp(mode="FAST_COMPILE")
    gradient = model.compile_dlogp(mode="FAST_COMPILE")
    for point in points(model):
        ...
        expected = t.logpdf(
            np.log(data.asking_rent),
            df=5,
            loc=mu,
            scale=sigmas[
                graph.bedroom_index(data.bedrooms, config["residual_bedroom_levels"])
            ],
        ).sum()
        ...
        assert float(likelihood(point)) == pytest.approx(
            expected, rel=1e-12, abs=len(data) * 1e-8
        )
        assert np.isfinite(density(point)) and np.isfinite(gradient(point)).all()
    initial = model.initial_point()
    initial["residual_bedroom_z_zerosum__"] = np.array([-0.7, -0.3, 0.1, 0.4, 0.9])
    assert np.ptp(evaluated(initial)[3]) > 0.01
...
def test_noise_alternative_does_not_change_mean_or_shared_priors():
    ...
    b = bedroom.compile_logp(
        vars=[bedroom[n] for n in common], sum=False, mode="FAST_COMPILE"
    )
    for point in points(shared):
        # Compile only common priors: new scale parameters have no influence.
        for left, right in zip(a(point), b(bedroom.initial_point() | point)):
...
@pytest.mark.parametrize(
    "kwargs", [{"building_prior_scale": 0.7}, {"unit_prior_scale": 0.5}]
)
def test_group_prior_alternative_changes_only_named_hyperprior(kwargs):
    data = training()
    design = feature.FeatureDesign(data)
    base = graph.build_model(data, design)
    changed = graph.build_model(data, design, **kwargs)
    name = "sigma_building" if "building_prior_scale" in kwargs else "sigma_unit"
    original_scale = 0.35 if name == "sigma_building" else 0.25
    ...
    assert float(second[index] - first[index]) == pytest.approx(
        halfnorm.logpdf(actual, scale=new_scale)
        - halfnorm.logpdf(actual, scale=original_scale),
        abs=5e-9,
    )
...
def test_bedroom_level_mapping_is_stable_and_unsupported_values_rejected():
    np.testing.assert_array_equal(
        graph.bedroom_index([3.0, 0.0, 1.0, 3.0], [0, 1, 3]), [2, 0, 1, 2]
    )
    data = training()
    config = graph.graph_configuration(data, residual_scale="bedroom")
    assert (
        graph.graph_configuration(
            data.sample(frac=1, random_state=8), residual_scale="bedroom"
        )
        == config
    )
    for values, levels in [
        ...
        with pytest.raises(ValueError):
            graph.bedroom_index(values, levels)
    for values in [[0] * len(data), [np.nan] * len(data), [1.5] * len(data)]:
        invalid = data.assign(bedrooms=values)
        with pytest.raises(ValueError):
            graph.graph_configuration(invalid, residual_scale="bedroom")


@pytest.mark.parametrize(
    "name", ["prior_multiplier", "building_prior_scale", "unit_prior_scale"]
)
@pytest.mark.parametrize("value", [0, -0.2, float("nan"), float("inf"), True])
