...
from apartments.research_pipeline import fit_dataset, publish_bundle, rank_candidates, read_bundle
...
def captures():
    for month in range(1, 5):
        for unit in range(12):
            ...
            yield {'observation': {
                'version_id': f'v{month}-{unit}', 'capture_id': f'c{month}-{unit}',
                'source': 'streeteasy', 'source_listing_id': f'b/{unit}',
                'building_slug': 'b', 'unit': str(unit),
                'collected_at': at, 'recorded_at': at,
            }, 'raw': {'status': 'ACTIVE', 'asking_rent': 2500 + unit * 130 + month * 25,
                       'attributes': {'bedrooms': unit % 3, 'bathrooms': 1,
                                      'elevator': bool(unit % 2)}}, 'provenance': {}}
...
def test_end_to_end_idempotent_analytics_model_and_frontier(tmp_path):
    ...
    assert build_dataset(reversed(list(captures())), dataset, as_of='2026-05-01') == source
    ...
    rows = [dict(unit_id='a', rent=3000, bedrooms=1, elevator=False, observed_at='2026-05-01'),
            dict(unit_id='b', rent=3100, bedrooms=1, elevator=True, observed_at='2026-05-01'),
            dict(unit_id='c', rent=3300, bedrooms=1, elevator=True, observed_at='2026-05-01')]
    ...
    ranked = [json.loads(line) for line in (ranking / 'rankings.jsonl').read_text().splitlines()]
    ...
    assert rank_candidates(candidates, prefs, ranking, budget=3200, model_bundle=fit) == result
...
def test_rank_cli_and_invalid_preference_fail_without_publishing(tmp_path):
    ...
    result = CliRunner().invoke(app, ['rank-apartments', str(candidates), str(preferences), str(tmp_path / 'out')])
...
def test_normalized_rental_flags_survive_analytical_model_boundary(tmp_path):
    ...
    observed = [json.loads(line) for line in (dataset / 'observations.jsonl').read_text().splitlines()]
...
def test_literal_unicode_separators_are_not_jsonl_boundaries(tmp_path):
    ...
    candidates.write_text(json.dumps({'rent':2500, 'bedrooms':1, 'description':text}, ensure_ascii=False)+'\n')
    prefs.write_text('{"bedrooms":100}')
    assert rank_candidates(candidates, prefs, tmp_path/'ranked')['input_candidates'] == 1
    source = tmp_path/'source'
    build_dataset(captures(), source, as_of='2026-05-01')
    manifest = read_bundle(source)
    rows = [json.loads(line) for line in (source/'observations.jsonl').read_text().split('\n') if line.strip()]
    for row in rows:
        row['description'] = text
    enriched = tmp_path/'enriched'
    publish_bundle(enriched, {'observations.jsonl':''.join(json.dumps(row, ensure_ascii=False)+'\n' for row in rows)},
