...
def test_missingness_ignores_known_values_but_tracks_unknowns():
    ...
    assert not np.array_equal(
        full.matrix(rows).toarray()[0], full.matrix(rows).toarray()[1]
    )
...
def test_single_known_category_cannot_reencode_missingness():
    ...
    fitted = [
        baseline.fit(
            ...
            unit_effect=False,
            iterations=12,
            encoder_class=ablation.encoder_class(v),
        )
        for v in ("full", "missingness")
    ]
    assert np.allclose(
        baseline.predict(fitted[0], data), baseline.predict(fitted[1], data)
    )
...
def test_known_category_contrasts_center_on_training_and_zero_unknowns():
    ...
    columns = [
        start + i
        for i, f in enumerate(encoder.amenity_features)
...
def test_crossed_splits_exclude_future_and_test_buildings():
    data = sample()
    for name, train, test in ablation.split_data(
        data, years=(), folds=range(5), cross_years=(2019,)
    ):
...
def test_run_resume_and_protocol_protection(tmp_path, monkeypatch):
    monkeypatch.setattr(
        model, "load_analytical", lambda _: (sample(), {"fixture": True}, {})
    )
    ...
    with pytest.raises(ValueError, match="Protocol changed"):
        ablation.run(
            tmp_path / "input", tmp_path / "output", **{**kwargs, "unit_effect": True}
        )
