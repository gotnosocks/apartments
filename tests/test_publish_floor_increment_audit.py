...
def test_binary_design_publication_replay_and_immutability(tmp_path):
    files={'time-design.npz':b'\x00\x01\xffbinary-design', 'audit.json':b'{"status":"design_only"}\n'}
    metadata={'version':'test-floor-audit','status':'design_only_no_fit_no_posterior'}
...
def test_tampered_saved_design_blocks_replay(tmp_path):
    ...
    with pytest.raises(ValueError,match='integrity'):_publish(tmp_path,files,{'version':'test'})
