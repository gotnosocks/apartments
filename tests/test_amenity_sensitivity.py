    ...
    for year in (2017, 2018, 2019):
        for unit in range(30):
            rows.append(dict(unit_id=f'u{unit}', building=f'b{unit//2}',
                ...
                bedrooms=1., bathrooms=1., square_feet=600., asking_rent=3000.+100*(unit%3),
...
def test_matched_reference_resume_and_source_rejection(tmp_path, monkeypatch):
    ...
    ablation.run(tmp_path/'input', reference, years=(2019,), cross_years=(2019,), folds=(0,), min_rows=1)
    ...
    stored = json.loads((root/'year-2019'/'building-10-units'/'predictions.json').read_text())
    ...
    monkeypatch.setattr(model, 'load_analytical', lambda _: (data, {'fixture':False}, {}))
...
def test_centered_category_renaming_preserves_fitted_predictions():
    data = sample()
    renamed = data.copy()
    renamed['laundry_type'] = data.laundry_type.map({'in_building':'zzz', 'in_unit':'aaa'})
    fits = [baseline.fit(frame, model.SETTINGS, iterations=20, unit_effect=False,
                         encoder_class=ablation.encoder_class('full')) for frame in (data, renamed)]
    assert np.allclose(baseline.predict(fits[0],data), baseline.predict(fits[1],renamed), atol=1e-7, rtol=0)
