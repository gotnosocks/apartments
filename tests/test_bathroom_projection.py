...
def cap(full, half):
    return {
        "capture_id": 1,
        "bathroom_fields": reported_bathrooms(
            {"propertyDetails": {"fullBathroomCount": full, "halfBathroomCount": half}}
        ),
    }
...
def test_unusual_pair_retained_and_flagged_never_repaired():
    ...
    assert r["bathroom_count_evidence"]["flags"] == [
        "multiple_reported_half_bathrooms_review_required"
    ]
...
def test_scalar_disagreement_explicit_not_overwritten():
    ...
    assert (
        "reported_counts_disagree_with_analytical_scalar"
        in r["bathroom_count_evidence"]["flags"]
    )
