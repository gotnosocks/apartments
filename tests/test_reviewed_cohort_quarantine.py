...
from docs.analysis.scripts.prepare_reviewed_listing_quarantines import (
    initial_price_evidence,
)
...
def quarantine_last(parent, rows):
    ...
    evidence = [
        {
            **{k: row[k] for k in ("audit_id", "unit_id", "source_listing_id")},
            ...
            "spans": [{"start": 0, "end": 30, "literal": text[:30]}],
        }
    ...
    decision = {
        **{k: row[k] for k in ("audit_id", "unit_id", "source_listing_id")},
        ...
        "reviewed_at": CLOCK,
        "reviewer": "reviewer",
        "reason": "Explicit short-term offer",
        "action": "quarantine_explicit_short_term_offer",
        "evidence": evidence,
    }
    ...
    manifest = {
        ...
        "files": {
            q.SIDECAR: q.records_hash(sidecar),
...
def fixture():
    ...
    parent = {
        "version": q.PARENT,
        "files": {"observations.jsonl": q.records_hash(source)},
    }
    decisions = []
        ...
        evidence = [
            {
                **{k: row[k] for k in ("audit_id", "unit_id", "source_listing_id")},
                ...
                "source_collected_at": row["known_at"],
                "known_at": row["known_at"],
                "spans": [{"start": 0, "end": len(text), "literal": text}],
            }
            for cid in row["capture_ids"]
        ]
        decision = {
...
def test_exact_inverse_retains_other_ads_and_current_row_without_mutation():
    ...
    assert q.parent_rows(manifest, kept, sidecar) == (
...
def test_inverse_rejects_changed_source_membership_and_bindings(fault):
    ...
    if fault == "price":
        ...
        kept.pop()
    elif fault == "sidecar_missing":
        sidecar.pop()
    elif fault == "position":
        sidecar[0]["source_index"] = 1
    elif fault == "duplicate_position":
        sidecar[1]["source_index"] = 0
    elif fault == "source_count":
        m["source_rows"] += 1
    elif fault == "duplicate_kept":
        kept[1] = deepcopy(kept[0])
    elif fault == "decision_manifest":
        m["decisions_manifest"]["version"] = "other"
...
def test_decisions_require_exact_evidence_and_review_scope(fault):
    ...
    if fault == "missing_capture":
        ...
        c["capture_id"] = str(c["capture_id"])
    elif fault == "foreign_ad":
        c["source_listing_id"] = "foreign"
    elif fault == "span":
        c["spans"][0]["end"] -= 1
    elif fault == "description":
        c["description"] += "!"
    elif fault == "future_capture":
        c["known_at"] = "2027-01-01T00:00:00Z"
    elif fault == "capture_before_collection":
        c["source_collected_at"] = CLOCK
    elif fault == "early_review":
        d["reviewed_at"] = "2020-01-01T00:00:00Z"
    elif fault == "action":
        d["action"] = "replace_price"
    elif fault == "row_hash":
        d["source_row_sha256"] = "c" * 64
    elif fault == "current":
        row["analysis_price_basis"] = "current_capture_gross_ask"
...
def test_publication_is_idempotent_and_preserves_source_evidence(tmp_path):
    source, decisions, *_ = fixture()
    parent, decision_path, output = [
        tmp_path / n for n in ("parent", "decisions", "output")
    ]
    publish_bundle(
        parent,
        {
...
def test_initial_event_date_is_not_price_change_utc_date():
    row = {
        "source_listing_id": "2675026",
        "asking_rent": 2878,
        "price_at": "2019-03-12T00:00:00+00:00",
    }
    prices = {
        "priceChanges": [
            {"price": 2878, "changedAt": "2019-03-12T20:56:30-04:00"},
            {"price": 3100, "changedAt": "2019-03-25T11:00:06-04:00"},
        ]
    }
    events = [
        {
            "event_listing_id": "2675026",
    ...
    for key, value in [
        ...
        with pytest.raises(ValueError):
            initial_price_evidence({**row, key: value}, prices, events)
