...
spec = importlib.util.spec_from_file_location(
    "modal_scrape", Path(__file__).parents[1] / "models/modal_scrape.py"
)
...
def test_secret_allowlist():
    assert m.credentials(
        {"OXYLABS_USER": "u", "OXYLABS_PASSWORD": "p", "OTHER_SECRET": "private"}
...
def test_clone_preserves_source(tmp_path):
    ...
    with sqlite3.connect(target / "archive.sqlite3") as db:
        db.execute("UPDATE evidence SET value=8")
    with sqlite3.connect(base / "archive.sqlite3") as db:
        assert db.execute("SELECT value FROM evidence").fetchone()[0] == 7
    m.clone_snapshot(base, target, "test")
    assert json.loads((target / "origin.json").read_text())["snapshot"] == "test"
    with pytest.raises(ValueError):
        m.clone_snapshot(base, target, "different")
...
def test_publish_new_snapshot_and_readiness(tmp_path):
    ...
    (workspace / "last-run.json").write_text(
        json.dumps({"exit_code": 0, "queue": {"pending": 12}})
    )
    with sqlite3.connect(workspace / "archive.sqlite3") as db:
        db.execute("CREATE TABLE evidence(value)")
        db.execute("INSERT INTO evidence VALUES(8)")
...
def test_batch_log_is_streamed_and_persisted(tmp_path, capsys):
    ...
    assert (
        m.run_logged(
            [sys.executable, "-c", 'print("crawl progress", flush=True)'], log, 5
        )
        == 0
    )
...
def test_submit_uses_deployed_spawn_and_records_call(monkeypatch):
    ...
    def lookup(app, name):
        ...
        return SimpleNamespace(
            spawn=lambda *a: calls.append(a) or SimpleNamespace(object_id="fc-test")
        )
...
def test_submit_refuses_existing_controller(monkeypatch):
    monkeypatch.setattr(m, "locks", {})
    monkeypatch.setattr(
        m, "runs", {"latest:chelsea-resume": "run", "run": {"status": "running"}}
    )
