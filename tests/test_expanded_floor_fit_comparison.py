...
def protocols():
    ...
    for name in m.LOADER_CODE:
        a["implementation_sha256"][name] = "old"
        b["implementation_sha256"][name] = "new"
    return a, b
...
def test_source_revision_cannot_hide_specification_or_sampling_changes(fault):
    ...
    if fault == "math":
    ...
    elif fault == "sampler_code":
        b["implementation_sha256"]["bayesian_disk_sampling.py"] = "changed"
    elif fault == "added_code":
        b["implementation_sha256"]["surprise.py"] = "changed"
    elif fault == "removed_code":
        b["implementation_sha256"].pop("bayesian_floor_spline_design.py")
    elif fault == "missing_contract":
        b["implementation_sha256"].pop("expanded_floor_projection.py")
    elif fault == "prior":
        b["floor_prior_scale"] = 0.2
    elif fault == "group_prior":
        ...
        b["source_version"] = "wrong"
    elif fault == "floor_order":
        b["floor_levels"].reverse()
    elif fault == "floor_nonfinite":
...
def test_observed_support_can_expand_inside_identical_basis():
    a, b = protocols()
    b["floor_levels"] = sorted(b["floor_levels"] + [4.0])
    assert set(m.check_protocols(a, b)) == m.LOADER_CODE
...
AFTER = AFTER.replace(
    "'observations.jsonl', floor_label_sidecar}",
    "'observations.jsonl', floor_label_sidecar, expanded_floor_sidecar}",
)
AFTER = AFTER.replace(
    "floor_label_changes=None)",
    "floor_label_changes=None, expanded_floor_changes=[json.loads(s) for s in files[expanded_floor_sidecar].decode().split('\\n') if s.strip()] if expanded_floor_sidecar in files else None)",
)
AFTER = AFTER.replace(
    "modules = (base, reviewed_source_lineage.floor_label_projection)",
...
@pytest.mark.parametrize(
    "fault",
    [None, "model", "sampler", "loader_transform", "plumbing_value", "new_import"],
)
def test_v3_filename_allowance_does_not_allow_arbitrary_math_or_loader_changes(fault):
    after = AFTER
    if fault == "model":
        after = after.replace("data * 2", "data * 3")
    elif fault == "sampler":
        after += "\ndef sample():\n    return approximate_sampler()\n"
    elif fault == "loader_transform":
        after = after.replace("pd.DataFrame(rows)", "pd.DataFrame(rows).fillna(0)")
    elif fault == "plumbing_value":
        after = after.replace(
            "files[expanded_floor_sidecar].decode()", "unrelated().decode()"
        )
    elif fault == "new_import":
        after = "import unrelated\n" + after
    if fault:
        with pytest.raises(ValueError):
            m.check_v3_loader_change(BEFORE, after)
    else:
...
def fits(tmp_path, spline_train):
    a = spline_train.copy()
    b = a.copy()
    b.loc[b.listed_floor.isna(), "listed_floor"] = 7.0
    result = []
    for name, frame in [("a", a), ("b", b)]:
        design = m.spline.FeatureDesign(frame)
        target = tmp_path / name / "fit"
        design.save(target)
        manifest = publish_binary(
            target, {p.name: p.read_bytes() for p in target.iterdir()}, {}
        )
        result.append(
            {
                "root": target.parent,
                "data": frame,
                "design": design,
                "provenance": {"fit_manifest": manifest},
...
def test_floor_coverage_change_preserves_nonfloor_and_time_design(fits, fault):
    ...
    design = b["design"]
    index = next(
        i for i, name in enumerate(design.features) if not m.smooth.is_floor(name)
    )
    if fault == "value":
        b["data"].loc[0, "bedrooms"] = 6
    elif fault == "prior":
        design.prior_scales[index] *= 2
    elif fault == "center":
        design.means[index] += 0.1
    elif fault == "order":
        design.features[index] = "unexpected"
    elif fault == "time":
        b["reconstruction"]["time_arrays"]["same"] = "different"
    elif fault == "floor_prior":
        design.floor_prior_scale *= 2
    elif fault == "knots":
        ...
        b["design"].prior_scales[
            b["design"].features.index("listed_floor.unknown")
        ] *= 2
    if fault:
        with pytest.raises(ValueError):
            m.check_designs(a, b)
...
def test_residual_slices_distinguish_new_inferences_and_photo_corrections():
    ...
    after[1].update(
        ...
        listed_floor=6,
        expanded_floor_provenance={"correction_id": "c"},
    )
    movement = [
        {
            **row,
            "reference": {"residual_log": 0.1},
            "candidate": {"residual_log": 0.05},
            "fitted_rent_change": 2.0,
        }
        for row in after
    ]
    ...
    assert {key: value["rows"] for key, value in result.items()} == {
        "all": 4,
        "current_capture": 1,
        "newly_inferred_floor": 1,
        "corrected_photo_floor": 1,
        "previously_known_floor": 1,
        "remaining_unknown_floor": 1,
    }
    with pytest.raises(ValueError):
        m.residual_slices(before, after[::-1], movement)
    with pytest.raises(ValueError):
        m.residual_slices(before, after, movement[:-1])


@pytest.mark.parametrize(
    "fault", [None, "bedrooms", "price", "identity", "sidecar", "wrong_reference"]
)
def test_published_revision_requires_exact_inverse_and_nonfloor_identity(
    tmp_path, fault
):
    ...
    if fault == "bedrooms":
        ...
        after[0]["asking_rent"] = 99000
    elif fault == "identity":
        after[0]["audit_id"] = "other"
    elif fault == "sidecar":
        changes[0]["source_row_sha256"] = "a" * 64
    publish_bundle(
        destination,
        {
    ...
    if fault:
        with pytest.raises(ValueError):
            m.verify_revision(source, destination)
    else:
...
def test_curves_use_joint_within_fit_draws_without_cross_fit_pairing(spline_train):
    ...
    beta[:, :, j] = -0.95 * beta[:, :, i] + rng.normal(scale=0.01, size=beta.shape[:2])
    curve = m.smooth.curve_from_draws(design, beta, 1.0)
    target = next(row for row in curve["points"] if row["floor"] == 10.0)
    joint = np.einsum("cdf,f->cd", beta, design.contrast_vector(2.0, 10.0)).ravel()
...
def test_support_driven_boundary_change_allowed_and_joint_prior_difference_exposed(
    spline_train,
):
    a, b = protocols()
    ...
    revised = source.copy()
    revised.loc[revised.listed_floor.isna(), "listed_floor"] = 57.0
    designs = [m.spline.FeatureDesign(frame) for frame in (source, revised)]
    m.check_design_arrays(*designs, source, revised)
    result = m.prior_comparison(*designs, 1.0)
    assert result["reference_knots"][-1] == 52.0
    assert result["candidate_knots"][-1] == 57.0
    assert result["maximum_absolute_log_sd_change"] > 0
    assert result["maximum_absolute_log_covariance_change"] > 0
    i = result["common_floors"].index(10.0)
    j = result["common_floors"].index(20.0)
    for key, design in zip(["reference", "candidate"], designs, strict=True):
        first = design.contrast_vector(2.0, 10.0) * design.prior_scales
        second = design.contrast_vector(2.0, 20.0) * design.prior_scales
        assert result[key + "_log_curve_covariance"][i][j] == pytest.approx(
            first @ second
        )
    same = m.prior_comparison(designs[0], designs[0], 1.0)
    ...
    def bound(root, name, manifest):
        if name == "expanded_floor_projection.py":
            return (
                b"wrong contract"
                if fault == "contract"
                else Path(m.projection.__file__).read_bytes()
            )
        if root == a["root"] / "protocol":
            return BEFORE.encode()
        return (
            AFTER.replace("data * 2", "data * 9") if fault == "v3_math" else AFTER
        ).encode()

    monkeypatch.setattr(m.shared.common, "bound_bytes", bound)
    if fault:
        with pytest.raises(ValueError):
            m.check_implementation_sources(a, b, ["bayesian_feature_experiment_v3.py"])
    else:
        assert m.check_implementation_sources(
            a, b, ["bayesian_feature_experiment_v3.py"]
        ) == {
            "v3_exact_sidecar_plumbing_only": True,
            "expanded_contract_matches_archive": True,
        }
