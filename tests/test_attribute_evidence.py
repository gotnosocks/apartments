...
def test_nested_structured_evidence_and_compatibility():
    ...
    assert any(
        e["source_path"] == "/propertyDetails/amenities/doormanTypes/0"
        and e["literal"] == "FULL_TIME"
...
def test_absence_unit_number_and_history_never_supply_attributes():
    result = extract(
        {
            "propertyDetails": {
                "address": {"displayUnit": "#14A"},
                "features": {"list": []},
                "amenities": {"list": []},
            },
            "propertyHistory": [{"bedroomCount": 2}],
            "latestListing": {"floor": 14},
        }
    )
    assert all(
        v is None for k, v in result["attributes"].items() if not isinstance(v, dict)
    )
...
def test_negation_conflict_and_literal_offsets():
    desc = (
        "No elevator. Pets are not allowed. North-facing windows; no courtyard views."
    )
    result = extract(
        {"propertyDetails": {"amenities": {"list": ["ELEVATOR"]}}, "description": desc}
    )
...
@pytest.mark.parametrize(
    "phrase",
    ["non-elevator", "non elevator", "non–elevator", "non—elevator", "non- elevator"],
)
def test_non_elevator_denial_is_not_a_positive_claim(phrase):
    ...
    assert (
        text[assertions[0]["start"] : assertions[0]["end"]] == assertions[0]["literal"]
    )
    conflicting = extract(
        {"description": text, "propertyDetails": {"amenities": {"list": ["ELEVATOR"]}}}
    )
...
def test_unrelated_non_prefix_does_not_negate_an_elevator():
    assert (
        extract({"description": "Non-smoking building with an elevator."})[
            "attributes"
        ]["elevator"]
        is True
    )
    assert (
        extract({"description": "An elevator in a non-doorman building."})[
            "attributes"
        ]["elevator"]
        is True
    )
    assert (
        extract({"description": "This is not a non-elevator building."})["attributes"][
            "elevator"
        ]
        is None
    )
...
def test_unit_floor_and_exposure_are_distinct_from_amenity_copy():
    result = extract(
        {
            "description": "This apartment is on the 14th floor. The second floor gym has west-facing windows. Rooftop terrace offers city views. Courtyard-facing windows. South-facing windows."
        }
    )
    a = result["attributes"]
...
def test_unresolved_description_and_unknown_shape():
    assert extract({"description": "$3f"})["warnings"] == [
        "unresolved_description_reference"
    ]
    assert extract({"description": {"text": "elevator"}})["warnings"] == [
        "unsupported_description_shape"
    ]
...
def test_uncertain_negated_optional_and_installation_not_positive():
    a = extract(
        {
            "description": "No in-unit laundry. Central AC may be installed. Washer and dryer in the unit hookups. Pets allowed upon approval. Not a full-time doorman."
        }
    )["attributes"]
    assert (
        a["laundry_type"] is None
        and a["hvac_type"] is None
        and a["doorman_type"] is None
    )
...
def test_service_and_hvac_conflicts_remain_visible():
    result = extract(
        {
            "propertyDetails": {
                "features": {"list": ["CENTRAL_AC"]},
                "amenities": {"doormanTypes": ["FULL_TIME", "PART_TIME"]},
            },
...
def test_subtype_negation_preserves_conflict_and_not_other_type_absence():
    result = extract(
        {
            "propertyDetails": {"features": {"list": ["CENTRAL_AC", "WASHER_DRYER"]}},
    ...
    a = extract(
        {
            ...
            "description": "No in-unit laundry. No central air conditioning.",
        }
    )["attributes"]
...
def test_common_section_scope_survives_newlines_and_resets_at_unit_heading():
    ...
    assert (
        extract({"description": html})["attributes"]["view_exposures"]["skyline"]
        is None
    )
...
def test_mixed_central_and_wall_mounted_split_copy_is_ambiguous():
    ...
    assert (
        out["evidence"][0]["ambiguous_equipment_description"]
        == "- CENTRAL A/C Wall Mounted Split Unit"
    )
    assert (
        extract({"description": "Central A/C throughout the apartment."})["attributes"][
            "hvac_type"
        ]
        == "central_ac"
    )
...
def test_coordinated_cardinal_exposures_preserve_partial_unknowns():
    ...
    examples = [
        ...
        (
            "North, South and East facing windows, lots of light!",
            ("north", "south", "east"),
        ),
    ]
    for description, positive in examples:
        ...
        assert {key for key, value in directions.items() if value is True} == set(
            positive
        )
        assert all(directions[key] is None for key in directions.keys() - set(positive))
        for evidence in out["evidence"]:
            assert (
                description[evidence["start"] : evidence["end"]] == evidence["literal"]
            )
...
def test_coordinated_directions_respect_negation_and_common_scope():
    out = extract(
        {
            "description": "No north and south exposures. Community Amenities\nEast and west facing windows."
        }
    )
...
def test_case_by_case_pet_policies_and_hyphenated_prohibition():
    ...
    for text in (
        ...
        assert (
            extract({"description": text})["attributes"]["pet_policy"]
            == "approval_required"
        )
    assert (
        extract({"description": "Sorry no-pets."})["attributes"]["pet_policy"]
        == "not_allowed"
    )
    assert (
        extract({"description": "Pets are welcome."})["attributes"]["pet_policy"]
...
def test_stacked_in_unit_laundry_and_optional_connection():
    assert (
        extract(
            {"description": "In unit stacked washer/dryer in dedicated laundry room."}
        )["attributes"]["laundry_type"]
        == "in_unit"
    )
    assert (
        extract({"description": "In unit stacked washer/dryer connections."})[
            "attributes"
        ]["laundry_type"]
        is None
    )
...
def test_neighborhood_walking_directions_do_not_establish_walkup_building():
    ...
    assert (
        extract({"description": "Walk up to the roof for sunset."})["attributes"][
            "elevator"
        ]
        is None
    )
    assert (
        extract({"description": "This unit is on the 2nd floor - walk up one flight."})[
            "attributes"
        ]["elevator"]
        is False
    )
    assert (
        extract({"description": "Apartment in a walk-up building."})["attributes"][
            "elevator"
        ]
        is False
    )
...
def test_hvac_permission_is_not_installed_equipment():
    ...
    assert (
        extract({"description": "Window air conditioners are allowed."})["attributes"][
            "hvac_type"
        ]
        is None
    )
    assert (
        extract(
            {"description": "Window air conditioning is installed. Pets are permitted."}
        )["attributes"]["hvac_type"]
        == "room_ac"
    )
...
def test_some_building_residences_do_not_establish_this_units_view():
    ...
    assert (
        extract({"description": text})["attributes"]["view_exposures"]["city"] is None
    )
    assert (
        extract({"description": "Select apartments have south-facing windows."})[
            "attributes"
        ]["window_exposures"]["south"]
        is None
    )
    assert (
        extract({"description": "This residence offers expansive city views."})[
            "attributes"
        ]["view_exposures"]["city"]
        is True
    )
...
def test_no_building_laundry_does_not_deny_private_equipment():
    ...
    conflict = extract(
        {"description": text, "propertyDetails": {"amenities": {"list": ["LAUNDRY"]}}}
    )
...
def test_nearby_negation_does_not_reverse_affirmed_laundry():
    for text in (
        "The building doesn't have a gym. On-site laundry is available.",
        "The building offers on-site laundry; pets are not permitted.",
    ):
        assert (
            extract({"description": text})["attributes"]["laundry_type"]
            == "in_building"
        )
    ...
    out = extract(
        {
            "description": "The building does not only offer on-site laundry, but also a gym."
        }
    )
...
def test_reference_media_does_not_override_structured_floor_or_other_features():
    result = extract(
        {
            "propertyDetails": {"floor": 8, "physicalFloor": 7},
