...
def extend(parent, rows):
    ...
    witnesses = [
        {
            "capture_id": cid,
            "source_collected_at": before["known_at"],
            "raw_listing_sha256": "b" * 64,
            "description_sha256": "c" * 64,
            "claims": [
                {
                    "attribute": "advertised_floor",
                    "value": floor,
                    "source_path": "/description",
                    "rule": "explicit-dwelling-floor-offer-v1",
                    "start": 0,
                    "end": 5,
                    "literal": "floor",
                }
        ...
        for cid in before["capture_ids"]
    ]
    case = {
        "audit_id": before["audit_id"],
        "source_listing_id": before["source_listing_id"],
        "source_row_sha256": sha(before),
        "action": "add_explicit_advertised_floor",
        "advertised_floor": floor,
        "proposed_fields": dict.fromkeys(projection.FIELDS, floor),
        "witnesses": witnesses,
    }
    policy = {
        "version": "prepared-direct-floor-additions-v1",
        "applied": False,
    ...
    manifest = {
        ...
        "source_manifest_sha256": records_hash([parent]),
        "policy": policy,
        "policy_sha256": records_hash([policy]),
        "reviewed_at": clock,
        "source_rows": len(rows),
        "files": {
            "observations.jsonl": records_hash(after),
            projection.SIDECAR: records_hash(changes),
        },
    }
...
def example():
    ...
    witness = {
        "capture_id": 12,
        "source_collected_at": "2026-09-17T00:00:00Z",
    ...
    case = {
        ...
        "action": "add_explicit_advertised_floor",
        "advertised_floor": 2,
        "proposed_fields": {"listed_floor": 2, "advertised_floor": 2},
        "witnesses": [witness],
    }
    parent = {
        "version": projection.residual_scope_projection.VERSION,
...
def test_decision_rejects_unsafe_additions(example, mutation):
    ...
    if mutation == "known_floor":
        row["advertised_floor"] = 4
    if mutation == "masked":
        row["expanded_floor_provenance"]["status"] = "reviewed_mask"
    if mutation == "bool_floor":
        case["advertised_floor"] = True
    if mutation == "typed_capture":
        case["witnesses"][0]["capture_id"] = "12"
    if mutation == "duplicate_capture":
        case["witnesses"] *= 2
    if mutation == "missing_capture":
        case["witnesses"] = []
    if mutation == "future_capture":
        case["witnesses"][0]["source_collected_at"] = "2027-01-01T00:00:00Z"
    if mutation == "bad_claim":
        case["witnesses"][0]["claims"][0]["value"] = 3
...
def test_rehashed_unchanged_row_tampering_rejected(example):
    ...
    m["source_manifest"]["files"]["observations.jsonl"] = records_hash(
        [changes[0]["before"], sibling]
    )
    ...
    rows[0] = projection.apply_case(
        changes[0]["before"], changes[0]["case"], m["reviewed_at"], m["policy_sha256"]
    )
