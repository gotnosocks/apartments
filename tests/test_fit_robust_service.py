...
def test_refit_parity_reuse_and_future_source_rejection(tmp_path,monkeypatch):
    ...
    for period in pd.date_range('2017-01-01','2019-12-01',freq='MS'):
        for unit in range(6):
            rent=3000.+unit*100
            records.append(dict(period=period,unit_id=f'u{unit}',building=f'b{unit//2}',building_id=f'b{unit//2}',
                audit_id=f'{period}-{unit}',source_listing_id=f'ad-{period}-{unit}',bedrooms=1.,bathrooms=1.,square_feet=600.,
                asking_rent=rent,log_rent=np.log(rent),known_at='2020-01-01T00:00:00Z'))
    data=pd.DataFrame(records);source={'fixture':True,'as_of':'2020-01-01T00:00:00Z'}
    ...
    def forbidden(*args,**kwargs):
        raise AssertionError('Verified serving artifact must be reused without refitting')
    monkeypatch.setattr(baseline,'fit',forbidden)
    assert serving.run(tmp_path,validation,output,prediction_month='2020-01')==result
    with pytest.raises(ValueError,match='month immediately before'):
        serving.run(tmp_path,validation,tmp_path/'stale',prediction_month='2020-02')
    source['as_of']='2099-01-01T00:00:00Z'
    with pytest.raises(ValueError,match='future'):
        serving.run(tmp_path,validation,tmp_path/'future',prediction_month='2020-01')
