...
class Analysis:
    def __init__(self, rows, shift=0.0, acceptable=True):
    ...
    def detail(self, audit_id):
        ...
        return {
            ...
            "contribution_diagnostics": {"acceptable": self.acceptable},
            "draws": 24000,
            "warnings": [],
        }
...
def rank(analysis, weights=None, **kwargs):
    return rank_current(
        analysis,
        {"bedrooms": 600} if weights is None else weights,
...
def signature(results):
    return [
        (r["record"]["audit_id"], r["monthly_surplus"], r["pareto_efficient"])
        for r in results
    ]
...
def test_frontier_and_utility_are_independent_of_posterior_prices_uncertainty_and_gates():
    ...
    assert {r["record"]["audit_id"] for r in baseline if r["pareto_efficient"]} == {
        "a",
        "b",
    }
...
def test_current_freshness_budget_and_status_filter_before_any_posterior_request():
    analysis = Analysis(
        [
            ...
            row("rented", 1000, listing_status="RENTED"),
            row("stale", 1800, collected_at="2026-08-01T00:00:00Z"),
            row("expensive", 7000),
            row("future", 1500, known_at="2026-09-20T00:00:00Z"),
        ]
    )
...
def test_review_conflicts_mask_only_preferences_without_rewriting_source():
    ...
    note = {
        ...
        "message": "Own description disagrees.",
    ...
    assert (
        other[0]["eligible"] and other[0]["market_comparison"]["source_review"] == note
    )
...
def test_fitted_candidate_mismatch_and_unknown_preference_keys_fail():
    ...
    with pytest.raises(ValueError, match="fitted observation"):
        rank(analysis)
    with pytest.raises(ValueError, match="Unknown"):
        rank(Analysis([]), {"bedroms": 500})
    with pytest.raises(ValueError, match="Duplicate"):
        rank(Analysis([row("a", 1), row("a", 2)]))


def test_unscoped_source_conflict_is_not_silently_treated_as_known():
    analysis = Analysis([row("a", 3000)])
    result, excluded, summary = rank(
        analysis,
        source_notes={"a": {"kind": "new_conflict", "interpretation_limited": True}},
    )
...
def test_merging_compatible_ads_cannot_hide_another_ads_source_conflict():
    analysis = Analysis(
        [row("a", 3000, unit_id="same"), row("z", 3000, unit_id="same")]
    )
...
def test_cli_routes_to_selected_bayesian_ranking_without_legacy_fallback(monkeypatch):
    ...
    def run(*args, **kwargs):
        ...
        return {"frontier_units": 2}

    monkeypatch.setattr(bayesian_candidate_search, "run", run)
    monkeypatch.setattr(
        candidate_search,
        "score_candidates",
        lambda *a, **k: pytest.fail("No legacy model fallback"),
    )
    result = CliRunner().invoke(
        app,
        [
            "rank-current-apartments",
            "prefs.json",
            "out",
            "--as-of",
            "2026-09-19",
    ...
    def fail(*a, **k):
        raise ValueError("Posterior does not match source")

    monkeypatch.setattr(bayesian_candidate_search, "run", fail)
    result = CliRunner().invoke(
        app, ["rank-current-apartments", "prefs.json", "out", "--as-of", "2026-09-19"]
    )
