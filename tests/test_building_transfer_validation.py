...
def buildings():
    ...
    for i in range(100):
        name = f"building-{i}"
        fold = transfer.ablation.building_fold(name)
    ...
    for date in pd.date_range("2016-01-01", "2019-12-01", freq="MS"):
        for fold, names in buildings().items():
            for index, name in enumerate(names):
                rent = 3000.0 + 200 * fold + 100 * index + 5 * (date.year - 2016)
                rows.append(
                    dict(
                        building=name,
                        unit_id=name + "-unit",
                        audit_id=f"{name}-{date.date()}",
                        period=date,
                        asking_rent=rent,
...
def test_all_dates_of_calibration_and_evaluation_buildings_are_reserved():
    ...
    for fold in range(5):
        ...
        changed = data.copy()
        changed.loc[changed.building.isin(test.building), "asking_rent"] = 1e9
        newtrain, newcal, _ = transfer.partition(changed, origin, fold)
        pd.testing.assert_frame_equal(train, newtrain)
        pd.testing.assert_frame_equal(cal, newcal)
        ...
        for i in range(100):
            ...
            rows.append(
                dict(
                    ...
                    prediction=3000.0,
                    asking_rent=3000.0 * np.exp(0.7 if i == 99 else 0),
                )
            )
    return pd.DataFrame(rows)
...
def test_building_balanced_quantiles_and_strict_past_only_calibration():
    hist = history()
    origin = pd.Timestamp("2019-01-01")
    kwargs = dict(calibration_fold=1, min_buildings=2)
    rule = transfer.rules(hist, origin, **kwargs)
    assert rule["row_weighted"]["quantiles"]["95"] == [0.0, 0.0]
    assert rule["building_balanced"]["quantiles"]["95"] == pytest.approx([0.0, 0.7])
    assert (
        rule["building_balanced"]["rows"] == 1200
        and rule["building_balanced"]["months"] == 12
    )
    changed = hist.copy()
    changed.loc[changed.period >= "2019-01-01", "asking_rent"] = 1e10
    assert transfer.rules(changed, origin, **kwargs) == rule
    assert rule["building_balanced"]["end"] == "2018-12-01"
    bad = hist.copy()
    bad.loc[0, "building"] = buildings()[0][0]
    ...
    with pytest.raises(ValueError, match="positive weights"):
        transfer.quantile([0, 1], [0, 1], [0.5])


def test_interval_scoring_denominators_and_building_equal_weight():
    hist = history().iloc[:100].copy()
    hist["unit_id"] = hist.building
    hist["audit_id"] = range(100)
    rule = {
        p: {"status": "estimated", "quantiles": {"80": [-0.1, 0.1], "95": [-0.1, 0.1]}}
        for p in transfer.POLICIES
    }
    table = transfer.add_bands(hist, rule)
    report = transfer.score(table)["intervals"]["row_weighted"]["95"]
    assert (
        report["coverage_percent"] == 99
        and report["equal_building_coverage_percent"] == 50
    )
    assert report["above_percent"] == 1 and report["buildings"] == 2
    unavailable = transfer.add_bands(
        hist, {p: {"status": "unavailable"} for p in transfer.POLICIES}
    )
...
def test_resumable_real_fits_replayed_rules_and_binding_rejection(
    ...
    assert (
        partial["phase"] == "paused_at_declared_limit"
        and partial["completed_months"] == 2
    )
    ...
    assert (
        report["pooled"]["intervals"]["building_balanced"]["95"]["unavailable_rows"]
        == 0
    )
    ...
    source = root / "fold-0/2018-01/fit"
    destination = root / "fold-0/2018-02/fit"
...
def test_protocol_support_threshold_changes_require_new_artifact(tmp_path, monkeypatch):
    monkeypatch.setattr(
        amenities, "load_analytical", lambda _: (sample(), {"fixture": True}, {})
    )
