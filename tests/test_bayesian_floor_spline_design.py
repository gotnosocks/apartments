...
def spline_train(train):
    data = train.copy()
    data['listed_floor'] = np.resize(np.r_[np.arange(1., 53.), np.nan], len(data))
    return data
...
def test_nonfloor_values_centering_priors_and_time_unchanged(spline_train, spec):
    ...
    for index, name in enumerate(old_names):
        ...
        if name in previous.features:
            np.testing.assert_array_equal(previous.matrix(spline_train)[:, previous.features.index(name)],
...
def test_full_floor_coverage_unknown_separation_and_floor_two_anchor(spline_train):
    ...
    assert design.floor_levels == list(np.arange(1., 53.))
    assert design.floor_knots == [1., 5., 10., 20., 35., 52.]
    ...
    np.testing.assert_array_equal(matrix[:, names.index('listed_floor.unknown')], [0, 0, 0, 0, 1])
    assert np.isfinite(design.matrix(spline_train)).all()
    assert sum(row['rows'] for row in design.floor_support['levels']) == spline_train.listed_floor.notna().sum()
    assert not any(name.startswith('listed_floor_gt_') for name in design.features)
    assert not any('physical' in name for name in names if name.startswith('listed_floor'))
...
def test_spline_is_twice_continuous_and_natural_at_boundaries(spline_train):
    ...
    for knot in design.floor_knots[1:-1]:
        ...
        right_second = (2*values[3] - 5*values[4] + 4*values[5] - values[6])/h**2
        np.testing.assert_allclose(left_second, right_second, atol=5e-8)
    for endpoint, direction in [(design.floor_knots[0], 1), (design.floor_knots[-1], -1)]:
...
def test_knot_to_knot_prior_is_independent_of_number_of_knots(train, high):
    data = train.copy()
    data['listed_floor'] = np.resize(np.r_[np.arange(1., high+1), np.nan], len(data))
    design = FeatureDesign(data, floor_prior_scale=.1)
    for low, upper in zip(design.floor_knots[:-1], design.floor_knots[1:]):
        direction = design.contrast_vector(low, upper)
        assert np.linalg.norm(direction*design.prior_scales) == pytest.approx(np.sqrt(2)*.1)
    direction = design.contrast_vector(1, high)
    assert np.linalg.norm(direction*design.prior_scales) == pytest.approx(np.sqrt(2)*.1)
    np.testing.assert_array_equal(design.contrast_vector(2, 2), np.zeros(len(design.features)))
...
def test_unobserved_interior_floor_can_interpolate_but_cannot_extrapolate(train):
    data = train.copy()
    data['listed_floor'] = np.resize([1., 3., 5., 10., np.nan], len(data))
    design = FeatureDesign(data)
    assert 2 not in design.floor_levels
    assert np.isfinite(design.matrix(cases(data, [2., 4., 7.5]))).all()
    for value in [0., 10.001, np.inf, -np.inf]:
...
def test_advertised_alias_preserves_canonical_priority(spline_train):
    ...
    np.testing.assert_array_equal(raw[:, [names.index(f'listed_floor_spline_{i}') for i in range(5)]], coordinates[[1]])


def test_saved_design_reconstructs_identical_features_and_contrasts(spline_train, tmp_path):
    design = FeatureDesign(spline_train, floor_prior_scale=.08)
    design.save(tmp_path)
    restored = FeatureDesign.load(tmp_path)
    np.testing.assert_array_equal(restored.matrix(spline_train), design.matrix(spline_train))
    np.testing.assert_array_equal(restored.prior_scales, design.prior_scales)
    np.testing.assert_array_equal(restored.contrast_vector(2, 52), design.contrast_vector(2, 52))
    assert restored.floor_support == design.floor_support


@pytest.mark.parametrize('fault', ['knots', 'anchor', 'basis', 'policy', 'features', 'numeric_order',
                                  'category_order', 'raw_order', 'priors', 'active', 'version', 'means'])
def test_corrupt_saved_design_rejected(spline_train, tmp_path, fault):
    design = FeatureDesign(spline_train)
    design.save(tmp_path)
    path = tmp_path/'feature-design.json'
    metadata = json.loads(path.read_text())
    if fault == 'knots': metadata['floor_knots'][1] += 1
    elif fault == 'anchor': metadata['floor_reference'] = 1
    elif fault == 'basis': metadata['floor_basis'][0][0] += .01
    elif fault == 'policy': metadata['floor_policy']['unknown'] = 'zero means ground floor'
    elif fault == 'features': metadata['features'].reverse()
    elif fault == 'numeric_order': metadata['numeric_order'].reverse()
    elif fault == 'category_order': metadata['category_order'].reverse()
    elif fault == 'raw_order': metadata['raw_feature_names'].reverse()
    elif fault == 'priors': metadata['floor_prior_scale'] = .4
    elif fault == 'active': metadata['active'] = [int(value) for value in metadata['active']]
    elif fault == 'version': metadata['version'] = 'unsupported'
    elif fault == 'means': metadata['means'] = metadata['means'][:-1]
    path.write_text(json.dumps(metadata))
    with pytest.raises(ValueError):
        FeatureDesign.load(tmp_path)


@pytest.mark.parametrize('levels', [[], [1.], [2., 1.], [1., 1.], [1., np.nan], [1., np.inf]])
...
def test_anchor_outside_floor_two_range_uses_observed_minimum(train):
    data = train.copy()
    data['listed_floor'] = np.resize([6., 8., 10., 20., np.nan], len(data))
    design = FeatureDesign(data)
