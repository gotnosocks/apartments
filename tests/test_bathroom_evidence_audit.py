...
def test_counts_preserve_two_halves_instead_of_arithmetic_reconstruction():
    result = consensus(
        [item({"fullBathroomCount": 1, "halfBathroomCount": 2, "bathroomCount": 2})]
    )
...
def test_conflicting_captures_not_silently_selected():
    result = consensus(
        [
            item({"fullBathroomCount": 2, "halfBathroomCount": 0}),
            item({"fullBathroomCount": 1, "halfBathroomCount": 2}),
        ]
    )
...
def test_partial_capture_not_silently_ignored():
    assert (
        consensus(
            [
                item({"fullBathroomCount": 2, "halfBathroomCount": 0}),
                item({"bathroomCount": 2}),
            ]
        )["status"]
        == "incomplete_explicit_counts"
    )
...
def test_exact_spans_and_distinct_bath_access_marketing_candidates():
    ...
    assert (
        "non_bath_ensuite_context"
        in next(f for f in findings if f["family"] == "ensuite")["flags"]
    )
...
def audit_fixture(tmp_path):
    ...
    def make(wrong_raw_hash=False):
        ...
        raw = canonical(
            {
                "propertyDetails": {"fullBathroomCount": 1, "halfBathroomCount": 2},
                    ...
                    [
                        {
                            "snapshot_id": 1,
                            "listing_id": "ad1",
                            "canonical_unit_url": "url1",
                            "raw_listing_json": raw,
                        }
                    ]
                )
            ).write_parquet(str(shard))
        historical = tmp_path / "history"
        hm = publish_bundle(
            historical,
            {
                "source-files.json": canonical(
                    {"listing_observations/part.parquet": digest(shard)}
                )
            },
            {"version": "fixture"},
        )
        ...
        publish_bundle(
            descriptions,
            {"evidence.jsonl": canonical(evidence) + "\n"},
...
def test_source_bound_audit_and_projection_replay_preserve_cohort(
...
def test_raw_source_hash_mismatch_cannot_publish(audit_fixture):
    ...
    with pytest.raises(ValueError, match="Raw source identity or hash mismatch"):
        run(*args)
