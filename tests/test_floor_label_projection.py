...
def project(*args, **kwargs):
    if len(args) < 5:
        kwargs.setdefault("as_of", "2026-09-19T15:55:00Z")
...
def building(count):
    return {
        "floor_count": count,
        "source_collected_at": "2026-09-18T00:00:00Z",
        "source_path": "/floorCount",
        "source_url": "https://streeteasy.com/building/b",
        "body_sha256": "a" * 64,
        "raw_building_sha256": "b" * 64,
    }
...
def test_all_captures_must_resolve_and_agree():
    row, c = example()
    c[1]["literal"] = "4D"
    c[1]["candidate_floor"] = 4
    assert "listed_floor" not in project(row, c, [])
    c[1]["literal"] = "PH"
    c[1]["candidate_floor"] = None
    assert "listed_floor" not in project(row, c, [])
...
def test_inverse_exact_and_mutation_rejected():
    ...
    m = {
        ...
        "source_rows": 1,
        "excluded_buildings": [],
        "interpreted_at": "2026-09-19T15:55:00Z",
        "files": {f.SIDECAR: records_hash([change])},
    }
    ...
    for field, value in [
        ("listed_floor", 4),
        ("bedrooms", 7),
        ("source_listed_floor", 1),
    ]:
        bad = deepcopy(after)
        bad[field] = value
        with pytest.raises(ValueError):
            f.parent_rows(m, [bad], [change])
    bad = deepcopy(change)
    bad["captures"][0]["literal"] = "4D"
    m["files"][f.SIDECAR] = records_hash([bad])
    with pytest.raises(ValueError):
        f.parent_rows(m, [after], [bad])
        ...
        changes.append(
            {
                "source_index": index,
                "source_row_sha256": sha(row),
                "listed_floor_was_present": "listed_floor" in row,
                "before_listed_floor": row.get("listed_floor"),
                "captures": captures,
            }
        )
    manifest = {
...
def test_building_count_is_compatibility_filter_not_floor_mapping():
    row, c = example()
    for cap in c:
        cap.update(literal="14B", candidate_floor=14)
    assert (
        project(row, c, [])["floor_label_provenance"]["status"]
...
def test_interpretation_and_capture_clocks_are_validated():
    row, c = example()
    with pytest.raises(ValueError):
        project(row, c, [], as_of="2026-09-17T00:00:00Z")
