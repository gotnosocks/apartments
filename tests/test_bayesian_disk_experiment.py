...
def configure(setup, monkeypatch, floor=False):
    ...
    monkeypatch.setattr(
        ...
        "floor_contrasts",
        lambda *a: {
            "version": m.increments.FLOOR_CONTRAST_VERSION,
    ...
    def sample(model, **kwargs):
        ...
        raw = kwargs["trace_root"]
        raw.mkdir(exist_ok=True)
        identity = {
            "version": m.storage.VERSION,
            "protocol_sha256": kwargs["protocol_hash"],
            **{
                k: kwargs[k]
                for k in (
                    "chains",
            ...
            "contract": {
                ...
                "draws": args.draws,
                "variables": {
                    name: list(v.dims[2:]) for name, v in p.data_vars.items()
                },
            },
        }
        ...
        result = {
            ...
            "chains": args.chains,
            "draws": args.draws,
            "posterior_sha256": digest(kwargs["output"]),
            "trace_manifest_sha256": digest(raw / "complete.json"),
            "warmup_exported": False,
            "raw_event_statistics_preserved": True,
            "block_budget_bytes": m.storage.MAX_BLOCK_BYTES,
            "maximum_array_block_bytes": 64,
...
def test_disk_runner_preserves_specification_and_replays_completed_fit(
    setup, monkeypatch, floor
):
    ...
    } <= json.loads((args.output / "fit/complete.json").read_text())["files"].keys()
    ...
    assert cache["implementation_sha256"] == digest(
        args.output / "fit/bayesian_report_cache.py"
    )
...
def test_reports_resume_from_exported_checkpoint_without_sampling(setup, monkeypatch):
    ...
    monkeypatch.setattr(
        m.linear.v2,
        "write_reports",
        lambda *a: (_ for _ in ()).throw(RuntimeError("Interrupted report")),
    )
...
@pytest.mark.parametrize(
    "field,value",
    [
        ("execution_version", "invented"),
        ("storage_policy", {}),
        ("storage_versions", {}),
    ],
)
def test_disk_protocol_rejects_unrecognized_execution(setup, monkeypatch, field, value):
    args, _, _ = configure(setup, monkeypatch)
    m.run(args)
    protocol = json.loads((args.output / "protocol/protocol.json").read_text())
    protocol[field] = value
    with pytest.raises(ValueError, match="disk execution"):
        m.disk_protocol.verify_protocol(protocol)


@pytest.mark.parametrize(
    "damage",
    ["warmup", "posterior_hash", "block_size", "trace_binding", "retained_draws"],
)
def test_storage_product_gate_rejects_inconsistent_evidence(setup, monkeypatch, damage):
    ...
    if damage == "warmup":
        storage["warmup_exported"] = True
    if damage == "posterior_hash":
        storage["posterior_sha256"] = "0" * 64
    if damage == "block_size":
        storage["maximum_array_block_bytes"] = m.storage.MAX_BLOCK_BYTES + 1
    if damage == "trace_binding":
        storage["trace_manifest_sha256"] = "0" * 64
    if damage == "retained_draws":
        manifest["identity"]["draws"] -= 1
    with pytest.raises(ValueError, match="storage products differ"):
        m.disk_protocol.verify_products(
            protocol, storage, manifest, digest(args.output / "fit/posterior.nc")
        )
