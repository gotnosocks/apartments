...
def sources(tmp_path, *, mismatched=False, future=False):
    row = {'audit_id':'ad', 'source_listing_id':'1', 'unit_id':'u1', 'capture_ids':[1],
           'asking_rent':3000, 'private_outdoor_category':'TERRACE', 'shared_outdoor_category':'ROOF_DECK',
    ...
    capture = {'audit_id':'ad', 'source_listing_id':'1', 'unit_id':'other' if mismatched else 'u1',
               'capture_id':1, 'source_collected_at':'2026-01-04T00:00:00Z' if future else '2026-01-02T00:00:00Z',
               'property_details':details, 'description':text, 'extraction':extract({'propertyDetails':details,'description':text})}
    evidence = tmp_path/'evidence'
    em = publish_bundle(evidence, {'evidence.jsonl':canonical(capture)+'\n'}, {'version':'fixture'})
    dataset = tmp_path/'dataset'
    publish_bundle(dataset, {'observations.jsonl':canonical(row)+'\n'},
                   {'dataset_version':'advertised-outdoor-types-v1', 'outdoor_audit_manifest':em})
...
@pytest.mark.parametrize('kwargs,match', [({'mismatched':True},'identity mismatch'),({'future':True},'knowledge clock')])
def test_scope_driver_rejects_unbound_identity_and_future_evidence(tmp_path,kwargs,match):
