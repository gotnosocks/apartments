...
def sample():
    ...
    for year in (2017,2018,2019):
        for unit in range(30):
            rows.append(dict(unit_id=f'u{unit}',building=f'b{unit//2}',audit_id=f'{year}-{unit}',
                period=pd.Timestamp(year,1,1),bedrooms=1.,bathrooms=1.,square_feet=600.,
                asking_rent=3000.+150*(unit%3),laundry_type=('in_building','in_unit',None)[unit%3],
...
def test_positive_unknown_exposure_design_has_no_value_contrast():
    ...
    assert support['window_exposures.south']=={'positive':45,'negative':0,'unknown':45}
...
def test_matched_reference_reuse_exposure_equivalence_and_wrong_checkpoint(tmp_path,monkeypatch):
    data=sample()
    monkeypatch.setattr(model,'load_analytical',lambda _: (data,{'fixture':True},{}))
    reference=tmp_path/'reference'
    ablation.run(tmp_path/'input',reference,years=(2019,),folds=tuple(range(5)),min_rows=1)
    root=tmp_path/'blocks'
    kwargs=dict(year=2019,folds=tuple(range(5)),blocks=('laundry','exposures'),min_rows=1,error_draws=20)
    ...
    for split in report['splits']:
        assert split['block_fits']['exposures']['exposure_diagnostic']['predictions_match_missingness']
        assert split['metrics']['exposures']['log_rmse']==pytest.approx(split['metrics']['missingness']['log_rmse'],abs=1e-10)
    ...
    donor=root/'building-0'/'laundry'; recipient=root/'building-0'/'exposures'
    shutil.rmtree(recipient);shutil.copytree(donor,recipient)
...
def test_changed_reference_row_membership_is_rejected(tmp_path,monkeypatch):
    ...
    changed=data.copy();changed['audit_id']=changed.audit_id+'-changed'
    monkeypatch.setattr(model,'load_analytical',lambda _: (changed,{'fixture':True},{}))
    with pytest.raises(ValueError,match='membership changed'):
        blocks.run(tmp_path/'input',reference,tmp_path/'bad',year=2019,folds=(0,),min_rows=1)
