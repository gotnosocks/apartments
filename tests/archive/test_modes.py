...
def test_backfill_defaults_to_historical_profile(tmp_path, monkeypatch):
    seen = []
    monkeypatch.setattr(cli, 'run_crawler', lambda args, generation, lock: seen.append(args) or 0)
...
def test_resume_inherits_profile_and_allows_override(tmp_path, monkeypatch):
    ...
    store.enqueue(generation, [{'url': 'https://streeteasy.com/building/a', 'kind': 'building'}])
    with store._tx():
        store.db.execute(
            'INSERT INTO metadata VALUES(?,?)',
            (f'crawl_profile:{generation}', json.dumps({
                'include_unavailable': True, 'transport': 'http', 'delay': 10,
                'concurrency': 5, 'api_rps': 2,
            })),
        )
    store.close()
    seen = []
