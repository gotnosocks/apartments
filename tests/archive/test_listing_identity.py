...
def data(identifier="123", category="rental", history=True):
    value = {
        "id": identifier,
        "propertyDetails": {
            "address": {"street": "123 Test Street", "displayUnit": "#04C"}
        },
    }
    if history:
        value["propertyHistory"] = [
            {
                "listingId": "100",
                category + "EventsOfInterest": [{"date": "2015-01-31", "price": 3120}],
            }
        ]
    stream = "0:" + json.dumps({"listing": value})
    body = (
        "<script>self.__next_f.push(" + json.dumps([1, stream]) + ")</script>"
    ).encode()
...
def setup(tmp_path):
    ...
    store.enqueue(
        gen, [{"url": DIRECT, "kind": "listing"}, {"url": NESTED, "kind": "listing"}]
    )
    return store, gen
...
def record(store, gen, url=DIRECT, **kwargs):
    ...
    return store.record(
        gen, url, 200, {"ETag": "own-route"}, body, "text/html", extracted
    )
...
def test_identity_keeps_types_intents_and_zero_padded_unit_labels_separate():
    ...
    for url in (
        DIRECT + "/media_gallery",
        DIRECT + "?page=2",
        DIRECT + "?archive_view=x",
        "https://streeteasy.com/building/example/04c",
        "https://streeteasy.com/building/example/4c",
        "https://other.test/rental/123",
        DIRECT + "-unknown",
    ):
...
def test_failed_or_unproven_capture_does_not_suppress_other_route(tmp_path, failure):
    ...
    s.claim(g)
    if failure == "partial":
        record(s, g, history=False)
    elif failure == "wrong_id":
        record(s, g, identifier="999")
    elif failure == "http_error":
        s.record_gap(g, DIRECT, 404, {}, "not found", body=b"not found")
    elif failure == "missing_body":
        s.record(g, DIRECT, 200, {}, None, extracted=data()[1])
    elif failure == "missing_snapshot":
        s.record(g, DIRECT, 200, {}, b"no extraction")
    else:
        body, extracted = data()
        extracted["scripts"][0]["flight_chunks"][0][1] = (
            '0:{"listing":{"id":"123","propertyDetails":{"address":{"street":"x"}},"propertyHistory":"$a"}}'
        )
        s.record(g, DIRECT, 200, {}, body, extracted=extracted)
    assert s.claim(g)["url"] == NESTED


def test_transient_failure_leaves_alternate_available_and_restart_recovers_reservation(
    tmp_path,
):
    s, g = setup(tmp_path)
    s.claim(g)
...
def test_source_scope_transfers_and_old_generation_never_satisfies_new(tmp_path):
    ...
    assert s.db.execute(
        "SELECT 1 FROM scope_urls WHERE generation=? AND url=?", (g, DIRECT)
    ).fetchone()
    new = s.new_generation("refresh")
    s.revisit_known(new, interval=0)
    assert s.claim(new)["url"] in (DIRECT, NESTED)
    assert s.claim(new) is None
    claimed = s.db.execute(
        "SELECT url FROM frontier WHERE generation=? AND state='inflight'", (new,)
    ).fetchone()[0]
    record(s, new, claimed)
    assert s.claim(new) is None
    assert (
        s.db.execute(
            "SELECT count(*) FROM observations WHERE generation=?", (new,)
        ).fetchone()[0]
        == 1
    )
...
def test_gallery_cleanup_preserves_old_observations_and_prevents_refresh(tmp_path):
    ...
    with s._tx():
        s.db.execute(
            "INSERT INTO frontier(generation,url,kind) VALUES(?,?,?)",
            (g, gallery, "listing"),
        )
    assert s.apply_listing_rules(g)["galleries_excluded"] == 1
    assert s.apply_listing_rules(g)["galleries_excluded"] == 0
    s.record(
        g, gallery, 200, {}, b"old gallery", extracted={"images": [{"src": "old.jpg"}]}
    )
    with s._tx():
        s.db.execute("UPDATE frontier SET state='done'")
    new = s.new_generation()
    s.revisit_known(new)
    assert not s.db.execute(
        "SELECT 1 FROM frontier WHERE generation=? AND url=?", (new, gallery)
    ).fetchone()
    assert (
        s.db.execute(
            "SELECT count(*) FROM observations WHERE url=?", (gallery,)
        ).fetchone()[0]
        == 1
    )
...
def test_upgrade_populates_existing_typed_keys_without_reading_bodies(tmp_path):
    ...
    db.execute("DROP INDEX frontier_listing_identity")
    db.execute("ALTER TABLE frontier DROP COLUMN listing_key")
    db.commit()
    db.close()
    s = ArchiveStore(tmp_path)
    assert {r[0] for r in s.db.execute("SELECT listing_key FROM frontier")} == {
        "rental:123:detail"
    }
...
def test_sale_requires_sale_history_and_does_not_reuse_rental_history():
    ...
    assert (
        capture_evidence(data(category="sale")[1], url)["listing_key"]
        == "sale:123:detail"
    )
...
def test_interval_refresh_defers_alias_using_proof_without_copying_validators(tmp_path):
    s, g = setup(tmp_path)
    record(s, g)
    s.apply_listing_rules(g)
    new = s.new_generation("refresh")
    s.revisit_known(new, interval=86400)
    assert s.claim(new) is None
    assert dict(
        s.db.execute("SELECT url,state FROM frontier WHERE generation=?", (new,))
    ) == {DIRECT: "deferred", NESTED: "deferred"}
...
def test_reuse_lookup_uses_identity_index_instead_of_scanning_generation(tmp_path):
    ...
    record(s, g)
    queries = []
    s.db.set_trace_callback(queries.append)
    s.apply_listing_rules(g)
    s.db.set_trace_callback(None)
    lookup = next(
        q for q in queries if "SELECT url FROM frontier" in q and "listing_key=" in q
    )
    plan = s.db.execute("EXPLAIN QUERY PLAN " + lookup).fetchall()
    assert any(
        "frontier_listing_identity" in row[3] and "listing_key=?" in row[3]
        for row in plan
    )
