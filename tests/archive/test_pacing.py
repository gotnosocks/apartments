...
def test_api_migrates_legacy_delay_and_preserves_new_override():
    assert (
        resolve_delay(None, "oxylabs", "chelsea", {"transport": "oxylabs", "delay": 35})
        == 0
    )
    assert (
        resolve_delay(
            ...
            "oxylabs",
            "chelsea",
            {"transport": "oxylabs", "delay": 2, "pacing_version": 2},
        )
        == 2
    )
    ...
    assert (
        resolve_delay(
            ...
            "firefox",
            "chelsea",
            {"transport": "oxylabs", "delay": 0, "pacing_version": 2},
        )
        == 60
    )
    with pytest.raises(ValueError):
        resolve_delay(0, "firefox", "chelsea", {})
    with pytest.raises(ValueError):
        resolve_delay(float("nan"), "oxylabs", None, {})
...
def test_api_pacing_does_not_throttle_on_provider_latency(tmp_path):
    ...
    spider = ArchiveSpider.from_crawler(
        crawler, data_dir=tmp_path, transport="oxylabs", delay=0
    )
