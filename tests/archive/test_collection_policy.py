...
from streeteasy_archive.collection_policy import setup, annotate, exclusion_reason
...
def body(canonical=UNIT, category='rental'):
    listing = {'id': '123', 'propertyDetails': {'address': {'street': '123 Test Street'}},
               'propertyHistory': [{'listingId': '123', category + 'EventsOfInterest': [{'date': '2020-01-01', 'price': 3000}]}]}
    head = f'<link rel="canonical" href="{canonical}">' if canonical else ''
    return ('<html><head>' + head + '</head><body><script type="application/json">' +
            json.dumps({'listing': listing}) + '</script></body></html>').encode()
...
def test_claim_never_spends_on_sales_or_unassociated_episodes(tmp_path):
    ...
    urls = [EPISODE, 'https://streeteasy.com/sale/123',
            'https://streeteasy.com/building/example?archive_view=unavailable-sales', UNIT]
...
def test_canonical_rental_evidence_unlocks_episode_and_survives_resume(tmp_path):
    s = ArchiveStore(tmp_path); g = s.new_generation(); setup(s, g)
    s.enqueue(g, [{'url': EPISODE, 'kind': 'listing'}]); assert s.claim(g) is None
    data = {}; annotate(s, g, data, body(), UNIT)
    assert data['collection_policy']['eligible']
    assert exclusion_reason(s, g, EPISODE) is None
    s.close(); s = ArchiveStore(tmp_path)
...
def test_missing_canonical_sales_and_conflicting_units_do_not_expand(tmp_path):
    ...
    for raw in [body(canonical=None), body(category='sale')]:
        ...
        assert not s.db.execute('SELECT 1 FROM scope_urls WHERE url=?', (EPISODE,)).fetchone()
...
def test_archived_unit_replay_establishes_membership_without_new_observations(tmp_path):
    ...
    raw = body(); data = extract(raw, UNIT, 'text/html')
...
def test_parsing_before_raw_capture_does_not_authorize_requests(tmp_path):
    ...
    assert exclusion_reason(s, g, EPISODE + '-unknown') == 'unsupported_advertisement_route'
    s.close()
...
def test_same_advertisement_unit_capture_reused_without_new_fetch(tmp_path):
    ...
    state = s.db.execute('SELECT state,attempts FROM frontier WHERE url=?', (EPISODE,)).fetchone()
    assert tuple(state) == ('superseded', 0)
    proof = json.loads(s.db.execute('SELECT reason FROM url_aliases WHERE url=?', (EPISODE,)).fetchone()[0])
...
def test_missing_body_or_later_failure_prevents_unit_reuse(tmp_path):
    for failure in ('missing_body', 'later_failure'):
        s = ArchiveStore(tmp_path / failure); g = s.new_generation(); setup(s, g)
        digest = save_unit(s, g)
...
def inventory_row(ad, label):
    ...
    return (f'<tr><td>{link}09/15/26</a></td><td>{link}{label}</a></td>'
            '<td><h6>$4,995</h6></td><td>1 bed</td><td>1 bath</td><td>- ft²</td><td>-</td></tr>')


def inventory_data(rows):
    return {'inventory': {'rows': [inventory_row(ad, label) for ad, label in rows]}, 'links': [], 'scripts': []}
...
def test_inventory_labels_become_unit_probes_only_for_rental_ads():
    ...
    data = inventory_data([(EPISODE, '#4C'), ('https://streeteasy.com/rental/124', '#9C-EAST'),
                           ('https://streeteasy.com/rental/125', ''), ('https://streeteasy.com/sale/126', '#5A'),
                           ('https://streeteasy.com/rental/127', '#Photos'), ('https://streeteasy.com/rental/128', '#4c')])
    assert inventory_unit_probes(data, INVENTORY) == {
        UNIT: EPISODE, 'https://streeteasy.com/building/example/9ceast': 'https://streeteasy.com/rental/124'}
...
def test_archived_in_scope_inventories_enroll_probes_offline(tmp_path):
    ...
    for url in (INVENTORY, other):
        s.record(g, url, 200, {}, b'<html></html>', 'text/html', inventory_data([(EPISODE, '#4C')]))
    s.db.execute('INSERT INTO scope_buildings VALUES(?,?)', (g, 'https://streeteasy.com/building/example'))
    s.db.commit()
    before = s.db.execute('SELECT count(*) FROM observations').fetchone()[0]
    setup(s, g); setup(s, g)
    reasons = s.db.execute("SELECT url, reason FROM scope_urls WHERE reason LIKE 'inventory-label%'").fetchall()
    assert [tuple(r) for r in reasons] == [(UNIT, f'inventory-label-unit-probe-v1 from {EPISODE}')]
...
def test_live_inventory_expansion_enrolls_probe_for_verified_building(tmp_path):
    ...
    s.db.execute('INSERT INTO scope_buildings VALUES(?,?)', (g, 'https://streeteasy.com/building/example'))
    s.db.commit()
    expand(s, g, inventory_data([(EPISODE, '#4C')]), INVENTORY, neighborhood='west-village')
    assert s.db.execute('SELECT 1 FROM scope_urls WHERE url=?', (UNIT,)).fetchone()
    expand(s, g, inventory_data([('https://streeteasy.com/rental/9', '#7')]),
           'https://streeteasy.com/building/elsewhere?archive_view=unavailable-rentals', neighborhood='west-village')
    assert not s.db.execute("SELECT 1 FROM scope_urls WHERE url LIKE '%elsewhere/7'").fetchone()
    s.close()
...
def test_unit_routes_are_claimed_before_associated_ads(tmp_path):
    ...
    s.enqueue(g, [{'url': EPISODE, 'kind': 'listing'}, {'url': other, 'kind': 'listing'}])
