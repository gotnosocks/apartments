...
@pytest.mark.parametrize(
    "status,parser_fails",
    [(200, False), (200, True), (403, False), (404, False), (302, False)],
)
def test_every_response_preserves_redacted_capture(
    tmp_path, monkeypatch, status, parser_fails
):
    ...
    list(
        spider.parse(
            HtmlResponse(
                URL, status=status, body=b"<html>original</html>", request=request
            )
        )
    )
    ...
    assert capture["provider"]["results"][0]["headers"] == [
        {"name": "ETag", "value": "safe"}
    ]
...
def test_repeated_body_keeps_distinct_capture_and_export(tmp_path):
    ...
    for job in ["first", "second"]:
        store.record(
            ...
            {},
            b"<html/>",
            extracted={"extraction_version": 1},
    ...
    assert [r["capture_metadata"] for r in rows] == [
        {"job_id": "first"},
        {"job_id": "second"},
    ]
    store.close()
...
def test_old_archive_adds_capture_column_without_altering_raw_data(tmp_path):
    ...
    db.execute(
        "CREATE TABLE observations(id INTEGER PRIMARY KEY, generation INTEGER, url TEXT, fetched REAL, status INTEGER, content_type TEXT, headers TEXT, body_hash TEXT, not_modified INTEGER DEFAULT 0, error TEXT)"
    )
    db.execute(
        "INSERT INTO observations(generation,url,fetched,status,headers) VALUES(1,?,10,200,'{}')",
        (URL,),
    )
    db.commit()
    db.close()
...
def test_default_spider_installs_provider_handler(tmp_path):
    ...
    assert instance.settings.getdict("DOWNLOAD_HANDLERS")["https"].endswith(
        "OxylabsDownloadHandler"
    )
    spider.store.close()
