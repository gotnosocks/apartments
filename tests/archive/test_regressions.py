...
def recorded(tmp_path):
    ...
    store.record(
        gen, URL, 200, {"ETag": "v1"}, b'<a href="/rental/123">history</a>', "text/html"
    )
    store.finish(gen)
    return store, gen
...
def test_304_rediscovery_and_missing_body_gap(tmp_path):
    ...
    assert store.db.execute(
        "SELECT 1 FROM frontier WHERE generation=? AND url=?",
        (second, "https://streeteasy.com/rental/123"),
    ).fetchone()
...
def test_error_archived_and_cooldown_atomic(tmp_path, status):
    ...
    assert (
        store.get_body(next(iter(store.observations(gen)))["body_hash"])
        == b"blocked body"
    )
...
def test_har_idempotence_missing_body_and_export_errors(tmp_path):
    ...
    entry = {
        ...
        "response": {
            ...
            "content": {"text": "<title>Example</title>", "mimeType": "text/html"},
        },
    }
    evil = {
        "request": {"url": "https://streeteasy.com.evil.test/building/a"},
...
def test_challenge_not_normal_captcha_library():
    assert not is_challenge(
        b'<title>Listing</title><script src="recaptcha.js"></script>'
    )
...
def test_detail_pages_take_priority_over_searches(tmp_path):
    ...
    store.record(
        gen,
        "https://streeteasy.com/building/example/5a",
        200,
        {},
...
def test_search_discovers_and_archives_actual_detail_body(tmp_path):
    ...
    search_response = Response(
        search,
        body=f'<a href="{detail}">Apartment 4A</a>'.encode(),
        request=Request(search, meta={"archive_url": search}),
    )
    ...
    snapshot = json.loads(
        store.db.execute(
            "SELECT extracted FROM snapshots WHERE url=?", (detail,)
        ).fetchone()[0]
    )
