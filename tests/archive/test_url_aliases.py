...
def legacy(store, generation, url, kind="building", state="pending"):
    with store._tx():
        store.db.execute(
            "INSERT INTO frontier(generation,url,kind,state) VALUES(?,?,?,?)",
            (generation, url, kind, state),
        )
        store.db.execute(
            "INSERT INTO scope_urls VALUES(?,?,?)", (generation, url, "legacy scope")
        )


def state(store, generation, url):
    row = store.db.execute(
        "SELECT state FROM frontier WHERE generation=? AND url=?", (generation, url)
    ).fetchone()
...
def test_migration_is_evidenced_idempotent_and_preserves_history_and_intents(tmp_path):
    ...
    alias = ROOT + "?unit_type=rentals"
    legacy(s, g, alias)
    captured = ROOT + "?similar=1"
    legacy(s, g, captured)
    s.record(g, captured, 200, {}, b"old variant", extracted={"title": "variant"})
    separate = [
        ROOT + "?archive_view=unavailable-rentals",
        ROOT + "?archive_view=unavailable-sales",
        ROOT + "?page=2&unit_type=rentals",
        ROOT + "?future=1",
        ROOT + "/4c",
        "https://streeteasy.com/rental/123",
        "https://streeteasy.com/rental/124",
    ]
    for url in separate:
        legacy(
            s,
            ...
            url,
            "inventory"
            if "archive_view" in url
            else "listing"
            if "/4c" in url or "/rental/" in url
            else "building",
        )
    ...
    assert (
        s.db.execute(
            "SELECT target_url FROM url_aliases WHERE url=?", (alias,)
        ).fetchone()[0]
        == ROOT
    )
...
def test_alias_only_scope_transfers_to_unfetched_canonical(tmp_path):
    ...
    assert (
        s.db.execute("SELECT reason FROM scope_urls WHERE url=?", (ROOT,)).fetchone()[0]
        == "building card"
    )
...
def test_no_success_or_old_generation_does_not_suppress_target(tmp_path):
    s = ArchiveStore(tmp_path)
    old = s.new_generation()
    s.enqueue(old, [{"url": ROOT, "kind": "building"}])
    s.record(old, ROOT, 200, {}, b"old")
    g = s.new_generation("update")
    alias = ROOT + "?similar=1"
    legacy(s, g, alias)
    ...
    another = ROOT + "?unit_type=rentals"
    legacy(s, g, another)
    assert s.deduplicate_building_views(g)["awaiting_canonical_capture"] == 1
    assert state(s, g, another) == "pending"


def test_incremental_copies_one_canonical_and_preserves_validators_and_freshness(
    tmp_path,
):
    s = ArchiveStore(tmp_path)
    g = s.new_generation()
    ...
    new = s.new_generation("update")
    s.revisit_known(new, interval=86400)
    assert (
        s.db.execute(
            "SELECT count(*) FROM frontier WHERE generation=?", (new,)
        ).fetchone()[0]
        == 1
    )
...
def test_alias_capture_does_not_supply_canonical_validator_or_freshness(tmp_path):
    ...
    alias = ROOT + "?similar=1"
    legacy(s, g, alias)
    s.record(g, alias, 200, {"ETag": "variant"}, b"variant")
    new = s.new_generation("update")
    s.revisit_known(new, interval=86400)
...
def test_reporting_counts_resources_separately_from_completed_variants(tmp_path):
    ...
    s = ArchiveStore(tmp_path)
    g = s.new_generation()
    enroll(s, g, {ROOT: "main"}, roots=[ROOT])
    s.record(g, ROOT, 200, {}, b"main")
    legacy(s, g, ROOT + "?similar=1")
    s.record(g, ROOT + "?similar=1", 200, {}, b"variant")
    legacy(s, g, ROOT + "?unit_type=sales")
    s.deduplicate_building_views(g)
    view = ROOT + "?archive_view=unavailable-rentals"
    s.enqueue(g, [{"url": view, "kind": "inventory"}])
    s.record(g, view, 200, {}, b"inventory")
    expected = {
        "known_scope_roots": 1,
        "main_pages_captured": 1,
        "expanded_inventories_captured": 1,
        "building_urls_done": 2,
        "building_urls_superseded": 1,
    }
