...
class FixtureDownloadHandler:
    ...
    async def download_request(self, request, spider=None):
        ...
        return Response(
            url=url, status=status, headers=response_headers, body=body, request=request
        )
...
def test_scrapy_lifecycle_queue_304_block_and_external_redirect(
    tmp_path, fixture_handler
):
    ...
    store.record(
        ...
        200,
        {"ETag": "old"},
        b"<html><title>cached body</title></html>",
        "text/html",
    )
    ...
    fixture_handler.responses.update(
        {
            root: (
                ...
                f'<a href="{discovered}">new</a>'.encode(),
            ),
    ...
    redirect_store.enqueue(
        redirect_generation, [{"url": redirected, "kind": "building"}]
    )
    ...
    restart_store.enqueue(
        restart_generation, [{"url": restart_url, "kind": "building"}]
    )
    assert restart_store.claim(restart_generation)["url"] == restart_url
    restart_store.close()
    restart_store = ArchiveStore(restart_dir)
    restart_store.recover_inflight(restart_generation)
    fixture_handler.responses[restart_url] = (
        200,
        {"Content-Type": "text/html"},
        b"<title>restart</title>",
    )
    ...
    assert urls.count(blocked) == 1, (
        "403 must close the crawl instead of retrying immediately"
    )
    assert urls.count(restart_url) == 1
    assert crawlers[1].stats.get_value("spider_exceptions/count", 0) == 0
    assert (
        store.get_body(store.latest_body(None, cached))
        == b"<html><title>cached body</title></html>"
    )
    ...
    redirect_rows = [
        row
        for row in ArchiveStore(redirect_dir).observations(redirect_generation)
        if row["url"] == redirected
    ]
...
def test_incremental_enqueue_deduplicates_known_completed_url(tmp_path):
    ...
    store.enqueue(
        generation, [{"url": url, "kind": "building", "lastmod": "2026-09-07"}]
    )

    row = store.db.execute(
        "SELECT state, attempts, lastmod FROM frontier WHERE generation=? AND url=?",
        (generation, url),
    ).fetchone()
