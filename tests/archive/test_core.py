...
def test_crash_recovery_and_atomic_discovery(tmp_path):
    ...
    s.record(
        g,
        "https://streeteasy.com/building/a",
        200,
        {"ETag": "v1", "Cookie": "secret"},
        b"<html/>",
        "text/html",
        {"extraction_version": 1},
        discovered=[{"url": "https://streeteasy.com/rental/1", "kind": "listing"}],
    )
...
def test_content_dedup_and_304_reuses_previous_generation(tmp_path):
    s = ArchiveStore(tmp_path)
    g1 = s.new_generation("one")
    url = "https://streeteasy.com/building/a"
    s.enqueue(g1, [{"url": url, "kind": "building"}])
    s.claim(g1)
    digest = s.record(g1, url, 200, {"ETag": "v1"}, b"body", "text/plain")
    assert s.body_path(digest).exists()
    g2 = s.new_generation("two")
    s.enqueue(g2, [{"url": url, "kind": "building"}])
    s.claim(g2)
...
def test_cooldown_keeps_frontier_and_redirect_scope(tmp_path):
    s = ArchiveStore(tmp_path)
    g = s.new_generation("blocked")
    url = "https://streeteasy.com/building/a"
    s.enqueue(g, [{"url": url, "kind": "building"}])
    s.claim(g)
    s.cooldown(g, 120, "403")
    assert s.status(g)["pending"] == 1
    assert approved_links(
        [
            {"url": "/login", "kind": "building"},
            {"url": "/building/a", "kind": "building"},
        ],
        url,
    )[0]["url"].endswith("/building/a")
...
def test_unavailable_inventory_gets_turn_before_growing_listing_queue(tmp_path):
    ...
    store.enqueue(
        ...
        [
            ...
            {
                "url": "https://streeteasy.com/building/example?archive_view=unavailable-rentals",
                "kind": "inventory",
            },
