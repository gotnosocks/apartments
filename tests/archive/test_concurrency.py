...
def spider(tmp_path, budget=0, transport="oxylabs"):
    ...
    s.store.enqueue(
        s.generation,
        [
            {"url": f"https://streeteasy.com/rental/{n}", "kind": "listing"}
...
def test_five_slots_refilled_without_duplicates(tmp_path):
    ...
    more = list(
        s.parse(
            HtmlResponse(
                url=batch[2].url, body=b"<html>Listing</html>", request=batch[2]
            )
        )
    )
...
def test_budget_less_than_concurrency(tmp_path):
    ...
    assert len(batch) == 3
    assert (
        list(s.parse(HtmlResponse(url=batch[0].url, body=b"<html/>", request=batch[0])))
        == []
    )
    assert s.sent == 3
    s.store.close()
...
def test_challenge_stops_refills_but_saves_other_inflight_results(tmp_path):
    ...
    assert (
        list(
            s.parse(
                HtmlResponse(
                    url=batch[0].url, status=403, body=b"denied", request=batch[0]
                )
            )
        )
        == []
    ...
    assert (
        list(
            s.parse(
                HtmlResponse(
                    url=batch[1].url, body=b"<html>Listing</html>", request=batch[1]
                )
            )
        )
        == []
