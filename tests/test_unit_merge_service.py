...
def save(service, detail, request_id="merge-test"):
    return service.merge(
        {
            "listing_ids": detail["listing_ids"],
            "identity_revision": detail["identity_revision"],
            "review_revision": detail["review_revision"],
            "request_id": request_id,
            "author": "Ben",
            "reason": "Verified rental IDs refer to the same home",
        }
    )
...
def test_merge_creates_unit_record_and_one_history_without_changing_source(service):
    ...
    for sid, original in zip([1, 2], before):
        ...
        assert {k: v for k, v in updated.items() if k != "unit_id"} == {
            k: v for k, v in original.items() if k != "unit_id"
        }
    assert {r["unit_id"] for r in s.observations({})["rows"]} == {event["unit_id"]}
    assert reopened.mapping({})["listing_to_unit"] == {
        "1": event["unit_id"],
        "2": event["unit_id"],
    }
    ...
    reopened.undo(
        {
            "merge_id": event["id"],
            "identity_revision": unit["identity_revision"],
            "author": "Ben",
            "reason": "undo",
            "request_id": "undo-test",
        }
    )
...
def test_corrected_identity_candidates_and_conflicting_history_remain_separate(service):
    ...
    s.ledger.correct(
        [2],
        [
            {"op": "replace", "path": "/unit_label", "value": "4C"},
            {"op": "replace", "path": "/bedrooms", "value": 2},
            {
                "op": "replace",
                "path": "/archive_listing/propertyHistory/0/rentalEventsOfInterest/0/price",
                "value": 100,
            },
        ],
        "Ben",
...
def test_listing_count_sort_across_pages_filters_and_merged_units(service):
    ...
    for building, ids in groups[1:]:
        for lid in ids:
            s.db.execute(
                "INSERT INTO rental SELECT * REPLACE(? AS snapshot_id,? AS listing_id,? AS building_slug) FROM rental WHERE snapshot_id=1",
                [int(lid), lid, building],
            )
    u = UnitMergeService(s)
    ...
    assert [
        r["listing_count"] for r in u.candidates({**desc, "direction": "asc"})["rows"]
    ] == [2, 2]
    assert u.candidates({**desc, "search": "b"})["rows"][0]["listing_count"] == 4
    partial = u.ledger.write(
        "merge",
        listing_ids=["10", "11"],
        author="test",
        reason="fixture",
    ...
    assert (
        u.candidates({"limit": 1, "exclude_unit_id": partial["unit_id"]})["rows"][0][
            "building"
        ]
        == "c"
    )
    ...
    assert (
        u.candidates({"limit": 1, "exclude_unit_id": partial["unit_id"]})["rows"] == []
    )
...
def test_source_association_preview_apply_export_and_undo(service):
    ...
    assert u.ledger.events() == []
    assert u.proposal({"token": preview["token"]})["proposals"][0]["evidence"][
        "canonical_url"
    ].endswith("/a/4c")
    ...
    detail = u.inspect({"listing_ids": ["1"]})
    assert (
        detail["listing_ids"] == ["1", "2"]
        and detail["association_basis"] == "streeteasy"
    )
    ...
    u.undo(
        {
            "merge_id": detail["latest_merge"]["id"],
            "author": "Ben",
    ...
    assert (
        u.candidates({"queue": "review"})["total"] == 1
    )  # Undo is respected, not re-suggested in bulk.
...
def test_canonical_groups_surface_different_labels_without_overlapping_actions(service):
    ...
    assert (
        "Building or unit labels differ" in groups["rows"][0]["assessment"]["reasons"]
    )
    assert u.candidates({"queue": "supported"})["total"] == 0
    # A label match overlapping that canonical group becomes one reviewable group.
    s.db.execute(
        "INSERT INTO rental SELECT * REPLACE(5 AS snapshot_id,'5' AS listing_id) FROM rental WHERE snapshot_id=2"
    )
    u = UnitMergeService(s)
    u._source = evidence()
    groups = u.candidates({})
...
def test_shared_latest_group_saved_identity_does_not_follow_changing_latest(service):
    ...
    unit_id = before["1"]
    assert unit_id.startswith("unit:") and unit_id != "2"
    u._source.latest = {1: "99", 2: "99"}
    u._assessments = None
...
def test_eligible_latest_subgroup_is_not_blocked_by_other_same_label_listings(service):
    u = ready_service(service)
    service.db.execute(
        "INSERT INTO rental SELECT * REPLACE(5 AS snapshot_id,'5' AS listing_id) FROM rental WHERE snapshot_id=1"
    )
    ...
    return u.separate(
        {
            ...
            "request_id": request_id,
            "author": "Ben",
            "reason": "These are different homes",
        }
    )
...
def test_keep_separate_removes_suggestions_and_counts_with_persistent_undo(service):
    ...
    assert reopened.candidates({"mode": "separated"})["rows"][0]["listing_ids"] == [
        "1",
        "2",
    ]
    assert (
        reopened.candidates({"mode": "separated", "search": "nonexistent"})["total"]
        == 0
    )
    ...
    reopened.undo_separate(
        {
            "separation_id": event["id"],
            "identity_revision": detail["identity_revision"],
            "author": "Ben",
            "reason": "New evidence",
            "request_id": "undo",
...
def test_exclusion_removes_candidates_preserves_membership_and_marks_exports(service):
    ...
    s.listing_inclusion(
        {**args, "ledger_revision": s.ledger.revision(), "request_id": "exclude-again"}
    )
    mapping = u.mapping({})
    assert mapping["listing_to_unit"] == {
        "1": merged["unit_id"],
        "2": merged["unit_id"],
    }
