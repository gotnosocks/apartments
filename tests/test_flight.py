...
def test_text_byte_lengths_adjacent_frames_and_embedded_fake_json():
    ...
    decoded = decode_records(frame('a', text) + frame('b', 'yes') + 'c:{"description":"$a"}\n')
...
def test_legacy_json_model_scalars_references_and_ignored_tags():
    assert decode_records(':HL["style"]\n1:I["module"]\n2:{"id":42}\n3:["$2"]\n4:"$2"\n5:null\n') == {
        '2': {'id': 42}, '3': ['$2'], '4': '$2', '5': None}


@pytest.mark.parametrize('stream, message', [
    ('a:Tff,short', 'Truncated'), ('a:Txyz,foo', 'Malformed'),
    ('a:T1,é', 'UTF-8'), ('a:T1,long\nb:{}\n', 'boundary'),
    ('a:T9999999999999999999999999,foo', 'Malformed'),
    ('a:T-1,foo', 'Malformed'), ('a:T,foo', 'Malformed'),
    ('a:T1,\ud800', 'Unicode'),
])
