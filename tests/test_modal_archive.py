...
def module():
    spec = importlib.util.spec_from_file_location(
        "modal_archive", Path(__file__).parents[1] / "models/modal_archive.py"
    )
...
def test_resumable_allowlisted_upload(module, tmp_path, monkeypatch):
    ...
    (archive / "bodies/aa").mkdir(parents=True)
    body = archive / "bodies/aa/hash.gz"
    body.write_bytes(b"body")
    (archive / ".env").write_text("do-not-upload")
    db = sqlite3.connect(archive / "archive.sqlite3")
    db.execute("create table observations(id integer)")
    db.commit()
    db.close()
    paths = []
    ...
    assert first == [
        "/snapshots/test/archive.sqlite3",
        "/bodies/aa/hash.gz",
        "/snapshots/test/complete.json",
    ]
