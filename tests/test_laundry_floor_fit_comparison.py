...
def protocols():
    a = {
        ...
        "implementation_sha256": {
            "math.py": "same",
            "bayesian_feature_experiment_v3.py": "old",
            "reviewed_source_lineage.py": "old",
        },
    }
    b = deepcopy(a)
    b["source_version"] = m.split.VERSION
    b["implementation_sha256"].update(
        {
            "laundry_floor_split.py": "new",
            "bayesian_feature_experiment_v3.py": "new",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("adaptation", "flow"),
        ("adaptation", None),
        ("building_prior_scale", 0.1),
        ("extra_sampler_setting", True),
    ],
)
def test_adaptation_option_is_not_a_general_protocol_bypass(key, value):
    ...
    with pytest.raises(ValueError):
        m.check_protocols(a, b, allow_adaptation_change=True)


@pytest.mark.parametrize(
    "fault", ["prior", "noise", "draws", "floor", "math", "inventory"]
)
...
def test_actual_category_split_preserves_other_model_columns(fault):
    ...
    if fault == "area":
        after.loc[0, "square_feet"] *= 2
    elif fault == "prior":
        b.prior_scales[b.features.index("bedrooms_gt_0")] *= 2
    elif fault == "centering":
        b.means[b.features.index("bedrooms_gt_0")] += 0.1
    if fault:
        with pytest.raises(ValueError, match="Nonlaundry"):
            m.check_designs(a, b, before, after)
    else:
...
def residual_fixture():
    ...
    before = [
        ...
        | {"fitted_rent": 2800.0, "residual_log": 0.1}
        for r in rows
    ]
    after = deepcopy(before)
    after[0].update(fitted_rent=2850.0, residual_log=0.08)
    after[1].update(fitted_rent=2600.0, residual_log=0.14)
...
def test_movements_align_identity_not_row_position_and_include_worse_residuals():
    ...
    assert [r["fitted_rent_change"] for r in result] == [-200.0, 50.0]
    summary = m.summarize_slice(result)
    assert summary["candidate_median_absolute_log_residual"] == pytest.approx(0.11)
    assert summary["reference_median_absolute_log_residual"] == pytest.approx(0.1)
    assert summary["median_absolute_fitted_change"] == 125.0
...
def test_residual_membership_or_targets_must_not_drift(fault):
    ...
    if fault == "price":
        ...
        b.pop()
    with pytest.raises(ValueError):
        m.movement_rows(a, b, rows)


@pytest.mark.parametrize(
    "fault",
    [None, "posterior", "source", "fit_binding", "draw_count", "gate", "interval"],
)
def test_category_analysis_must_bind_the_exact_complete_posterior(tmp_path, fault):
    ...
    value = {
        "version": m.categories.VERSION,
        "bindings": {
            k: digest(p / "complete.json")
            for k, p in [
                ("fit", experiment / "fit"),
        ...
        "source_observations_sha256": "source",
        "status": "all_supported_contrasts_converged",
        "all_joint_beta_draws": True,
        "chains": 4,
        "draws_per_chain": 6000,
        "design_features": ["x"],
        "contrasts": [
            {
                "diagnostics": {"acceptable": True},
                "log_effect": {},
                "percent_effect": {},
            }
        ],
    ...
    if fault == "posterior":
        value["posterior_sha256"] = "different"
    elif fault == "source":
        value["source_observations_sha256"] = "different"
    elif fault == "fit_binding":
        value["bindings"]["fit"] = "different"
    elif fault == "draw_count":
        value["draws_per_chain"] = 50
    elif fault == "gate":
        value["contrasts"][0]["diagnostics"]["acceptable"] = False
    elif fault == "interval":
        value["contrasts"][0]["percent_effect"] = None
    publish_bundle(
        analysis, {"contrasts.json": canonical(value) + "\n"}, {"version": "test"}
    )
    if fault:
        with pytest.raises(ValueError):
            m.category_result(analysis, experiment, dataset, fit)
    else:
