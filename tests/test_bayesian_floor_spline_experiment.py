...
def configure(setup):
    args, _, _ = setup
    args.floor_prior_scale = 0.1
    args.maxdepth = 10
    frame, source = m.load_data(args.dataset)
    frame["listed_floor"] = [1.0, 2.0, 10.0, 52.0]
    code = {path.name: m.digest(path) for path in m.implementation_paths()}
    configuration = m.graph.graph_configuration(
        frame, args.prior_multiplier, **m.graph_kwargs(args)
    )
...
def test_protocol_binds_spline_prior_support_and_sampler_settings(setup):
    ...
    assert protocol["floor_knots"] == [1.0, 5.0, 10.0, 20.0, 35.0, 52.0]
    assert protocol["floor_reference"] == 2
    assert protocol["floor_prior_scale"] == 0.1
    assert protocol["floor_levels"] == [1.0, 2.0, 10.0, 52.0]
    assert protocol["graph_configuration"] == configuration
    assert protocol["maxdepth"] == 10
    assert (
        protocol["source_observations_sha256"] == source["files"]["observations.jsonl"]
    )
    assert m.disk_protocol.verify_protocol(protocol)
    assert {
        "bayesian_floor_spline_design.py",
        "bayesian_floor_spline_experiment.py",
    } <= code.keys()
    assert "floor_increment_prior_scale" not in protocol
    args.floor_prior_scale = 0.05
    tighter = m.make_protocol(args, frame, source, code, configuration)
    assert tighter["floor_prior_scale"] == 0.05
    assert (
        tighter["source_observations_sha256"] == protocol["source_observations_sha256"]
    )
...
def test_real_sampling_defaults_are_long_enough_for_diagnostics():
    ...
    assert (args.chains, args.tune, args.draws, args.target_accept, args.maxdepth) == (
        4,
        4000,
        6000,
        0.93,
    ...
    assert args.floor_prior_scale == 0.1
...
def posterior_fixture(design):
    ...
    beta = rng.normal(0, 0.01, (4, 1200, len(design.features)))
    ...
    beta[:, :, first] = rng.normal(0, 0.3, (4, 1200))
    beta[:, :, second] = (-direction[first] / direction[second]) * beta[
        :, :, first
    ] + rng.normal(0.04, 0.002, (4, 1200))
    posterior = xr.Dataset(
        {"beta": (("chain", "draw", "feature"), beta)},
        coords={"feature": design.features},
    )
    stats = xr.Dataset(
        {
...
def test_joint_floor_contrasts_preserve_draw_covariance_and_direction(spline_train):
    ...
    for case in result["contrasts"]:
        expected = beta @ design.contrast_vector(
            case["lower_floor"], case["upper_floor"]
        )
        interval = np.quantile(expected, [0.025, 0.5, 0.975])
        keys = ["lower_95", "median", "upper_95"]
        np.testing.assert_allclose([case["log_effect"][key] for key in keys], interval)
        np.testing.assert_allclose(
            [case["percent_effect"][key] for key in keys], 100 * np.expm1(interval)
        )
    ...
    assert (
        full_range["log_effect"]["upper_95"] - full_range["log_effect"]["lower_95"]
        < 0.2
    )
