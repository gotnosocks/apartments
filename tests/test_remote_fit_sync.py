...
def test_roundtrip_uploads_only_changes_and_verifies_download(repo, tmp_path):
    ...
    request, sources = sync.build_request(repo=root, dataset=dataset, runner='models.fake_runner',
                                          runner_args=['--draws', '5'], run_id='one')
    assert len(upload(request, sources, blobs)) == len(request['files'])
    run_root, scratch = tmp_path/'volume/runs/one', tmp_path/'scratch'
    scratch.mkdir()
    result = sync.execute(request, blob_root=blobs, run_root=run_root, scratch=scratch, filesystem_root=fs)
    assert result['succeeded'] and (fs/str(dataset).lstrip('/')/'observations.jsonl').is_file()
    assert {f['path'] for f in result['files']} == {'fit/posterior.nc', 'fit/complete.json'}

    local = tmp_path/'local/fit-one'
    stats = sync.download(result, lambda p: sync.file_chunks(run_root/'output'/p), local)
    assert stats['files'] == 2 and (local/'fit/posterior.nc').read_text() == '{"rent": 1}\n5'
    ...
    request2, sources2 = sync.build_request(repo=root, dataset=dataset, runner='models.fake_runner',
                                            runner_args=['--draws', '5'], run_id='two')
...
def test_download_rejects_corruption_and_resumes(repo, tmp_path):
    ...
    sync.download(result, lambda p: sync.file_chunks(run_root/'output'/p), local)
    assert json.loads((local/'fit/complete.json').read_text())['files']['posterior.nc'] == \
