...
def protocols():
    ...
    candidate.update(
        source_version=checks.projection.VERSION,
        source_manifest_sha256="new source",
        source_observations_sha256="new rows",
    )
...
def test_actual_archived_to_current_loader_changes(name):
    ...
    path = (
        Path("models") if name.startswith("bayesian") else Path("src/apartments")
    ) / name
    ...
    tampered = (
        after.replace("v2.pd.DataFrame(rows)", "v2.pd.DataFrame(rows).fillna(0)")
        if name.startswith("bayesian")
        else after.replace("current, direct_floor_changes)", "current, None)")
    )
