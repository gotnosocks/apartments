    ...
    result.append(
        {
            **deepcopy(rows[-1]),
            "audit_id": SENTINEL,
            "unit_id": "scope-excluded-unit",
            "source_listing_id": "scope-excluded-ad",
            "capture_ids": ["scope-excluded-capture"],
            "capture_id": None,
            "period": "2018-01-01",
    ...
    evidence = [
        {
            **deepcopy(c),
            ...
            "spans": [{"start": 0, "end": 24, "literal": text[:24]}],
        }
        for c in captures
    ]
    d = {
        **{k: row[k] for k in q.IDENTITIES},
        "action": action,
        "reason": "Own advertisement offers commercial office space",
        "reviewer": "test reviewer",
        "reviewed_at": CLOCK,
        "source_row_sha256": q.sha(row),
...
def bundle(parent, rows, decisions):
    ...
    dm = {
        "version": q.DECISION_VERSION,
        "reviewed_at": CLOCK,
        "source_manifest_sha256": q.records_hash([parent]),
        "source_observations_sha256": parent["files"]["observations.jsonl"],
        "files": {
            "decisions.jsonl": q.records_hash(
                sorted(decisions, key=q.decision_sort_key)
            )
        },
    }
    manifest = {
        "version": q.VERSION,
        "source_manifest": deepcopy(parent),
        "source_manifest_sha256": q.records_hash([parent]),
        "source_rows": len(rows),
        "reviewed_at": CLOCK,
        "decisions_manifest": dm,
        "decisions_manifest_sha256": q.records_hash([dm]),
        "decision_ids": sorted(d["decision_id"] for d in decisions),
        "files": {
            q.SIDECAR: q.records_hash(changes),
            "observations.jsonl": q.records_hash(kept),
        },
    }
...
def extend(parent, rows, expanded_changes):
    matches = [
        (row, change)
        for row, change in zip(rows, expanded_changes, strict=True)
        if row["audit_id"] == SENTINEL
    ]
    assert len(matches) == 1
    row, change = matches[0]
    return bundle(
        parent, rows, [decision_for(row, change["original_change"]["captures"])]
    )
...
def fixture():
    ...
    for i in range(4):
        ...
        for field in ("floor_label_provenance", "expanded_floor_provenance"):
            row[field] = {
                "source_capture_evidence_sha256": q.records_hash(captures),
                "source_known_at": row["known_at"],
                "interpreted_at": "2026-09-19T23:00:00Z",
            }
        rows.append(row)
        if i in (0, 2):
            decisions.append(
                decision_for(row, captures, list(sorted(q.ACTIONS))[i // 2])
            )
    parent = {
        "version": q.PARENT,
        "interpreted_at": "2026-09-19T23:00:00Z",
...
def test_inverse_rejects_tampering_even_with_rehashed_mutable_files(fault):
    ...
    if fault == "floor":
        kept[0]["listed_floor"] = 90
    elif fault == "order":
        kept.reverse()
    elif fault == "missing":
        kept.pop()
    elif fault == "duplicate_kept":
        kept[1] = deepcopy(kept[0])
    elif fault == "position":
        changes[0]["source_index"] = 1
    elif fault == "bool_position":
        changes[0]["source_index"] = False
    elif fault == "duplicate_position":
        changes[1]["source_index"] = 0
    elif fault == "count":
        m["source_rows"] += 1
    elif fault == "bool_count":
        m["source_rows"] = True
    elif fault == "parent_version":
        m["source_manifest"]["version"] = "old-source"
    elif fault == "nested":
        m["source_manifest"]["version"] = q.VERSION
    elif fault == "parent_hash":
        m["source_manifest_sha256"] = "c" * 64
    elif fault == "decision_version":
        m["decisions_manifest"]["version"] = "old-decisions"
    elif fault == "decision_source":
        m["decisions_manifest"]["source_manifest_sha256"] = "c" * 64
    elif fault == "decision_observations":
        m["decisions_manifest"]["source_observations_sha256"] = "c" * 64
    elif fault == "decision_file":
        m["decisions_manifest"]["files"]["decisions.jsonl"] = "c" * 64
    elif fault == "decision_coverage":
        m["decision_ids"].pop()
    elif fault == "review_clock":
        m["reviewed_at"] = "2020-01-01"
    elif fault == "sidecar_footprint":
        changes[0]["extra"] = 1
    elif fault == "empty_sidecar":
        changes.clear()
    elif fault == "sidecar_row":
        changes[0]["observation"]["asking_rent"] += 1
    elif fault == "parent_clock":
        m["source_manifest"]["interpreted_at"] = "2027-01-01"
    ...
    if fault == "observations_hash":
        m["files"]["observations.jsonl"] = "c" * 64
    if fault == "sidecar_hash":
        m["files"][q.SIDECAR] = "c" * 64
...
def test_decision_rejects_unbound_or_inexact_evidence(fault):
    ...
    if fault == "typed_audit":
        ...
        d["evidence"].append(deepcopy(c))
    elif fault == "missing_capture":
        d["evidence"].pop()
    elif fault == "duplicate_source":
        d["source_captures"].append(deepcopy(s))
    elif fault == "source_hash":
        s["raw_listing_sha256"] = "c" * 64
    elif fault == "source_clock":
        s["known_at"] = "2026-09-17"
    elif fault == "evidence_clock":
        c["known_at"] = "2026-09-17"
    elif fault == "future_capture":
        c["known_at"] = s["known_at"] = "2027-01-01"
    elif fault == "collection_clock":
        c["source_collected_at"] = s["source_collected_at"] = CLOCK
    elif fault == "row_hash":
        d["source_row_sha256"] = "c" * 64
    elif fault == "description_hash":
        c["description"] += "!"
    elif fault == "raw_hash":
        c["raw_listing_sha256"] = "c" * 64
    elif fault == "body_hash":
        c["body_sha256"] = "c" * 64
    elif fault == "path":
        c["source_path"] = "/other/description"
    elif fault == "source_path":
        s["source_path"] = "/other/displayUnit"
    elif fault == "span":
        c["spans"][0]["end"] -= 1
    elif fault == "bool_span":
        c["spans"][0]["start"] = False
    elif fault == "empty_spans":
        c["spans"] = []
    elif fault == "duplicate_span":
        c["spans"].append(deepcopy(c["spans"][0]))
    elif fault in ("reviewer", "reason"):
        d[fault] = ""
    elif fault == "action":
        d["action"] = "quarantine_explicit_short_term_offer"
    elif fault == "early_review":
        d["reviewed_at"] = "2020-01-01"
    elif fault == "provenance":
        row["expanded_floor_provenance"]["source_capture_evidence_sha256"] = "c" * 64
        d["source_row_sha256"] = q.sha(row)
    elif fault == "provenance_clock":
        row["floor_label_provenance"]["interpreted_at"] = "2027-01-01"
        d["source_row_sha256"] = q.sha(row)
    elif fault == "known_clock":
        row["floor_label_provenance"]["source_known_at"] = CLOCK
        d["source_row_sha256"] = q.sha(row)
    elif fault == "typed_evidence_ad":
        c["source_listing_id"] = 100
    elif fault == "raw_witness":
        c["raw_listing_json"] = "{}"
    elif fault == "raw_nonobject":
        c["raw_listing_json"] = "[]"
    rehash_decision(d)
    if fault == "decision_hash":
        d["decision_id"] = "c" * 64
    with pytest.raises(ValueError):
...
def test_add_and_extend_full_expanded_fixture():
    ...
    added_caps = [
        {
            **captures[0],
            **{k: source_rows[-1][k] for k in q.IDENTITIES},
            "capture_id": "scope-excluded-capture",
        }
    ]
    second = old.project_row(
        source_rows[-1],
        added_caps,
        [],
        parent["building_floor_evidence"]["b"],
        parent["interpreted_at"],
    )
    second_change = {
        **change,
        "source_index": 1,
        "source_row_sha256": q.sha(source_rows[-1]),
        "captures": added_caps,
    }
    parent["source_rows"] = 2
    parent["source_manifest"]["files"]["observations.jsonl"] = q.records_hash(
        source_rows
    )
...
def test_optional_exact_raw_json_witness(raw, valid):
    rows, decisions, _ = fixture()
    row, d = rows[0], decisions[0]
    d["source_captures"][0]["raw_listing_sha256"] = hashlib.sha256(
        raw.encode()
    ).hexdigest()
    d["evidence"][0]["raw_listing_sha256"] = hashlib.sha256(raw.encode()).hexdigest()
    d["evidence"][0]["raw_listing_json"] = raw
    for field in ("floor_label_provenance", "expanded_floor_provenance"):
        row[field]["source_capture_evidence_sha256"] = q.records_hash(
            d["source_captures"]
        )
    d["source_row_sha256"] = q.sha(row)
    rehash_decision(d)
