...
def test_audit_dataset_reports_quality_without_dropping_duplicate_events(tmp_path: Path):
    db = duckdb.connect()
    try:
        ...
        db.execute("COPY listings TO ? (FORMAT PARQUET)", [str(tmp_path / "listing_observations" / "part.parquet")])
        ...
        db.execute("COPY events TO ? (FORMAT PARQUET)", [str(tmp_path / "event_mentions" / "part.parquet")])
    finally:
        db.close()

    report = audit_dataset(tmp_path)
    assert report["tables"]["counts"] == {"listing_observations": 2, "event_mentions": 3, "snapshots": 0, "fetch_observations": 0, "building_observations": 0, "inventory_rows": 0}
    assert report["event_mentions"]["event_key"] == {"rows": 3, "distinct": 2, "duplicate_rows": 1}
    ...
    assert report["event_mentions"]["occurrence_uniqueness"] == {"rows": 3, "distinct_occurrences": 3}
...
def test_audit_optional_inventory_and_building_diagnostics(tmp_path: Path):
    ...
    try:
        ...
        db.execute("CREATE TABLE s AS SELECT 1::BIGINT snapshot_id, 'u' url, 'h' body_hash, 'listing' kind, 1.0 observed_at")
        db.execute("COPY s TO ? (FORMAT PARQUET)", [str(tmp_path / "snapshots" / "p.parquet")])
        db.execute("CREATE TABLE b AS SELECT 1::BIGINT snapshot_id, 'b' building_slug, 'id' building_id, 2.0 residential_units, 1.0 latitude, 2.0 longitude, '{}' raw_building_json, 'partial' parse_status, 'x' AS \"error\"")
        db.execute("COPY b TO ? (FORMAT PARQUET)", [str(tmp_path / "building_observations" / "p.parquet")])
        db.execute("CREATE TABLE i AS SELECT 1::BIGINT snapshot_id, 3::BIGINT \"count\", 2::BIGINT row_count, '[]' expected_counts_json, '{}' raw_inventory_json, 'ok' parse_status, NULL::VARCHAR AS \"error\"")
        db.execute("COPY i TO ? (FORMAT PARQUET)", [str(tmp_path / "inventory_observations" / "p.parquet")])
    finally:
        db.close()
    report = audit_dataset(tmp_path)
    assert report["parse_failures"]["building_observations"]["by_status"] == {"partial": 1}
