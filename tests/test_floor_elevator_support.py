...
def test_opposing_claim_sensitivity_removes_whole_building_and_preserves_unknown_only():
    rows = [
        row(2, 0, "a", "opposing"),
        row(6, 0, "b", "opposing"),
        row(None, 1, "c", "opposing"),
        row(4, None, "d", "opposing"),
        row(2, 1, "e", "lift"),
        row(8, 1, "f", "lift"),
        row(3, None, "g", "unknown"),
    ]
...
def test_source_only_audit_uses_canonical_alias_without_fabricating_label_candidates(
    tmp_path,
):
    source, output = tmp_path / "source", tmp_path / "output"
    rows = [
        dict(
            audit_id="a", unit_id="u", building="b", advertised_floor=3, elevator=None
        ),
        dict(
            audit_id="b", unit_id="v", building="b", advertised_floor=6, elevator=True
        ),
    ]
    publish_bundle(
        source,
    ...
    captured = [
        json.loads(line)
        for line in (output / "explicit-observations.jsonl").read_text().splitlines()
    ]
