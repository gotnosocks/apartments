...
spec = importlib.util.spec_from_file_location('modal_fit', Path(__file__).parents[1]/'models/modal_fit.py')
module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(module)

@pytest.mark.parametrize('values', [(1,100,4), (100,0,4), (100,100,1), (100,100,100)])
...
def test_cpu_default_submits_only_volume_identifiers(monkeypatch):
    calls = []
    monkeypatch.setattr(module.fit_cpu, 'remote', lambda *args: calls.append(args) or {'files': []})
    module.main(snapshot='chelsea-test', run_id='test-run', draws=20, tune=20, chains=2)
    assert calls == [('chelsea-test', 'test-run', {'draws':20, 'tune':20, 'chains':2})]


def test_optional_gpu_dispatch(monkeypatch):
    calls = []
    monkeypatch.setattr(module, 'run_gpu', lambda *args: calls.append(args) or {'files': []})
    module.main(snapshot='chelsea-test', run_id='gpu-run', gpu=True, draws=20, tune=20, chains=2)
