...
def capture(listing="old", snapshot=1, beds=0, when="2026-09-01T00:00:00+00:00"):
    return {
        ...
        "raw_listing_json": json.dumps(
            {"id": listing, "pricing": {}, "propertyDetails": {}}
        ),
    }
...
def extraction(monkeypatch):
    monkeypatch.setattr(
        historical,
        "_extract",
        lambda raw: {"attributes": {}, "evidence": [], "conflicts": {}},
    )


def project(captures=None, events=None, memberships=None, **kwargs):
    return historical.project_advertisement(
        captures or [capture()],
        memberships or [membership()],
...
def test_retrospective_clock_and_own_advertisement_only():
    row, audit = project(
        events=[event(), event("another", date="2010-01-01", price=900)]
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            [event(price=2500), event(price=3000)],
            "conflicting_or_missing_initial_prices",
        ),
        (
            [event(price=None), event(price=3000)],
            "conflicting_or_missing_initial_prices",
        ),
...
def test_membership_ambiguity_and_capture_mismatch():
    ...
    assert (
        "unresolved_or_conflicting_canonical_url"
        in project(captures=[bad])[1]["reasons"]
    )
...
def test_effective_correction_and_raw_preservation(tmp_path, monkeypatch):
    ...
    assert (
        audit["captures"][0]["projections"][0]["corrections"][0]["id"] == record["id"]
    )
    assert (
        project(events=[event(date="2016-01-01")], overlay=overlay)[0]["bedrooms"] == 0
    )
    assert (
        project(overlay=corrections.Overlay(ledger, as_of="2026-09-14"))[0]["bedrooms"]
        == 0
    )
...
def test_correction_can_resolve_conflicting_initial_prices(tmp_path, monkeypatch):
    ...
    row, audit = project(
        events=[event(price=2500), event(price=3000)],
        overlay=corrections.Overlay(ledger, as_of="2026-09-18"),
    )
    assert row["rent"] == 2800
    assert {p["raw_initial_ask"] for p in audit["captures"][0]["projections"]} == {
        2500,
        3000,
    }
...
def test_bundle_real_parquet_hashes_idempotence_and_rejected_month(tmp_path):
        ...
        json.dumps(
            {
                ...
                "finished_at": corrections.instant("2026-09-17").timestamp(),
                "counts": {
                    "listing_observations": 2,
                    "rental_unit_memberships": 2,
                    "event_mentions": 2,
                },
            }
        )
    ...
    for i, e in enumerate(events):
        e["event_index"] = i
    records = {
        "listing_observations": captures,
        "rental_unit_memberships": [membership(), membership("new")],
        "event_mentions": events,
    }
    for name, records in records.items():
        (dataset / name).mkdir()
        pq.write_table(
            pa.Table.from_pylist(records), dataset / name / "part-00000.parquet"
        )
    ...
    assert first == historical.build_historical_dataset(
        dataset, output, as_of="2026-09-18"
    )
    assert read_bundle(output) == first
    assert first["coverage"]["accepted"] == 0
    assert first["coverage"]["exclusion_reasons"] == {
        "conflicting_layout_same_unit_month": 2
    }
...
def recovered_source(tmp_path):
    ...
    dataset.mkdir()
    c = capture()
    c["raw_listing_json"] = json.dumps(
        {"id": "old", "description": "$d", "pricing": {}}
    )
    c["collected_at"] = corrections.instant(c["collected_at"]).timestamp()
    c["parsed_at"] = c["collected_at"]
    e = {**event(), "event_index": 0}
    tables = {
        "listing_observations": [c],
        "rental_unit_memberships": [membership()],
        "event_mentions": [e],
        "snapshots": [
            {
                "snapshot_id": 1,
                "body_hash": "body-digest",
                "url": c["canonical_unit_url"],
    ...
    (dataset / "canonical-units.json").write_text(
        json.dumps(
            {
                "counts": counts,
    ...
    source_files = {
        str(path.relative_to(dataset)): digest(path)
    ...
    recovered = {
        "snapshot_id": 1,
        "listing_id": "old",
        "source_body_sha256": "body-digest",
        "original_raw_listing_sha256": hashlib.sha256(
            c["raw_listing_json"].encode()
        ).hexdigest(),
        "original_description_reference": "$d",
        "resolved_description": "This home has central air.",
        "interpreted_at": "2026-09-18T03:00:00+00:00",
        "parser_version": 7,
        "recovery_version": "description-recovery-v1",
    }
    recovered["description_sha256"] = hashlib.sha256(
        recovered["resolved_description"].encode()
    ).hexdigest()
...
def test_recovery_preserves_original_then_corrects_and_adds_knowledge_clock(
    ...
    monkeypatch.setattr(
        ...
        lambda raw: {
            "attributes": {
                ...
                if raw.get("description", "").startswith("This")
                else None
            },
            "evidence": [],
            "conflicts": {},
    ...
    ledger = tmp_path / "corrections.jsonl"
    monkeypatch.setattr(
        corrections, "now", lambda: corrections.instant("2026-09-18T04:00:00+00:00")
    )
    corrections.append(
        ledger,
        author="reviewer",
        reason="window AC verified",
        edit={
            "target": {"source": "streeteasy", "source_listing_id": "old"},
            "validity": {"all_time": True},
            "patch": [
                {"op": "test", "path": "/attributes/hvac_type", "value": "central_ac"},
                {
                    "op": "replace",
                    "path": "/attributes/hvac_type",
                    "value": "window_ac",
                },
            ],
        },
    ...
    historical.build_historical_dataset(
        dataset,
        tmp_path / "earlier",
        as_of="2026-09-18T02:00:00+00:00",
        description_recovery=tmp_path / "recovery",
    )
...
def test_source_manifest_count_and_membership_digest_fail_closed(tmp_path):
    ...
    with pytest.raises(ValueError, match="row count mismatch for event_mentions"):
        historical.build_historical_dataset(
            dataset, tmp_path / "output", as_of="2026-09-19"
        )
...
def test_recovery_jsonl_unicode_line_separator_is_not_a_record_boundary(tmp_path):
    ...
    record["description_sha256"] = hashlib.sha256(
        record["resolved_description"].encode()
    ).hexdigest()
    ...
    publish_bundle(
        tmp_path / "unicode",
        {
            ...
            "source-files.json": (
                tmp_path / "original" / "source-files.json"
            ).read_text(),
        },
        {k: v for k, v in manifest.items() if k != "files"},
    )
    result = historical.build_historical_dataset(
        dataset,
        tmp_path / "output",
        as_of="2026-09-19",
