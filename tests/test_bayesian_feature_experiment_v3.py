...
def posterior(args, design, configuration):
    scalar = np.ones((args.chains, args.draws)) * 0.2
    variables = {
        name: (("chain", "draw"), scalar.copy())
        for name in (
            "alpha",
            "annual_drift",
            "sigma_building",
    ...
    coords = {
        "chain": range(args.chains),
        "draw": range(args.draws),
        "feature": design.features,
        "building": design.time.buildings,
        "unit": design.time.unit_ids,
        "trend_basis": range(design.time.time_matrix.shape[1]),
        "season_basis": range(11),
    }
    for name, dim in [
        ("beta", "feature"),
        ...
        variables[name] = (
            ("chain", "draw", dim),
            np.repeat(scalar[:, :, None], len(coords[dim]), axis=2),
        )
    if args.residual_scale == "bedroom":
        coords["residual_bedroom"] = configuration["residual_bedroom_levels"]
        values = np.broadcast_to(
            np.arange(1, len(coords["residual_bedroom"]) + 1) * 0.1,
            (*scalar.shape, len(coords["residual_bedroom"])),
        ).copy()
        variables["sigma_by_bedroom"] = (("chain", "draw", "residual_bedroom"), values)
        variables["residual_bedroom_z"] = (
            ("chain", "draw", "residual_bedroom"),
        ...
        if configuration.get("residual_bedroom_parameterization") == "centered":
            variables["residual_bedroom_offset"] = (
                ("chain", "draw", "residual_bedroom"),
                np.zeros_like(values),
            )
...
def setup(tmp_path, monkeypatch):
    ...
    def build(data, observed_design, multiplier, **kwargs):
        events["build"] += 1
        model = Model()
        model.graph_configuration = m.graph.graph_configuration(
            data, multiplier, **kwargs
        )
        model.compression_summary = {
            "observations": len(data),
            "features": len(observed_design.features),
        }
    ...
    def prior(**kwargs):
        events["prior"] += 1
        events["prior_arguments"] = kwargs
        return FakeInference()

    def sample(model, **kwargs):
        events["sample"] += 1
        events["sample_arguments"] = kwargs
    ...
    def reports(target, inference, observed_design, data, protocol_hash):
        ...
        for name in (
            "summary.json",
            "diagnostics.json",
            "derived-diagnostics.json",
            "parameter-diagnostics.csv",
            "derived-diagnostics.csv",
            "bathroom-contrasts.json",
            "residuals.jsonl",
            "coefficients.json",
            "group-effects.jsonl",
        ):
            (target / name).write_text(
                canonical(result if name == "summary.json" else {}) + "\n"
            )
        return result
...
def test_explicit_graph_protocol_archival_residual_scales_and_no_resample(setup, mode):
    ...
    args.building_prior_scale = 0.7
    args.unit_prior_scale = 0.125
    result = m.run(args)
    ...
    assert (
        protocol["building_prior_scale"] == 0.7
        and protocol["unit_prior_scale"] == 0.125
    )
    assert protocol["source_observations_sha256"] == digest(
        args.dataset / "observations.jsonl"
    )
    assert {
        "bayesian_feature_experiment_v3.py",
        "bayesian_feature_experiment_v2.py",
        "bayesian_feature_experiment.py",
        "bayesian_feature_graph_v3.py",
        "bayesian_feature_graph.py",
        ...
        "reviewed_source_lineage.py",
        "laundry_floor_split.py",
        "reviewed_cohort_quarantine.py",
        "elevator_corrections.py",
        "floor_label_projection.py",
        "expanded_floor_projection.py",
        "residual_scope_projection.py",
        "direct_floor_projection.py",
    } == set(protocol["implementation_sha256"])
    ...
    assert summary["global_sigma"]["median"] == 0.2
    if mode == "bedroom":
        ...
        assert [r["sigma"]["median"] for r in summary["by_bedroom"]] == pytest.approx(
            [0.1, 0.2, 0.3]
        )
        assert summary["by_bedroom"][1]["support"] == {
            "rows": 2,
            "units": 2,
            "buildings": 2,
        }
    ...
    assert (
        events["build"] == events["prior"] == events["sample"] == events["reports"] == 1
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("seed", -1),
        ("prior_multiplier", 0),
        ("prior_multiplier", float("nan")),
        ("building_prior_scale", float("inf")),
        ("unit_prior_scale", -0.1),
        ("unit_prior_scale", True),
        ("target_accept", 0),
        ("target_accept", 1),
        ("target_accept", float("nan")),
        ("residual_scale", "unknown"),
        ("residual_parameterization", "unknown"),
        ("adaptation", "unknown"),
        ("spec", "unknown"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("unit_prior_scale", 0.5),
        ("residual_scale", "bedroom"),
        ("draws", 4),
        ("seed", 7),
    ],
)
...
def test_valid_checkpoint_resumes_reporting_without_sampling(setup, monkeypatch):
    ...
    def interrupted(*a):
        raise RuntimeError("interrupted before reports")
...
@pytest.mark.parametrize(
    "mutation", ["bytes", "protocol", "draws", "bedroom_coordinate"]
)
def test_bad_checkpoint_rejected_without_resampling(setup, monkeypatch, mutation):
    ...
    original = m.v2.write_reports
    monkeypatch.setattr(
        m.v2, "write_reports", lambda *a: (_ for _ in ()).throw(RuntimeError("stop"))
    )
    ...
    if mutation == "bytes":
        ...
        value = json.loads((fit / "posterior-checkpoint.json").read_text())
        value["protocol_sha256"] = "wrong"
        (fit / "posterior-checkpoint.json").write_text(canonical(value) + "\n")
    elif mutation == "draws":
        events["inference"]["posterior"] = events["inference"]["posterior"].isel(
            draw=slice(0, 2)
        )
    else:
        events["inference"]["posterior"] = events["inference"][
            "posterior"
        ].assign_coords(residual_bedroom=[0, 1, 3])
    monkeypatch.setattr(m.v2, "write_reports", original)
    with pytest.raises(ValueError, match="checkpoint|coordinate"):
        m.run(args)
...
def test_sampler_incomplete_return_never_becomes_checkpoint(setup, monkeypatch):
    ...
    monkeypatch.setattr(m.v2.sampler, "sample", sample)
    with pytest.raises(ValueError, match="Incomplete posterior"):
        m.run(args)
...
def test_actual_graph_configuration_cannot_differ(setup, monkeypatch):
    ...
    def wrong(*a, **k):
        model = original(*a, **k)
        model.graph_configuration["unit_prior_scale"] = 999
        return model

    monkeypatch.setattr(m.graph, "build_model", wrong)
    with pytest.raises(ValueError, match="Built graph settings"):
        m.run(args)
...
def test_completed_version_required_products_and_hash_integrity(setup):
    ...
    del manifest["files"]["residual-scales.json"]
    path.write_text(canonical(manifest) + "\n")
    with pytest.raises(ValueError, match="required products"):
        m.run(args)
...
def test_optional_graph_evidence_requires_v3_hash(setup, tmp_path):
    ...
    value = {
        "passed": True,
        "hashes": {"bayesian_feature_graph.py": code["bayesian_feature_graph.py"]},
        "rows": 4,
        "source_observations_sha256": "source",
    }
    publish_bundle(
        bad, {"parity.json": canonical(value) + "\n"}, {"version": "synthetic-parity"}
    )
    with pytest.raises(ValueError, match="v3 implementation"):
        m.graph_verification(bad, code)
    good = tmp_path / "good-parity"
    value["hashes"]["bayesian_feature_graph_v3.py"] = code[
        "bayesian_feature_graph_v3.py"
    ]
    publish_bundle(
        good, {"parity.json": canonical(value) + "\n"}, {"version": "synthetic-parity"}
    )
...
@pytest.mark.parametrize(
    "mutation", ["reordered", "invalid_labels", "missing_coordinate", "truncated"]
)
...
def test_loader_preserves_review_overlay_without_mutating_v2(setup, tmp_path, version):
    ...
    rows = [
        json.loads(line)
        for line in (args.dataset / "observations.jsonl").read_text().splitlines()
    ]
    rows[0]["bathroom_count_evidence"]["flags"] = [
        "reviewed_external_shared_bathroom_or_toilet_access"
    ]
    rows[0]["scope_composition_review_history"] = [
        {"decision": "source-bound review", "capture_id": "captured"}
    ]
    ...
    assert (
        data.iloc[0].scope_composition_review_history
        == rows[0]["scope_composition_review_history"]
    )
...
def test_reviewed_revision_name_without_exact_parent_lineage_is_rejected(
    setup, tmp_path, version
):
    args, _, _ = setup
    source = tmp_path / "unsupported_revision"
    publish_bundle(
        source,
        {"observations.jsonl": (args.dataset / "observations.jsonl").read_text()},
        {"version": version},
    )
    with pytest.raises(
        ValueError,
        match="lineage|Elevator|Floor projection|Expanded floor|Residual scope|direct floor|Direct floor",
    ):
        m.load_data(source)


@pytest.mark.parametrize(
    "mutation", ["duplicate_audit", "duplicate_unit_period", "zero_rent", "bad_version"]
)
def test_loader_retains_exact_cohort_validation(setup, tmp_path, mutation):
    ...
    if mutation == "duplicate_unit_period":
        rows[1]["unit_id"] = rows[0]["unit_id"]
    if mutation == "zero_rent":
        rows[0]["asking_rent"] = 0
    ...
    publish_bundle(
        ...
        {"observations.jsonl": "".join(canonical(r) + "\n" for r in rows)},
        {
            ...
            else "reviewed-scope-composition-projection-v2"
        },
    )
    with pytest.raises(ValueError):
        m.load_data(source)
