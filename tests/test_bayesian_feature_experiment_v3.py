...
def test_explicit_graph_protocol_archival_residual_scales_and_no_resample(setup,mode):
    ...
    assert {'bayesian_feature_experiment_v3.py','bayesian_feature_experiment_v2.py',
            ...
            'reviewed_source_lineage.py','laundry_floor_split.py','reviewed_cohort_quarantine.py','elevator_corrections.py','floor_label_projection.py','expanded_floor_projection.py'} == set(protocol['implementation_sha256'])
...
def test_reviewed_revision_name_without_exact_parent_lineage_is_rejected(setup, tmp_path, version):
    ...
    with pytest.raises(ValueError, match='lineage|Elevator|Floor projection|Expanded floor'):
        m.load_data(source)
