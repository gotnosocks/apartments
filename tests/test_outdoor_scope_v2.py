...
def capture(text, private=(), shared=(), key=1):
    details = {'features':{'privateOutdoorSpaceTypes':list(private)},
               'amenities':{'sharedOutdoorSpaceTypes':list(shared)}}
    return {'audit_id':'ad', 'source_listing_id':'123', 'unit_id':'unit:1', 'capture_id':key,
            'body_sha256':'body', 'raw_listing_sha256':'raw', 'source_collected_at':'2026-01-02T00:00:00Z',
...
@pytest.mark.parametrize('text,expected', [
    ...
    ('The primary bedroom has access to the terrace.', [('TERRACE','unit_access')]),
    ...
    ('Building features: shared garden. Apartment features: private patio.', [('GARDEN','shared'),('PATIO','private_explicit')]),
    ('A private terrace and a common roof deck.', [('TERRACE','private_explicit'),('ROOF_DECK','shared')]),
    ('The building offers a roof deck, but this apartment has a private roof deck.', [('ROOF_DECK','shared'),('ROOF_DECK','private_explicit')]),
    ('Views of a private garden.', [('GARDEN','view')]),
    ('The apartment has windows overlooking the garden.', [('GARDEN','view')]),
    ('Private balcony overlooking the garden.', [('BALCONY','private_explicit'),('GARDEN','view')]),
    ...
    ('Some apartments have patios. This unit has a balcony.', [('PATIO','unresolved'),('BALCONY','unit_access')]),
    ...
    ('Private backyard. Shared courtyard.', [('YARD','private_explicit'),('COURTYARD','shared')]),
    ...
    ('The master bedroom has a window looking out to the garden.', [('GARDEN','view')]),
...
def test_coexisting_private_shared_and_negative_retained_without_global_veto():
    ...
    assert [c['scope'] for c in actual['claims']] == ['private_explicit','shared','negative']
...
def test_text_and_structured_scope_are_independent_dimensions():
    ...
    assert measure([capture('A private terrace.')])['types_by_scope']['private_explicit'] == ['TERRACE']
...
def test_nested_source_and_unicode_boundaries():
    ...
    source['property_details']['description'] = 'Garden views.\u2028Private balcony.<br/>Shared garden.'
    source['extraction'] = extract({'description':source['description'], 'propertyDetails':source['property_details']})
...
def test_view_subject_is_separate_from_garden_ownership():
    ...
    building = scope_text('Windowed elevator lobbies overlooking a serene courtyard garden.')
    assert [(c['scope'],c['subject']) for c in building] == [('view','building'),('view','building')]


def test_non_amenity_names_do_not_become_planned_or_shared_facilities():
    claims = scope_text('The building is near Hudson Yards. The future London Terrace Towers project.')
    assert all(c['scope']=='unresolved' and c['subject']=='not_an_amenity' for c in claims)
