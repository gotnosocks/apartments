...
@pytest.mark.parametrize(
    "module", [outdoor_scope_evaluation, outdoor_scope_evaluation_v2]
)
def test_evaluation_counts_disagreements_and_unlabeled_claims_without_hiding_them(
    tmp_path, module
):
    ...
    publish_bundle(
        review,
        {"review.json": canonical({"cases": [case]}) + "\n"},
        {"version": "fixture"},
    )
    ...
    assert summary["confusion"] == [
        {"label": "private_explicit", "prediction": "negative", "count": 1}
    ]
    ...
    rows = [
        json.loads(s)
        for s in (output / "comparisons.jsonl").read_text().split("\n")
        if s
    ]
...
@pytest.mark.parametrize(
    "mutation,match", [("hash", "hash mismatch"), ("offset", "offsets")]
)
def test_evaluation_rejects_labels_not_bound_to_source_text(tmp_path, mutation, match):
    ...
    case = {
        ...
        "description_sha256": hashlib.sha256(text.encode()).hexdigest(),
        "annotations": [
            {"start": 8, "end": 15, "text": "balcony", "scope": "private_explicit"}
        ],
    }
    if mutation == "hash":
        case["description_sha256"] = "wrong"
    else:
        case["annotations"][0]["start"] = 7
    review = tmp_path / "review"
    publish_bundle(
        review,
        {"review.json": canonical({"cases": [case]}) + "\n"},
