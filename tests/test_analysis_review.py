...
from models import amenity_ablation,amenity_rent_model,minimal_rent_model,residual_review
...
def source_fit():
    ...
    for period in pd.date_range('2024-01-01','2024-06-01',freq='MS'):
        for i in range(12):
            ...
            rows.append(dict(audit_id=f'{period.month}-{i}',unit_id=f'u{i}',building=f'b{i//3}',building_id=f'b{i//3}',
                source_listing_id=str(period.month*100+i),period=period.strftime('%Y-%m-%d'),
                asking_rent=rent,log_rent=math.log(rent),bedrooms=i%3,bathrooms=1+.5*(i%3),square_feet=450+50*i,
                listed_floor=i+1,physical_floor=None,elevator=bool(i%2),
                laundry_type=['in_building','in_unit',None][i%3],doorman_type='full_time',hvac_type=None,
                pet_rules=None,pet_policy='approval_required' if i%2 else 'not_allowed',
                description='Source\u2028description',capture_ids=[f'c{period.month}-{i}'],
                known_at='2024-07-01T00:00:00Z',collected_at='2024-06-30T00:00:00Z',
                analysis_price_basis='historical_initial_own_advertisement_ask'))
    train=pd.DataFrame(rows);train['period']=pd.to_datetime(train.period)
    fit=minimal_rent_model.fit(train,amenity_rent_model.SETTINGS,iterations=20,encoder_class=amenity_ablation.encoder_class('full'))
    artifact={'version':robust_pricing.VERSION,'encoder':fit['encoder'].metadata(),
        'coefficients':fit['beta'].tolist(),'center':fit['center'],
        'training':{'knowledge_cutoff':'2024-07-01T00:00:00Z','membership_sha256':'fixture-membership',
...
def workspace(tmp_path,source_fit):
    ...
    dataset=tmp_path/'dataset';model_path=tmp_path/'model';residual_path=tmp_path/'residuals'
    ...
    mm=publish_bundle(model_path,{'model.json':canonical(artifact)+'\n'},
    ...
    publish_bundle(residual_path,{'residuals.jsonl':''.join(canonical(r)+'\n' for r in residuals),'summary.json':'{}\n'},
        {'version':'fitted-residual-review-v1','model_manifest':mm,'dataset_manifest':dm,
         'runtime_sha256':mm['runtime_sha256'],'pricing_features_sha256':mm['pricing_features_sha256']})
    return AnalysisWorkspace.load(model_path,dataset,residual_path),(model_path,dataset,residual_path)


def test_detail_replays_components_history_and_normalizes_aliases(workspace):
    w,_=workspace;d=w.detail('1-0')
    assert math.exp(math.fsum(d['log_contributions_by_family'].values()))==pytest.approx(d['residual']['fitted_rent'])
...
def test_supported_joint_contrast_matches_full_reencoding(workspace):
    w,_=workspace;changes={'bedrooms':1,'bathrooms':1.5,'square_feet':550,'laundry_type':'in_unit'}
    ...
    assert result['estimate']['changed_rent']==pytest.approx(expected['predicted_rent'])
...
def test_no_magnitude_zero_is_presented_for_unobserved_floor_or_reporting_change(workspace):
    w,_=workspace
...
@pytest.mark.parametrize('changes',[{'building_id':'new'},{'bedrooms':1.2},{'bathrooms':None},
    {'square_feet':float('nan')},{'elevator':'yes'},{'laundry_type':'invented'},{'bedrooms':0}])
def test_invalid_identity_layout_values_and_noop_rejected(workspace,changes):
    w,_=workspace
    with pytest.raises(ValueError):w.contrast('1-0',changes)


def test_residual_mismatch_cache_invalidation_and_changed_bundle_fail_closed(workspace):
    w,paths=workspace
    wrong=copy.deepcopy(w.residuals);wrong[0]['asking_rent']+=1
    with pytest.raises(ValueError,match='target or identity'):
        AnalysisWorkspace(w.model,w.rows,wrong,w.summary,w.manifests)
    wrong=copy.deepcopy(w.residuals);wrong[0]['asking_minus_fitted']+=10
    with pytest.raises(ValueError,match='arithmetic'):
        AnalysisWorkspace(w.model,w.rows,wrong,w.summary,w.manifests)
    before=bundle_signature(*paths)
    file=paths[2]/'residuals.jsonl';file.write_text(file.read_text()+'\n')
    assert bundle_signature(*paths)!=before
    with pytest.raises(ValueError):AnalysisWorkspace.load(*paths)


def test_same_length_different_residual_membership_rejected(workspace):
    w,_=workspace;wrong=copy.deepcopy(w.residuals);wrong[0]['audit_id']='another-row'
    with pytest.raises(ValueError,match='membership'):
        AnalysisWorkspace(w.model,w.rows,wrong,w.summary,w.manifests)


@pytest.mark.parametrize('fault', [None,'wrong_ad','future_capture','future_recovery','text_hash','wrong_dataset'])
def test_archived_descriptions_bind_identity_clock_and_exact_source_without_promoting_claims(workspace,tmp_path,fault):
    w,_=workspace
    capture={'audit_id':'1-0','unit_id':'u0','source_listing_id':'wrong' if fault=='wrong_ad' else '100',
             'capture_id':'c1-0','source_collected_at':'2024-08-01' if fault=='future_capture' else '2024-06-30',
             'description':'top-level fallback','property_details':{'description':'Archived <b>private terrace</b>\u2028wording.'},
             'extraction':{'experimental_inference':'must not be displayed as a physical fact'}}
    if fault=='future_recovery':capture['description_interpreted_at']='2024-08-01'
    if fault=='text_hash':capture['description_sha256']='invalid'
    path=tmp_path/'evidence'
    publish_bundle(path,{'evidence.jsonl':canonical(capture)+'\n'},
        {'version':'cohort-outdoor-evidence-v1','dataset_manifest':{} if fault=='wrong_dataset' else w.manifests['dataset']})
    if fault:
        with pytest.raises(ValueError):w.attach_evidence(path)
        assert w.detail('1-0')['source_captures']==[]
    else:
        w.attach_evidence(path)
        detail=w.detail('1-0');shown=detail['source_captures'][0]
