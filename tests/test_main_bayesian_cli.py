...
def fit_route(monkeypatch):
    ...
    def run(args, route):
        ...
        assert all(
            p["num_threads"] == 1 for p in threadpool_info() if p["user_api"] == "blas"
        )
    ...
    monkeypatch.setattr(
        research_pipeline,
        "fit_dataset",
        lambda *a, **kw: pytest.fail("No robust fallback"),
    )
    return seen
...
def test_fit_pricing_defaults_to_exact_spline_disk_runner(fit_route):
    ...
    assert (
        json.loads(result.output)["status"] == "diagnostic_only"
    )  # Never silently promotes diagnostics.
    assert fit_route == [
        {
            "dataset": Path("source"),
            "output": Path("posterior"),
            "draws": 4000,
            "tune": 2000,
            "chains": 4,
            "seed": 20260918,
            ...
            "residual_parameterization": "centered",
            "graph_validation": None,
            "floor_increments": False,
...
def test_explicit_sampler_and_model_options_are_preserved(fit_route):
    ...
    assert fit_route[0] == {
        ...
        "residual_parameterization": "noncentered",
        "graph_validation": Path("proof"),
        "floor_increments": True,
        "floor_increment_prior_scale": 0.15,
        "floor_prior_scale": 0.10,
        "maxdepth": None,
        "_runner": "legacy_disk",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ["--tune", "0"],
        ["--target-accept", "1"],
        ["--prior-multiplier", "nan"],
        ["--spec", "surrogate"],
        ["--residual-scale", "robust"],
        ["--adaptation", "variational"],
        ["--residual-parameterization", "other"],
        ["--execution", "surrogate"],
        ["--floor-increments", "--floor-increment-prior-scale", "0"],
        ["--floor-model", "unknown"],
        ["--floor-model", ""],
        ["--floor-prior-scale", "0"],
        ["--floor-prior-scale", "nan"],
        ["--floor-prior-scale", "inf"],
        ["--maxdepth", "0"],
        ["--maxdepth", "21"],
        ["--execution", "memory"],
        ["--graph-validation", "legacy-proof"],
...
def test_explicit_floor_design_routes_to_disk_with_its_prior(fit_route):
    ...
    assert fit_route[0]["floor_increment_prior_scale"] == 0.08
...
def test_explicit_floor_model_routes_without_legacy_flag(fit_route, model):
    result = CliRunner().invoke(
        app, ["fit-pricing", "source", "posterior", "--floor-model", model]
    )
    assert result.exit_code == 0, result.output
...
def test_explicit_spline_prior_and_depth_are_preserved(fit_route):
    ...
    assert result.exit_code == 0, result.output
    assert fit_route[0]["floor_prior_scale"] == 0.05
    assert fit_route[0]["maxdepth"] == 12
    assert (fit_route[0]["tune"], fit_route[0]["draws"], fit_route[0]["seed"]) == (
        4000,
        6000,
        20260924,
    )
...
def test_legacy_disk_depth_is_explicit(fit_route):
    result = CliRunner().invoke(
        app,
        ["fit-pricing", "source", "posterior", "--linear-floor", "--maxdepth", "14"],
    )
    assert result.exit_code == 0, result.output
    assert fit_route[0]["_runner"] == "legacy_disk"
...
def test_memory_execution_is_explicit_for_older_protocol_replay(
    fit_route, monkeypatch, increments
):
    ...
    monkeypatch.setattr(
        runner, "run", lambda args: calls.append(vars(args)) or {"status": "replayed"}
    )
    result = CliRunner().invoke(
        app,
        [
...
def test_linear_floor_requires_explicit_legacy_choice(fit_route):
    result = CliRunner().invoke(
        app, ["fit-pricing", "source", "posterior", "--linear-floor"]
    )
    assert result.exit_code == 0, result.output
...
def test_legacy_fitter_has_explicit_separate_command(monkeypatch):
    ...
    monkeypatch.setattr(
        research_pipeline,
        "fit_dataset",
        lambda *args, **kwargs: calls.append((args, kwargs)) or {"legacy": True},
    )
    result = CliRunner().invoke(
        app,
        [
    ...
    assert calls == [
        ((Path("source"), Path("old-model")), {"holdout_fraction": 0.0, "ridge": 2.0})
    ]
...
def analysis_route(monkeypatch):
    ...
    workspace = SimpleNamespace(
        detail=lambda key: (
            calls.append(("detail", key)) or {"audit_id": key, "draws": 16000}
        ),
        counterfactual=lambda key, changes: (
            calls.append(("counterfactual", key, changes))
            or {"status": "reporting_change"}
        ),
        close=lambda: calls.append(("close",)),
    )

    def selected(path):
        calls.append(("selection", path))
        return selection, Path("accepted-fit"), Path("exact-source")

    def load(experiment, dataset):
        calls.append(("load", experiment, dataset))
        return workspace
...
def test_analysis_uses_selected_binding_detail_and_joint_source_values(analysis_route):
    ...
    result = CliRunner().invoke(
        ...
        [
            ...
            "chosen.json",
            "--changes",
            '{"bedrooms":2,"full_bathrooms":2,"window_exposures.north":true}',
        ],
    )
    assert result.exit_code == 0, result.output
    value = json.loads(result.output)
    assert (
        value["detail"]["draws"] == 16000
        and value["counterfactual"]["status"] == "reporting_change"
    )
    assert calls == [
        ("selection", Path("chosen.json")),
        ("load", Path("accepted-fit"), Path("exact-source")),
        ("detail", "capture:123"),
        (
            "counterfactual",
            "capture:123",
...
def test_selected_binding_failure_has_no_fallback_or_posterior_load(
    analysis_route, monkeypatch
):
...
def test_unsupported_change_closes_workspace_and_returns_no_partial_result(
    analysis_route,
):
    calls, workspace = analysis_route

    def fail(key, changes):
        raise ValueError("Unsupported field")

    workspace.counterfactual = fail
    result = CliRunner().invoke(
        app, ["analyze-apartment", "a1", "--changes", '{"foo":1}']
    )
...
def test_runner_scope_restores_caller_blas_thread_count(fit_route):
    ...
    with threadpool_limits(limits=2, user_api="blas"):
        ...
        assert result.exit_code == 0, result.output
        assert all(
            p["num_threads"] == 2 for p in threadpool_info() if p["user_api"] == "blas"
        )
    assert len(fit_route) == 1


def test_analysis_exposes_merged_review_and_issue_notes_without_losing_detail(
    analysis_route, monkeypatch
):
    ...
    issue = {
        ...
        "message": "Headline price is net rather than gross.",
        "interpretation_limited": True,
        "issues": [{"id": "price-case"}],
    }
    review = {
        "kind": "bedroom_count_conflict",
        "message": "Room count remains disputed.",
        "interpretation_limited": True,
    }
    monkeypatch.setattr(
        source_issues, "load_source_issues", lambda *a, **k: {"a1": issue}
    )
    monkeypatch.setattr(
        bayesian_source_review, "load_source_review", lambda *a, **k: {"a1": review}
    )
    result = CliRunner().invoke(
        app, ["analyze-apartment", "a1", "--changes", '{"bedrooms":2}']
    )
    assert result.exit_code == 0, result.output
    value = json.loads(result.output)
    ...
    assert any(
        "does not resolve the source conflict" in w
        for w in value["counterfactual"]["warnings"]
    )
    assert calls[-1] == ("close",)


def test_invalid_selected_source_issues_fail_before_posterior_load(
    analysis_route, monkeypatch
):
    ...
    def selected(path):
        value, fit, dataset = original(path)
        return (
            {**value, "source_issues": "issues", "evidence": "evidence"},
            fit,
            dataset,
        )
    ...
    def fail(*a, **k):
        raise ValueError("Issues belong to another source")
