...
from apartments.reviewed_source_lineage import (
    FLOOR,
    LAUNDRY,
    REFRESHED,
    manifest_hash,
...
def revise(parent, rows, version, field, identity, index=0):
    ...
    manifest = {
        ...
        "overlay": {
            "active_ids": [identity],
            "corrections_as_of": "2026-09-19T01:00:00Z",
        },
    }
...
def test_lineage_rejects_unintended_changes_even_with_rehashed_output(fault):
    ...
    if fault == "price":
        ...
        rows[0]["known_at"] = "2026-09-17T00:00:00Z"
    elif fault == "unit":
        rows[1]["unit_id"] = "different"
    elif fault == "order":
        rows.reverse()
    elif fault == "before":
        change["before_advertised_floor"] = 4
    elif fault == "missing_patch":
        rows[1]["attribute_review_history"] = []
    elif fault == "extra_change":
        change["changes"].append({"op": "replace", "path": "/asking_rent", "value": 1})
        ...
        manifest["overlay"]["active_ids"].append("floor")
    elif fault == "recorded_before_source":
        change["recorded_at"] = "2026-09-17T00:00:00Z"
    elif fault == "recorded_after_cutoff":
        change["recorded_at"] = "2026-09-20T00:00:00Z"
...
def test_unexpected_residual_scope_sidecar_is_rejected_for_older_sources():
    _, manifest, rows = fixture()
