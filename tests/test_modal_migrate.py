...
def module():
    spec = importlib.util.spec_from_file_location(
        "migration_test_module", Path(__file__).parents[1] / "models/modal_migrate.py"
    )
...
def test_plan_excludes_sqlite_shared_memory_but_keeps_wal(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        m, "volume", SimpleNamespace(reload=lambda: None, commit=lambda: None)
    )
    plan = m.plan.local()
...
def test_controller_retains_successful_work_after_one_pack_fails(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        m, "plan", SimpleNamespace(remote=lambda: {"groups": [{"id": 0}, {"id": 1}]})
    )
