...
from models.amenity_solver_diagnostic import (
    objective_gradient,
    refine,
    prediction_change,
)
...
def test_stationarity_matches_independent_finite_difference_and_penalty():
    x = sparse.csr_matrix([[1.0, -2.0], [1.0, 0.5], [1.0, 3.0]])
    p = sparse.diags([0.1, 2.0])
    y, beta = np.array([-0.5, 0.3, 1.2]), np.array([0.07, 0.11])
    ...
    for index in range(2):
        ...
        numerical.append(
            (
                objective_gradient(x, p, y, beta + shift)["objective"]
                - objective_gradient(x, p, y, beta - shift)["objective"]
            )
            / 2e-6
        )
...
def test_tight_irls_matches_independent_convex_optimizer_and_does_all_steps():
    ...
    p = sparse.diags([0.1, 0.3])
    y = 0.25 + 0.5 * np.linspace(-1, 1, 12)
    y[-1] += 3.0
    initial = np.zeros(2)
    beta, history = refine(x, p, y, initial, steps=30)
    reference = minimize(
        lambda b: objective_gradient(x, p, y, b)["objective"],
        initial,
        jac=lambda b: -objective_gradient(x, p, y, b)["negative_gradient"],
        method="BFGS",
        options={"gtol": 1e-11},
    )
    ...
    assert all(
        b["objective"] <= a["objective"] + 1e-10 for a, b in zip(history, history[1:])
    )
...
def test_tiny_rare_contrast_gradient_can_hide_behind_large_unchanged_loss():
    ...
    x = sparse.csr_matrix(np.array([[0.0, 0.0]] * 100 + [[1.0, 0.0], [0.0, 1.0]]))
    p = sparse.diags([0.01, 0.01])
    y = np.array([100.0] * 100 + [0.05, -0.05])
    ...
    assert abs(beta[1] - beta[0]) > 0.099
    assert after["gradient_inf"] < 1e-10
    assert len(history) == 3


def test_prediction_change_reports_log_percent_and_dollars_consistently():
    change = prediction_change(np.log([100.0, 200.0]), np.log([110.0, 180.0]))
    assert change["max_absolute_percent_change"] == pytest.approx(10.0)
    assert change["max_absolute_dollar_change"] == pytest.approx(20.0)
    assert change["mean_absolute_dollar_change"] == pytest.approx(15.0)
...
def _implementation_fixture(tmp_path, version):
    ...
    if version.endswith("-v2"):
        ...
        for name, content in contents.items():
            (root / "implementation" / name).write_text(content)
...
@pytest.mark.parametrize(
    "version", ["amenity-shrinkage-sensitivity-v1", "amenity-shrinkage-sensitivity-v2"]
)
def test_archived_producer_allows_replay_but_executed_core_must_match(
    tmp_path, version
):
...
def test_bad_or_missing_producer_snapshot_is_rejected(tmp_path, version, damage):
    ...
    if damage == "missing":
        ...
        path.write_text("unbound producer\n")
...
def test_v2_snapshot_bundle_must_bind_protocol(tmp_path):
    ...
    root, protocol, sha, paths = _implementation_fixture(
        tmp_path, "amenity-shrinkage-sensitivity-v2"
    )
