...
def inputs(tmp_path, monkeypatch, *, exclude_current=False):
    ...
    original = next(d for d in original_decisions if d['audit_id'] == rows[index]['audit_id'])
    decision = decision_for(rows[index], original['source_captures'])
    parent = tmp_path/'parent'
    products = {'observations.jsonl': ''.join(canonical(r)+'\n' for r in rows),
        'summary.json': '{}\n', 'quarantined.jsonl': '[]\n',
    ...
    pm = publish_bundle(parent, products,
                        {'version': contract.PARENT, 'interpreted_at': '2026-09-19T23:00:00Z'})
    decisions = tmp_path/'decisions'
    publish_bundle(decisions, {'decisions.jsonl': canonical(decision)+'\n', 'policy.json': '{}\n'},
        {'version': contract.DECISION_VERSION, 'source_manifest_sha256': digest(parent/'complete.json'),
         'source_observations_sha256': pm['files']['observations.jsonl'], 'reviewed_at': CLOCK})
    calls = []
    ...
    def lineage(manifest, kept, **sidecars):
        assert contract.parent_rows(manifest, kept, sidecars['residual_scope_changes']) == (pm, rows)
        assert all(name in sidecars for name in ('quarantined', 'elevator_changes',
                                                'floor_label_changes', 'expanded_floor_changes'))
        calls.append(deepcopy(manifest))
    monkeypatch.setattr(project, 'source_lineage', lineage)
    return parent, decisions, rows, calls


def test_publication_preserves_parent_sidecars_values_and_replays(tmp_path, monkeypatch):
    parent, decisions, rows, calls = inputs(tmp_path, monkeypatch)
    target = tmp_path/'result'
    result = project.run(parent, decisions, target)
    assert result['rows'] == 3 and result['quarantined_rows'] == 1 and result['current_rows'] == 1
    assert [json.loads(l) for l in (target/'observations.jsonl').read_text().splitlines()] == [rows[0], rows[1], rows[3]]
    for name in ('quarantined.jsonl', 'floor-label-projection.jsonl',
                 'expanded-floor-projection.jsonl', 'elevator-corrections.jsonl', 'ancestor-policy.txt'):
...
def test_experiment_rejects_current_membership_change_before_publication(tmp_path, monkeypatch):
