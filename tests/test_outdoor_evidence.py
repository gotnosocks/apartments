...
def test_structured_private_and_shared_remain_separate_and_literal():
    result = extract({'features': {'privateOutdoorSpaceTypes': ['BALCONY', 'TERRACE'],
                                   'list': ['PRIVATE_OUTDOOR_SPACE']},
                      'amenities': {'sharedOutdoorSpaceTypes': ['GARDEN'], 'list': ['SHARED_OUTDOOR_SPACE']}})
    assert not result['warnings']
    assert [x['value'] for x in result['assertions'] if x['attribute'] == 'private_type'] == ['BALCONY', 'TERRACE']
    assert [x['value'] for x in result['assertions'] if x['attribute'] == 'shared_type'] == ['GARDEN']
    assert result['assertions'][0] == {'attribute': 'private_type', 'value': 'BALCONY',
        'literal': 'BALCONY', 'source_path': '/features/privateOutdoorSpaceTypes/0'}
    assert all(x['value'] is True for x in result['assertions'] if x['attribute'].endswith('_outdoor'))


@pytest.mark.parametrize('payload', [None, {}, {'features': {}},
    {'features': {'privateOutdoorSpaceTypes': []}, 'amenities': {'sharedOutdoorSpaceTypes': []}},
...
def test_unknown_codes_and_malformed_fields_remain_reviewable():
    result = extract({'features': {'privateOutdoorSpaceTypes': ['LOGGIA', None, 4, '']},
                      'amenities': {'sharedOutdoorSpaceTypes': 'GARDEN'}})
    assert result['assertions'] == [{'attribute': 'private_type', 'value': 'LOGGIA',
                                   'source_path': '/features/privateOutdoorSpaceTypes/0', 'literal': 'LOGGIA'}]
    assert [x['code'] for x in result['warnings']] == ['unknown_type_code', 'invalid_type_code',
        'invalid_type_code', 'invalid_type_code', 'invalid_array_type']


def test_nested_payload_and_source_paths():
    result = extract({'propertyDetails': {'features': {'privateOutdoorSpaceTypes': ['PATIO']},
                                         'description': 'Private terrace.'}})
    assert result['assertions'][0]['source_path'] == '/propertyDetails/features/privateOutdoorSpaceTypes/0'
    assert result['description_candidates'][0]['source_path'] == '/propertyDetails/description'
    assert extract({'propertyDetails': {}, 'description': 'Shared garden.'})['description_candidates'][0]['source_path'] == '/description'


@pytest.mark.parametrize('text,expected', [
    ('6,000 SF of total living space and 7,000 SF of private outdoor space', [7000]),
...
def test_exact_unicode_offsets_and_independent_private_shared_flags():
    ...
    for item in (private, shared):
        assert text[item['measurement_start']:item['measurement_end']] == item['measurement_text']
...
@pytest.mark.parametrize('text,flag', [
    ...
    ('Total 800 SF of private outdoor space including two terraces.', 'area_sum_or_total_language'),
])
...
def test_shared_section_persists_and_unit_section_resets():
    items = screen('Building Amenities:\nGarden\nPatio\nUnit Features:\nPrivate terrace')
...
@pytest.mark.parametrize('text', ['200-300 SF terrace', '200 to 300 sq ft garden', '200 SF–300 SF patio'])
