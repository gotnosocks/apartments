...
def finished():
    protocol = dict(
        chains=2, draws=7, tune=9, seed=11, adaptation="diag", target_accept=0.93
    )
    identity = {
        **protocol,
    ...
    progress = {
        "chains": [
            dict(chain=i, finished_draws=16, total_draws=16, tuning=False)
            for i in range(2)
        ]
    }
            ...
            "/": xr.Dataset(
                attrs={
                    ...
                    "sampler_settings": {
                        ...
                        "seed": 11,
                        "adapt_options": {
                            "step_size_settings": {"target_accept": 0.93}
                        },
                    },
                }
            ),
...
@pytest.mark.parametrize(
    "damage", ["short", "tuning", "chain_duplicate", "seed", "raw_seed", "empty_steps"]
)
def test_no_completion_from_partial_or_mismatched_evidence(finished, damage):
    ...
    if damage == "short":
        progress["chains"][0]["finished_draws"] -= 1
    if damage == "tuning":
        progress["chains"][0]["tuning"] = True
    if damage == "chain_duplicate":
        progress["chains"][0]["chain"] = 1
    if damage == "seed":
        identity["seed"] += 1
    if damage == "raw_seed":
        trace.attrs["sampler_settings"]["seed"] += 1
    if damage == "empty_steps":
        trace["sample_stats"].n_steps.values[0, 0] = 0
