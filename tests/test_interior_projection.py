...
def capture(text, identity=1, **kwargs):
    return {'capture_id': identity, 'description': text, 'findings': screen(text),
            'description_sha256': hashlib.sha256(text.encode()).hexdigest(),
            'audit_id': 'row1', 'source_listing_id': 'ad1', **kwargs}


def test_advertised_room_height_is_not_typical_home_height():
    result = project([capture('☀ This duplex has 18’ skylit livingroom ceilings. A floor-through home.')])
    assert result['features'] == {'advertised_ceiling_feet': 18., 'advertised_levels': 2,
                                   'floor_through_mention': 1, 'skylight_mention': 1}
    height = next(e for e in result['evidence'] if e['feature'] == 'advertised_ceiling_feet')
...
def test_unknown_not_absence_and_no_latest_capture_backfill():
    ...
    assert 'conflicting_accepted_values' in result['decisions']['advertised_ceiling_feet']['reasons']


def test_agreeing_captures_and_unmentioned_capture_do_not_mean_absence():
    result = project([capture('A single-level home. 10 foot ceilings.', 1),
                      capture('10-foot ceilings.', 2), capture('A pleasant home.', 3)])
    assert result['features']['advertised_ceiling_feet'] == 10
    assert result['features']['advertised_levels'] == 1
    assert result['decisions']['advertised_ceiling_feet']['accepted_capture_ids'] == [1, 2]


@pytest.mark.parametrize('text', [
    '10" ceilings.', '10 to 12 foot ceilings.', '10′ 6″ ceilings.',
    'Around 10 foot ceilings.', '10 foot plus ceilings.', '10 foot ceilings +.',
    'Between 10 and 12 foot ceilings.', '20 x 12 foot ceilings.',
    '12 foot x 20 foot ceilings.', 'Over 12 foot ceilings.',
    'Approximately 12 foot ceilings.', '12 foot ceilings or more.',
...
@pytest.mark.parametrize('text', [
    'No skylights.', 'Could convert to a duplex.', 'Select apartments feature a duplex.',
    'A multi-level home.', 'The gallery has a skylight.',
...
def test_ambiguous_candidate_blocks_other_accepted_candidate_same_feature():
    result = project([capture('This duplex is spacious. Other duplex apartments available.')])
...
def test_flags_not_silently_ignored_and_completed_renovation_not_global_plan():
    ...
    assert project([capture('Newly renovated duplex with 12 foot ceilings.')])['features']['advertised_levels'] is None
...
def test_shared_play_recreation_and_pool_scope(text):
    ...
    assert all('local_nonunit_scope_ambiguity' in e['rejection_reasons'] for e in result['evidence'])


@pytest.mark.parametrize('text', [
    '10.5 &#39; CEILINGS FOUR CLOSETS 28 FT LONG LIVING ROOM',
    "11'?2? ceiling height and thirteen 7'?9? double-paned quiet windows",
    "11'5 ft ceilings", "11' 5 ft ceilings", "Ceiling height 11'?5?",
])
...
@pytest.mark.parametrize('text', ['Ceiling height: 11 feet.', 'Ceilings are 11 feet.', 'Ceiling heights of 11 feet.'])
...
def test_clean_one_level_layout_preserved():
    assert project([capture('The unit boasts an efficient one-level layout.')])['features']['advertised_levels'] == 1
