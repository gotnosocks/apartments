...
def trace():
    ...
    p = xr.Dataset(
        {
            "beta": (
                ("chain", "draw", "feature"),
                np.arange(2 * 17 * 3, dtype=float).reshape(2, 17, 3),
            ),
            "sigma": (("chain", "draw"), np.ones(shape)),
            "sigma_log__": (("chain", "draw"), np.zeros(shape)),
        },
    ...
    tree = xr.DataTree.from_dict(
        {
            "/": xr.Dataset(attrs={"sampler_settings": {"draws": 17}}),
            "posterior": p,
            "sample_stats": xr.Dataset(stats),
            "warmup_posterior": p.isel(draw=slice(0, 4)),
        }
    )
    expected = {
        "chains": 2,
        "draws": 17,
        "variables": {"beta": ["feature"], "sigma": []},
        "coordinates": {"feature": ["a", "b", "c"]},
    }
    return tree, expected


def test_bounded_export_preserves_all_draws_coordinates_and_boolean_stats(
    trace, tmp_path
):
    raw, expected = trace
    ...
    with xr.open_datatree(path, engine="h5netcdf") as actual:
        ...
        for name in expected["variables"]:
            np.testing.assert_array_equal(
                actual.posterior[name].values, raw.posterior[name].values
            )
        ...
        assert (
            actual.attrs["raw_trace_attrs_json"] == '{"sampler_settings":{"draws":17}}'
        )


@pytest.mark.parametrize(
    "damage", ["coordinate", "count", "missing", "dimensions", "warmup", "nonfinite"]
)
def test_corrupt_trace_cannot_publish(trace, tmp_path, damage):
    raw, expected = trace
    if damage == "coordinate":
        expected["coordinates"]["feature"] = ["b", "a", "c"]
    if damage == "count":
        expected["draws"] = 18
    if damage == "missing":
        del raw["posterior"]["sigma"]
    if damage == "dimensions":
        expected["variables"]["beta"] = []
    if damage == "warmup":
        raw.sample_stats.tuning.values[0, 0] = True
    if damage == "nonfinite":
        raw.posterior.beta.values[0, 0, 0] = np.nan
    path = tmp_path / "posterior.nc"
    with pytest.raises(ValueError):
        m.export_trace(raw, expected, path, max_bytes=48)
...
def test_trace_inventory_checks_chunks_and_rejects_symlinks(tmp_path):
    ...
    (root / "nested").mkdir()
    (root / "nested/chunk").write_bytes(b"123")
...
def test_interrupted_sampling_refuses_automatic_restart(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        nutpie, "sample", lambda *a, **k: pytest.fail("Must not resample")
    )
    with pytest.raises(ValueError, match="cannot resume"):
        m.sample_to_netcdf(
            model,
            output=tmp_path / "p.nc",
            trace_root=root,
            protocol_hash="a",
            draws=4,
            tune=4,
...
def test_storage_chunks_match_parameter_slices_without_changing_export_values(
    trace, tmp_path
):
    import h5netcdf
    ...
    raw["posterior"] = xr.DataTree(
        raw.posterior.to_dataset()
        .assign_coords(unit=units)
        .assign(unit_z=(("chain", "draw", "unit"), values))
    )
    expected["variables"]["unit_z"] = ["unit"]
    expected["coordinates"]["unit"] = units
    path = tmp_path / "posterior.nc"
...
def test_chain_coordinates_are_not_silently_relabelled(trace, tmp_path):
    raw, expected = trace
    raw["posterior"] = xr.DataTree(
        raw.posterior.to_dataset().assign_coords(chain=[1, 0])
    )
...
def test_completed_trace_recovery_rejects_identity_or_chunk_changes(tmp_path, damage):
    ...
    (raw / "zarr.json").write_text("{}")
    settings = dict(
        draws=4, tune=4, chains=2, seed=3, adaptation="diag", target_accept=0.93
    )
    identity = {
        "version": m.VERSION,
        "protocol_sha256": "original",
        "contract": m.contract(model, chains=2, draws=4),
        **settings,
    }
    m.atomic_json(
        root / "complete.json", {"identity": identity, "files": m.trace_files(raw)}
    )
    if damage == "raw_chunk":
        (raw / "zarr.json").write_text('{"changed":true}')
    with pytest.raises(ValueError, match="completion or identity differs"):
        m.sample_to_netcdf(
            model,
            output=tmp_path / "p.nc",
            trace_root=root,
            protocol_hash="changed" if damage == "protocol" else "original",
            status_path=tmp_path / "status.json",
            **settings,
        )
