...
def test_centering_cancels_masks_apply_and_support_is_counted():
    ...
    beta[start+before] = .1
    beta[start+after] = .3
    fit = {'encoder': encoder, 'beta': beta, 'center': 8.}
    result = contrasts.category_contrast(fit, data, 'laundry_type', 'in_building', 'in_unit')
    assert result['log_difference'] == pytest.approx(.2)
    assert result['percent_difference'] == pytest.approx(100*np.expm1(.2))
    assert result['centering_cancellation_error'] < 1e-12
    assert result['support']['in_unit'] == {'unit_months':12,'units':6,'buildings':6}
    assert result['buildings_with_both'] == 6
    assert result['units_with_both'] == 0
    encoder.amenity_active_columns[before] = False
    result = contrasts.category_contrast(fit, data, 'laundry_type', 'in_building', 'in_unit')
    assert result['log_difference'] == pytest.approx(.3)


def test_unsupported_category_does_not_report_a_premium():
    data = sample()
    encoder = ablation.encoder_class('full')(data)
    fitted = {'encoder': encoder, 'beta': np.zeros(encoder.n_parameters), 'center': 8.}
    result = contrasts.category_contrast(fitted, data, 'pet_rules', 'not_allowed', 'approval_required')
    ...
    with pytest.raises(ValueError, match='known amenity values'):
        contrasts.category_contrast(fitted, data, 'laundry_type', '__unknown__', 'in_unit')


def test_verified_replay_and_source_protocol_rejection(tmp_path, monkeypatch):
    source = {'fixture': True}
    monkeypatch.setattr(model, 'load_analytical', lambda _: (sample(), source.copy(), {}))
    experiment = tmp_path/'fit'
    ablation.run(tmp_path/'input', experiment, years=(2019,), folds=(), unit_effect=False, min_rows=10)
    ...
    assert contrasts.run(tmp_path/'input', experiment, output, split='year-2019') == manifest
    monkeypatch.setattr(model, 'load_analytical', lambda _: (sample(), {'wrong_source': True}, {}))
    with pytest.raises(ValueError, match='source manifest'):
        contrasts.run(tmp_path/'input', experiment, tmp_path/'wrong', split='year-2019')
    (experiment/'protocol.json').write_text((experiment/'protocol.json').read_text()+'\n')
    with pytest.raises(ValueError, match='protocol hash'):
        contrasts.run(tmp_path/'input', experiment, tmp_path/'wrong-protocol', split='year-2019')
