...
def sample(batch, scratch=None):
    ...
    with separate_phases(
        timings,
        lambda name, **kw: events.append((name, kw)),
        batch_draws=batch,
        scratch=scratch,
    ):
        result = sample_numpyro_nuts(
            model=model,
            draws=21,
            tune=30,
            chains=2,
    ...
    for group in ("posterior", "sample_stats"):
        for name in whole[group].data_vars:
            np.testing.assert_allclose(
                whole[group][name].values,
                batched[group][name].values,
                rtol=1e-10,
                atol=1e-10,
            )
    counts = [
        v["completed_draws_per_chain"]
        for _, v in events
    ...
    with pytest.raises(RuntimeError, match="test interruption"):
        with separate_phases({}, fail):
            sampler = numpyro.infer.MCMC(
                numpyro.infer.NUTS(potential_fn=lambda x: (x * x).sum() / 2),
                num_warmup=3,
                num_samples=3,
            )
