...
def test_progress_atomically_records_chain_state_and_nonfinite_initial_step(
    tmp_path, capsys
):
...
def test_sampling_forwards_settings_and_records_terminal_failure(tmp_path, monkeypatch):
    ...
    def compile_model(actual, **kw):
        ...
        assert (
            json.loads((tmp_path / "progress.json").read_text())["phase"] == "compiling"
        )
        return compiled
    ...
    assert {
        k: seen[k]
        for k in ("draws", "tune", "chains", "seed", "adaptation", "target_accept")
    } == {
        k: args[k]
        for k in ("draws", "tune", "chains", "seed", "adaptation", "target_accept")
    }
    ...
    monkeypatch.setattr(
        nutpie,
        "sample",
        lambda *a, **k: {"posterior": SimpleNamespace(sizes={"chain": 4, "draw": 99})},
    )
