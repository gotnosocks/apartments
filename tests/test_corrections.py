...
RAW = {'source': 'streeteasy', 'source_listing_id': 'b/4c', 'building_slug': 'b',
       'unit': '4C', 'street_easy_rental_id': '123', 'captured_at': '2026-09-07T12:00:00Z',
       'attributes': {'square_feet': 700, 'bedrooms': 1, 'bathrooms': 1},
       'home_features': ['DISHWASHER'], 'archive_listing': {'createdAt': '2015-01-01'},
       'price_history': [{'date': '2015-01-01', 'base_rent': 2000, 'event': 'Listed'}]}
CTX = {'source': 'streeteasy', 'source_listing_id': 'b/4c', 'building_slug': 'b',
       'unit': '4C', 'episode_id': '123', 'version_id': 'v', 'capture_id': 'cap'}


def edit(value=750, *, target=None, validity=None, patch=None, **extra):
    return {'target': target or {'source': 'streeteasy', 'episode_id': '123'},
            'validity': validity or {'all_time': True},
            'patch': patch or [{'op': 'replace', 'path': '/attributes/square_feet', 'value': value}], **extra}
...
def add(ledger, spec=None):
    return c.append(ledger, author='Ben', reason='Measured floor plan',
                    evidence=['floorplan.pdf'], edit=spec or edit())


def test_independent_recorded_and_effective_time_with_revision_and_withdrawal(ledger, monkeypatch):
    ...
    assert c.Overlay(ledger, as_of='2026-09-15').apply(RAW, CTX, effective_at='2016-01-01')[0] == RAW
    overlay = c.Overlay(ledger)
    assert overlay.apply(RAW, CTX, effective_at='2015-01-01')[0]['attributes']['square_feet'] == 750
    ...
    second = add(ledger, {**spec, 'supersedes': first['id'], 'patch': edit(760)['patch']})
    assert c.Overlay(ledger, as_of='2026-09-16').apply(RAW, CTX, effective_at='2016-01-01')[0]['attributes']['square_feet'] == 750
    assert c.Overlay(ledger).apply(RAW, CTX, effective_at='2016-01-01')[0]['attributes']['square_feet'] == 760
    ...
    assert len(c.Overlay(ledger).records) == 3  # Original edit never resurrected/deleted.
...
def test_arbitrary_nested_attributes_null_arrays_and_provenance(ledger):
    raw = deepcopy(RAW)
    patch = [{'op': 'test', 'path': '/attributes/square_feet', 'value': 700},
...
def test_array_index_shifts_conflict(ledger):
    ...
    add(ledger, edit(patch=[{'op': 'add', 'path': '/home_features/0', 'value': 'OTHER'}]))
    with pytest.raises(c.CorrectionError, match='Conflicting'):
        c.Overlay(ledger).apply(RAW, CTX)
...
def test_disjoint_edits_and_failed_test_are_atomic(ledger):
    add(ledger)
    add(ledger, edit(patch=[{'op': 'replace', 'path': '/attributes/bedrooms', 'value': 2}]))
    assert c.Overlay(ledger).apply(RAW, CTX)[0]['attributes'] == {'square_feet': 750, 'bedrooms': 2, 'bathrooms': 1}
    add(ledger, edit(patch=[{'op': 'test', 'path': '/attributes/bathrooms', 'value': 2},
...
def test_changed_scope_requires_explicit_retraction(ledger):
    ...
    with pytest.raises(c.CorrectionError, match='preserve target'):
        add(ledger, edit(supersedes=first['id'], target={'source': 'streeteasy', 'building_slug': 'b'}))
    assert len(c.Overlay(ledger).records) == 1


@pytest.mark.parametrize('patch', [[{'op': 'replace', 'path': '/captured_at', 'value': '1900-01-01'}],
...
def test_export_raw_preserved_selectors_and_clocks_independent(tmp_path, ledger):
    ...
    before = db.execute("SELECT version_id,structured_json,attributes_json,CAST(collected_at AS VARCHAR),CAST(recorded_at AS VARCHAR) FROM attribute_versions").fetchall()
    ...
    assert db.execute("SELECT version_id,structured_json,attributes_json,CAST(collected_at AS VARCHAR),CAST(recorded_at AS VARCHAR) FROM attribute_versions").fetchall() == before
    db.close()
    ...
    assert frozen.apply(RAW, {**CTX, 'version_id': version})[0]['attributes']['square_feet'] == 750
    raw_manifest = export_observations(path, tmp_path/'raw', c.Overlay(ledger, enabled=False))
    assert raw_manifest['source_observations_sha256'] == manifest['source_observations_sha256']
...
def test_cli_add_list_retract(tmp_path, ledger):
    ...
    result = runner.invoke(app, ['corrections', 'add', str(spec), '--ledger', str(ledger), '--author', 'Ben', '--reason', 'Test'])
    ...
    result = runner.invoke(app, ['corrections', 'retract', record['id'], '--ledger', str(ledger), '--author', 'Ben', '--reason', 'Test withdrawal'])
    assert result.exit_code == 0, result.output
...
def test_known_as_of_rejects_future_corrections(tmp_path, ledger):
    ...
    assert list(iter_observations(db, c.Overlay(ledger, as_of='2026-01-01'), known_as_of='2026-01-01')) == []
    db.close()


def test_raw_nested_source_attributes_editable_but_episode_id_protected(ledger):
    add(ledger, edit(patch=[{'op':'add','path':'/archive_listing/custom_attribute','value':{'courtyard':True}}]))
    assert c.Overlay(ledger).apply(RAW, CTX)[0]['archive_listing']['custom_attribute']['courtyard']
    with pytest.raises(c.CorrectionError):
        add(ledger, edit(patch=[{'op':'replace','path':'/archive_listing/id','value':'other'}]))
