...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Enjoy a private south-facing terrace.",
            "TERRACE",
        ...
        (
            "A private landscaped 250 sq ft terrace.",
            "TERRACE",
            "private landscaped 250 sq ft terrace",
        ),
        (
            "Features a private rooftop deck.",
            "PRIVATE_ROOF_DECK",
            "private rooftop deck",
        ),
        ("No fee! Your own sunny balcony.", "BALCONY", "Your own sunny balcony"),
        (
            "Building amenities: roof deck. This apartment has a private balcony.",
            "BALCONY",
            "private balcony",
        ),
        (
            "Some units have patios. This unit has a private balcony.",
            "BALCONY",
            "private balcony",
        ),
        (
            "Some units have balconies, but this apartment has a private terrace.",
            "TERRACE",
            "private terrace",
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "Two roof decks and 12,000 square feet of private outdoor space.",
            "PRIVATE_ROOF_DECK",
        ),
    ],
)
...
def test_no_cross_capture_join_and_no_new_type_from_text():
    result = corroborate(
        [
            capture("Private garden.", ["BALCONY"]),
            capture("Outdoor access.", ["GARDEN"], 2),
        ]
    )
    assert result["private_types"] == []
    assert any(
        "no_same_capture_structured_type" in e["reasons"]
        for e in result["ambiguous_evidence"]
    )
    assert corroborate([capture("Private garden and private balcony.", ["BALCONY"])])[
        "private_types"
    ] == ["BALCONY"]
...
@pytest.mark.parametrize(
    ...
    [
        "No balcony.",
        "Shared balcony.",
        "This unit has a shared balcony.",
        "The balcony is not private.",
...
def test_contradictory_same_type_blocks_across_captures(contrary):
    result = corroborate(
        [
            capture("Private balcony and private patio.", ["BALCONY", "PATIO"]),
            capture(contrary, ["BALCONY"], 2),
        ]
    )
    assert result["private_types"] == ["PATIO"]
    assert any(
        e["reasons"] == ["contradictory_type_evidence"]
...
def test_source_assertions_candidates_and_version_must_reproduce():
    ...
    for mutate in [
        lambda s: s["extraction"]["assertions"].clear(),
