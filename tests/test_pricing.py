...
def sample():
    ...
    for month in range(1, 11):
        for unit in range(12):
            ...
            physical = unit % 6 + 1
            elevator = unit % 2 == 0
            rows.append({"unit_id": str(unit), "building_id": str(unit % 3),
                         "observed_at": f"2025-{month:02d}-10T10:00:00Z",
                         "bedrooms": beds, "bathrooms": 1 + unit % 2,
                         "physical_floor": physical, "advertised_floor": physical + (physical > 4),
                         "elevator": elevator, "laundry_type": "in_unit" if unit % 3 else "none",
                         "window_exposures": ["south"] if unit % 2 else None,
                         "rent": math.exp(7.5 + .2*beds + .04*physical*elevator + .005*month)})
    return rows
...
def test_chronological_encoder_and_fit_ignore_future_targets_and_categories(tmp_path):
    ...
    for row in changed:
        if row["observed_at"] >= "2025-09":
            row.update(rent=1000000, square_feet=10000000, building_id="future-building")
    ...
    assert model.report["holdout"]["rmse_log"] < .03
    ...
    assert sum(model.decomposition(rows[0]).values()) == pytest.approx(model.predict(rows[0])["log_rent"])
...
def test_unit_month_selection_resists_capture_frequency_and_orders():
    ...
    assert duplicate_model.report["selection"]["exclusions"]["superseded_unit_month_capture"] == 200
...
def test_empty_invalid_and_short_windows():
    ...
    for options in ({"ridge": 0}, {"holdout_fraction": 1}, {"holdout_fraction": float("nan")}):
        with pytest.raises(ValueError):
            fit_pricing_model(sample(), **options)
    rows = sample() + [{"rent": -1}, {"rent": 10}, {"rent": 10, "unit_id": "x", "observed_at": "bad"}]
    exclusions = fit_pricing_model(rows).report["selection"]["exclusions"]
    assert exclusions["invalid_rent"] == exclusions["missing_unit_id"] == exclusions["invalid_observation_time"] == 1


def test_preference_frontier_negative_preferences_unknowns_and_duplicate_ties():
    rows = [{"unit_id": "a", "rent": 3000, "bedrooms": 1, "physical_floor": 2},
...
def test_indicator_preferences_and_boolean_unknown():
    rows = [{"rent": 3000, "laundry_type": "in_unit", "elevator": True, "window_exposures": ["south"]},
            {"rent": 3000, "laundry_type": "none", "elevator": False, "window_exposures": []}]
    ranking = rank_apartments(rows, {"laundry_type=in_unit": 150, "elevator": 200, "window_exposures.south": 100})
    assert ranking[0]["monthly_surplus"] == -2550
    assert ranking[0]["pareto_efficient"] and not ranking[1]["pareto_efficient"]
    assert not rank_apartments([dict(rows[0], elevator="unspecified")], {"elevator": 200})[0]["eligible"]


def test_support_warnings_audited_exclusions_and_preference_typos():
    rows = sample()
    extras = [dict(rows[0], unit_id="f", furnished=True),
              dict(rows[0], unit_id="s", short_term=True),
              dict(rows[0], unit_id="c", concession=True)]
    model = fit_pricing_model(rows + extras)
    assert model.report["selection"]["exclusions"] == {"excluded_furnished": 1, "excluded_short_term": 1, "excluded_concession": 1}
    support = model.report["feature_support"]["numeric"]["square_feet"]
    assert support["known_rows"] == 0 and support["missing_rows"] == 96
    effect = model.marginal_contributions(rows[0], {"square_feet": 900, "laundry_type": "future"})
    ...
    alias = rank_apartments([{"rent": 2000, "pet_policy": "allowed"}], {"pet_policy=allowed": 100})
    assert alias[0]["monthly_surplus"] == -1900


@pytest.mark.parametrize("field,value,preference", [
...
def test_partial_directional_evidence_preserves_unknowns_in_encoding_and_search():
    ...
    row = {"rent": 3000, "observed_at": "2025-01-01", "window_exposures": {"south": True, "west": False}}
...
def test_short_temporal_coverage_freezes_time_effects_and_future_extrapolation():
    ...
    assert model.predict(rows[0])["predicted_rent"] == model.predict(dict(rows[0], observed_at="2028-09-01"))["predicted_rent"]
    assert len(model.marginal_contributions(rows[0], {"observed_at": "2028-09-01"})["warnings"]) == 3


def test_temporal_support_enables_trend_and_full_year_seasonality():
    rows = sample()
    for month in (11, 12):
        rows.extend(dict(r, observed_at=f"2025-{month:02d}-10T10:00:00Z") for r in sample()[:12])
    model = fit_pricing_model(rows, holdout_fraction=0)
    assert all(model.encoder["numeric"][f]["enabled"] for f in ("trend_years", "season_sin", "season_cos"))
