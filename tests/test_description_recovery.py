...
def sample(tmp_path, snapshot=1, description='North-facing café windows.', **extra):
    listing = {'id': snapshot, 'description': '$b', 'propertyDetails': {'address': {'displayUnit': '#4A'}, 'bedroomCount': 1},
               'propertyHistory': [{'listingId': snapshot, 'rentalEventsOfInterest':
                                    [{'date': '2025-01-01', 'price': 3000, 'status': 'ACTIVE'}]}], **extra}
    stream = 'a:' + json.dumps(listing) + '\nb:T' + format(len(description.encode()), 'x') + ',' + description
    body = ('<link rel="canonical" href="/building/demo/4a"><script>self.__next_f.push(' + json.dumps([1, stream]) + ')</script>').encode()
    ...
    old['description'] = '$b'
    row.update(raw_listing_json=json.dumps(old, sort_keys=True, separators=(',', ':')), snapshot_id=snapshot,
               body_hash=body_hash, snapshot_url=url, snapshot_observed_at=100., collected_at=100., parsed_at=200.)
    return row, body_root


def source_dataset(tmp_path, count=2, description='North-facing café windows.'):
    dataset = tmp_path / 'source'
    captures = [sample(tmp_path, i, description=description)[0] for i in range(1, count + 1)]
    ...
    listings = [{k: v for k, v in c.items() if k not in ('body_hash', 'snapshot_url', 'snapshot_observed_at')} for c in captures]
    snapshots = [{'snapshot_id': c['snapshot_id'], 'body_hash': c['body_hash'], 'url': c['url'], 'observed_at': c['collected_at']} for c in captures]
    pq.write_table(pa.Table.from_pylist(listings), dataset / 'listing_observations' / 'part.parquet')
    pq.write_table(pa.Table.from_pylist(snapshots), dataset / 'snapshots' / 'part.parquet')
...
def test_recovery_binds_exact_capture_and_raw_payload(tmp_path):
    ...
    assert result['original_raw_listing_sha256'] == hashlib.sha256(row['raw_listing_json'].encode()).hexdigest()
    assert result['source_body_sha256'] == row['body_hash']
    assert result['resolved_description'] == 'North-facing café windows.'
    assert result['description_sha256'] == hashlib.sha256(result['resolved_description'].encode()).hexdigest()
...
def test_capture_hash_identity_and_other_payload_changes_quarantined(tmp_path):
    ...
    old = json.loads(row['raw_listing_json']);old['propertyDetails']['bedroomCount'] = 2
    changed = dict(row, raw_listing_json=json.dumps(old))
    assert recover_capture(changed, bodies)['reason'] == 'non_description_payload_changed'
...
def test_unresolved_invalid_or_oversize_body_quarantined(tmp_path):
    ...
    assert result['reason'] == 'body_read_failure' and result['detail'] == 'body_decompression_limit'
    assert recover_capture(dict(row, snapshot_url='https://example.com'), bodies)['reason'] == 'snapshot_url_mismatch'
...
def test_completed_bundle_reuses_stable_hashes_and_catches_source_change(tmp_path):
    ...
    assert manifest['coverage'] == {'candidates': 2, 'accepted': 2, 'quarantined': 0, 'quarantine_reasons': {}, 'batches': 2}
    assert build_description_recovery(dataset, bodies, out, workers=1, batch_size=1) == manifest
    (dataset / 'complete.json').write_text(json.dumps({'unit_association_rule': 'canonical-url-v1', 'changed': True}))
...
def test_interruption_resume_keeps_committed_interpretation_time(tmp_path):
    ...
    with pytest.raises(RuntimeError, match='interrupted'):
        build_description_recovery(dataset, bodies, out, workers=1, batch_size=1, progress=stop)
...
def test_corrupt_checkpoint_fails_instead_of_silent_recompute(tmp_path):
    dataset, bodies = source_dataset(tmp_path)
    out = tmp_path / 'recovered'
    def stop(progress): raise RuntimeError('stop')
...
def test_interrupted_resume_preserves_literal_unicode_line_separators(tmp_path):
    ...
    assert b'\xe2\x80\xa8' in committed and b'\xe2\x80\xa9' in committed and b'\xc2\x85' in committed
