...
def test_recovery_preserves_unicode_and_rejects_identity_source_and_future_knowledge():
    ...
    row = {'source_listing_id': '123', 'canonical_unit_url': 'unit:1', 'known_at': '2026-09-18T16:00:00Z'}
    recovery = {'original_raw_listing_sha256': hashlib.sha256(raw.encode()).hexdigest(),
        'source_body_sha256': 'body', 'listing_id': '123', 'canonical_unit_url': 'unit:1',
        'interpreted_at': '2026-09-18T15:00:00Z', 'resolved_description': 'Duplex\u2028with skylight.'}
    description, _ = checked_description(raw, 'body', recovery, row)
    assert description == recovery['resolved_description']
    assert len(json_rows((json.dumps({'description': description}, ensure_ascii=False) + '\n').encode())) == 1
    for field, wrong in [('listing_id', '456'), ('canonical_unit_url', 'unit:2'),
                         ('source_body_sha256', 'wrong'), ('original_raw_listing_sha256', 'wrong'),
...
def test_cohort_audit_binds_sources_counts_rows_and_replays(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr('models.interior_feature_audit.granular_parse.parse_listing',
        lambda body, url: ({'listing_id': '456', 'canonical_unit_url': 'unit:2',
                           'raw_listing_json': canonical({'description': 'Floor-thru apartment.'})}, None))
