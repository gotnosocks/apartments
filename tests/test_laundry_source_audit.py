...
def case(*, structured=True):
    ...
    payload = {
        "id": "123",
        "description": "$resolvedLater",
        "propertyDetails": {
            "features": {"list": ["WASHER_DRYER"] if structured else []}
        },
        "latestListing": {
            "propertyDetails": {"features": {"list": ["WASHER_DRYER", "LAUNDRY"]}}
        },
    }
    ...
    evidence = {
        ...
        "known_at": "2026-09-18T11:00:00+00:00",
        "source_path": "/description",
    }
    candidate = {
        **row,
        **evidence,
        "findings": [{"kind": "hookups", "start": 0, "end": 20, "literal": text[:20]}],
    }
...
def test_recovered_description_and_structured_claim_are_kept_separate():
    ...
    assert (
        result["laundry_assertions"][0]["source_path"]
        == "/propertyDetails/features/list/0"
    )
...
def test_raw_advertisement_identity_is_independently_checked():
    ...
    payload["id"] = "456"
    raw = canonical(payload)
    e["raw_listing_sha256"] = hashlib.sha256(raw.encode()).hexdigest()
...
def source_fixture(tmp_path):
            ...
            "summary.json": canonical(
                {
                    ...
                    "description_manifest_sha256": digest(
    ...
    return dict(
        ...
        refresh=tmp_path / "unused-refresh",
