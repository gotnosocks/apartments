    ...
    for building in range(5):
        for unit in range(3):
            for year in (2018,2019):
                rows.append({'building':f'b{building}','building_id':f'b{building}',
                    'unit_id':f'b{building}:u{unit}','audit_id':f'{building}-{unit}-{year}',
                    'period':pd.Timestamp(year,1,1),'bedrooms':1.,'bathrooms':1.,'square_feet':600.,
                    'laundry_type':('in_building','in_unit',None)[unit],
                    'asking_rent':3000.+100*unit,'log_rent':np.log(3000.+100*unit)})
    return pd.DataFrame(rows)


def protocol(frame):
    return boot.prepare_protocol(frame, {'settings':boot.model.SETTINGS,'unit_effect':True},draws=200,seed=123)


def test_duplicate_draw_copies_keep_all_rows_independent_ids_and_source_provenance():
    source=sample();before=source.copy(deep=True)
    ...
    assert drawn[boot.SOURCE_BUILDING].nunique()==2 and drawn[boot.SOURCE_UNIT].nunique()==6
    assert drawn.audit_id.value_counts().max()==2
    assert not set(drawn[drawn[boot.COPY_POSITION]==0].unit_id)&set(drawn[drawn[boot.COPY_POSITION]==1].unit_id)
...
def test_fit_records_absent_categories_and_centered_contrasts():
    ...
    assert all(c['status']=='absent_category' and c['log_difference'] is None for c in result['contrasts'][1:])
...
def test_failed_replicate_is_explicit_not_zero_or_replaced(monkeypatch):
    frame=boot.prepare_frame(sample())
    def fail(*args,**kwargs):raise RuntimeError('simulated numerical failure')
...
def test_summary_keeps_failed_and_absent_denominators():
    frame=boot.prepare_frame(sample());p=protocol(frame)
    record,_=boot.fit_replicate(frame,p,2)
    results=[deepcopy(record) for _ in range(20)]
    for i,r in enumerate(results):
        r['replicate']=i;r['contrasts'][0]['log_difference']=.01+i*.001
    failed=deepcopy(record);failed['status']='fit_failed'
    failed['contrasts'][0].update(status='fit_failed',log_difference=None,percent_difference=None)
    results.append(failed)
    summary=boot.summarize(results,p)
    assert summary['completed_replicates']==21 and summary['planned_replicates']==200
    ...
    assert contrast['approximate_replicates_in_each_2_5_percent_tail']==.5
...
def test_worker_checkpoints_match_protocol_and_deterministic_fit(tmp_path):
    ...
    manifest,files=boot._verified_bundle(tmp_path/'replicates'/'00002',retain={'result.json'})
    assert manifest['protocol_sha256']==boot._hash(p)
    assert boot._worker(2)==(2,'converged')
    assert boot._verified_bundle(tmp_path/'replicates'/'00002',retain={'result.json'})==(manifest,files)
