...
def listing(history, label="#3D"):
    return {
        "id": "100",
        "propertyDetails": {"address": {"displayUnit": label}, "bedroomCount": 4},
...
def test_rental_history_and_label_are_separate_corroboration():
    result = identity_evidence(
        TARGET, listing([history("10"), history("11")]), [member("10"), member("11")]
    )
...
def test_sale_history_and_label_only_cannot_become_rental_history_match():
    result = identity_evidence(
        TARGET, listing([history("10", "saleEventsOfInterest")]), [member("10")]
    )
...
def test_missing_and_unresolved_history_remain_explicit():
    ...
    result = identity_evidence(
        TARGET, listing([history("10"), history("11")]), [unresolved]
    )
