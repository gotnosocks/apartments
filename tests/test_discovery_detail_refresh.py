...
def fixture(tmp_path):
    ...
    for identifier, url in [
        ("100", "https://streeteasy.com/building/demo/1d"),
        ...
        cards.append(
            {
                ...
                "canonical_url": url,
                "href": url,
                "in_chelsea_scope": True,
                "source_fields": {"furnished": True, "monthsFree": 1},
            }
        )
    ...
    cards.append(
        {
            ...
            "canonical_url": "https://streeteasy.com/building/elsewhere/3d",
            "href": "https://streeteasy.com/building/elsewhere/3d",
            "in_chelsea_scope": False,
        }
    )
    page = {
        "seed_path": "/for-rent/chelsea",
        "page": 1,
        "source_url": "https://streeteasy.com/for-rent/chelsea",
        "source_clock": {"capture_started_at": "2026-09-18T20:00:00Z"},
        "body_sha256": "a" * 64,
        "capture_reference": {"example": True},
        "cards": cards,
    }
    report = tmp_path / "discovery"
    publish_bundle(
        report, {"pages.jsonl": canonical(page) + "\n"}, {"version": "test-discovery"}
    )
    rows = []
    for c in cards[:2]:
        observation = {
        ...
        rows.append(
            {
                ...
                "canonical_unit_url": c["canonical_url"],
                "observed_detail_urls": [c["href"]],
    ...
    publish_bundle(
        review,
        {"detail-review-queue.jsonl": "".join(canonical(r) + "\n" for r in rows)},
        manifest,
    )
...
def test_preflight_resume_and_no_refetch_establish_detail_identity(tmp_path):
    ...
    async def fetch(request):
        ...
        return HtmlResponse(
            request.url,
            request=request,
            body=html(row, status="RENTED" if identifier == "101" else "ACTIVE"),
        )

    paused = asyncio.run(
        m.collect(review, report, output, fetch=fetch, max_new_requests=1)
    )
    ...
    rows = [
        json.loads(l)
        for l in (output / "snapshot/candidates.jsonl").read_text().splitlines()
    ]
    assert rows[1]["canonical_unit_url"].endswith("/2d")
    assert (
        rows[1]["discovery_provenance"]["unit_identity_established_from_detail"] is True
    )
    assert (
        rows[0]["discovery_provenance"]["unit_identity_established_from_detail"]
        is False
    )
    changes = [
        json.loads(l)
        for l in (output / "snapshot/changes.jsonl").read_text().splitlines()
    ]
...
def test_mismatched_detail_never_inherits_search_identity(tmp_path, damage):
    ...
    async def fetch(request):
        identifier = request.url.rsplit("/", 1)[1]
        url = "https://streeteasy.com/building/demo/" + (
            "1d" if identifier == "100" else "2d"
        )
        if damage == "known_unit" and identifier == "100":
            url = "https://streeteasy.com/building/demo/wrong"
        if damage == "unknown_building" and identifier == "101":
            url = "https://streeteasy.com/building/other/2d"
        row = {
            "source_listing_id": "999" if damage == "advertisement" else identifier,
...
def test_interrupted_request_is_not_repeated(tmp_path):
    ...
    publish_bundle(
        ...
        {"intent.json": "{}\n"},
        {
            "version": m.VERSION,
            "plan_sha256": ph,
            "target_sha256": m.refresh._hash(plan["targets"][0]),
            "index": 0,
        },
    )
    calls = []
    ...
    assert (
        result["report"]["failure_reasons"]["interrupted_request_outcome_unknown"] == 1
    )
