...
from models.cohort_spatial_features import (
    check_listing,
    resolve_building,
    feature_summary,
    verified_manifest,
    bound_archived_building,
)
...
def point():
    return {
        "status": "source_bound",
        "source_building_id": "12",
        "latitude": 40.7,
        "longitude": -74.0,
        "street_candidate": "seventh avenue",
    }


def test_coordinate_and_street_conflicts_are_distinguished():
    a = point()
    b = {**a, "street_candidate": "7th avenue"}
    ...
    b["latitude"] += 0.0001
    assert (
        resolve_building("demo", [a, b], {"12"})["status"]
        == "missing_or_conflicting_location"
    )


@pytest.mark.parametrize(
    "fault", ["missing", "building_id", "invalid_capture", "multiple_ids"]
)
def test_all_capture_and_listing_id_evidence_must_agree(fault):
    captures, ids = [point()], {"12"}
    if fault == "missing":
        captures = []
    elif fault == "building_id":
        ids = {"13"}
    elif fault == "invalid_capture":
        captures.append({"status": "invalid_or_conflicting_coordinates"})
    elif fault == "multiple_ids":
        ids.add("13")
    assert (
        resolve_building("demo", captures, ids)["status"]
        == "missing_or_conflicting_location"
    )
...
def test_historical_coordinate_join_requires_own_raw_building_identity():
    ...
    for position, wrong in [
        ...
        (0, "4"),
        (1, "11"),
        (2, "https://streeteasy.com/building/other/3d"),
        (3, "other"),
        (4, None),
        (5, "99"),
    ]:
...
def test_building_center_and_street_support_do_not_count_ads_as_buildings():
    ...
    b = {
        "building": "b",
        **point(),
        "source_building_id": "13",
        "latitude": 40.8,
        "street_candidate": "west 20th street",
    }
    c = {"building": "c", "status": "missing_or_conflicting_location"}
    rows = [
        {"building": building, "unit_id": "u" + str(i), "analysis_price_basis": basis}
        for i, (building, basis) in enumerate(
            [("a", "historical_initial_own_advertisement_ask")] * 5
...
def flight_records():
    return {
        ...
        "2": {"latitude": 40.7, "longitude": -74},
        "3": {"street": "140 Seventh Avenue"},
    }


def test_building_page_spatial_references_are_resolved_with_source_paths():
    result = bound_archived_building(
        "12", "https://streeteasy.com/building/demo", flight_records()
    )
    ...
    assert {
        "source_path": "/1/geoCenter",
        "reference": "$2",
        "target_path": "/2",
    } in result["field_references"]


@pytest.mark.parametrize(
    "fault", ["missing", "cycle", "conflict", "slug", "literal_text"]
)
def test_reference_resolution_does_not_hide_bad_building_evidence(fault):
    ...
    if fault == "missing":
        ...
        decoded["2"]["latitude"] = 40.8
    elif fault == "slug":
        decoded["1"]["slug"] = "other"
    elif fault == "literal_text":
        decoded["1"]["geoCenter"] = FlightText("$2")
    assert (
        bound_archived_building("12", "https://streeteasy.com/building/demo", decoded)[
            "status"
        ]
        != "source_bound"
    )
...
@pytest.mark.parametrize(
    "fault", [None, "body_hash", "snapshot_url", "exported_coordinate"]
)
def test_complete_local_source_join_decodes_real_flight_and_is_replayable(
    tmp_path, fault
):
    import gzip
    import hashlib
    ...
    archive, bodies, history, dataset, current, output = [
        tmp_path / n
        for n in ("archive", "bodies", "history", "dataset", "current", "output")
    ]
    archive.mkdir()
    ...
    body = (
        "<script>self.__next_f.push(" + json.dumps([1, frames]) + ")</script>"
    ).encode()
    sha = hashlib.sha256(body).hexdigest()
    (bodies / sha[:2]).mkdir(parents=True)
    (bodies / sha[:2] / (sha + ".gz")).write_bytes(
        gzip.compress(body if fault != "body_hash" else b"changed")
    )
    ...
    parquet(
        "listing_observations/part.parquet",
        [
            {
                "snapshot_id": 1,
                "listing_id": "10",
    ...
    parquet(
        "snapshots/metadata.parquet",
        [
            {
                "snapshot_id": 2,
                "body_hash": sha,
                "kind": "building",
                "observed_at": 1672531200.0,
                "url": "https://streeteasy.com/building/"
                + ("other" if fault == "snapshot_url" else "demo"),
            }
        ],
    )
    raw = decoded["1"]
    parquet(
        "building_observations/part.parquet",
        [
            {
                "snapshot_id": 2,
                "building_slug": "demo",
                "building_id": "12",
                "latitude": 40.8 if fault == "exported_coordinate" else 40.7,
                "longitude": -74.0,
                "raw_building_json": json.dumps(raw),
            }
        ],
    )
    inventory = {
        name: digest(archive / name)
        for name in (
            "complete.json",
            "listing_observations/part.parquet",
            "snapshots/metadata.parquet",
    ...
    arguments = dict(
        ...
        interpreted_at="2026-09-19T08:30:50Z",
        output=output,
    )
    if fault:
        with pytest.raises(ValueError):
        ...
        assert (
            result["historical_listing_captures"]
            == result["building_page_captures"]
            == 1
        )
