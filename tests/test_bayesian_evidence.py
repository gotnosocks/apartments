    ...
    captures = [
        {
            ...
            "capture_id": 12,
            "description": literal,
            "description_sha256": hashlib.sha256(literal.encode()).hexdigest(),
            "source_collected_at": "2026-09-10T10:00:00+00:00",
            "known_at": "2026-09-18T10:00:00+00:00",
            "description_interpreted_at": "2026-09-17T10:00:00+00:00",
            "body_sha256": "body-hash",
            "raw_listing_sha256": "raw-hash",
            "extraction": {"inferred_ensuite_count": 9},
        },
        {
            "audit_id": "quarantined",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("capture_id", "12"),
        ("capture_id", True),
        ("source_collected_at", "2026-09-19"),
        ("known_at", "2026-09-19"),
        ("known_at", "2026-09-09"),
        ("description_interpreted_at", "2026-09-19"),
        ("description_interpreted_at", "2026-09-09"),
        ("description_sha256", "bad"),
        ("description", {"not": "literal"}),
    ],
)
def test_surviving_identity_clock_or_literal_mismatch_rejected(tmp_path, field, value):
    parts = fixtures()
    parts[3][0][field] = value
    with pytest.raises(ValueError):
        load_evidence(*publish(tmp_path, parts))


def test_recovery_cannot_postdate_capture_knowledge_even_before_row_cutoff(tmp_path):
    parts = fixtures()
    parts[3][0]["description_interpreted_at"] = "2026-09-18T11:00:00+00:00"
...
def test_duplicate_capture_and_duplicate_observation_rejected(tmp_path):
    parts = fixtures()
    parts[3].append(copy.deepcopy(parts[3][0]))
    with pytest.raises(ValueError, match="duplicated"):
        load_evidence(*publish(tmp_path, parts))
    parts = fixtures()
    parts[1].append(copy.deepcopy(parts[1][0]))
...
def test_lineage_is_explicit_not_an_arbitrary_nested_manifest_match(tmp_path, fault):
    parts = fixtures(cleaned=True)
    manifest = parts[0]
    if fault == "wrong_original":
        parts[2]["dataset_manifest"] = {
            "dataset_version": "historical-plus-current-capture-analysis-v1",
            "files": {},
        }
    if fault == "unrelated_nested_match":
        manifest["source_manifest"]["projection_manifest"]["dataset_manifest"] = {
            "dataset_version": "wrong"
        }
        manifest["unrelated_dataset_manifest"] = parts[2]["dataset_manifest"]
    if fault == "parent_hash":
        manifest["source_manifest_sha256"] = "bad"
    if fault == "wrong_parent_version":
        manifest["source_manifest"]["version"] = "unrelated-projection"
    if fault == "decision_conflict":
        manifest["source_manifest"]["decision_manifest"] = {"projection_manifest": {}}
    if fault != "parent_hash":
        manifest["source_manifest_sha256"] = hashlib.sha256(
            (canonical(manifest["source_manifest"]) + "\n").encode()
        ).hexdigest()
    with pytest.raises(ValueError):
        load_evidence(*publish(tmp_path, parts))
...
def test_property_details_literal_precedence_and_missing_text(tmp_path):
    ...
    missing.update(capture_id=13, description=None, description_sha256=None)
    del missing["property_details"]
    parts[3].append(missing)
    result = load_evidence(*publish(tmp_path, parts))["kept"]
    assert (
        result[0]["description"] == literal
        and result[0]["source_path"] == "/propertyDetails/description"
    )
...
def test_fitted_archive_requires_capture_knowledge_clock(tmp_path):
    parts = fixtures()
    del parts[3][0]["known_at"]
    with pytest.raises(ValueError, match="knowledge clock"):
        load_evidence(*publish(tmp_path, parts))
...
def test_refreshed_evidence_exact_binding_full_coverage_and_literal_text(tmp_path):
    ...
    archive = tmp_path / "archive"
    publish_bundle(
        archive, {"evidence.jsonl": "".join(canonical(c) + "\n" for c in captures)}, em
    )
...
@pytest.mark.parametrize(
    "fault", ["manifest", "observations", "missing_capture", "capture_type", "clock"]
)
def test_refreshed_evidence_rejects_wrong_binding_coverage_or_clock(tmp_path, fault):
    dataset, em, captures = refreshed_parts(tmp_path)
    if fault == "manifest":
        em["dataset_manifest_sha256"] = "bad"
    if fault == "observations":
        em["dataset_observations_sha256"] = "bad"
    if fault == "missing_capture":
        captures.pop()
    if fault == "capture_type":
        captures[0]["capture_id"] = "12"
    if fault == "clock":
        captures[0]["known_at"] = "2026-09-19"
    archive = tmp_path / "archive"
    publish_bundle(
        archive, {"evidence.jsonl": "".join(canonical(c) + "\n" for c in captures)}, em
    )
...
@pytest.mark.parametrize(
    "fault", [None, "missing_sidecar", "changed_excluded_row", "changed_retained_row"]
)
def test_quarantine_evidence_requires_inverse_lineage_and_returns_survivors_only(
    tmp_path, fault
):
    ...
    rows = [
        json.loads(s) for s in (old / "observations.jsonl").read_text().split("\n") if s
    ]
    for row in rows:
        row.update(
    ...
    em.update(
        dataset_manifest_sha256=digest(original / "complete.json"),
    ...
    if fault == "changed_excluded_row":
        sidecar[0]["observation"]["advertised_floor"] = 9
    if fault == "changed_retained_row":
        kept[0]["unit_id"] = "different"
    files = {"observations.jsonl": "".join(canonical(r) + "\n" for r in kept)}
    if fault != "missing_sidecar":
        files["quarantined.jsonl"] = "".join(canonical(r) + "\n" for r in sidecar)
...
def test_expanded_floor_evidence_verifies_each_inverse_stage(
    tmp_path, fault, outer_scope
):
    ...
    files = {
        ...
        "expanded-floor-projection.jsonl": "".join(
            canonical(r) + "\n" for r in expanded
        ),
    }
    if outer_scope:
        files["residual-scope-quarantine.jsonl"] = "".join(
