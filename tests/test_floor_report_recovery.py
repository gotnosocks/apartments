...
def fixture():
    ...
    code = {
        m.RUNNER: sha(fixed),
        "recover_floor_report.py": "b" * 64,
        "bayesian_feature_report.py": "c" * 64,
    }
    manifest = {"files": {**products, **code, "pre-floor-summary.json": "d" * 64}}
    recovery = {
        "version": m.VERSION,
        "protocol_sha256": "ph",
        "original_runner_sha256": sha(ORIGINAL),
        "implementation_sha256": code,
        "preserved_product_sha256": products,
        "original_summary_sha256": "d" * 64,
    }
...
def test_recovery_refuses_changes_outside_naming_fix(fault):
    ...
    if fault == "math":
        new += "\nchanged_formula()"
    if fault == "original":
        r["original_runner_sha256"] = "bad"
    if fault == "posterior":
        fm["files"]["posterior.nc"] = "different"
    if fault == "diagnostics":
        fm["files"]["diagnostics.json"] = "different"
    if fault == "runner":
        fm["files"][m.RUNNER] = "different"
    if fault == "summary":
        after["diagnostics"] = {"acceptable": False}
    if fault == "status":
        after["status"] = "published"
    if fault == "missing_product":
        r["preserved_product_sha256"].pop("residuals.jsonl")
    with pytest.raises(ValueError):
        m.verify_recovery(p, ph, fm, r, old, new, before, after)


def test_failed_new_floor_diagnostics_cannot_retain_accepted_status():
    parts = list(fixture())
    parts[-1]["floor_diagnostics"]["acceptable"] = False
...
def test_unexpected_or_repeated_original_code_refused():
    with pytest.raises(ValueError):
        m.coordinate_fix(ORIGINAL + ORIGINAL)
    with pytest.raises(ValueError):
        m.coordinate_fix("different implementation")
