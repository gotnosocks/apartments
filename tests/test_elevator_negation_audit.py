...
def inputs(structured=False):
    payload = {'id': 42, 'description': '$reference', 'propertyDetails': {
        'amenities': {'list': ['ELEVATOR'] if structured else []}}}
    raw = json.dumps(payload)
    row = {'source_listing_id': '42', 'building': 'b', 'analysis_price_basis': 'historical_initial_own_advertisement_ask',
           'elevator': True, 'audit_id': 'a', 'unit_id': 'u'}
    capture = {'description': 'One flight up in a non-elevator building.',
...
def test_replay_preserves_original_structured_claim_and_recovers_literal_description(structured, expected):
    ...
    assert 'non-elevator' in next(c['literal'] for c in result['after']['claims'] if c['source_path'] == '/description')
...
def test_wrong_original_payload_or_unrelated_extraction_changes_are_refused(damage):
    ...
    if damage == 'hash': raw += ' '
    if damage == 'advertisement': row['source_listing_id'] = '43'
    if damage == 'unrelated':
        def old(payload):
            result = extract_attribute_evidence(payload)
            result['attributes']['bedrooms'] = 5
            return result
    with pytest.raises(ValueError): replay(row, capture, raw, old)
