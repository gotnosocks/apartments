...
def source(tmp_path, monkeypatch):
    monkeypatch.setattr(
        corrections, "now", lambda: datetime(2026, 9, 19, 3, tzinfo=timezone.utc)
    )
    row = {
        "audit_id": "a",
        "unit_id": "u",
    ...
    captures = [
        {
            ...
            "source_listing_id": "123",
            "capture_id": cid,
            "body_sha256": str(cid) * 32,
            "raw_listing_sha256": str(cid) * 32,
            "description": text,
    ...
    audit = [
        {
            **c,
            "building": "b",
            "laundry_type": "in_building",
            "replay_version": "attribute-evidence-v4",
            "replayed_laundry_type": None,
            "structured_in_unit_claim": False,
            "structured_in_building_claim": False,
            "laundry_assertions": spans,
        }
        for c in captures
    ]
    ...
    corrections.append(
        ledger, author="test reviewer", reason="Verified scoped denial.", edit=edit
    )
...
def test_exact_patch_preserves_source_clock_prices_and_original_category(source):
    ...
    assert {
        ...
        if k not in ("laundry_type", "attribute_review_history")
...
def test_incomplete_or_conflicting_capture_evidence_is_rejected(source, fault):
    ...
    if fault == "raw_hash":
        audit[0]["raw_listing_sha256"] = "wrong"
    if fault == "positive_structured":
        audit[0]["structured_in_unit_claim"] = True
    if fault == "wrong_literal":
        audit[0]["laundry_assertions"][0]["literal"] = "wrong"
    if fault == "wrong_replay":
        audit[0]["replay_version"] = "attribute-evidence-v3"
    if fault == "missing_denial":
        audit[0]["laundry_assertions"] = []
    if fault == "duplicate_audit":
        audit.append(deepcopy(audit[0]))
...
def test_changed_row_cannot_silently_drop_or_reuse_the_correction(source):
    ...
    with pytest.raises(ValueError, match="exact source row version"):
        apply(source)
...
def test_other_patch_fields_are_not_allowed(source, tmp_path):
    ...
    corrections.append(
        ...
        edit={
            ...
            "validity": {"all_time": True},
            "patch": [{"op": "replace", "path": "/asking_rent", "value": 1}],
        },
    )
...
def test_publication_and_idempotent_replay_with_real_ledger_and_description_binding(
    source, tmp_path
):
    row, captures, audit, ledger = source
    dataset = tmp_path / "dataset"
    descriptions = tmp_path / "descriptions"
    source_audit = tmp_path / "audit"
    dm = publish_bundle(
        dataset,
