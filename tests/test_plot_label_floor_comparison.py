...
def fitted(tmp_path, *, elevated=False, unmixed=False):
    frame = data()
    frame.loc[frame.advertised_floor.isna(), "advertised_floor"] = 1.0
    d = m.interaction.FeatureDesign(frame) if elevated else m.floor.FeatureDesign(frame)
    rng = np.random.default_rng(14)
    beta = rng.normal(0, 0.05, (4, 1000, len(d.features)))
    if elevated:
        # Covariance yields a tight floor2->3 curve at known elevator=True.
        beta[:, :, d.features.index("listed_floor_gt_2")] = -0.5 * beta[
            :, :, -1
        ] / np.sqrt(3) + rng.normal(0.02, 0.001, (4, 1000))
    if unmixed:
        beta[0, :, d.features.index("listed_floor_gt_2")] += 4
    target = tmp_path / "fit"
    target.mkdir()
    xr.Dataset(
        {"beta": (("chain", "draw", "feature"), beta)}, coords={"feature": d.features}
    ).to_netcdf(target / "posterior.nc", group="posterior", engine="h5netcdf")
    return {
        "root": tmp_path,
        "data": frame,
        "design": d,
        "protocol": {"chains": 4, "draws": 1000, "prior_multiplier": 1.0},
    }
...
def test_elevator_curve_preserves_joint_covariance_and_reverse_floor(tmp_path):
    ...
    with pytest.raises(ValueError, match="explicit elevator"):
        m.vector(f["design"], f["data"], 3.0)
    curve = m.curve(f, True)
    point = next(r for r in curve["points"] if r["floor"] == 3.0)
    assert point["diagnostics"]["acceptable"]
    assert (
        point["percent_effect"]["upper_95"] - point["percent_effect"]["lower_95"] < 0.5
    )
    assert (
        m.vector(f["design"], f["data"], 1.0, False)[
            f["design"].features.index("listed_floor_gt_1")
        ]
        == -1
    )
...
def test_unmixed_cumulative_contrasts_are_withheld(tmp_path):
    f = fitted(tmp_path, unmixed=True)
    curve = m.curve(f)
    point = next(r for r in curve["points"] if r["floor"] == 3.0)
...
def test_source_counts_preserve_explicit_proxy_and_missing():
    frame = data().iloc[:3].copy()
    frame["advertised_floor"] = [2, 2, None]
    frame["floor_label_provenance"] = [{}, {"status": "label_proxy"}, {}]
    result = m.source_counts(frame)
    assert result["missing_rows"] == 1
    assert (
        result["levels"][0]["explicit_rows"] == 1
        and result["levels"][0]["label_proxy_rows"] == 1
    )
...
def test_scientific_plot_exports_deterministic_binary_artifacts(tmp_path):
    f = fitted(tmp_path)
    curve = m.curve(f)
    counts = m.source_counts(f["data"])
    result = {
        "curves": dict.fromkeys(
            (
                "explicit_source_baseline",
                "expanded_label_floor",
                "expanded_with_elevator",
    ...
    first = m.render(result)
    second = m.render(result)
    assert first == second
    assert (
        first["floor-curves.png"].startswith(b"\x89PNG")
        and b"<svg" in first["floor-curves.svg"]
    )
...
def test_empty_joint_floor_access_cell_withholds_display_but_keeps_diagnostics(
    tmp_path,
):
    f = fitted(tmp_path, elevated=True)
    f["data"].loc[f["data"].advertised_floor == 8.0, "elevator"] = True
    result = m.curve(f, False)
    unsupported = next(r for r in result["points"] if r["floor"] == 8.0)
    assert unsupported["diagnostics"]["acceptable"]
    assert unsupported["status"] == "supported_converged"
    assert (
        unsupported["source_support_status"]
        == "withheld_missing_joint_floor_access_endpoint"
    )
    ...
    supported = next(r for r in result["points"] if r["floor"] == 3.0)
