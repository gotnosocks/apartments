...
from models import bayesian_feature_checks_v2 as m


@pytest.fixture
def posterior_case(tmp_path):
    rng = np.random.default_rng(812)
    data = pd.DataFrame({'asking_rent': [3000., 4500., 8000.], 'audit_id': ['a','b','c'],
                         'source_listing_id': ['11','12','13'], 'unit_id': ['u1','u0','u1'],
                         'building': ['b1','b0','b1'], 'period': pd.to_datetime(['2020-01-01','2020-02-01','2020-03-01'])})
    time = SimpleNamespace(time_matrix=rng.normal(size=(3,2)), time_center=np.array([.1,.3]),
                           linear_time=np.array([0.,1.,2.]), linear_center=.8,
                           season_matrix=rng.normal(size=(12,2)), season_weights=np.ones(12)/12,
                           buildings=['b0','b1'], unit_ids=['u0','u1'])
    indices = {'period': np.array([0,1,2]), 'season': np.array([0,1,2]),
               'building': np.array([1,0,1]), 'unit': np.array([1,0,1])}
    time.arrays = lambda frame: indices
    matrix = rng.normal(size=(3,2))
    design = SimpleNamespace(time=time, features=['x','z'], matrix=lambda frame: matrix)
    coordinates = {'chain': [10,20], 'draw': list(range(9)), 'feature': design.features,
                   'building': time.buildings, 'unit': time.unit_ids, 'trend_basis': [0,1], 'season_basis': [0,1]}
    variables = {}
    for name, dims in m.VARIABLE_DIMS.items():
        shape = (2,9)+tuple(len(coordinates[d]) for d in dims)
        values = rng.normal(size=shape)*.1
        if name == 'alpha': values += 8
        if name in ('sigma','sigma_unit'): values = abs(values)+.01
...
def test_balanced_selection_boundaries_and_validation():
    ...
    for args in [(1,100,50),(4,100,51),(2,0,1),(2,100,0),(True,100,1)]:
        with pytest.raises(ValueError): m.select_draws(*args)


def test_lazy_joint_selection_and_reconstruction_matches_original_summary(posterior_case):
    from models.bayesian_feature_experiment import fitted_summary
    path, posterior, design, data = posterior_case
    samples, selection = m.load_draws(path, {'chains':2,'draws':9}, design, per_chain=3)
    ...
    for name in m.VARIABLE_DIMS:
        expected = np.concatenate([posterior[name].isel(chain=c,draw=[1,4,7]).values for c in range(2)])
    ...
    for s in range(6):
        for i in range(3):
            d = design.time; a = d.arrays(data)
            expected = (samples['alpha'][s]+design.matrix(data)[i]@samples['beta'][s]
                        +(d.time_matrix[i]-d.time_center)@samples['trend_coefficients'][s]
                        +samples['annual_drift'][s]*(d.linear_time[i]-d.linear_center)
                        +(d.season_matrix[i]-d.season_weights@d.season_matrix)@samples['season_coefficients'][s]
    ...
    np.testing.assert_allclose([r['fitted_rent'] for r in rows], np.exp(np.median(mu,axis=0)), atol=1e-9)
    np.testing.assert_allclose([r['latent_rent_lower_95'] for r in rows], np.exp(np.quantile(mu,.025,axis=0)), atol=1e-9)
...
def test_asymmetric_omission_detected_despite_matching_mean_absolute_error():
    ...
    replicated = rng.standard_t(5, size=(200,2000))*.07
    ...
    assert result['mean_absolute_log_residual']['observed'] == result['mean_absolute_log_residual']['replicated']
    assert result['median_signed_log_residual']['probability_replicated_greater_or_equal'] == 0
    assert result['signed_tail_balance_10pct']['observed_minus_replicated']['lower_95'] > .1
...
def test_sparse_current_slices_are_explicitly_withheld():
    data = pd.DataFrame({'bedrooms': [1]*20+[2]*2, 'unit_id': ['u'+str(i) for i in range(22)],
                         'building': ['b'+str(i%3) for i in range(22)],
                         'analysis_price_basis': ['historical_initial_own_advertisement_ask']*20+['current_capture_gross_ask']*2})
    included, omitted = m.slices(data)
    assert {v['slice'] for v,_ in included} == {'overall','history','bedrooms=1','bedrooms=1/history'}
...
def test_rejected_fit_never_reads_posterior(monkeypatch,tmp_path):
    ...
    monkeypatch.setattr(m, 'load_draws', lambda *a,**k: pytest.fail('Read rejected posterior'))
...
def test_frozen_reconstruction_dependency_hashes_enforced():
    ...
    names = ['bayesian_feature_model.py','bayesian_rent_model.py','bayesian_feature_experiment.py',
             'amenity_rent_model.py','minimal_rent_model.py','pricing.py','corrections.py','research_pipeline.py']
    paths = [next(p for p in (root/'models'/name,root/'src'/'apartments'/name) if p.exists()) for name in names]
