...
def test_cohort_audit_binds_sources_counts_rows_and_replays(tmp_path, monkeypatch):
    ...
    raw = canonical(
        {
            ...
            "propertyDetails": {"features": {"privateOutdoorSpaceTypes": ["BALCONY"]}},
        }
    )
    for name, frame in {
    ...
    monkeypatch.setattr(
        "models.outdoor_feature_audit.granular_parse.parse_listing",
        lambda body, url: (
            {
                "listing_id": "456",
                ...
                "raw_listing_json": canonical(
                    {
                        "description": "Shared roof deck.",
                        "propertyDetails": {
                            "amenities": {"sharedOutdoorSpaceTypes": ["ROOF_DECK"]}
                        },
                    }
                ),
            },
...
def test_category_union_keeps_positive_scope_changes_and_unknown_codes():
    ...
    result = category(
        [capture(["BALCONY"]), capture(["BALCONY", "TERRACE"])], "private"
    )
    assert result["category"] == "BALCONY+TERRACE"
    assert result["reported_type_sets_changed"] is True
    assert (
        category([capture(["BALCONY", "UNKNOWN_CODE"])], "private")["category"] is None
    )
