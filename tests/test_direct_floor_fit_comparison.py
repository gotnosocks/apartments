...
def test_exact_source_revision_and_tampering(example, tmp_path):
    ...
    publish_bundle(
        candidate, files, {k: v for k, v in manifest.items() if k != "files"}
    )
    ...
    before = [
        {
            ...
            "listed_floor": value,
            "advertised_floor": alias,
        }
        for i, (value, alias) in enumerate([(None, None), (None, 4), (None, None)])
    ]
    ...
    movements = [
        {
            "audit_id": str(i),
            "unit_id": str(i),
            "reference": {"residual_log": 0.2},
            "candidate": {"residual_log": 0.1},
            "fitted_rent_change": 10,
        }
        for i in range(3)
    ]
