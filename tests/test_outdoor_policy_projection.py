...
def test_corroboration_preserves_cohort_raw_categories_shared_and_replay(tmp_path):
    rows = []
    captures = []
    for key, types, text in [
        ("1", ["BALCONY", "PATIO"], "This apartment has a private balcony."),
        ...
        rows.append(
            {
                ...
                "capture_ids": [key],
                "asking_rent": 3000,
                "private_outdoor_category": None if key == "3" else "+".join(types),
                "shared_outdoor_category": "GARDEN",
            }
        )
        ...
        captures.append(
            {
                ...
                "description": text,
                "extraction": extract(
                    {"propertyDetails": details, "description": text}
                ),
            }
        )
    ...
    output = tmp_path / "out"
    manifest = run(structured, evidence, output)
    assert manifest["support"]["known_private_rows"] == 2
    actual = [
        json.loads(s)
        for s in (output / "observations.jsonl").read_text().split("\n")
        if s
    ]
    ...
    assert (
        actual[2]["private_outdoor_category"] == "BALCONY"
    )  # Unknown roof rights do not invent an outdoor type.
    assert actual[2]["structured_private_outdoor_category"] is None
    assert all(
        r["shared_outdoor_category"] == "GARDEN" and r["asking_rent"] == 3000
        for r in actual
    )
