...
@pytest.mark.parametrize(
    "label,value", [("3D", 3), ("# 14b", 14), (" 2A ", 2), ("99Z", 99)]
)
...
def record(audit, label, explicit, unit="unit-a"):
    return {
        ...
        "unit_id": unit,
        "building": "building-a",
        "explicit_floor": explicit,
        **infer_label(label),
    }


def test_repeated_captures_are_not_independent_agreement_units():
    observations, summary = summarize(
        [record("a", "3D", 3)] * 4 + [record("b", "3D", 2)]
    )
...
def source_fixture(tmp_path):
    ...
    raw = canonical(
        {
            "id": "ad-a",
            "propertyDetails": {"address": {"displayUnit": "#3D"}},
