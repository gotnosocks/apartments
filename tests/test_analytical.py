...
def record(at='2026-01-01', beds=0, *, version='v1', recorded='2026-01-02', **attrs):
    return {'observation': {'source': 'streeteasy', 'source_listing_id': 'b/1d',
            'version_id': version, 'capture_id': version, 'episode_id': '123',
            'building_slug': 'b', 'unit': '1D', 'collected_at': at,
            'recorded_at': recorded},
            'raw': {'attributes': {'bedrooms': beds, 'bathrooms': 1, **attrs},
                    'building_slug': 'b', 'status': 'ACTIVE', 'asking_rent': 2500,
...
def test_dated_intervals_never_backproject_price_or_attributes():
    ...
    assert [(r['bedrooms'], r['valid_from'][:10], r['valid_until'] and r['valid_until'][:10])
            for r in result['intervals']] == [(0, '2026-01-01', '2026-02-01'), (1, '2026-02-01', None)]
    assert [r['rent'] for r in result['observations']] == [2500, 2500]
    assert all('change date unverified' in r['attribute_time_basis'] for r in result['intervals'])
    assert result == project_records([first, second, deepcopy(first)], as_of='2026-03-01')
...
def test_nulls_false_and_floor_labels_are_not_inferred():
    ...
    assert attributes({'attributes': {}, 'building_amenities': ['ELEVATOR', 'DOORMAN', 'LAUNDRY']})['laundry_type'] == 'in_building'
    assert attributes({'attributes': {'laundry_type': None}, 'home_features': ['WASHER_DRYER']})['laundry_type'] is None


def test_overlay_splits_effective_intervals_without_duplicate_price_rows(tmp_path, monkeypatch):
    ledger = tmp_path/'edits.jsonl'
    monkeypatch.setattr(corrections, 'now', lambda: datetime(2026, 2, 20, tzinfo=UTC))
    edit = corrections.append(ledger, author='Ben', reason='Renovation records', edit={
    ...
    assert len(project_records([record()], as_of='2026-02-01', overlay=old)['intervals']) == 1
...
def test_inactive_and_missing_current_prices_remain_audited():
    ...
    old['raw']['archive_listing']['pricing'] = {'netEffectiveRent': 2200, 'rentedPrice': 2000}
    result = project_records([old], as_of='2026-03-01')
    assert not result['observations']
    assert result['quarantine'][0]['reason'] == 'missing_or_invalid_contemporary_asking_rent'
...
def test_database_adapter_reads_existing_capture_history(tmp_path):
    ...
    raw = record()['raw']
    raw.update(source='streeteasy', source_listing_id='b/1d',
               canonical_url='https://streeteasy.com/building/b/1d',
               captured_at='2026-01-01T00:00:00Z', unit='1D')
    retain_capture(db, 'capture', raw)
    db.execute("UPDATE attribute_versions SET recorded_at=TIMESTAMPTZ '2026-01-02'")
    db.close()
    manifest = build_dataset(tmp_path/'source.duckdb', tmp_path/'out', as_of='2026-03-01')
...
def test_explicit_historical_assertions_do_not_backfill_unmentioned_fields(tmp_path, monkeypatch):
    ledger = tmp_path/'historical.jsonl'
    monkeypatch.setattr(corrections, 'now', lambda: datetime(2026, 2, 20, tzinfo=UTC))
    for start, until, beds in [('2015-01-01', '2020-01-01', 0), ('2020-01-01', None, 1)]:
        validity = {'from': start}
        if until:
            validity['until'] = until
        corrections.append(ledger, author='Ben', reason='Dated renovation evidence', edit={
            'target': {'source': 'streeteasy', 'source_listing_id': 'b/1d'},
            'validity': validity,
            'patch': [{'op': 'replace', 'path': '/attributes/bedrooms', 'value': beds}]})
    result = project_records([record(beds=1)], as_of='2026-03-01', overlay=corrections.Overlay(ledger, as_of='2026-03-01'))
    facts = sorted(result['attribute_assertions'], key=lambda r: r['valid_from'])
    assert [(r['value'], r['valid_from'][:10]) for r in facts] == [(0, '2015-01-01'), (1, '2020-01-01')]
...
def test_conflicting_historical_assertions_fail_before_first_capture(tmp_path, monkeypatch):
    ledger = tmp_path/'conflict.jsonl'
    monkeypatch.setattr(corrections, 'now', lambda: datetime(2026, 2, 20, tzinfo=UTC))
    for beds in (0, 1):
        corrections.append(ledger, author='Ben', reason='Conflicting histories', edit={
            'target': {'source': 'streeteasy', 'source_listing_id': 'b/1d'},
            'validity': {'from': '2015-01-01', 'until': '2020-01-01'},
            'patch': [{'op': 'replace', 'path': '/attributes/bedrooms', 'value': beds}]})
    with pytest.raises(corrections.CorrectionError, match='Conflicting'):
        project_records([record()], as_of='2026-03-01', overlay=corrections.Overlay(ledger, as_of='2026-03-01'))
...
def test_furnished_short_term_and_concession_flags_preserve_price_basis():
    row = record()
    row['raw']['archive_listing']['pricing'] = {'price': 2500, 'monthsFree': 1,
                                               'furnishedRent': 2500, 'leaseTermMonths': 3,
...
def test_assertions_reflect_final_sequential_patch_value(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(corrections, 'now', lambda: datetime(2026, 2, 20, tzinfo=UTC))
    corrections.append(ledger, author='Ben', reason='Sequential JSON Patch', edit={
        'target': {'source': 'streeteasy', 'source_listing_id': 'b/1d'},
        'validity': {'from': '2015-01-01', 'until': '2020-01-01'},
        'patch': [{'op': 'replace', 'path': '/attributes/bedrooms', 'value': 0},
                  {'op': 'replace', 'path': '/attributes/bedrooms', 'value': 1}]})
    out = project_records([record()], as_of='2026-03-01', overlay=corrections.Overlay(ledger, as_of='2026-03-01'))
