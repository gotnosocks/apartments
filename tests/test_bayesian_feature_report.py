...
def interval(median=0.1):
    return {
        "median": median,
        "lower_95": median - 0.05,
        "upper_95": median + 0.05,
        "probability_positive": 0.98 if median > 0 else 0.02,
    }
...
def experiment(tmp_path):
        ...
        rows.append(
            {
                ...
                "canonical_unit_url": "https://streeteasy.com/building/example/"
                + str(i),
            }
        )
    dataset = tmp_path / "dataset"
    source = publish_bundle(
        dataset,
        {"observations.jsonl": "".join(canonical(r) + "\n" for r in rows)},
        {"version": "reviewed-bathroom-counts-projection-v1"},
    )
    protocol = {
        "version": m.EXPERIMENT_VERSION,
        ...
        "likelihood": "Student-t log ask",
        "bathroom_policy": "Source counts",
        "uncertainty": "Conditional associations",
        "chains": 4,
        "draws": 1000,
        "tune": 1000,
    ...
    increments = [
        {
            "bedrooms": 2,
            "before_full_half": [1, 0],
            "after_full_half": [2, 0],
            "log_effect": interval(),
            "percent_effect": interval(10),
            "support_before": counts,
            "support_after": counts,
    ...
    half = {
        ...
        "before_half": 0,
        "after_half": 1,
        "log_effect": interval(0.04),
        "percent_effect": interval(4),
        "support_before": counts,
        "support_after": counts,
        "supported_endpoints": True,
        "encoded_contrast": True,
    }
    balance = {
        "bedrooms": 2,
        "difference": {**interval(0.03), "probability_positive": 0.8},
        "probability_first_increment_larger": 0.8,
        "support": [counts, counts, counts],
    }
    contrasts = {
        "increments": increments,
        "half_bath_increments": [half],
        "balance": [balance],
    }
    names = [
        "bedrooms_gt_1",
        "listed_floor",
        "bathroom_composition_unknown",
        "laundry_type.contrast_0",
        "laundry_type.unknown",
    ]
    design_support = {
        "rows": 4,
        "bathroom_composition_known": 4,
        "features": len(names),
    }
    design = {
        ...
        "features": names,
        "numeric": {"listed_floor": {"center": 5.0, "scale": 3.0}},
    }
    coefficients = [{"feature": name, **interval()} for name in names]
    residuals = []
    for i, row in enumerate(rows):
        fit = row["asking_rent"] * (0.9 if i % 2 == 0 else 1.2)
        residuals.append(
            {
                k: row[k]
                for k in (
    ...
    group_rows = [
        {
            "kind": kind,
            "id": identity,
            "log_effect": interval(0.1 if i % 2 == 0 else -0.1),
    ...
    summary = {
        ...
        "diagnostics": diag,
        "derived_diagnostics": diag,
        "design_support": design_support,
        "bathroom_balance": [balance],
        "median_absolute_log_residual": (
            abs(residuals[0]["residual_log"]) + abs(residuals[1]["residual_log"])
        )
        / 2,
    }
    files = {
        ...
        "time-design.json": canonical(
            {"buildings": ["b0", "b1"], "unit_ids": ["u0", "u1", "u2", "u3"]}
        )
        + "\n",
        "time-design.npz": bytes(range(256)),
        "posterior.nc": bytes(reversed(range(256))),
        "residuals.jsonl": "".join(canonical(r) + "\n" for r in residuals),
        "group-effects.jsonl": "".join(canonical(r) + "\n" for r in group_rows),
    }
    publish_binary(
        root / "fit", files, {"version": m.EXPERIMENT_VERSION, "protocol_sha256": ph}
    )
...
def test_verified_report_deduplicates_contrasts_separates_reporting_and_replays(
    experiment, tmp_path
):
    ...
    assert report["coefficients"]["omitted_category_basis_coefficients"] == [
        "laundry_type.contrast_0"
    ]
    assert {r["feature"] for r in report["coefficients"]["reporting_coefficients"]} == {
        "bathroom_composition_unknown",
        "laundry_type.unknown",
    }
    notes = {
        r["feature"]: r["encoded_unit"]
    ...
    assert (
        b"<script" not in saved["report.html"]
        and b"<iframe" not in saved["report.html"]
    )


@pytest.mark.parametrize(
    "mutation", ["status", "parameter_failure", "derived_failure", "optimistic_flag"]
)
def test_diagnostic_only_or_unacceptable_fit_refused(experiment, mutation):
    ...
    if mutation == "status":
        ...
        key = (
            "diagnostics" if mutation == "parameter_failure" else "derived_diagnostics"
        )
        updated = copy.deepcopy(summary[key])
        if mutation == "optimistic_flag":
            updated["max_rhat"] = 1.05
        else:
            updated["acceptable"] = False
        summary[key] = updated
        files[
            "diagnostics.json" if key == "diagnostics" else "derived-diagnostics.json"
        ] = canonical(updated) + "\n"
...
def test_contrast_support_recomputed_from_source(experiment):
    ...
    contrasts["increments"][0]["support_before"] = {
        "rows": 2,
        "units": 1,
        "buildings": 1,
    }
...
def v3_fixture(
    experiment,
    mode="bedroom",
    source_version="reviewed-scope-composition-projection-v2",
):
    ...
    config = {
        ...
        "residual_scale": mode,
        "student_t_nu": 5.0,
        "beta_prior_multiplier": 1.0,
        "building_prior_scale": 0.7,
        "unit_prior_scale": 0.125,
        "residual_sigma_prior_scale": 0.25,
        "residual_bedroom_levels": [1, 2] if mode == "bedroom" else [],
        "residual_bedroom_counts": [1, 3] if mode == "bedroom" else [],
        "residual_bedroom_offset_scale_prior": 0.3 if mode == "bedroom" else None,
        "parameterization": "Synthetic saved configuration; no graph compiled.",
    }

    def scale(value):
        return {**interval(value), "probability_positive": 1.0}

    scales = {
        "version": "bayesian-residual-scale-summary-v1",
        "graph_configuration": config,
        "student_t_nu": 5.0,
        "scale_units": "Log advertised asking rent; Student-t scale, not its standard deviation.",
        "global_sigma": scale(0.2),
        "global_sigma_role": "Geometric mean of bedroom-level scales (equal level weights)"
        if mode == "bedroom"
        else "Shared observation scale",
        "by_bedroom": [
            {
                "bedrooms": 1,
                "sigma": scale(0.15),
                "support": {"rows": 1, "units": 1, "buildings": 1},
            },
            {
                "bedrooms": 2,
                "sigma": scale(0.25),
                "support": {"rows": 3, "units": 3, "buildings": 2},
            },
        ]
        if mode == "bedroom"
        else [],
...
@pytest.mark.parametrize(
    ...
    [
        (v, f)
        for v in sorted(m.reviewed_source_lineage.VERSIONS)
        for f in [
            None,
            "missing_lineage",
            "unintended_change",
        ...
        if f not in ("missing_sidecar", "changed_sidecar")
        or v
        in (
            m.reviewed_source_lineage.reviewed_cohort_quarantine.VERSION,
            m.reviewed_source_lineage.elevator_corrections.VERSION,
            m.reviewed_source_lineage.expanded_floor_projection.VERSION,
            m.reviewed_source_lineage.residual_scope_projection.VERSION,
        )
    ],
)
def test_corrected_source_report_requires_exact_review_lineage(
    experiment, tmp_path, version, fault
):
    ...
    for row in rows:
        row.update(
            ...
            advertised_floor=3,
            capture_ids=[row["audit_id"]],
            elevator=True,
        )
    if version == m.reviewed_source_lineage.direct_floor_projection.VERSION:
        rows[2]["advertised_floor"] = None
    sidecar_files = {}
    if version in (
        m.reviewed_source_lineage.residual_scope_projection.VERSION,
        m.reviewed_source_lineage.direct_floor_projection.VERSION,
    ):
    ...
    if version in (
        m.reviewed_source_lineage.reviewed_cohort_quarantine.VERSION,
        m.reviewed_source_lineage.elevator_corrections.VERSION,
        m.reviewed_source_lineage.floor_label_projection.VERSION,
        m.reviewed_source_lineage.expanded_floor_projection.VERSION,
        m.reviewed_source_lineage.residual_scope_projection.VERSION,
        m.reviewed_source_lineage.direct_floor_projection.VERSION,
    ):
        from tests.test_reviewed_cohort_quarantine import (
            add_excluded_row,
            quarantine_last,
        )
    ...
    parent = {
        "version": m.reviewed_source_lineage.REFRESHED,
        "files": {"observations.jsonl": observations_hash(rows)},
    }
    manifest, rows = revise(
        parent, rows, m.reviewed_source_lineage.LAUNDRY, "laundry_type", "laundry"
    )
    if version in (
        m.reviewed_source_lineage.FLOOR,
        m.reviewed_source_lineage.laundry_floor_split.VERSION,
        m.reviewed_source_lineage.reviewed_cohort_quarantine.VERSION,
        m.reviewed_source_lineage.elevator_corrections.VERSION,
        m.reviewed_source_lineage.floor_label_projection.VERSION,
        m.reviewed_source_lineage.expanded_floor_projection.VERSION,
    ...
    if version in (
        ...
        sidecar_files["quarantined.jsonl"] = "".join(
            canonical(r) + "\n" for r in sidecar
        )
    if version in (
        m.reviewed_source_lineage.elevator_corrections.VERSION,
        ...
        sidecar_files["elevator-corrections.jsonl"] = "".join(
    ...
    if version in (
        ...
        manifest, rows, label_sidecar = extend(
            manifest,
            rows,
            unsupported_index=2
            if version == m.reviewed_source_lineage.direct_floor_projection.VERSION
        ...
        sidecar_files["floor-label-projection.jsonl"] = "".join(
    ...
    if version in (
        ...
        sidecar_files["expanded-floor-projection.jsonl"] = "".join(
    ...
    if version in (
        ...
        sidecar_files["residual-scope-quarantine.jsonl"] = "".join(
            canonical(r) + "\n" for r in scope_sidecar
        )
    if version == m.reviewed_source_lineage.direct_floor_projection.VERSION:
        from tests.test_direct_floor_projection import extend

        manifest, rows, direct_sidecar = extend(manifest, rows)
        sidecar_files["direct-floor-changes.jsonl"] = "".join(
            canonical(r) + "\n" for r in direct_sidecar
        )
    ...
    if fault:
        with pytest.raises(
            ValueError,
            match="lineage|Reconstructed parent|Reconstructed laundry|Reconstructed quarantine|Quarantine|Elevator|Floor projection|Expanded floor|Residual scope|direct floor|Direct floor|Floor forward replay|Reconstructed scope",
        ):
            m.build_report(root, dataset)
    else:
        report, _ = m.build_report(root, dataset)
        ...
        assert (output / "reviewed_source_lineage.py").read_text() == Path(
            m.reviewed_source_lineage.__file__
        ).read_text()
...
def test_v3_source_bound_scale_summary_and_html(
    experiment, tmp_path, mode, source_version
):
    root, dataset, protocol, files, scales = v3_fixture(
        experiment, mode, source_version
    )
    report, _ = m.build_report(root, dataset)
    ...
    assert report["method"]["building_prior_scale"] == 0.7
    assert report["method"]["unit_prior_scale"] == 0.125
...
def test_v3_missing_or_tampered_noise_products_refused(experiment, name):
    ...
    del files[name]
    rewrite_v3(root, protocol, files)
    with pytest.raises(ValueError, match="missing inference products"):
        m.build_report(root, dataset)
...
def test_v3_rehashed_configuration_contradictions_refused(experiment, mutation):
    ...
    if mutation == "protocol_mode":
        protocol["residual_scale"] = "shared"
    if mutation == "protocol_prior":
        protocol["building_prior_scale"] = 0.8
    if mutation == "configuration_only":
        config["unit_prior_scale"] = 0.9
    if mutation == "summary_only":
        scales["graph_configuration"] = {**config, "unit_prior_scale": 0.9}
    if mutation == "graph_version":
        config["version"] = "unsupported"
    if mutation == "nu":
        config["student_t_nu"] = 7
    if mutation == "sigma_prior":
        config["residual_sigma_prior_scale"] = 0.7
    if mutation == "bedroom_offset_prior":
        config["residual_bedroom_offset_scale_prior"] = 0.6
    if mutation == "source_levels":
        config["residual_bedroom_levels"] = [1, 3]
    if mutation == "source_counts":
        config["residual_bedroom_counts"] = [2, 2]
    if mutation == "boolean_level":
        config["residual_bedroom_levels"] = [True, 2]
    if mutation not in (
        "protocol_mode",
        "protocol_prior",
        "configuration_only",
    ...
    files["residual-scales.json"] = canonical(scales) + "\n"
    rewrite_v3(root, protocol, files)
    with pytest.raises(ValueError, match="Residual|residual|Unsupported"):
        m.build_report(root, dataset)
...
def test_v3_rehashed_scale_claims_refused(experiment, mutation):
    root, dataset, protocol, files, scales = v3_fixture(experiment)
    if mutation.startswith("support_"):
        scales["by_bedroom"][0]["support"][mutation.removeprefix("support_")] += 1
    if mutation == "missing_level":
        scales["by_bedroom"].pop()
    if mutation == "duplicate_level":
        scales["by_bedroom"][1]["bedrooms"] = 1
    if mutation == "negative_scale":
        scales["global_sigma"]["lower_95"] = -0.1
    if mutation == "zero_scale":
        scales["by_bedroom"][0]["sigma"]["lower_95"] = 0
    if mutation == "unordered_interval":
        scales["by_bedroom"][0]["sigma"]["upper_95"] = 0.01
    if mutation == "positive_probability":
        scales["global_sigma"]["probability_positive"] = 0.5
    if mutation == "wrong_scale_units":
        scales["scale_units"] = "Standard deviation"
    if mutation == "wrong_role":
        scales["global_sigma_role"] = "Frequency-weighted mean"
    if mutation == "wrong_summary_nu":
        scales["student_t_nu"] = 8
    files["residual-scales.json"] = canonical(scales) + "\n"
    rewrite_v3(root, protocol, files)
    with pytest.raises(ValueError, match="Residual|residual|posterior interval"):
        m.build_report(root, dataset)
