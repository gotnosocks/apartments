...
@pytest.mark.parametrize('ordinal,floor', list(zip(ORDINALS, range(1, 21))) +
    [('1st', 1), ('2nd', 2), ('3rd', 3), ('4th', 4), ('11th', 11), ('12th', 12),
     ('13th', 13), ('21st', 21), ('102nd', 102), ('111th', 111), ('18', 18)])
def test_explicit_floor_token_not_label_digits(ordinal, floor):
    result = extract({'description': f'Welcome to Apartment 9B on the {ordinal} floor with bright windows.'})
    assert result['attributes']['advertised_floor'] == floor


@pytest.mark.parametrize('prefix', ['', 'Welcome to ', '\n\tWelcome to ', '\r\n Welcome\tto\n'])
...
def test_structured_conflict_and_unrelated_attributes_are_preserved():
    raw = {'propertyDetails': {'floor': 1, 'physicalFloor': 1, 'bedroomCount': 2},
    ...
    assert [e['source_path'] for e in floor_claims(result)] == ['/propertyDetails/floor', '/description']


def test_two_incompatible_own_description_claims_remain_conflicted():
    result = extract({'description': 'Apartment 1A on the second floor. This unit is on the 3rd floor.'})
    assert result['attributes']['advertised_floor'] is None
