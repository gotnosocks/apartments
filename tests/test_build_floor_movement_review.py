...
def fixture(expanded=True, masked=False):
    ...
    before["asking_rent"] = 3000.0
    row["asking_rent"] = 3000.0
    ...
    if expanded:
        ...
        files = {
            "observations.jsonl": (canonical(rows[0]) + "\n").encode(),
            m.floor_label_projection.SIDECAR: (canonical(old_change) + "\n").encode(),
            m.expanded_floor_projection.SIDECAR: (
                canonical(changes[0]) + "\n"
            ).encode(),
        }
...
def test_loads_correct_building_evidence_and_every_projection_stage(expanded):
    ...
    version = (
        m.EXPANDED_COMPARISON if expanded else "matched-floor-spline-fit-comparison-v1"
    )
    got, old_changes, new_changes, evidence = m.verified_floor_source(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "wrong_comparison_version",
        "missing_old_sidecar",
        "missing_new_sidecar",
        "tampered_old_sidecar",
        "tampered_new_sidecar",
        "tampered_row",
        "tampered_building_context",
    ],
)
def test_expanded_source_rejects_wrong_versions_and_sidecar_ancestry(fault):
    ...
    if fault == "wrong_source_version":
        manifest["version"] = m.floor_label_projection.VERSION
    elif fault == "wrong_comparison_version":
        version = "matched-floor-spline-fit-comparison-v1"
    elif fault == "missing_old_sidecar":
        files.pop(m.floor_label_projection.SIDECAR)
    elif fault == "missing_new_sidecar":
        files.pop(m.expanded_floor_projection.SIDECAR)
    elif fault == "tampered_old_sidecar":
        changes = m.records(files[m.floor_label_projection.SIDECAR])
        changes[0]["source_index"] = 2
        files[m.floor_label_projection.SIDECAR] = (
            canonical(changes[0]) + "\n"
        ).encode()
        manifest["files"][m.floor_label_projection.SIDECAR] = records_hash(changes)
    elif fault == "tampered_new_sidecar":
        changes = m.records(files[m.expanded_floor_projection.SIDECAR])
        changes[0]["source_index"] = 2
        files[m.expanded_floor_projection.SIDECAR] = (
            canonical(changes[0]) + "\n"
        ).encode()
        manifest["files"][m.expanded_floor_projection.SIDECAR] = records_hash(changes)
    elif fault == "tampered_row":
        rows = m.records(files["observations.jsonl"])
    ...
    with pytest.raises(ValueError):
        m.verified_floor_source(manifest, files, version)
...
def test_legacy_comparison_rejects_hidden_expanded_stage():
    ...
    with pytest.raises(ValueError, match="Unexpected expanded"):
        m.verified_floor_source(
            manifest, files, "matched-label-floor-fit-comparison-v1"
        )
...
def test_scope_sidecars_are_selected_after_exact_outer_reconstruction(monkeypatch):
    ...
    source = {
        ...
        m.residual_scope_projection.SIDECAR: "".join(
            canonical(c) + "\n" for c in changes
        ).encode(),
    }
    ...
    def inner(parent, files, version):
        assert (
            parent == manifest["source_manifest"] and version == m.EXPANDED_COMPARISON
        )
    ...
    with pytest.raises(ValueError):
        implementation(manifest, damaged, m.SCOPE_COMPARISON)
...
@pytest.mark.parametrize("expanded", [False, True])
@pytest.mark.parametrize(
    "fault", [None, "wrong_source_binding", "movement_identity", "ranked_movement"]
)
def test_published_cases_include_both_stages_and_keep_existing_identity_guards(
    tmp_path, monkeypatch, expanded, fault
):
    manifest, files, rows = fixture(expanded, masked=expanded)
    dataset, comparison, evidence, output = [
        tmp_path / name for name in ("dataset", "comparison", "evidence", "output")
    ]
    publish_bundle(
        dataset,
        {k: v.decode() for k, v in files.items()},
    ...
    movement = {
        **{
        ...
        "fitted_rent_change": 100.0,
    }
    result = {
        "version": m.EXPANDED_COMPARISON
        if expanded
        else "matched-floor-spline-fit-comparison-v1",
    ...
    if fault == "wrong_source_binding":
        result["fits"][1]["bindings"]["source"] = "a" * 64
    elif fault == "movement_identity":
        movement["source_listing_id"] = "different"
    elif fault == "ranked_movement":
        result["largest_distinct_unit_movements"][0]["fitted_rent_change"] = 20.0
    publish_bundle(
        comparison,
        {
    ...
    monkeypatch.setattr(
        m,
        "load_evidence",
        lambda *args: {rows[0]["audit_id"]: [{"description": "own literal"}]},
    )
    if fault:
        with pytest.raises(ValueError):
            m.run(comparison, dataset, evidence, output)
        ...
        if expanded:
            assert (
                case["expanded_floor_projection"]["original_change"]
                == case["floor_projection"]
            )
            assert (
                case["expanded_floor_mask"]["correction_record"]["id"] == "correction-1"
            )
            assert (
                sha(case["expanded_floor_mask"])
                == case["observation"]["expanded_floor_provenance"]["floor_mask_sha256"]
            )
