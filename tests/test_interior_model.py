...
def sample():
    ...
    for month in range(1, 5):
        for unit in range(24):
            ...
            rent = 3000 * np.exp(0.04 * (height - 10) if height is not None else -0.03)
            rows.append(
                dict(
                    unit_id=f"u{unit}",
                    building=f"b{unit // 4}",
...
def test_known_only_centering_and_constant_mentions():
    ...
    assert np.allclose(
        columns["advertised_ceiling_feet"][data.advertised_ceiling_feet.notna()].mean(),
        0,
    )
    assert (
        columns["advertised_ceiling_feet"][data.advertised_ceiling_feet.isna()] == 0
    ).all()
    ...
    probe["advertised_ceiling_feet"] = [12.0, None]
    probe["advertised_levels"] = [3.0, None]
    encoded = encoder.matrix(probe).toarray()[:, start:stop]
    assert encoded[0, 0] == 2.0  # Frozen mean 10, standard deviation 1.
...
def test_reporting_ignores_known_magnitudes_and_baseline_matches_existing():
    ...
    probe["advertised_ceiling_feet"] = [9.0, 12.0, None]
    ...
    assert np.array_equal(
        original.matrix(data).toarray(), experimental.matrix(data).toarray()
    )
    assert np.array_equal(
        original.penalty(amenities.SETTINGS).toarray(),
        experimental.penalty(amenities.SETTINGS).toarray(),
    )
...
def test_real_fit_roundtrip_penalties_convergence_and_contrasts(variant):
    data = sample()
    settings = {**amenities.SETTINGS, "interior_penalty": 3.0}
    fitted = baseline.fit(
        data, settings, iterations=20, encoder_class=interior.encoder_class(variant)
    )
    ...
    if stop > start:
        assert np.allclose(
            np.asarray(penalty.power(2).sum(axis=0)).ravel()[start:stop], 3.0
        )
    saved = json.loads(
        json.dumps(
            {
    ...
    probe["advertised_ceiling_feet"] = [12.0, None, 8.0]
    assert np.array_equal(
    ...
    if variant == "values":
        ...
        actual = (
            baseline.predict(loaded, changed)[0]
            - baseline.predict(loaded, probe.iloc[:1])[0]
        )
...
def test_malformed_evidence_and_saved_fit_rejected():
    data = sample()
    for field, value in [
        ("advertised_ceiling_feet", np.inf),
        ("advertised_levels", 1.5),
        ("skylight_mention", 2.0),
    ]:
        bad = data.copy()
    ...
    fitted = baseline.fit(
        data, amenities.SETTINGS, encoder_class=interior.encoder_class("values")
    )
