...
def fixture_case(action="mark_shared_composition_unknown"):
    ...
    row = {
        ...
        "bathroom_count_evidence": {
            "full_status": "consistent_explicit_count",
            "half_status": "consistent_explicit_count",
            "flags": ["keep_this_flag"],
            "capture_ids": [1],
        },
    }
    evidence = {k: row[k] for k in revision.SOURCE_ID_FIELDS} | {
        "capture_id": 1,
        "description": text,
    ...
    d = {k: row[k] for k in revision.SOURCE_ID_FIELDS} | {
        "action": action,
        "source_projection_row_sha256": revision.sha(row),
        "interpreted_at": "2026-09-18T20:00:00+00:00",
        "evidence": [evidence],
        "reason": "fixture",
    }
...
def test_shared_access_preserves_all_reported_counts_and_original_analytical_fields():
    ...
    assert r["bathroom_count_evidence"]["flags"] == [
        "keep_this_flag",
        revision.SHARED_FLAG,
    ]
    assert r["bathroom_count_evidence"]["full_status"] == "consistent_explicit_count"
    assert (
        r["bathroom_count_evidence"]["composition_status"]
        == "reviewed_external_shared_access_composition_unknown"
    )
    assert r["bathroom_count_before_review"] == {
        k: row[k] for k in revision.REVIEW_FIELDS
    }
...
def test_acceptance_removes_only_generic_review_flag_and_keeps_counts():
    ...
    row["reported_half_bathrooms"] = 2
    row["bathroom_count_evidence"]["flags"].append(revision.MULTIPLE_FLAG)
    rebind(row, d)
    r = revision.apply_decision(row, d)
    assert r["reported_half_bathrooms"] == 2
    assert r["bathroom_count_evidence"]["flags"] == ["keep_this_flag"]
    assert (
        revision.MULTIPLE_FLAG
        in r["bathroom_count_before_review"]["bathroom_count_evidence"]["flags"]
    )


def test_office_quarantine_does_not_change_source_row():
    row, d = fixture_case("quarantine_nonresidential")
    original = deepcopy(row)
...
def test_tampered_source_binding_fails(problem, expected):
    ...
    if problem == "row":
        ...
        if problem == "span":
            d["evidence"][0]["literal"] = "wrong"
        elif problem == "capture":
            d["evidence"][0]["capture_id"] = 2
        elif problem == "identity":
            d["evidence"][0]["unit_id"] = "wrong"
        rebind(row, d)
    with pytest.raises(ValueError, match=expected):
        revision.apply_decision(row, d)
...
def test_source_excerpt_refuses_changed_reviewed_wording_or_count():
    ...
    capture = {
        **d["evidence"][0],
        "source_listing_id": "1945700",
        "body_sha256": "b",
        "raw_listing_sha256": "r",
        "source_collected_at": "2026-09-18T19:00:00Z",
        "description_interpreted_at": None,
        "known_at": "2026-09-18T19:00:00Z",
        "bathroom_fields": {
            "fullBathroomCount": {"value": 1},
            "halfBathroomCount": {"value": 2},
        },
    }
    with pytest.raises(ValueError, match="Reviewed source wording missing"):
        revision.source_excerpt(capture, "accept_corroborated_multiple_half")
    capture["description"] = "Three baths: (1 Full, 2 Half Baths)"
    capture["description_sha256"] = hashlib.sha256(
        capture["description"].encode()
    ).hexdigest()
    assert (
        revision.source_excerpt(capture, "accept_corroborated_multiple_half")["literal"]
        == "(1 Full, 2 Half Baths)"
    )
...
def test_projection_replays_and_preserves_untargeted_rows(tmp_path):
    ...
    publish_bundle(
        decisions,
        {"decisions.jsonl": canonical(d) + "\n"},
        {
            "version": revision.DECISION_VERSION,
            "projection_manifest": pm,
            "summary": {"interpreted_at": d["interpreted_at"]},
            "implementation_sha256": {
                Path(revision.__file__).name: digest(Path(revision.__file__))
            },
        },
    )
    output = tmp_path / "output"
    result = revision.project(projection, decisions, output)
    ...
    r = [
        json.loads(x)
        for x in (output / "observations.jsonl").read_text().split("\n")
        if x
    ]
