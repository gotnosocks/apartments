...
def test_reader_location_terms_match_graph_groups_and_skip_other_versions():
    ...
    assert terms.row_terms({'version': contract.EXPERIMENT}, {}, pd.DataFrame(), None) == {}
...
def test_reader_location_term_indexes_group_and_period():
    ...
    result = terms.row_terms({'version': terms.BEDROOM_TIME_EXPERIMENT}, draws, frame, Design())
    np.testing.assert_array_equal(result['bedroom_time'], draws['bedroom_time'][:, 3, 1])
