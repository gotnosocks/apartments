...
def test_canonical_unit_url_extraction_is_explicit_and_bounded(tmp_path):
    ...
    with gzip.open(path,'wt') as stream:
        stream.write('<html><head><link href="/building/a/4c" rel="canonical"></head><body>unitId=unrelated</body>')
    assert extract(path,'https://streeteasy.com/rental/1') == ('https://streeteasy.com/building/a/4c',None)
    with gzip.open(path,'wt') as stream:
        stream.write('<head><link rel="canonical" href="/building/a/4c"><link rel="canonical" href="/building/a/5c"></head>')
    ...
    assert unit_page('http://www.streeteasy.com/building/a/4c/') == 'https://streeteasy.com/building/a/4c'


def evidence():
    source = SourceEvidence.__new__(SourceEvidence)
    source.digest='fixture';source.error=None
    source.pages={sid:{'canonical_url':'https://streeteasy.com/building/a/4c'} for sid in [1,2]}
    source.latest={1:'2',2:'2'};source.history={1:{'1','2'},2:{'1','2'}}
    return source
...
def test_generic_canonical_page_cannot_automatically_establish_a_home():
    ...
    catalog={lid:{'captures':[{'snapshot_id':int(lid),'building_slug':'a','unit_label':'Studio'}]} for lid in ['1','2']}
    result=source.assess(['1','2'],catalog,{'https://streeteasy.com/building/a/studio':{'1','2'}},{'1':{'1','2'},'2':{'1','2'}},set(),set())
    assert not result['eligible'] and any('generic' in reason for reason in result['reasons'])
...
def test_backfill_is_bound_to_the_exact_dataset_snapshots(service):
    ...
    root=service.root/'snapshots';root.mkdir()
    pq.write_table(pa.Table.from_pylist([
        {'snapshot_id':1,'body_hash':'a'*64}, {'snapshot_id':2,'body_hash':'b'*64}
    ]),root/'part.parquet')
    expected=snapshot_rows(service.root)
    rows=[{'snapshot_id':sid,'url':url,'body_hash':digest,'canonical_url':'https://streeteasy.com/building/a/4c','error':None} for sid,url,digest in expected]
    metadata={b'version':VERSION.encode(),b'dataset':service.dataset.encode(),b'snapshots':fingerprint(expected).encode()}
    ...
    pq.write_table(table.replace_schema_metadata({**metadata,b'dataset':b'wrong-dataset'}),path)
