...
@pytest.mark.parametrize('text', ['10-12 foot ceilings', '10′–12′ ceilings', 'ceilings range from 10 to 12 feet'])
...
@pytest.mark.parametrize('text', ['10" ceilings', '10 inch ceilings', '10 inches ceilings', '10″ ceilings'])
...
@pytest.mark.parametrize('phrase', ['floor through', 'floor-through', 'floor thru', 'floor-thru', 'floor–through'])
...
def test_multiple_levels_and_other_units_scope():
    items = by_feature('Single-level home. Two-level unit. A triplex. Multi-level layout. Other duplex apartments available.', 'levels')
...
def test_missing_description_or_unmentioned_feature_is_unknown():
    for description in (None, '', {}, '$3a', 'A spacious apartment with high ceilings.'):
...
@pytest.mark.parametrize('phrase', ['12-foot ceilings', '12-ft. ceilings', '12′-high ceilings'])
