...
def example():
    ...
    case.update(
        ...
        kinds=["furnished_offer_unmodeled"],
        messages=[message],
    )
    notes = {
        "a": {
...
def test_legacy_defaults_are_preserved_without_expanded_annotations():
    ...
    assert spec["counts"] == {
        "rows": 52653,
        "current_rows": 172,
        "known_floor_rows": 29907,
        "source_review_cases": 8,
    }
...
def test_explicit_expanded_expectations_bind_four_issues_three_historical_ads():
    spec = m.load_expectations(
        m.ROOT / "docs/analysis/expanded-spline-main-page-expectations-20260919.json"
    )
    ...
    assert by_ad["4141846"]["floor_scenario"] == {"before": 6.0, "after": 7.0}
...
def test_invalid_explicit_expectations_fail(tmp_path, fault):
    ...
    if fault == "bool_count":
        spec["counts"]["rows"] = True
    if fault == "negative_count":
        spec["counts"]["rows"] = -1
    if fault == "missing_count":
        del spec["counts"]["source_review_cases"]
    if fault == "duplicate_audit":
        spec["historical_issues"].append(deepcopy(spec["historical_issues"][0]))
    if fault == "missing_hash":
        del spec["source_issues_manifest_sha256"]
    if fault == "missing_message":
        spec["historical_issues"][0]["messages"] = []
    if fault == "bad_current_count":
        spec["known_current_floor_rows"] = 173
    if fault == "unchanged_scenario":
        spec["historical_issues"][1]["floor_scenario"] = {"before": 6.0, "after": 6.0}
    path = tmp_path / "expectations.json"
    path.write_text(json.dumps(spec))
    with pytest.raises(ValueError):
        m.load_expectations(path)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "missing_annotation",
        "extra_annotation",
        "strengthened_furnishing",
        "changed_message",
        "dropped_merged_message",
        "not_limited",
    ],
)
def test_changed_identity_or_furnishing_strength_fails(fault):
    row, case, notes = example()
    if fault == "wrong_unit":
        row["unit_id"] = "unit:other"
    if fault == "wrong_ad":
        row["source_listing_id"] = "456"
    ...
    if fault == "missing_annotation":
        notes.clear()
    if fault == "extra_annotation":
        notes["other"] = deepcopy(notes["a"])
    if fault == "strengthened_furnishing":
        notes["a"]["issues"][0]["kind"] = "furnished_only_offer_unmodeled"
    if fault == "changed_message":
        notes["a"]["issues"][0]["message"] = "Furnished only."
    if fault == "dropped_merged_message":
        notes["a"]["message"] = "Dropped reviewed statement."
    if fault == "not_limited":
        notes["a"]["interpretation_limited"] = False
    with pytest.raises(ValueError):
        m.verify_historical_notes([row], notes, [case])
