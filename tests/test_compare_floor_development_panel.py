...
def inputs(tmp_path, version, *, source_index=0, damage=False):
    panel, comparison, output = (
        tmp_path / name for name in ("panel", "comparison", "output")
    )
    cases, movements = [], []
    for i in range(26):
        source = {
        ...
        movements.append(
            {
                **source,
                ...
                "fitted_rent_change": 10.0,
            }
        )
    if damage:
        movements[0]["asking_rent"] = 4000.0
    publish_bundle(
        panel,
        {"cases.jsonl": "".join(canonical(r) + "\n" for r in cases)},
        {
            "version": "floor-source-development-panel-v1",
            "dataset_manifest_sha256": "panel-source",
        },
    )
    ...
    publish_bundle(
        comparison,
        {
            "comparison.json": canonical({"version": version, "fits": bindings}),
...
def test_fixed_panel_uses_bound_source_without_changing_membership(
    tmp_path, version, index
):
    args = inputs(tmp_path, version, source_index=index)
    run(*args)
    summary = json.loads((args[2] / "summary.json").read_text())
    cases = [
        json.loads(line) for line in (args[2] / "cases.jsonl").read_text().splitlines()
    ]
    ...
    assert summary["candidate_median_absolute_log_residual"] == 0.09
...
def test_expanded_panel_does_not_accept_changed_targets(tmp_path):
    with pytest.raises(ValueError, match="source identity/target changed"):
        run(
            *inputs(
                tmp_path, "matched-expanded-floor-spline-fit-comparison-v1", damage=True
            )
        )


def scope_inputs(tmp_path, damage=None):
    panel, ancestor, _ = inputs(
        tmp_path, "matched-expanded-floor-spline-fit-comparison-v1"
    )
    result = json.loads((ancestor / "comparison.json").read_text())
    common = dict(result["fits"][1]["bindings"])
    rows = [
        json.loads(line)
        for line in (ancestor / "residual-movements.jsonl").read_text().splitlines()
    ]
    for row in rows:
        row["reference"] = dict(row["candidate"])
        row["candidate"] = {"residual_log": 0.08}
    if damage == "source":
        common["source"] = "unrelated-source"
    if damage == "fit":
        common["fit"] = "different-fit-on-same-source"
    if damage == "residual":
        rows[0]["reference"]["residual_log"] = 0.099
    if damage == "identity":
        rows[0]["unit_id"] = "another-unit"
    if damage == "missing":
        rows.pop()
...
def test_scope_panel_retains_original_cells_and_proves_common_fit(tmp_path):
    ...
    assert summary["reference_median_absolute_log_residual"] == 0.09
    assert summary["candidate_median_absolute_log_residual"] == 0.08
...
def test_other_comparisons_reject_unexpected_ancestry(tmp_path):
    panel, comparison, output = inputs(
        tmp_path, "matched-expanded-floor-spline-fit-comparison-v1"
    )
