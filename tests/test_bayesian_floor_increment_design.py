    ...
    return pd.DataFrame(
        {
            ...
            "unit_id": ["u" + str(i % 70) for i in range(n)],
            ...
            "listed_floor": rng.choice([-1.0, 0.0, 1.0, 3.0, np.nan], n),
...
def test_exact_increment_boundary_negative_ground_gaps_and_unknown(train):
    ...
    assert design.floor_levels == [-1.0, 0.0, 1.0, 3.0]
    assert design.floor_thresholds == [-1.0, 0.0, 1.0]
    ...
    np.testing.assert_array_equal(
        matrix[:, [names.index(n) for n in floor_names]],
    ...
    beta[[design.features.index(n) for n in floor_names[:-1]]] = [0.1, -0.05, 0.2]
    predictions = design.matrix(cases(train, [-1, 0, 1, 3])) @ beta
    np.testing.assert_allclose(np.diff(predictions), [0.1, -0.05, 0.2], atol=1e-14)
...
def test_every_nonfloor_column_and_prior_exactly_unchanged(train, spec):
    previous = Previous(train, spec)
    design = FeatureDesign(train, spec)
    ...
    for i, name in enumerate(oldnames):
        ...
        if name in previous.features:
            np.testing.assert_array_equal(
                previous.matrix(train)[:, previous.features.index(name)],
...
def test_alias_and_physical_height_are_independent(train):
    ...
    np.testing.assert_array_equal(listed_floor_values(data), listed_floor_values(train))
    data.loc[0, "listed_floor"] = 0
    data.loc[0, "advertised_floor"] = 3
...
def test_unobserved_floor_rejected_at_raw_and_public_transform(train, floor):
    design = FeatureDesign(train)
    with pytest.raises(ValueError, match="absent from fitted support"):
        design.matrix(cases(train, [floor]))
    with pytest.raises(ValueError, match="absent from fitted support"):
        design.raw_features(cases(train, [floor]))


def test_level_and_adjacent_endpoint_support(train):
    design = FeatureDesign(train)
    assert (
        sum(v["rows"] for v in design.floor_support["levels"])
        == train.listed_floor.notna().sum()
    )
    for item in design.floor_support["levels"]:
        rows = train.loc[train.listed_floor.eq(item["level"])]
        assert (
            item["units"] == rows.unit_id.nunique()
            and item["buildings"] == rows.building.nunique()
        )
    for item in design.floor_support["adjacent_supported_contrasts"]:
        lo = set(
            train.loc[train.listed_floor.eq(item["lower_supported_level"]), "building"]
        )
        hi = set(
            train.loc[train.listed_floor.eq(item["upper_supported_level"]), "building"]
        )
        assert item["shared_buildings"] == len(lo & hi)


def test_explicit_column_order_roundtrip(train, tmp_path):
    design = FeatureDesign(train, floor_increment_prior_scale=0.08)
    ...
    assert restored.floor_policy["cumulative_extreme_log_prior_sd"] == 0.08 * np.sqrt(3)
...
def test_saved_order_and_encoding_tamper_rejected(train, tmp_path, fault):
    ...
    if fault == "thresholds":
        meta["floor_thresholds"] = [-1.0, 1.0]
    if fault == "features":
        meta["features"] = meta["features"][::-1]
    if fault == "numeric_order":
        meta["numeric_order"] = meta["numeric_order"][::-1]
    if fault == "category_order":
        meta["category_order"] = meta["category_order"][::-1]
    if fault == "raw_order":
        meta["raw_feature_names"] = meta["raw_feature_names"][::-1]
    if fault == "priors":
        meta["floor_increment_prior_scale"] = 0.4
    if fault == "active":
        meta["active"] = [int(v) for v in meta["active"]]
    if fault == "version":
...
def test_all_unknown_or_single_supported_level_adds_no_fabricated_thresholds(
    train, floor
):
    data = train.copy()
    data["listed_floor"] = floor
...
def test_invalid_increment_prior_rejected(train, scale):
    with pytest.raises(ValueError, match="prior scale"):
        FeatureDesign(train, floor_increment_prior_scale=scale)


def test_independently_observed_physical_floor_and_gap_columns_preserved(train):
    data = train.copy()
    rng = np.random.default_rng(723)
    data["physical_floor"] = rng.choice([0.0, 1.0, 2.0, 4.0, np.nan], len(data))
    old = Previous(data)
    new = FeatureDesign(data)
    oldmat, oldnames, _ = old.raw_features(data)
    newmat, newnames, _ = new.raw_features(data)
    for name in (
        "physical_floor",
        "physical_floor.unknown",
        "physical_floor_x_elevator",
        "physical_floor_x_elevator.unknown",
        "floor_label_gap",
        "floor_label_gap.unknown",
    ):
        np.testing.assert_array_equal(
            oldmat[:, oldnames.index(name)], newmat[:, newnames.index(name)]
        )
    np.testing.assert_array_equal(listed_floor_values(data), listed_floor_values(train))
