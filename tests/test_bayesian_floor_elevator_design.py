...
def test_existing_design_columns_priors_and_time_are_preserved(mode):
    frame = data(); design = m.FeatureDesign(frame, mode=mode)
    base = m.floor.FeatureDesign(frame)
    np.testing.assert_array_equal(design.matrix(frame)[:, :len(base.features)], base.matrix(frame))
    np.testing.assert_array_equal(design.prior_scales[:len(base.features)], base.prior_scales)
    assert design.time.unit_ids == base.time.unit_ids
    np.testing.assert_allclose(design.matrix(frame)[:, len(base.features):].mean(axis=0), 0, atol=1e-15)
...
def test_known_no_yes_symmetric_unknown_separate_and_upper_range_saturates():
    ...
    np.testing.assert_array_equal(arrays[1], np.array([[0,0,0],[.5,0,0],[.5,.5,0],[.5,.5,.5],[.5,.5,.5],[0,0,0]]))
...
def test_pooled_and_separate_priors_match_full_range_difference_but_not_local_shape():
    ...
    for mode in m.MODES:
        ...
        vectors.append(diff)
        assert np.linalg.norm(diff*design.prior_scales) == pytest.approx(.15*np.sqrt(3))
        ...
        assert np.linalg.norm(yes*design.prior_scales) == pytest.approx(np.sqrt(3*.15**2+3*.15**2/4))
        local = (m.floor_contrast_vector(design, frame, 2, 3, True)
                 -m.floor_contrast_vector(design, frame, 2, 3, False))
        expected = .15/np.sqrt(3) if mode == 'pooled' else .15
...
def test_saved_design_round_trip_and_tampered_order_rejection(tmp_path, mode):
    ...
    path = tmp_path/'interaction-design.json'; saved = json.loads(path.read_text())
    saved['features'].reverse(); path.write_text(json.dumps(saved))
    with pytest.raises(ValueError, match='feature order'): m.FeatureDesign.load(tmp_path)


@pytest.mark.parametrize('bad', [True, 0, -1, float('nan'), float('inf')])
def test_invalid_prior_scales_rejected(bad):
    with pytest.raises(ValueError): m.FeatureDesign(data(), interaction_prior_scale=bad)


def test_missing_endpoint_and_unsupported_counterfactual_rejected():
    frame = data()
    with pytest.raises(ValueError, match='endpoints'): m.FeatureDesign(frame.loc[frame.advertised_floor != 4])
    with pytest.raises(ValueError, match='both known'): m.FeatureDesign(frame.assign(elevator=None))
    design = m.FeatureDesign(frame)
    with pytest.raises(ValueError, match='explicit known'): m.floor_contrast_vector(design, frame, 2, 5, None)
    with pytest.raises(ValueError, match='supported'): m.floor_contrast_vector(design, frame, 2, 6, True)
    with pytest.raises(ValueError, match='absent from fitted'): design.matrix(frame.assign(listed_floor=6))
