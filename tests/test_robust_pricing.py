...
from apartments.candidate_search import (
    select_candidates,
    score_candidates,
    market_comparison,
)
...
def fitted():
    ...
    for period in pd.date_range("2017-01-01", "2019-12-01", freq="MS"):
        for unit in range(12):
            rent = 2800.0 + 200 * unit + 20 * (period.year - 2017)
            rows.append(
                dict(
                    unit_id=f"u{unit}",
                    building=f"b{unit // 3}",
                    building_id=f"b{unit // 3}",
                    period=period,
                    asking_rent=rent,
                    log_rent=np.log(rent),
                    bedrooms=unit % 3,
                    bathrooms=1.0 + 0.5 * (unit % 2),
                    square_feet=None if unit % 3 == 0 else 500.0 + 50 * unit,
                    listed_floor=unit + 1,
                    physical_floor=unit,
...
def test_portable_parity_components_and_joint_floor_interaction(fitted):
    ...
    test = train.tail(12).copy()
    test["period"] = pd.Timestamp("2020-01-01")
    test.loc[test.index[0], ["unit_id", "building", "building_id"]] = [
        "new",
        "new-b",
        "new-b",
    ]
    ...
    np.testing.assert_allclose(
        [r["predicted_rent"] for r in actual], expected, rtol=1e-12
    )
    ...
    for row in actual:
        assert math.exp(math.fsum(row["log_components"].values())) == pytest.approx(
            row["predicted_rent"]
        )
    record = test.iloc[2].to_dict()
    changes = {"physical_floor": 9, "elevator": True}
    contrast = portable.marginal_contributions(record, changes, "2020-01")
    pair = pd.DataFrame([record, {**record, **changes}])
    scientific_pair = np.exp(baseline.predict(scientific, pair))
    assert contrast["dollar_change"] == pytest.approx(
        scientific_pair[1] - scientific_pair[0], abs=1e-8
    )
    assert (
        portable.marginal_contributions(record, {"laundry_type": "in_unit"}, "2020-01")[
            "status"
        ]
        == "unknown_attribute_contrast"
    )
...
def capture(unit="u0", **changes):
    return dict(
        ...
        bathrooms=1,
        source="streeteasy",
        source_listing_id="listing-" + unit,
        collected_at="2020-01-04T00:00:00Z",
...
def test_latest_inactive_conflicting_stale_and_future_captures():
    ...
    other = {**conflict, "rent": 3500}
    good = capture("good")
    selected, rejected, stats = select_candidates(
        [active, inactive, stale, future, conflict, other, good, good],
        as_of="2020-01-06",
        max_age_days=7,
    )
    ...
    assert (
        ...
        and stats["identical_latest_duplicates"] == 1
    )
    assert stats["exclusion_counts"] == {
        "conflicting_latest_capture": 2,
        "latest_capture_not_confirmed_active": 1,
        "not_known_at_cutoff": 1,
        "stale_capture": 1,
    }
    # A later-knowledge observation cannot replace the earlier known snapshot.
    new = {
        **active,
        "collected_at": "2020-01-05",
        "known_at": "2020-01-08",
...
def test_old_advertisement_does_not_supersede_active_episode_and_conflicts_precede_budget():
    ...
    duplicate = {
        **active,
        "source_listing_id": "another-active-ad",
        "collected_at": "2020-01-05",
    }
    ...
    selected, _, stats = select_candidates(
        [active, expensive], as_of="2020-01-06", budget=3200
    )
    assert not selected
...
def test_fresh_search_cli_keeps_preferences_separate_and_replays(tmp_path, fitted):
    _, _, artifact = fitted
    model = bundle(tmp_path / "model", artifact)
    candidates = tmp_path / "candidates.jsonl"
    prefs = tmp_path / "prefs.json"
    records = [
        capture("u0"),
        {**capture("u1"), "rent": 3100, "elevator": False},
    ...
    rankings = [
        json.loads(s) for s in (output / "rankings.jsonl").read_text().split("\n") if s
    ]
    ...
    assert (
        rankings[-1]["market_comparison"]["uncertainty"]["status"]
        == "unsupported_new_building"
    )
    ...
    changed = json.loads(canonical(artifact))
    changed["center"] += 0.3
    other = bundle(tmp_path / "other-model", changed)
    score_candidates(
        candidates,
        prefs,
        tmp_path / "other-scores",
        model_bundle=other,
        as_of="2020-01-06",
        budget=3200,
    )
    other_rows = [
        json.loads(s)
        for s in (tmp_path / "other-scores" / "rankings.jsonl").read_text().split("\n")
        if s
    ]
    assert [(r["monthly_surplus"], r["pareto_efficient"]) for r in rankings] == [
        (r["monthly_surplus"], r["pareto_efficient"]) for r in other_rows
    ]
    assert (
        rankings[0]["market_comparison"]["predicted_rent"]
        != other_rows[0]["market_comparison"]["predicted_rent"]
    )
...
def test_model_clock_horizon_tampering_and_missing_capture_clock(tmp_path, fitted):
    ...
    assert (
        market_comparison(model, capture(), as_of="2019-12-31")["status"]
        == "model_not_known_at_cutoff"
    )
    assert (
        market_comparison(model, capture(), as_of="2020-02-01")["status"]
        == "model_outside_serving_horizon"
    )
    assert (
        market_comparison(model, {**capture(), "bedrooms": 6}, as_of="2020-01-06")[
            "status"
        ]
        == "unsupported_candidate_attributes"
    )
    assert (
        market_comparison(
            model, {**capture(), "building_id": "wrong-building"}, as_of="2020-01-06"
        )["status"]
        == "unsupported_candidate_attributes"
    )
    malformed = capture()
    del malformed["collected_at"]
    malformed["observed_at"] = "2020-01-04"
    selected, rejected, _ = select_candidates([malformed], as_of="2020-01-06")
    assert (
        not selected
        and rejected[0]["reason"] == "missing_or_invalid_collection_knowledge_clock"
    )
    root = bundle(tmp_path / "model", artifact)
