...
def inputs():
    return (
        pd.DataFrame(
            {"ess_bulk": [600, 2400], "ess_tail": [300, 1200]},
            index=["beta[a]", "unit_z[u]"],
        ),
        pd.DataFrame(
            {"ess_bulk": [1000, 200], "ess_tail": [600, 100]},
            index=["unit_z[u]", "beta[a]"],
        ),
    )
...
@pytest.mark.parametrize(
    "fault",
    ["missing", "duplicate", "negative_ess", "nan", "reversed_bounds", "zero_time"],
)
def test_invalid_alignment_or_rates_cannot_produce_ranking(fault):
    ...
    if fault == "missing":
        ...
        gpu.index = ["beta[a]", "beta[a]"]
    elif fault == "negative_ess":
        cpu.iloc[0, 0] = -1
    elif fault == "nan":
        gpu.iloc[0, 1] = np.nan
...
def comparison_bundles(root, fault=None):
    ...
    cpu, gpu = inputs()
    cp = dict(
        source_manifest_sha256="dataset",
    ...
    gp.update(
        ...
        warmup_per_chain=4000,
        feature_names=["a"],
        feature_prior_scales=[0.15],
        float64=True,
        dense_mass=False,
    )
    if fault == "cohort":
        gp["source_observations_sha256"] = "different"
    if fault == "prior":
        gp["feature_prior_scales"] = [0.30]
    if fault == "code":
        gp["implementation_sha256"] = {"graph.py": "different"}
    cx, gb, ce, ge = (
        root / n for n in ("cpu-fit", "gpu-fit", "cpu-rates", "gpu-rates")
    )
    publish_bundle(
        cx / "protocol",
        {"protocol.json": canonical(cp) + "\n"},
        {"protocol_sha256": "cpu-protocol"},
    )
    publish_bundle(
        cx / "fit",
        {
            "feature-design.json": canonical(
                {"features": ["a"], "prior_scales": [0.15]}
            )
            + "\n"
        },
        {"protocol_sha256": "cpu-protocol"},
    ...
    (gb / "sampled.json").write_text(
        canonical({"settings": gp, "posterior_sha256": posterior_hash}) + "\n"
    )
    diagnostic = {"max_rhat": 1.001, "min_ess_bulk": 600.0, "min_ess_tail": 500.0}
    cs = {
        "acceptable": True,
        "timing": {
            "retained_wall_seconds_lower": 100,
            "retained_wall_seconds_upper": 200,
        },
        "diagnostics": {k: diagnostic for k in ("parameters", "derived", "floor")},
    }
    gs = {
        "acceptable": fault != "diagnostics",
        "benchmark_record_sha256": digest(gb / "sampled.json"),
        "posterior_sha256": posterior_hash,
        "timing_denominators_seconds": {"retained_compute_and_storage": 100},
        **{
            k: diagnostic
            for k in (
                "parameters",
                "derived_unit_and_bathroom_contributions",
                "joint_floor_contrasts",
            )
        },
    }
    files = {
        n + "-efficiency.csv": cpu.to_csv(index_label="parameter")
        for n in ("parameter", "derived")
    }
    publish_bundle(
        ce,
        {**files, "summary.json": canonical(cs) + "\n"},
        {
    ...
    files = {
        n + "-efficiency.csv": gpu.to_csv(index_label="parameter")
        for n in ("parameter", "derived")
    }
    publish_bundle(
        ge / "complete", {**files, "diagnostics.json": canonical(gs) + "\n"}, {}
    )
    if fault == "posterior":
        (gb / "posterior.nc").write_bytes(b"changed after diagnostics")
...
def test_completed_comparison_publishes_separate_parameter_families(tmp_path):
    ...
    result = run(*args)
    assert [r["family"] for r in result["family_comparisons"]["parameter"]] == [
        "beta",
        "unit_z",
    ]
    assert result["diagnostic_family_minima"]["floor"]["cpu"][
        "minimum_bulk_ess_per_second_bounds"
    ] == [3, 6]
    assert (args[-1] / "complete.json").exists()


@pytest.mark.parametrize(
    "fault", ["cohort", "prior", "code", "diagnostics", "posterior"]
)
