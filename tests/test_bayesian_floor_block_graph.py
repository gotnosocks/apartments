...
def test_exact_blocks_preserve_means_parameters_priors_density_gradient(mode):
    frame = data()
    design = (
        floor.FeatureDesign(frame)
        if mode == "floor"
        else elevator.FeatureDesign(frame, mode=mode)
    )
    ...
    for point in points(old):
        np.testing.assert_allclose(
            functions[0][0](point), functions[1][0](point), rtol=1e-12, atol=1e-9
        )
        np.testing.assert_allclose(
            functions[0][1](point), functions[1][1](point), rtol=1e-11, atol=1e-9
        )
...
@pytest.mark.parametrize(
    "matrix,names",
    [
        ([[1, 2]], ["listed_floor_gt_1"]),
        ([[1, 2]], ["listed_floor_gt_1"] * 2),
        ([[np.nan]], ["listed_floor_gt_1"]),
        ([[1]], ["other"]),
    ],
)
