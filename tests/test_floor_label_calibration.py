...
def row(unit, prefix, explicit, audit=None, building="b"):
    return dict(
        audit_id=audit or unit,
        unit_id=unit,
        building=building,
        candidate_floor=prefix,
        explicit_floor=explicit,
        capture_label_conflict=False,
        unresolved_capture_labels=0,
    )
...
def test_whole_unit_holdouts_abstain_at_unsupported_endpoints():
    ...
    assert result["holdout_counts"] == {
        "agrees": 2,
        "outside_other_units_prefix_range": 2,
    }
...
def test_candidates_require_interpolation_and_consistent_capture_labels():
    ...
    assert [
        (r["unit_id"], r["inferred_floor_candidate"]) for r in result["candidates"]
    ] == [("new", 3)]
