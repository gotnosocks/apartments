...
def floor_archive(tmp_path, train):
    ...
    with threadpool_limits(limits=1, user_api="blas"):
        ...
        design = floor.FeatureDesign(converted(rows))
    ...
    files = {
        ...
        for name in verification.comparison.DESIGNS
    }
    fm = publish_binary(
...
def test_backend_floor_fields_and_source_values_keep_aliases(floor_archive):
    *_, design, rows = floor_archive
    ...
    assert not any(
        "linear standardized floor" in warning for warning in analysis._warnings(row)
    )
    assert any("gaps" in warning for warning in analysis._warnings(row))
    loaded = backend.load_design(
        floor_archive[0] / "fit", converted(rows), floor_archive[2]
    )
    np.testing.assert_array_equal(
        loaded.matrix(converted(rows)), design.matrix(converted(rows))
    )
