...
spec = importlib.util.spec_from_file_location(
    "minimal_rent_model", Path(__file__).parents[1] / "models/minimal_rent_model.py"
)
    ...
    members = [
        {
            "listing_id": lid,
            "unit_id": "u",
            "canonical_unit_url": "https://streeteasy.com/building/a/1",
        }
        for lid in ["1", "2"]
    ]
    for name, rows in [
        ...
        ("rental_unit_memberships", members),
    ]:
        path = tmp_path / name
        path.mkdir()
...
def fixture():
    ...
    data["log_rent"] = (
        8.0 + 0.2 * data.bedrooms + 0.01 * np.arange(n) / 10 + rng.normal(0, 0.03, n)
    )
...
def test_train_only_encoding_unknown_units_and_sparse_fit(tmp_path):
    train = fixture()
    model = m.fit(
        train, {"building_penalty": 10, "unit_penalty": 8, "trend_penalty": 100}
    )
    test = train.iloc[:2].copy()
    test["unit_id"] = "unseen"
    test["building"] = "unknown"
    test["period"] = pd.Timestamp("2025-06-01")
    assert np.isfinite(m.predict(model, test)).all()
    assert np.sqrt(np.mean((m.predict(model, train) - train.log_rent) ** 2)) < 0.06
    enc = model["encoder"]
    assert enc.size_default == 700 and enc.size_medians == {}
    a, b = enc.offsets["unit"]
    x = enc.matrix(test)
    assert x[:, a:b].nnz == 0
    a, b = enc.offsets["trend"]
    assert (x[:, b - 1].toarray() == 1).all()
    # Test-set sizes and prices cannot change training transformations.
    before = enc.metadata()
    test["square_feet"] = 1e9
    test["asking_rent"] = 1e12
    enc.matrix(test)
...
def test_model_round_trip_and_contemporaneous_baseline(tmp_path):
    ...
    np.savez_compressed(
        tmp_path / "model.npz", beta=model["beta"], center=model["center"]
    )
    ...
    assert (
        table.bedroom_baseline.iloc[0]
        == expected.loc[(train.period.iloc[0], train.bedrooms.iloc[0])]
    )
    assert model["robust_objective_relative_change"] < 1e-5


def test_incremental_bedrooms_sum_adjacent_increments():
    data = fixture().iloc[:6].copy()
    data["bedrooms"] = np.arange(6, dtype=float)
    enc = m.Encoder(data)
    matrix = enc.matrix(data)
    expected = np.array(
        [[float(b > threshold) for threshold in range(5)] for b in range(6)]
    )
    assert enc.features[1:6] == [f"bedrooms_gt_{n}" for n in range(5)]
    assert np.array_equal(matrix[:, 1:6].toarray(), expected)
    beta = np.zeros(enc.n_parameters)
    increments = np.array([0.20, 0.15, 0.12, 0.10, 0.08])
    beta[1:6] = increments
