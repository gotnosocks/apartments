...
def archive(tmp_path, request):
    ...
    protocol.update(
        ...
        feature_design_version=experiment.feature.VERSION,
        base_feature_design_version=experiment.feature.floor.VERSION,
        interaction_mode=request.param,
        interaction_prior_scale=0.15,
        interaction_thresholds=list(experiment.feature.THRESHOLDS),
    ...
    for name, dim in [
        ...
        values = rng.normal(0, 0.02, (*shape, *((len(coords[dim]),) if dim else ())))
        if name == "alpha":
            values += math.log(4000)
        if name in ("sigma_unit", "sigma"):
            values = np.exp(values) * 0.15
    ...
    beta[:, :, design.features.index("listed_floor_gt_2")] = -beta[
        :, :, design.features.index(experiment.feature.names(request.param)[0])
    ] / (2 * math.sqrt(3) if request.param == "pooled" else 2) + rng.normal(
        0.03, 0.001, shape
    )
    ...
    samples = {
        n: np.asarray(posterior[n]).reshape((-1, *posterior[n].shape[2:]))
        for n in posterior
    }
    mu = reconstruct_mu(samples, design, frame)
    q = np.exp(np.quantile(mu, [0.025, 0.5, 0.975], axis=0))
    residuals = []
    for i, r in enumerate(rows):
        residuals.append(
            ...
            | {
                "latent_rent_lower_95": float(q[0, i]),
                "fitted_rent": float(q[1, i]),
                "latent_rent_upper_95": float(q[2, i]),
                "residual_dollars": float(r["asking_rent"] - q[1, i]),
                "residual_log": float(math.log(r["asking_rent"] / q[1, i])),
            }
        )
    ...
    files.update(
        {
            n: (root / "fit" / n).read_bytes()
            for n in (*verification.comparison.DESIGNS, "interaction-design.json")
        }
    )
    ...
    files.update(
        {
            ...
            "floor-elevator-diagnostics.csv": table.to_csv(),
            "posterior.nc": (tmp_path / "posterior.nc").read_bytes(),
            "coefficients.json": canonical(
                [{"feature": n, **interval(0.01)} for n in design.features]
            )
            + "\n",
    ...
    fm = publish_binary(
        root / "fit", files, {"version": experiment.VERSION, "protocol_sha256": ph}
    )
    return (
        root,
        source,
        protocol,
        {"protocol_manifest": pm, "fit_manifest": fm},
        design,
        rows,
        posterior,
    )
...
def test_report_reconstruction_and_full_joint_counterfactual(archive):
    ...
    with closing(backend.load(root, source)) as analysis:
        ...
        assert sum(
            r["mean_log_contribution"] for r in item["contributions"]
        ) == pytest.approx(item["mean_log_rent"])
        scenario = analysis.counterfactual(rows[0]["audit_id"], {"listed_floor": 3})
        assert scenario["status"] == "accepted"
        vector = experiment.feature.floor_contrast_vector(
            design, converted(rows), 2, 3, True
        )
        delta = posterior.beta.values.reshape(-1, len(design.features)) @ vector
        np.testing.assert_allclose(
            list(scenario["delta_log"].values()),
            np.quantile(delta, [0.025, 0.5, 0.975]),
        )
        assert (
            scenario["delta_log"]["upper_95"] - scenario["delta_log"]["lower_95"] < 0.05
        )
        # Marginal floor 8 and no-elevator support exist; their joint cell does not.
        absent = analysis.counterfactual(
            rows[0]["audit_id"], {"listed_floor": 8, "elevator": False}
        )
...
def test_loader_refuses_base_only_or_mismatched_protocol(archive):
    ...
    for protocol in (
        None,
        {**p, "version": report.EXPERIMENT_V4},
        {**p, "interaction_prior_scale": 0.2},
    ):
        with pytest.raises(ValueError):
            backend.load_design(root / "fit", converted(rows), protocol)
    restored = backend.load_design(root / "fit", converted(rows), p)
    np.testing.assert_array_equal(
        restored.matrix(converted(rows)), design.matrix(converted(rows))
    )
...
def test_rehashed_interaction_center_rejected_by_source_reconstruction(archive):
    ...
    extra["interaction_means"][0] += 0.01
    files = {
        n: (root / "fit" / n).read_bytes() for n in manifests["fit_manifest"]["files"]
    }
    files["interaction-design.json"] = canonical(extra) + "\n"
    manifests["fit_manifest"] = publish_binary(
        root / "fit",
        files,
        {
            "version": experiment.VERSION,
            "protocol_sha256": manifests["fit_manifest"]["protocol_sha256"],
        },
    )
    with (
        threadpool_limits(limits=1, user_api="blas"),
        pytest.raises(ValueError, match="exact source reconstruction"),
    ):
