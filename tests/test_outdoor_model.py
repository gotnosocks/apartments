...
def sample():
    ...
    for month in range(1, 5):
        for unit in range(60):
            ...
            log_rent = np.log(3000)+{'TERRACE': .1, 'GARDEN': .2, None: -.03}.get(category, 0.)
            rows.append(dict(unit_id=f'u{unit}', building=f'b{unit//6}',
                period=pd.Timestamp(2024, month, 1), bedrooms=1., bathrooms=1.,
                square_feet=650., asking_rent=np.exp(log_rent), log_rent=log_rent,
                ...
                private_outdoor_category=category, shared_outdoor_category=shared[(unit+month//2) % 3]))
    return pd.DataFrame(rows)


def outdoor_columns(encoder, data):
    start, stop = encoder.offsets['outdoor']
    return dict(zip(encoder.outdoor_features, encoder.matrix(data).toarray()[:, start:stop].T))
...
def test_interior_variant_exact_matrix_and_penalty():
    ...
    settings = {**amenities.SETTINGS, 'interior_penalty': 3., 'outdoor_penalty': 7.}
    assert np.array_equal(original.matrix(data).toarray(), extended.matrix(data).toarray())
    assert np.array_equal(original.penalty(settings).toarray(), extended.penalty(settings).toarray())
...
def test_known_centering_separates_values_and_unknown_reporting():
    ...
    for field in outdoor.FIELDS:
        ...
        types = [columns[f'{field}={value}'] for value in encoder.outdoor_categories[field]]
        assert np.allclose(np.sum(types, axis=0), 0.)
        for values in types:
            assert np.mean(values[known]) == pytest.approx(0., abs=1e-14)
            assert (values[~known] == 0.).all()
    ...
    assert types['private_outdoor_category=BALCONY'][0] != types['private_outdoor_category=BALCONY'][1]
    reporting = outdoor_columns(outdoor.encoder_class('reporting')(data), changed)
    assert reporting['private_outdoor_category.unknown'].tolist() == [0., 0., 1.]
...
def test_rare_pool_and_unseen_category_use_training_only():
    ...
    assert encoder.outdoor_category_centers[field]['__rare__'] == pytest.approx(9/data[field].notna().sum())
    ...
    assert columns[field+'.unknown'].tolist() == [0., 0., 0., 1.]
...
def test_single_known_category_cannot_identify_presence_premium():
    data = sample()
    data['private_outdoor_category'] = ['BALCONY' if i % 2 else None for i in range(len(data))]
    encoder = outdoor.encoder_class('types')(data)
    columns = outdoor_columns(encoder, data)
...
def test_real_fit_roundtrip_penalties_and_contrast_prediction(variant):
    data = sample()
    settings = {**amenities.SETTINGS, 'interior_penalty': 3., 'outdoor_penalty': 2.}
    fitted = baseline.fit(data, settings, iterations=20, encoder_class=outdoor.encoder_class(variant))
    ...
    for family, amount in [('amenities', 10.), ('interior', 3.), ('outdoor', 2.)]:
        start, stop = encoder.offsets[family]
        assert np.allclose(diagonal[start:stop], amount)
    fallback = np.asarray(encoder.penalty(amenities.SETTINGS).power(2).sum(axis=0)).ravel()
    start, stop = encoder.offsets['outdoor']
    assert np.allclose(fallback[start:stop], 10.)
    ...
    assert np.array_equal(encoder.penalty(settings).toarray(), loaded['encoder'].penalty(settings).toarray())
    ...
    if variant == 'types':
        for contrast in contrasts:
            ...
            actual = baseline.predict(loaded, after)[0]-baseline.predict(loaded, before)[0]
            assert actual == pytest.approx(contrast['log_rent_change'])
        assert .07 < contrasts[0]['log_rent_change'] < .12
        assert .16 < contrasts[1]['log_rent_change'] < .22


def test_rare_contrast_not_reported_and_bad_inputs_rejected():
    data = sample()
    data.loc[data.private_outdoor_category.eq('GARDEN'), 'private_outdoor_category'] = None
    data.loc[0, 'private_outdoor_category'] = 'GARDEN'
    encoder = outdoor.encoder_class('types')(data)
    fitted = {'encoder': encoder, 'beta': np.zeros(encoder.n_parameters), 'center': 0.}
    ...
    for value in ['', 'balcony', 'TERRACE+BALCONY', 'BALCONY+BALCONY', '__rare__', 1., np.inf, ['BALCONY']]:
        bad = data.copy()
    ...
    with pytest.raises(ValueError, match='coefficients'):
        outdoor.load_saved({'encoder': encoder.metadata(), 'beta': [], 'center': 0.})
    with pytest.raises(ValueError, match='outdoor penalty'):
        encoder.penalty({**amenities.SETTINGS, 'outdoor_penalty': -1.})
...
def test_missing_columns_and_nullable_values_are_unknown():
    ...
    encoder = outdoor.encoder_class('types')(data)
    assert all((values == 1).all() for values in outdoor_columns(encoder, data).values())
