...
def candidate(**changes):
    return {'unit_id': 'u1', 'building_id': 'demo', 'source': 'streeteasy',
        'source_listing_id': '100', 'capture_id': 'capture1',
        'canonical_unit_url': 'https://streeteasy.com/building/demo/1d',
        'collected_at': '2026-09-18T10:00:00Z', 'known_at': '2026-09-18T10:01:00Z',
        'listing_status': 'ACTIVE', 'price_basis': 'gross_advertised_rent',
        'rent': 4000, 'bedrooms': 1, 'bathrooms': 1, **changes}


def failure(**changes):
    return {'source_listing_id': '100', 'collected_at': '2026-09-18T11:00:00Z',
            'interpreted_at': '2026-09-18T11:01:00Z', 'reason': 'canonical_unit_mismatch', **changes}


def test_failure_blocks_old_advertisement_but_is_not_inactivity():
    old = candidate(); f = failure()
...
def test_failure_knowledge_clock_and_later_success():
    ...
    newer = candidate(collected_at='2026-09-18T12:00:00Z', known_at='2026-09-18T12:01:00Z')
    assert m.combine_records([newer], [f], as_of='2026-09-18T13:00:00Z') == ([newer], [])


def test_identity_drift_is_not_two_separate_units_or_silent_merge():
    old = candidate(); changed = candidate(unit_id='u2', canonical_unit_url='https://streeteasy.com/building/demo/2d')
...
def historical(**changes):
    return {'audit_id': 'historical1', 'unit_id': 'u1', 'building': 'demo',
        'source_listing_id': 'old', 'known_at': '2026-09-17T00:00:00Z',
        'period': '2020-01-01', 'analysis_price_basis': 'historical_initial_own_advertisement_ask',
        'bedrooms': 0, 'bathrooms': 1, 'asking_rent': 2500,
        'research_review_history': [{'decision_id': 'keep-this-source-review'}], **changes}


def evidence(row, root):
    return {'capture_id': row['capture_id'], 'bathroom_fields': {
...
def test_preserves_reviewed_history_and_missing_search_ad(monkeypatch):
    ...
    old = candidate(); new = candidate(source_listing_id='101', unit_id='u2', capture_id='capture2',
                                      canonical_unit_url='https://streeteasy.com/building/demo/2d', bedrooms=2)
    parent = historical(); before = deepcopy(parent)
    rows, _, _, summary = m.assemble([parent], [old, new], [], {'capture1': None, 'capture2': None},
        as_of='2026-09-18T12:00:00Z', max_age_days=7)
...
def test_latest_inactive_capture_removes_old_success(monkeypatch):
    monkeypatch.setattr(m, 'capture_evidence', evidence)
    old = candidate(); inactive = candidate(capture_id='inactive', listing_status='RENTED',
        collected_at='2026-09-18T11:00:00Z', known_at='2026-09-18T11:01:00Z')
    another = candidate(source_listing_id='101', unit_id='u2', capture_id='c2')
    rows, _, _, summary = m.assemble([historical()], [old, inactive, another], [], {'c2': None},
        as_of='2026-09-18T12:00:00Z', max_age_days=7)
...
def test_same_month_and_knowledge_contract(monkeypatch):
    monkeypatch.setattr(m, 'capture_evidence', evidence)
    kwargs = dict(candidates=[candidate()], failures=[], sources={'capture1': None},
                  as_of='2026-09-18T12:00:00Z', max_age_days=7)
...
def test_saved_collection_transform_replay_and_body_integrity(tmp_path):
    ...
    async def fetch(request):
        ...
        row = {'source_listing_id': identifier, 'canonical_unit_url':
               'https://streeteasy.com/building/demo/' + ('1d' if identifier == '100' else '2d')}
        return HtmlResponse(request.url, request=request, body=html(row))
    collection = tmp_path / 'collection'
    result = asyncio.run(discovery_detail_refresh.collect(review, report, collection, fetch=fetch))
    cutoff = result['report']['latest_known_at']
    parent = tmp_path / 'parent'
    publish_bundle(parent, {'observations.jsonl': canonical(historical()) + '\n'}, {'version': m.PARENT_VERSION})
    ...
    import gzip
    (collection / 'archive/bodies' / sha[:2] / (sha + '.gz')).write_bytes(gzip.compress(b'changed'))
...
def test_existing_current_review_cannot_be_lost(monkeypatch):
    monkeypatch.setattr(m, 'capture_evidence', evidence)
    args = dict(candidates=[candidate()], failures=[], sources={'capture1': None},
                as_of='2026-09-18T12:00:00Z', max_age_days=7)
