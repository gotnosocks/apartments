import pandas as pd
import pytest
from models.bayesian_feature_report import (
    verify_reporting_recovery,
    verify_reporting_cache,
)
...
def valid():
    ...
    code = {
        n: n + "-hash"
        for n in (
            "recover_bayesian_reports.py",
            "bayesian_report_cache.py",
            "bayesian_feature_report.py",
        )
    }
    recovery = {
        ...
        "completed_diagnostic_sha256": diag,
        "implementation_sha256": code,
        "maximum_source_block_bytes": 1024,
    }
    return {"files": {"posterior.nc": "posterior", **diag, **code}}, recovery


@pytest.mark.parametrize(
    "damage", ["none", "posterior", "protocol", "diagnostic", "code", "block"]
)
def test_recovery_binds_completed_inference_and_override_code(damage):
    ...
    if damage == "none":
        ...
        return
    if damage == "posterior":
        recovery["posterior_sha256"] = "different"
    if damage == "protocol":
        recovery["protocol_sha256"] = "different"
    if damage == "diagnostic":
        manifest["files"]["diagnostics.json"] = "different"
    if damage == "code":
        manifest["files"]["bayesian_report_cache.py"] = "different"
    if damage == "block":
        recovery["maximum_source_block_bytes"] = 9 * 1024 * 1024
...
def test_completed_diagnostic_table_cannot_be_substituted():
    ...
    table = pd.DataFrame(
        {
            "r_hat": [1.0, 1.001],
            "ess_bulk": [1000.0, 1100.0],
            "ess_tail": [1200.0, 1300.0],
        }
    )
    diag = {
        "max_rhat": 1.001,
        "min_ess_bulk": 1000.0,
        "median_ess_bulk": 1050.0,
        "min_ess_tail": 1200.0,
        "parameters": 2,
        "rhat_over_1_01": 0,
...
@pytest.mark.parametrize(
    "damage", ["none", "draws", "posterior", "code", "order", "block"]
)
def test_forward_cache_binds_all_draws_and_implementation(damage):
    protocol = {"chains": 4, "draws": 6000, "units": 100}
    manifest = {
        "files": {"posterior.nc": "posterior", "bayesian_report_cache.py": "code"}
    }
    cache = {
        **protocol,
        ...
        "maximum_source_block_bytes": 4096,
    }
    if damage == "none":
    ...
    if damage == "draws":
        cache["draws"] = 1000
    if damage == "posterior":
        cache["posterior_sha256"] = "different"
    if damage == "code":
        cache["implementation_sha256"] = "different"
    if damage == "order":
        cache["sample_order"] = "unknown"
    if damage == "block":
        cache["maximum_source_block_bytes"] = 9 * 1024 * 1024
    with pytest.raises(ValueError, match="Reporting cache"):
        verify_reporting_cache(protocol, "p", manifest, cache)
