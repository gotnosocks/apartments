...
def bedroom_case(tmp_path):
    data = pd.DataFrame({"bedrooms": [3.0, 0.0, 3.0, 0.0]})
    config = m.graph.graph_configuration(data, residual_scale="bedroom")
    protocol = {
        "version": m.V3_EXPERIMENT,
        "chains": 2,
        "draws": 4,
    ...
    design = SimpleNamespace(
        ...
        time=SimpleNamespace(
            buildings=["a", "b"],
            unit_ids=["u", "v"],
            time_matrix=np.zeros((3, 2)),
            season_matrix=np.zeros((12, 2)),
        ),
    )
    coords = {
        "chain": [1, 3],
        ...
        "trend_basis": [0, 1],
        "season_basis": [0, 1],
        "residual_bedroom": [0, 3],
    }
    rng = np.random.default_rng(527)
    variables = {
        name: (
            ("chain", "draw", *dims),
            rng.normal(size=(2, 4) + tuple(len(coords[d]) for d in dims)),
        )
        for name, dims in m.common.VARIABLE_DIMS.items()
    }
    sigma = np.arange(8).reshape(2, 4) * 0.02 + 0.05
    z = np.stack(
        [-np.arange(8).reshape(2, 4) / 5, np.arange(8).reshape(2, 4) / 5], axis=2
    )
    scale = np.arange(8).reshape(2, 4) * 0.03 + 0.2
    variables.update(
        sigma=(("chain", "draw"), sigma),
        sigma_unit=(("chain", "draw"), sigma * 0.5),
        residual_bedroom_z=(("chain", "draw", "residual_bedroom"), z),
        residual_bedroom_offset=(
        ...
        sigma_by_bedroom=(
            ("chain", "draw", "residual_bedroom"),
            sigma[:, :, None] * np.exp(scale[:, :, None] * z),
        ),
    )
...
def test_verified_configuration_recomputed_against_exact_source_and_saved_graph(
    bedroom_case, tmp_path
):
    ...
    (tmp_path / "graph-configuration.json").write_text(
        json.dumps({**config, "student_t_nu": 7.0})
    )
    with pytest.raises(ValueError, match="configuration differs"):
...
def test_v4_uses_same_explicit_noise_contract_and_requires_floor_code(
    bedroom_case, tmp_path
):
    _, _, protocol, config, _, data = bedroom_case
    ...
    protocol["implementation_sha256"] = {
        "bayesian_feature_graph_v3.py": "x",
        "bayesian_feature_experiment_v3.py": "y",
    }
    with pytest.raises(ValueError, match="Missing v4"):
        m.verify_implementation(protocol)


def test_lazy_joint_noise_selection_and_replication_exactly_follow_bedroom_mapping(
    bedroom_case,
):
    ...
    for name in m.BEDROOM_DIMS:
        expected = np.concatenate(
            [posterior[name].isel(chain=c, draw=[1, 3]).values for c in range(2)]
        )
    ...
    seed = 817
    expected = (
        np.random.default_rng(seed).standard_t(5, size=(4, 4))
        * samples["sigma_by_bedroom"][:, [1, 0, 1, 0]]
    )
    np.testing.assert_array_equal(
        m.replicate_residuals(samples, config, data.bedrooms, seed), expected
    )
    ...
    assert np.max(abs(expected - wrong)) > 0.02
...
def test_invalid_bedroom_posterior_is_refused_before_replication(
    bedroom_case, mutation
):
    path, p, protocol, config, design, _ = bedroom_case
    if mutation == "missing":
        p = p.drop_vars("sigma_by_bedroom")
    elif mutation == "offset_missing":
        p = p.drop_vars("residual_bedroom_offset")
    elif mutation == "offset_identity":
        p["residual_bedroom_offset"].values[0, 1, 0] += 0.1
    elif mutation == "coordinate":
        p = p.assign_coords(residual_bedroom=[3, 0])
    elif mutation == "scale_identity":
        p["sigma_by_bedroom"].values[0, 1, 0] *= 1.1
    elif mutation == "zero_sum":
        p["residual_bedroom_z"].values[0, 1, 0] += 0.1
    elif mutation == "negative":
        p["residual_bedroom_scale"].values[0, 1] = -0.2
    else:
        p["sigma_by_bedroom"] = p["sigma_by_bedroom"].transpose(
            "draw", "chain", "residual_bedroom"
        )
    p.to_netcdf(path, group="posterior", engine="h5netcdf", mode="w")
    with pytest.raises(ValueError):
        m.load_draws(path, protocol, design, config, per_chain=2)
...
def test_bedroom_reconstruction_cannot_fall_back_or_accept_unseen_count(bedroom_case):
    ...
    with pytest.raises(ValueError, match="required"):
        m.replicate_residuals(missing, config, data.bedrooms, 1)
    with pytest.raises(ValueError, match="Unsupported bedroom"):
        m.replicate_residuals(samples, config, [2, 0], 1)
    with pytest.raises(ValueError, match="degrees of freedom"):
        m.replicate_residuals(
            samples, {**config, "student_t_nu": 7.0}, data.bedrooms, 1
        )


def test_shared_and_legacy_v2_use_exact_original_shared_replication(
    bedroom_case, tmp_path
):
    ...
    expected = (
        np.random.default_rng(61).standard_t(5, size=(4, 4)) * samples["sigma"][:, None]
    )
    np.testing.assert_array_equal(
        m.replicate_residuals(samples, shared, data.bedrooms, 61), expected
    )
    legacy = {"version": m.V2_EXPERIMENT, "prior_multiplier": 1.0}
    assert m.verified_configuration(legacy, data, tmp_path) == shared
    with pytest.raises(ValueError, match="cannot declare"):
        m.verified_configuration(
            {**legacy, "residual_scale": "bedroom"}, data, tmp_path
        )
...
def test_v3_implementation_and_report_gates_fail_closed(monkeypatch, tmp_path):
    with pytest.raises(ValueError, match="Missing v3 graph"):
        m.verify_implementation(
            {"version": m.V3_EXPERIMENT, "implementation_sha256": {}}
        )
    ...
    monkeypatch.setattr(
        m, "load_draws", lambda *a, **k: pytest.fail("Read unaccepted posterior")
    )
...
def test_spline_uses_explicit_noise_contract_and_requires_its_producer(
    bedroom_case, tmp_path
):
    _, _, protocol, config, _, data = bedroom_case
