...
from models import (
    quarantine_fit_comparison as m,
    reviewed_cohort_projection as projection,
)
from tests.test_laundry_floor_fit_comparison import (
    protocols as laundry_protocols,
    residual_fixture,
)
...
def protocols():
    ...
    for value in (a, b):
        value.update(
            execution_version=m.disk.VERSION,
            storage_policy=deepcopy(m.disk.POLICY),
            storage_versions=dict.fromkeys(
                ("zarr", "obstore", "xarray", "h5py"), "1.0"
            ),
        )
...
def test_source_comparison_rejects_conflated_experiments(fault):
    ...
    if fault == "math":
        ...
        b["implementation_sha256"]["extra.py"] = "new"
    elif fault == "removed_code":
        del b["implementation_sha256"]["math.py"]
    elif fault == "missing_helper":
        del b["implementation_sha256"]["reviewed_cohort_quarantine.py"]
    elif fault == "storage":
        b["storage_policy"]["all_retained_draws"] = False
    elif fault == "adaptation":
        b["adaptation"] = "low_rank"
    elif fault == "draws":
        b["draws"] = 50
    elif fault == "floor":
        b["floor_levels"].append(9)
    elif fault == "prior":
        b["building_prior_scale"] *= 2
    elif fault == "noise":
        b["residual_scale"] = "bedroom"
    elif fault == "environment":
        b["versions"] = {"numpy": "different"}
...
def test_published_source_revision_restores_exact_reference(tmp_path):
    ...
    parent, decision_path, candidate = [
        tmp_path / name for name in ("parent", "decisions", "candidate")
    ]
    publish_bundle(
        parent,
        {
    ...
    publish_bundle(
        ...
        {
            "version": m.quarantine.DECISION_VERSION,
            "reviewed_at": CLOCK,
    ...
    with pytest.raises(ValueError, match="exact reference"):
        m.verify_revision(wrong, candidate)
...
def test_residual_source_membership_and_target_are_bound(fault):
    ...
    if fault == "price":
        ...
        b[0]["source_listing_id"] = "other"
    elif fault == "missing":
        b.pop()
    elif fault == "duplicate":
        b.append(deepcopy(b[0]))
    with pytest.raises(ValueError):
        m.compare_residuals(a, b, rows, rows)
...
def test_group_comparison_reports_removed_buildings_and_units():
    ...
    a = [
        {"kind": kind, "id": identity, "log_effect": interval(0)}
    ...
    b[1]["log_effect"] = interval(0.2)
    changed, removed = m.compare_groups(a, b, rows, rows[:1])
    assert changed[0]["id"] == "a1" and changed[0]["log_effect"]["median_change"] == 0.2
    assert {(r["kind"], r["id"]) for r in removed} == {
        ("building", "b"),
        ("unit", "b1"),
    }
    for bad in (b[:1], b + [b[0]]):
        with pytest.raises(ValueError, match="membership"):
            m.compare_groups(a, bad, rows, rows[:1])
...
@pytest.mark.parametrize(
    "fault", ["missing", "duplicate", "single", "nan", "shape", "labels"]
)
def test_invalid_common_reference_is_rejected(fault):
    ...
    if fault == "missing":
        ...
        labels = ["a", "a", "b"]
    with pytest.raises(ValueError):
        m.common_building_draws(values, labels, shared)


@pytest.mark.parametrize("fault", [None, "unmixed_chains", "draw_count", "coordinates"])
def test_joint_building_analysis_reads_all_draws_and_enforces_convergence(
    tmp_path, fault
):
    ...
    if fault == "unmixed_chains":
        values[:, :, 0] += np.arange(4)[:, None] * 10
    labels = ["a", "b", "removed"]
    posterior = xr.Dataset(
        {"building_effect": (("chain", "draw", "building"), values)},
    ...
    (tmp_path / "fit").mkdir()
    posterior.to_netcdf(
        tmp_path / "fit/posterior.nc", group="posterior", engine="h5netcdf"
    )
    stats.to_netcdf(
        tmp_path / "fit/posterior.nc", group="sample_stats", mode="a", engine="h5netcdf"
    )
    fit = {
        "root": tmp_path,
        "protocol": {"chains": 4, "draws": 1000},
        "data": SimpleNamespace(building=labels),
    }
    if fault == "draw_count":
        fit["protocol"]["draws"] = 50
    elif fault == "coordinates":
        fit["data"].building = ["a", "b", "wrong"]
    if fault:
        with pytest.raises(ValueError):
            m.building_contrasts(fit, ["a", "b"])
    else:
        result = m.building_contrasts(fit, ["a", "b"])
        assert result["diagnostics"]["acceptable"]
        expected = np.quantile(
            (values[:, :, 0] - values[:, :, 1]) / 2, [0.025, 0.5, 0.975]
        )
        effect = result["contrasts"][0]["log_effect"]
        np.testing.assert_allclose(
            [effect[k] for k in ("lower_95", "median", "upper_95")], expected
        )
...
def test_floor_comparison_aligns_physical_endpoints():
    a = {
        "contrasts": [
            {"lower_floor": low, "upper_floor": high, "percent_effect": interval(1)}
    ...
    b["contrasts"].reverse()
    assert all(
        r["percent_effect"]["median_change"] == 0 for r in m.compare_floors(a, b)
    )
    b["contrasts"][0]["upper_floor"] = 5
    with pytest.raises(ValueError, match="endpoints"):
        m.compare_floors(a, b)
...
def test_category_comparison_preserves_physical_scenario(fault):
    ...
    if fault == "membership":
        ...
        b["contrasts"][0]["after"] = "on_floor"
    elif fault == "vector":
        b["contrasts"][0]["design_vector"] = [1, 0]
    elif fault == "vector_shape":
        b["contrasts"][0]["design_vector"] = [1]
    elif fault == "nonfinite":
        b["contrasts"][0]["design_vector"] = [0, np.nan]
    if fault:
        with pytest.raises(ValueError):
            m.compare_categories(a, b)
    else:
