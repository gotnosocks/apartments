...
def posterior():
    ...
    return xr.Dataset(
        ...
        coords={
            "chain": [0, 1],
            "draw": np.arange(65),
            "unit": ["u" + str(i) for i in range(1030)],
        },
    )
...
def test_all_draws_order_scaling_repeated_indices_and_replay(tmp_path):
    p = posterior()
    expected = p.unit_z.values.reshape(130, 1030)
    values, meta = build_cache(p, tmp_path, "sourcehash")
    assert (
        values.flags.f_contiguous
        and meta["maximum_source_block_bytes"] <= 8 * 1024 * 1024
    )
    np.testing.assert_array_equal(values, expected)
    lazy = UnitSamples(values)
    scale = np.linspace(0.1, 0.5, 130)
    ...
    for index in [0, 1029, [7, 1, 7, 1029], slice(500, 520)]:
        ...
        np.testing.assert_array_equal(
            scaled[:, index], (scale[:, None] * expected)[:, index]
        )
    values._mmap.close()
...
def test_scientific_reports_are_byte_identical_with_disk_unit_access(tmp_path):
    ...
    variables = {
        name: (("chain", "draw"), value + rng.normal(0, 0.01, shape))
        for name, value in [("alpha", 8.4), ("sigma_unit", 0.2), ("annual_drift", 0.03)]
    }
    for name, dim in [
        ("beta", "feature"),
        ("unit_z", "unit"),
        ...
        variables[name] = (
            ("chain", "draw", dim),
            rng.normal(0, 0.03, (*shape, len(coords[dim]))),
        )
    stats = xr.Dataset(
        {
    ...
    inference = xr.DataTree.from_dict(
        {"posterior": xr.Dataset(variables, coords=coords), "sample_stats": stats}
    )
