...
One CPU-only worker uses two cores and 8 GiB, a one-hour hard timeout and no Modal retries. A subprocess deadline at 55 minutes leaves time to checkpoint and commit its output. A new process for each run avoids reusing a stopped Twisted reactor. Normal completion publishes a standalone browser checkpoint when this workspace is authoritative, commits the SQLite archive, checkpoint and run log, then releases the distributed lock. The browser shows the last committed checkpoint between bounded runs, rather than reading the writer’s WAL. A non-authoritative workspace does not replace the selected browser checkpoint. Exit code 3 generally denotes the crawler's pause/cooldown state; examine the returned state and log before resuming.
