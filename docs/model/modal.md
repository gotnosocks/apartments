...
All fitting, diagnostics and artifact generation run on Modal. The laptop submits a small request and optionally streams finished files. The default worker uses **Nutpie/Numba on four cloud CPU cores**. `--gpu` selects **Nutpie/JAX on one T4** through a separately registered ephemeral app. Default CPU runs never build the CUDA image; GPU registration and image building happen only when requested. The rent model's priors, likelihoods, exclusions and historical attribute assumptions are unchanged.

The default is CPU because the measured T4 smoke test was slower: on the same 2,386-observation snapshot, 50 tuning steps plus 50 retained draws in two chains took about 110 seconds on T4, versus 12.5 seconds for the earlier local M1/Numba baseline. These short runs are execution tests, not converged estimates or comprehensive performance benchmarks. No further analysis or benchmarks need to run on the laptop. Cloud CPU performance is measured separately for each job.
