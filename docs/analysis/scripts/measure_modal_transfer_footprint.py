...
def inventory(path):
    ...
    return {
        ...
        "files": records,
        "file_count_including_manifest": len(records),
...
def analytical_compression(path, measured, level):
    ...
    with gzip.GzipFile(
        filename="", mode="wb", fileobj=sink, compresslevel=level, mtime=0
    ) as compressed:
        with tarfile.open(
    ...
    if inventory(path) != measured or any(
        digest(path / name) != expected[name] for name in names
    ):
        raise ValueError("Analytical source changed during compression measurement")
    return {
        "format": "deterministic USTAR stream inside gzip; dataset/ prefix; zero timestamps and owners",
        "gzip_level": level,
        "bytes": sink.bytes,
        "sha256": sink.hash.hexdigest(),
        "compressed_fraction": sink.bytes / measured["bytes"],
        "uncompressed_to_compressed_ratio": measured["bytes"] / sink.bytes,
        "compression_wall_seconds": elapsed,
        "wall_time_is_not_a_benchmark": True,
...
def run(dataset, experiment, output, level=6):
    ...
    if (
        fit != inventory(Path(experiment) / "fit")
        or protocol != inventory(Path(experiment) / "protocol")
    ...
    result = {
        "version": "local-modal-transfer-footprint-v1",
        "analytical_input": input_bundle,
        "baseline_fit": fit,
        "baseline_protocol": protocol,
        "baseline_matching_dataset": baseline_dataset,
        "baseline_offline_closure": {
            "bytes": sum(v["bytes"] for v in (fit, protocol, baseline_dataset)),
            "file_count_including_manifests": sum(
                v["file_count_including_manifest"]
                for v in (fit, protocol, baseline_dataset)
            ),
            "actual_stripped_bundle_load_performed": False,
        },
        "analytical_input_compression": compression,
        "planning_combination": {
            "bytes": input_bundle["bytes"] + fit["bytes"] + protocol["bytes"],
            "file_count_including_manifests": sum(
                v["file_count_including_manifest"]
                for v in (input_bundle, fit, protocol)
            ),
        ...
        "posterior_read_or_compressed": False,
        "remote_activity": False,
        "stripped_bundle_load_performed": False,
    ...
    print(
        json.dumps(
            {
                "output": str(output),
                "input_bytes": input_bundle["bytes"],
                "input_gzip_bytes": compression["bytes"],
                "baseline_fit_bytes": fit["bytes"],
