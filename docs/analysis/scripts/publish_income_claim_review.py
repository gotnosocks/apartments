...
TEMPORAL = ('The complete same-advertisement description was reviewed at its September 2026 '
...
def specifications(cases):
    ...
    def add(ad, kind, message, literals, **observed):
        row = by_ad[ad]
        result.append({'audit_id': row['audit_id'], 'kind': kind, 'message': message,
            'temporal_scope': TEMPORAL, 'literals': literals,
            'observed_fields': {'asking_rent': row['asking_rent'], 'period': row['period'], **observed}})

    for ad, minimum, maximum in (
        ('4758015', '$77,760', '$90,720 to $103,680'),
        ('4761346', '$83,314', '$83,314 to $103,680')):
        add(ad, 'advertised_income_ceiling_dollars_unmodeled',
    ...
    add('4276224', 'advertised_income_ceiling_dollars_unmodeled',
    ...
    add('4160254', 'advertised_income_ceiling_dollars_unmodeled',
        ...
        ['You MUST be within income specified below to qualify!', 'Minimum Income: $105,000',
         'Maximum Income: 1 Person People: $176,220'])
    add('4488024', 'advertised_income_ceiling_dollars_unmodeled',
    ...
    for ad, phrase in (
        ...
        ('4837062', 'max income per house hold of 120 percent Area Median Income (AMI)'),
        ('4902655', 'max income per house hold of 120 percent Area Median Income (AMI)')):
        add(ad, 'advertised_income_ceiling_ami_unmodeled',
            ...
            ['This is HDFC', phrase, 'use a guarantor if their income is below 40 times the rent.'])
    add('4810936', 'advertised_ami_eligibility_boundary_unspecified',
        ...
        ['This is HDFC', 'must meet the 120 percent Area Median Income (AMI) and use a guarantor if their income is below 40 times the rent.'])
    add('4780384', 'advertised_ami_eligibility_boundary_unspecified',
    ...
    add('3705384', 'advertised_income_restriction_terms_unspecified',
    ...
    for ad, floor, phrase in (
        ...
        add(ad, 'label_proxy_prose_floor_disagreement',
            ...
            [phrase], listed_floor=floor)
    for ad in ('4837062', '4902655'):
        add(ad, 'internally_conflicting_bedroom_description',
            ...
            ['a stunning, one bedroom', 'This two bedroom has a wonderful pre war charm.'], bedrooms=2.)
    add('4276224', 'furnished_short_term_utilities_offer_scope',
        ...
        ['FURNISHED, Short-Term only rental till April 30th.', 'This unit includes all utilities'])
    if len(by_ad) != 12 or len(result) != 19 or {r['audit_id'] for r in result} != {r['audit_id'] for r in cases}:
        raise ValueError('Manual review does not cover the exact twelve screen candidates')
    return result


def run(dataset, evidence, screen, output, reviewed_at):
    dataset, evidence, screen, output = map(Path, (dataset, evidence, screen, output))
    if [digest(p/'complete.json') for p in (dataset, evidence, screen)] != [SOURCE_HASH, EVIDENCE_HASH, SCREEN_HASH]:
    ...
    notes = source_issues.publish_source_issues(dataset, evidence, specifications(cases), output,
        reviewed_at=reviewed_at, reviewer='Codex: full own-description review of all twelve income-screen candidates')
    restored = source_issues.load_source_issues(dataset, output, evidence=evidence)
    if notes != restored:
        raise ValueError('Published income review differs from independently loaded annotations')
    print(json.dumps({'observations': len(notes), 'issues': sum(len(n['issues']) for n in notes.values()),
                      'manifest_sha256': digest(output/'complete.json')}), flush=True)
