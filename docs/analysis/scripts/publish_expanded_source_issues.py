    ...
    return [
        {
            ...
            "literals": [
                "Please note: The apartment is being offered furnished only, for 1 or 2 year lease."
            ],
        },
        {
            "audit_id": "15b0a695cf02ea4e2102d83f014a109ef7bdd423d20fdfbd2cedef7593922302",
            ...
            "literals": [
                "a utility closet with stacked front-loading Whirlpool washer/dryer"
            ],
        },
        {
            ...
            "literals": [
                "This furnished rental is the epitome of luxury and is available immediately for the most discerning resident."
            ],
        },
        {
...
def main():
    ...
    if (
        digest(args.dataset / "complete.json") != SOURCE_HASH
        or digest(args.evidence / "complete.json") != EVIDENCE_HASH
    ):
        raise ValueError(
            "Expected the reviewed expanded source and exact description archive"
        )
    notes = source_issues.publish_source_issues(
        ...
        reviewer="Codex: complete archived-capture source review",
    )
    restored = source_issues.load_source_issues(
        args.dataset, args.output, evidence=args.evidence
    )
    if notes != restored:
        raise ValueError(
            "Published source notes differ from independently loaded notes"
        )
    print(
        json.dumps(
            {
                ...
                "manifest_sha256": digest(args.output / "complete.json"),
            }
        ),
        flush=True,
    )
