...
- **Cleanup.** `clean --run-id` deletes a run's remote copy; `clean --unreferenced-blobs` prunes blobs no remaining run needs. Clean only after `complete`, if the posterior may be needed.
